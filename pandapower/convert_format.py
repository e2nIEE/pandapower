...
def _add_missing_columns(net, elements_to_deserialize):
    ...
    if _check_elements_to_deserialize('bus_geodata', elements_to_deserialize) and "coords" not in net.bus_geodata:
        net.bus_geodata["coords"] = None
    if _check_elements_to_deserialize('trafo3w', elements_to_deserialize) and not "tap_at_star_point" in net.trafo3w:
        net.trafo3w["tap_at_star_point"] = False
    if _check_elements_to_deserialize('trafo3w', elements_to_deserialize) and not "tap_step_degree" in net.trafo3w:
        net.trafo3w["tap_step_degree"] = 0
    if _check_elements_to_deserialize('load', elements_to_deserialize) and "const_z_percent" not in net.load or "const_i_percent" not in net.load:
    ...
    if _check_elements_to_deserialize('line', elements_to_deserialize) and "g_us_per_km" not in net.line:
        net.line["g_us_per_km"] = 0.

    if _check_elements_to_deserialize('gen', elements_to_deserialize) and "slack" not in net.gen:
        net.gen["slack"] = False

    if _check_elements_to_deserialize('trafo', elements_to_deserialize) and "tap_phase_shifter" not in net.trafo and "tp_phase_shifter" not in net.trafo:
        net.trafo["tap_phase_shifter"] = False

    # unsymmetric impedance
    if _check_elements_to_deserialize('impedance', elements_to_deserialize) and "r_pu" in net.impedance:
