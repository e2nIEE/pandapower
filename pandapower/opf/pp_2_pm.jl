...
function get_solver(optimizer::String, nl::String="ipopt", mip::String="cbc",
    ...
    mip_time_limit::Float64=Inf, tol::Float64=1e-8)
    
    if optimizer == "gurobi"
            solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => time_limit,
                    "OutputFlag" => log_level, "FeasibilityTol" => tol, "OptimalityTol" => tol)
    end
    ...
    if optimizer == "juniper" && nl == "ipopt" && mip == "cbc"
        mip_solver = JuMP.optimizer_with_attributes(Cbc.Optimizer, "logLevel" => log_level, "seconds" => mip_time_limit,
                    "tol" => tol)
        nl_solver = JuMP.optimizer_with_attributes(Ipopt.Optimizer, "print_level" => log_level,
                    "max_cpu_time" => nl_time_limit, "tol" => tol)
    ...
    if optimizer == "juniper" && nl == "gurobi" && mip == "cbc"
        mip_solver = JuMP.optimizer_with_attributes(Cbc.Optimizer, "logLevel" => log_level, "seconds" => mip_time_limit, "tol" => tol)
        nl_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => nl_time_limit, "FeasibilityTol" => tol, "OptimalityTol" => tol)
        solver = JuMP.optimizer_with_attributes(Juniper.Optimizer,
                     "nl_solver" => nl_solver,
                     "mip_solver" => mip_solver,
                     "log_levels" => [],
    ...
    if optimizer == "juniper" && nl == "gurobi" && mip == "gurobi"
        mip_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => mip_time_limit, "FeasibilityTol" => tol, "OptimalityTol" => tol)
        nl_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => nl_time_limit, "FeasibilityTol" => tol, "OptimalityTol" => tol)
        solver = JuMP.optimizer_with_attributes(Juniper.Optimizer,
                     "nl_solver" => nl_solver,
                     "mip_solver" => mip_solver,
    ...
    if optimizer == "cbc"
        solver = JuMP.optimizer_with_attributes(Cbc.Optimizer, "seconds" => time_limit, "tol" => tol)
    end
