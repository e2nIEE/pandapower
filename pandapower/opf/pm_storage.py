    ...
    for idx in net["storage"].index:
        energy = (net["storage"].at[idx, "soc_percent"] * 1e-2 *
                  (net["storage"].at[idx, "max_e_mwh"] -
                   net["storage"].at[idx, "min_e_mwh"])) # / pm["baseMVA"] testar
        qs = net["storage"].at[idx, "q_mvar"].item()  # / pm["baseMVA"] testar
        ps = net["storage"].at[idx, "p_mw"].item()  # / pm["baseMVA"] testar
        max_p_mw = ps
        max_q_mvar, min_q_mvar = qs, -qs
        if "max_p_mw" in net["storage"]:
            max_p_mw = net["storage"].at[idx, "max_p_mw"].item()  # / pm["baseMVA"] testar
        if "max_q_mvar" in net["storage"]:
            max_q_mvar = net["storage"].at[idx, "max_q_mvar"].item()  # / pm["baseMVA"] testar
        if "max_q_mvar" in net["storage"]:
            min_q_mvar = net["storage"].at[idx, "min_q_mvar"].item()  # / pm["baseMVA"] testar
