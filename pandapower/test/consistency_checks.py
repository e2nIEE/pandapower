...
def indices_consistent(net):
    ...
    for element in elements:
        ...
        assert len(e_idx) == len(res_idx), (
            "length of %s bus and res_%s indices do not match" % (element, element)
        )
        assert all(e_idx == res_idx), "%s bus and res_%s indices do not match" % (
            element,
            element,
        )
...
def branch_loss_consistent_with_bus_feed_in(net, atol=1e-2):
    ...
    try:
        ...
        raise AssertionError(
            "Branch losses are %.4f MW, but power generation at the buses exceeds the feedin by %.4f MW"
            % (branch_loss_p, bus_surplus_p)
        )
    try:
        assert isclose(bus_dc_surplus_p, branch_dc_loss, atol=atol)
    except AssertionError:
        raise AssertionError(
            "DC branch losses are %.4f MW, but power generation at the DC buses exceeds the feedin by %.4f MW"
            % (branch_dc_loss, bus_dc_surplus_p)
        )
    try:
        assert isclose(bus_surplus_q, branch_loss_q, atol=atol)
    except AssertionError:
        raise AssertionError(
            "Branch losses are %.4f MVar, but power generation at the buses exceeds the feedin by %.4f MVar"
            % (branch_loss_q, bus_surplus_q)
        )
...
def element_power_consistent_with_bus_power(net, rtol=1e-2, test_q=True):
    ...
    """
    bus_p = pd.Series(data=0.0, index=net.bus.index)
    bus_q = pd.Series(data=0.0, index=net.bus.index)
    bus_p_dc = pd.Series(data=0.0, index=net.bus_dc.index)
    ...
    assert allclose(
        net.res_bus_dc.p_mw.values, bus_p_dc.values, equal_nan=True, rtol=rtol
    )
    if test_q:
        assert allclose(
            net.res_bus.q_mvar.values, bus_q.values, equal_nan=True, rtol=rtol
        )
...
def branch_loss_consistent_with_bus_feed_in_3ph(net, atol=1e-2):
    ...
    branch_loss_p = (
        net.res_line_3ph.pl_a_mw.sum()
        + net.res_trafo_3ph.pl_a_mw.sum()
        + net.res_line_3ph.pl_b_mw.sum()
        + net.res_trafo_3ph.pl_b_mw.sum()
        + net.res_line_3ph.pl_c_mw.sum()
        + net.res_trafo_3ph.pl_c_mw.sum()
    )

    branch_loss_q = (
        net.res_line_3ph.ql_a_mvar.sum()
        + net.res_trafo_3ph.ql_a_mvar.sum()
        + net.res_line_3ph.ql_b_mvar.sum()
        + net.res_trafo_3ph.ql_b_mvar.sum()
        + net.res_line_3ph.ql_c_mvar.sum()
        + net.res_trafo_3ph.ql_c_mvar.sum()
    )
...
def element_power_consistent_with_bus_power_3ph(net, rtol=1e-2):
    ...
    The bus feed-in at each node has to be equal to the sum of the element feed ins at each node.
    """
    bus_p_a = pd.Series(data=0.0, index=net.bus.index)
    bus_q_a = pd.Series(data=0.0, index=net.bus.index)
    bus_p_b = pd.Series(data=0.0, index=net.bus.index)
    bus_q_b = pd.Series(data=0.0, index=net.bus.index)
    bus_p_c = pd.Series(data=0.0, index=net.bus.index)
    bus_q_c = pd.Series(data=0.0, index=net.bus.index)
    ...
    assert allclose(
        net.res_bus_3ph.p_a_mw.values, bus_p_a.values, equal_nan=True, rtol=rtol
    )
    assert allclose(
        net.res_bus_3ph.q_a_mvar.values, bus_q_a.values, equal_nan=True, rtol=rtol
    )
    assert allclose(
        net.res_bus_3ph.p_b_mw.values, bus_p_b.values, equal_nan=True, rtol=rtol
    )
    assert allclose(
        net.res_bus_3ph.q_b_mvar.values, bus_q_b.values, equal_nan=True, rtol=rtol
    )
    assert allclose(
        net.res_bus_3ph.p_c_mw.values, bus_p_c.values, equal_nan=True, rtol=rtol
    )
    assert allclose(
        net.res_bus_3ph.q_c_mvar.values, bus_q_c.values, equal_nan=True, rtol=rtol
    )
...
def get_trafo_s_3ph(net, tf_index, side):
    s = np.array(
        [
            (
                net.res_trafo_3ph.at[tf_index, "p_" + ph + "_" + side + "_mw"]
                + 1j * net.res_trafo_3ph.at[tf_index, "q_" + ph + "_" + side + "_mvar"]
            )
            for ph in phases
        ]
    )
...
def get_trafo_v_3ph(net, tf_index, side):
    ...
    v = np.array(
        [
            (
                net.res_bus_3ph.at[bus_id, "vm_" + ph + "_pu"]
                * net.bus.vn_kv[bus_id]
                * np.exp(
                    1j * np.deg2rad(net.res_bus_3ph.at[bus_id, "va_" + ph + "_degree"])
                )
            )
            for ph in phases
        ]
...
def trafo_currents_consistent_3ph(net):
    ...
    for vector_group, trafo_df in net.trafo.groupby("vector_group"):
        ...
        for index, trafo in trafo_df.iterrows():
            ...
            if vector_group == "Dyn":
                check_dyn_transformer_currents(
                    i_hv, i_lv, ratio[tf_index], trafo["shift_degree"], rtol
                )

            if vector_group == "YNyn":
                check_ynyn_traformer_currents(
                    i_hv, i_lv, ratio[tf_index], trafo["shift_degree"], rtol
                )
