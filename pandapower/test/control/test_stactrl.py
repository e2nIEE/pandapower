...
def simple_test_net():
    ...
    create_sgen(net, 2, p_mw=2.0, sn_mva=10, name="sgen1")
...
def test_voltctrl():
    ...
    BinarySearchControl(
        net,
        name="BSC1",
        ctrl_in_service=True,
        output_element="sgen",
        output_variable="q_mvar",
        output_element_index=[0],
        output_element_in_service=[True],
...
def test_voltctrl_droop():
    ...
    bsc = BinarySearchControl(
        ...
        output_element_in_service=[True],
        output_values_distribution=[1],
        input_element="res_trafo",
        input_variable="q_hv_mvar",
        input_element_index=[0],
        set_point=1.02,
        voltage_ctrl=True,
        bus_idx=1,
        tol=tol,
    ...
    assert (
        abs(
            net.res_bus.loc[1, "vm_pu"]
            - (1.02 + net.res_trafo.loc[0, "q_hv_mvar"] / 40)
        )
        < tol
    )
...
def test_qctrl_droop():
    ...
    assert (
        abs(
            net.controller.object[0].input_sign[0] * net.res_line.loc[0, "q_from_mvar"]
            - (
                net.controller.object[1].q_set_mvar_bsc
                + (0.995 - net.res_bus.loc[1, "vm_pu"]) * 40
            )
        )
        < tol
    )
...
def test_stactrl_pf_import():
    ...
    print(
        "Controlled line, constQ = 0.5 MVar - q_from_mvar and q_to_mvar: \n",
        net.res_line.loc[0, "q_to_mvar"],
        "\t",
        net.res_line.loc[0, "q_from_mvar"],
    )
    print(
        "Controlled line, constQ = 0.5 MVar - q_from_mvar and q_to_mvar: \n",
        net.res_line.loc[2, "q_to_mvar"],
        "\t",
        net.res_line.loc[1, "q_from_mvar"],
    )
    ...
    print(
        ...
        "\t",
        net.res_line.loc[4, "q_from_mvar"],
    )
    print(
        "Input Measurement q_from_mvar and q_to_mvar, expected:\n 0.2442 MVar, -0.6215 MVar: \n",
        net.res_line.loc[5, "q_to_mvar"],
        "\t",
        net.res_line.loc[5, "q_from_mvar"],
    )
    print(
    ...
    print(
        "Controlled bus, set point = 1.03 pu, vm_pu: \n", net.res_bus.loc[84, "vm_pu"]
    )
    ...
    assert (
        ...
        - (
            -(
                1
                + (0.999 - net.res_bus.loc[91, "vm_pu"])
                * net.controller.object[2].q_droop_mvar
            )
        )
        < tol
    )
