...
def _vm_in_desired_area(net, lower_vm, higher_vm, side, idx=None, trafo_table="trafo"):
    ...
    return (
        lower_vm
        <= net.res_bus.vm_pu.loc[net[trafo_table].loc[idx, f"{side}_bus"].tolist()]
    ) & (
        net.res_bus.vm_pu.loc[net[trafo_table].loc[idx, f"{side}_bus"].tolist()]
        <= higher_vm
    )
...
def test_discrete_tap_control_lv():
    ...
    logger.info(
        ...
        % (
            net.res_bus.vm_pu[net.trafo.lv_bus].values.item(),
            net.trafo.tap_pos.values.item(),
        )
    )
...
def test_discrete_tap_control_vectorized_lv():
    ...
    runpp(net)
    assert not all(
        _vm_in_desired_area(net, 1.01, 1.03, "lv")
    ...
    assert not np.allclose(
        net_ref.trafo.tap_pos.values, 0
    )  # since there is something to do, the
    # tap_pos shouldn't be 0
    assert all(_vm_in_desired_area(net_ref, 1.01, 1.03, "lv"))

    # now create the vectorized version
    DiscreteTapControl(
        net, net.trafo.index.values, side="lv", vm_lower_pu=1.01, vm_upper_pu=1.03
    )
    runpp(net, run_control=True)
...
def test_discrete_tap_control_vectorized_hv():
    ...
    runpp(net)
    assert not all(
        _vm_in_desired_area(net, 1.01, 1.03, "hv")
    )  # there should be something
    # to do for the controllers
    ...
    # now create the vectorized version
    DiscreteTapControl(
        net, net.trafo.index, side="hv", vm_lower_pu=1.01, vm_upper_pu=1.03
    )
    runpp(net, run_control=True)
...
def test_continuous_tap_control_side_mv():
    ...
    create_load(net, 2, 5.0, 2.0)
    create_load(net, 3, 5.0, 2.0)
    set_user_pf_options(net, init="dc", calculate_voltage_angles=True)
    tol = 1e-4
    ...
    runpp(net)
    assert not any(
        _vm_in_desired_area(net, 1.01, 1.03, "mv", trafo_table="trafo3w")
    )  # there should be
    ...
    assert not any(
        _vm_in_desired_area(net_ref, 1.01, 1.03, "mv", trafo_table="trafo3w")
    )
