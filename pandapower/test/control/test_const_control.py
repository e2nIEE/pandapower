...
def test_write():
    ...
    ds = DFData(pd.DataFrame(data=[[0.0, 1.0, 2.0], [2.0, 3.0, 4.0]]))
    c1 = ConstControl(
        net, "sgen", "p_mw", element_index=[0, 1], profile_name=[0, 1], data_source=ds
    )
    create_sgen(net, 0, 0)
    c2 = ConstControl(
        net,
    ...
    for t in range(2):
        ...
        c2.control_step(net)
        assert np.all(
            net.sgen.p_mw.values == ds.df.loc[t].values * np.array([1, 1, 0.5])
        )
...
def test_write_to_object_attribute():
    ...
    c1 = ContinuousTapControl(net, 0, 1.0)
    c2 = ConstControl(
        net,
        "controller",
        "object.vm_set_pu",
