...
def test_continuous_tap_control_vectorized_lv():
    ...
    assert not np.allclose(
        net.res_trafo.vm_lv_pu.values, 1.02, atol=tol
    )  # there should be
    ...
    ContinuousTapControl(
        net, net.trafo.index.values, side="lv", vm_set_pu=1.02, tol=tol
    )
    runpp(net, run_control=True)
...
def test_continuous_tap_control_vectorized_hv():
    ...
    runpp(net)
    assert not np.allclose(
        net.res_trafo.vm_hv_pu.values, 1.02, atol=tol
    )  # there should be
    # something to do for the controllers
    ...
    # now create the vectorized version
    ContinuousTapControl(
        net, net.trafo.index.values, side="hv", vm_set_pu=1.02, tol=tol
    )
    runpp(net, run_control=True)
...
def test_continuous_tap_control_side_mv():
    ...
    runpp(net)
    assert not np.allclose(
        net.res_trafo3w.vm_mv_pu.values, 1.02, atol=tol
    )  # there should be
    # something to do for the controllers
...
def test_continuous_tap_control_reindexed():
    ...
    create_load(net, 240, 5.0, 2.0)
    create_load(net, 100, 5.0, 2.0)
    set_user_pf_options(net, init="dc", calculate_voltage_angles=True)
    tol = 1e-4
