...
#TODO TESTS MIT tap_step_percent_neg machen
def test_continuous_tap_control_lv_tap_step_percent_neg():
    # --- load system and run power flow
    net = simple_four_bus_system()
    set_user_pf_options(net, init='dc', calculate_voltage_angles=True)
    # --- initial tap data
    ...
    # todo: rewrite to not compare to hardcoded values
    tid = 0
    ContinuousTapControl(net, element_index=tid, vm_set_pu=0.99, side='lv')
    # DiscreteTapControl(net, tid=0, side='lv', vm_lower_pu=0.95, vm_upper_pu=0.99)

    logger.info("case1: low voltage")
    logger.info("before control: trafo voltage at low voltage bus is %f, tap position is %u"
                % (net.res_trafo.vm_lv_pu.at[tid], net.trafo.tap_pos.to_numpy()))

    # run control
    runpp(net, run_control=True)
    runpp(net)
    logger.info("after ContinuousTapControl: trafo voltage at low voltage bus is %f, tap position is %f"
                % (net.res_trafo.vm_lv_pu.values, net.trafo.tap_pos.to_numpy()))

    assert np.isclose(net.res_trafo.vm_lv_pu.at[tid], 0.99, atol=1e-3)
    assert np.isclose(net.trafo.tap_pos.to_numpy(), .528643)
    # increase voltage from 1.0 pu to 1.03 pu
    net.ext_grid.vm_pu = 1.03
    # switch back tap position
    net.trafo.at[0, 'tap_pos'] = 0
    ...
    assert np.isclose(net.trafo.tap_pos.to_numpy(), -2)
    # increase voltage from 1.0 pu to 1.03 pu
    net.ext_grid.vm_pu = 0.98
    # switch back tap position
    net.trafo.at[0, 'tap_pos'] = 0
    ...
    logger.info(
        ...
        % (net.res_trafo.vm_lv_pu.at[tid], net.trafo.tap_pos.to_numpy()))
    assert np.isclose(net.res_trafo.vm_lv_pu.at[tid], 0.99, atol=1e-3)
    assert np.isclose(net.trafo.tap_pos.to_numpy(), 1.077656)


def test_continuous_tap_control_hv_tap_step_percent_eg():
    # --- load system and run power flow
    net = simple_four_bus_system()
    # --- initial tap data
    net.trafo.tap_side = 'hv'
    ...
    assert np.isclose(net.res_trafo.vm_lv_pu.at[tid], 0.99, atol=1e-3)
    assert np.isclose(net.trafo.tap_pos.to_numpy(), 0.528643)
    ...
    logger.info(
        ...
        % (net.res_trafo.vm_lv_pu.at[tid], net.trafo.tap_pos.to_numpy()))
    assert np.isclose(net.res_trafo.vm_lv_pu.at[tid], 0.99, atol=1e-3)
    assert np.isclose(net.trafo.tap_pos.to_numpy(), -1.07765621)


def test_continuous_tap_control_vectorized_lv_tap_step_percent_eg():
    # --- load system and run power flow
    net = create_empty_network()
    create_buses(net, 6, 110)
    create_buses(net, 5, 20)
    ...
    # create with individual controllers for comparison
    for tid in net.trafo.index.values:
        ContinuousTapControl(net_ref, element_index=tid, side='lv', vm_set_pu=1.02, tol=tol)
    ...
    # now create the vectorized version
    ContinuousTapControl(net, element_index=net.trafo.index.values, side='lv', vm_set_pu=1.02, tol=tol)
    runpp(net, run_control=True)
...
def test_continuous_tap_control_vectorized_hv_tap_step_percent_eg():
    # --- load system and run power flow
    net = create_empty_network()
    create_buses(net, 6, 20)
    create_buses(net, 5, 110)
    ...
    # create with individual controllers for comparison
    for tid in net.trafo.index.values:
        ContinuousTapControl(net_ref, element_index=tid, side='hv', vm_set_pu=1.02, tol=tol)
    ...
    # now create the vectorized version
    ContinuousTapControl(net, element_index=net.trafo.index.values, side='hv', vm_set_pu=1.02, tol=tol)
    runpp(net, run_control=True)
