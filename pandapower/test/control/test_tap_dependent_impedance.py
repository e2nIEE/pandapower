...
def test_tap_dependent_impedance_control():
    ...
    characteristic_vk = Characteristic.from_points(
        net, ((0, 13.5), (10, 12.3), (20, 11.1))
    )
    characteristic_vkr = Characteristic.from_points(
        net, ((0, 0.52), (10, 0.5), (20, 0.53))
    )
    TapDependentImpedance(
        net, 0, characteristic_vk.index, output_variable="vk_percent", restore=False
    )
    TapDependentImpedance(
        net, 0, characteristic_vkr.index, output_variable="vkr_percent", restore=False
    )
...
def test_tap_dependent_impedance_restore():
    ...
    TapDependentImpedance(
        net, 0, characteristic_vk.index, output_variable="vk_percent", restore=True
    )
    TapDependentImpedance(
        net, 0, characteristic_vkr.index, output_variable="vkr_percent", restore=True
    )
...
def test_characteristic():
    ...
    c4 = SplineCharacteristic(
        net, x_points, y_spline, fill_value=(y_spline[0], y_spline[-1])
    )
    assert c4(2) == 4
...
def test_trafo_characteristic_table_diagnostic():
    ...
    create_transformer(
        net, hv_bus=b2, lv_bus=cb, std_type="0.25 MVA 20/0.4 kV", tap_pos=2
    )
    ...
    net["trafo_characteristic_table"] = pd.DataFrame(
        {
            ...
            "angle_deg": [0, 0, 0, 0, 0],
            "vk_percent": [5, 5.2, 6, 6.8, 7],
            "vkr_percent": [1.3, 1.4, 1.44, 1.5, 1.6],
