...
def test_oc_relay_dtoc():
    ...
    for k in range(6):
        OCRelay(
            net, switch_index=k, oc_relay_type="DTOC", time_settings=[0.07, 0.5, 0.3]
        )

    net_sc = create_sc_bus(net, sc_line_id=4, sc_fraction=0.5)
    calc_sc(net_sc, bus=max(net_sc.bus.index), branch_results=True)
...
def test_oc_relay_idtoc():
    ...
    for k in range(6):
        OCRelay(
            ...
            switch_index=k,
            oc_relay_type="IDTOC",
            time_settings=[0.07, 0.5, 0.3, 1, 0.5],
        )
    net_sc = create_sc_bus(net, sc_line_id=4, sc_fraction=0.5)
    calc_sc(net_sc, bus=max(net_sc.bus.index), branch_results=True)
    protection_results = calculate_protection_times(net_sc, scenario="sc")
...
def test_oc_relay_plots():
    ...
    for k in range(6):
        OCRelay(
            net,
            switch_index=k,
            oc_relay_type=oc_relay_type_list[k],
            time_settings=time_settings_list[k],
        )
...
def test_plot_tripped_grid_protection_device():
    ...
    plot_tripped_grid_protection_device(
        net_sc, protection_results, sc_bus=max(net_sc.bus.index), sc_location=0.5
    )
...
def oc_relay_net():
    ...
    create_lines(
        net,
        from_buses=[0, 1, 2, 1, 4, 5],
        to_buses=[1, 2, 3, 4, 5, 6],
        length_km=[2, 5, 4, 4, 0.5, 0.5],
