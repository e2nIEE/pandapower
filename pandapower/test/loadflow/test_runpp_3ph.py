...
def net():
    ...
    create_ext_grid(
        net, bus=1, vm_pu=1.0, s_sc_max_mva=5000, rx_max=0.1, r0x0_max=0.1, x0x_max=1.0
    )
    create_std_type(
        net,
        {
    ...
    create_asymmetric_load(
        net, 5, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5
    )
    return net
...
def check_it(net):
    bus_pp = np.abs(
        net.res_bus_3ph[["vm_a_pu", "vm_b_pu", "vm_c_pu"]][
            ~np.isnan(net.res_bus_3ph.vm_a_pu)
        ].values
    )
    bus_pf = np.abs(
        np.array(
            [[0.96742893, 1.01302766, 1.019784], [0.74957533, 1.09137945, 1.05124282]]
        )
    )
...
def test_2bus_network_isolated_net_part(net):
    ...
    create_line(net, from_bus=b1, to_bus=b2, length_km=50.0, std_type="example_type")
    create_asymmetric_load(
        net, b2, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5
    )
    add_zero_impedance_parameters(net)
    runpp_3ph_with_consistency_checks(net)
    assert net["converged"]
...
def test_2bus_network_singel_oos_bus(net):
    ...
    create_line(net, from_bus=5, to_bus=b1, length_km=10.0, std_type="example_type")
    create_asymmetric_load(
        net, b1, p_a_mw=-5, q_a_mvar=5, p_b_mw=-1, q_b_mvar=1.5, p_c_mw=-1, q_c_mvar=0.5
    )
    add_zero_impedance_parameters(net)
    runpp_3ph_with_consistency_checks(net)
    assert net["converged"]
...
@pytest.mark.parametrize(
    "recycle", [None, {"bus_pq": True, "Ybus": True}, {"bus_pq": True, "Ybus": False}]
)
def test_4bus_network(init, recycle):
    ...
    create_line(
        net, from_bus=busn, to_bus=busm, length_km=1.0, std_type="example_type3"
    )
    create_line(
        net, from_bus=busn, to_bus=busp, length_km=1.0, std_type="example_type3"
    )
    create_line_from_parameters(
        net,
        from_bus=busn,
    ...
    create_asymmetric_load(
        net, busk, p_a_mw=50, q_a_mvar=20, p_b_mw=80, q_b_mvar=60, p_c_mw=20, q_c_mvar=5
    )
    create_asymmetric_load(
        net, busm, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5
    )
    create_asymmetric_load(
        net, busp, p_a_mw=50, q_a_mvar=20, p_b_mw=60, q_b_mvar=20, p_c_mw=10, q_c_mvar=5
    )
...
def test_3ph_bus_mapping_order():
    ...
    create_line(
        net, b1, b2, 1.0, std_type="N2XRY 3x185sm 0.6/1kV", index=3, in_service=False
    )
    ...
    assert np.allclose(
        net.res_bus_3ph.vm_a_pu.values, net.res_bus.vm_pu.values, equal_nan=True
    )
...
def test_3ph_two_bus_line_powerfactory():
    ...
    bus_pf = np.abs(
        np.array(
            [
                [0.99939853552, 1.0013885141, 0.99921580141],
...
def check_bus_voltages(net, result, trafo_vector_group):
    ...
    assert np.allclose(
        result, res_vm_pu, atol=tolerances[trafo_vector_group], rtol=0
...
def test_2trafos():
    ...
    assert net["converged"]
    assert np.allclose(
        net.res_ext_grid_3ph.iloc[0].values, net.res_ext_grid_3ph.iloc[1].values
    )
...
def test_3ph_isolated_nodes():
    ...
    create_ext_grid(
        net, bus=busn, vm_pu=1.0, name="Grid Connection", s_sc_max_mva=5000, rx_max=0.1
    )
    net.ext_grid["r0x0_max"] = 0.1
    net.ext_grid["x0x_max"] = 1.0
    create_std_type(
    ...
    create_asymmetric_load(
        net, busk, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5
    )
    ...
    create_line(
        net, from_bus=busn, to_bus=busk, length_km=50.0, std_type="example_type"
    )
    create_line(
        net, from_bus=busl, to_bus=busk, length_km=50.0, std_type="example_type"
    )
    add_zero_impedance_parameters(net)
    runpp_3ph_with_consistency_checks(net)
    assert net["converged"]
