...
def _get_xward_result(net):
    ...
    for b, x_id in zip(
        net.xward.query("in_service").bus.values,
        net.xward.query("in_service").index.values,
    ):
        p_bus = ppc["bus"][net._pd2ppc_lookups["bus"][b], PD]
        p_shunt = (
            ppc["bus"][net._pd2ppc_lookups["bus"][b], VM] ** 2
            * net["xward"].at[x_id, "pz_mw"]
        )
...
def _get_slack_weights(net):
    slack_weights = np.r_[
        net.gen.query("in_service").slack_weight,
...
def assert_results_correct(net, tol=1e-8):
    ...
    assert np.allclose(
        input_p_mw
        - (injected_p_mw - consumed_p_mw - consumed_xward_p_mw) * slack_weights,
        result_p_mw,
        atol=tol,
        rtol=0,
...
def check_xward_results(net, tol=1e-9):
    ...
    assert np.allclose(
        xward_pq_res + xward_internal, net.res_xward.p_mw, atol=tol, rtol=0
    )
...
def test_numba():
    ...
    create_gen(net, 2, 200, 1.0, slack_weight=2)
...
def test_two_gens():
    ...
    assert np.allclose(
        net.res_bus.va_degree, [0, -1.5537971468, 0.08132969181], rtol=0, atol=1e-6
    )
    # check gen p and q
    assert np.allclose(
        net.res_gen.p_mw, [33.548233528, 67.096467056], rtol=0, atol=1e-6
    )
    assert np.allclose(
        net.res_gen.q_mvar, [43.22006789, 63.226937869], rtol=0, atol=1e-6
    )
    # check line currents
    assert np.allclose(
        net.res_line.i_ka,
        [1.6717271156, 2.5572842138, 0.16309292919],
        rtol=0,
        atol=1e-9,
    )
...
def test_three_gens():
    net = small_example_grid()
    create_gen(net, 1, 200, 1.0, slack_weight=2)
    create_gen(net, 2, 200, 1.0, slack_weight=2)
...
def test_xward_manually():
    ...
    slack_power = (
        net_1.res_gen.p_mw.at[0] - net_1.gen.p_mw.at[0]
    ) * 3  # factor 3 since gen has
    ...
    assert np.isclose(
        200 - net.load.at[1, "p_mw"], slack_power * 2 / 3, rtol=0, atol=1e-6
    )
    runpp(net)

    assert np.isclose(
        net_1.res_gen.at[0, "p_mw"], net.res_gen.at[0, "p_mw"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net_1.res_gen.at[0, "q_mvar"], net.res_gen.at[0, "q_mvar"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net_1.res_bus.at[2, "p_mw"],
        net.res_bus.at[2, "p_mw"] + net.res_line.at[3, "p_from_mw"],
        rtol=0,
        atol=1e-6,
    )
    assert np.allclose(
        net_1.res_bus.vm_pu, net.res_bus.loc[0:2, "vm_pu"], rtol=0, atol=1e-6
    )
    assert np.allclose(
        net_1.res_bus.va_degree, net.res_bus.loc[0:2, "va_degree"], rtol=0, atol=1e-6
    )
...
def test_gen_oos():
    net = small_example_grid()
    create_gen(net, 2, 200, 1.0, slack_weight=2, in_service=False)
...
def case9_simplified():
    net = create_empty_network()
    create_buses(net, 9, vn_kv=345.0)
...
def test_multivoltage_example_with_controller():
    ...
    load_disp = (
        sum([net[elm].p_mw.sum() for elm in ["load", "shunt"]])
    ...
    net2.xward.ps_mw = (
        net._ppc["bus"][net._pd2ppc_lookups["bus"][net.xward.bus], PD]
        - net.load.loc[net.load.bus.isin([34, 32]), "p_mw"].values
    )
    runpp(net2)
    ...
    if do_output:
        ...
        logger.info(
            "slack generator p results: %.6f    %.6f"
    ...
    expected_slack_power = (
        slack_power_without_controller
        - losses_without_controller
    ...
    assert np.isclose(
        net.res_gen.p_mw.at[0], expected_slack_power / 2 + gen_p_disp, atol=1e-5
    )
