...
def add_test_line(net):
    ...
    create_load(net, b2, p_mw=1.2, q_mvar=1.1)
...
def add_test_ext_grid_gen_switch(net):
    ...
    b2 = create_bus(net, vn_kv=vn_kv, zone=zone)
    b3 = create_bus(net, vn_kv=vn_kv, zone=zone)
    b4 = create_bus(net, vn_kv=vn_kv, zone=zone)
    b5 = create_bus(net, vn_kv=vn_kv, zone=zone)
    ...
    create_ext_grid(net, bus=b1, vm_pu=1.01)
    create_gen(net, bus=b5, vm_pu=1.015, p_mw=0.3)
    return net
...
def add_test_load_sgen(net):
    ...
    create_load(net, b2, p_mw=pl, q_mvar=ql)
    create_sgen(net, b2, p_mw=ps, q_mvar=qs)
    # adding out of serivce loads and sgens should not change the result
    create_load(net, b2, p_mw=pl, q_mvar=ql, in_service=False, index=get_free_id(net.load) + 1)
    create_sgen(net, b2, p_mw=ps, q_mvar=qs, in_service=False, index=get_free_id(net.sgen) + 1)
...
def add_test_load_sgen_split(net):
    ...
    qs = -0.1
    for _ in range(nr):
        create_load(net, b2, p_mw=pl, q_mvar=ql, scaling=1.0 / nr)
        create_sgen(net, b2, p_mw=ps, q_mvar=qs, scaling=1.0 / nr)
...
def add_test_trafo(net):
    ...
    create_load(net, b3, p_mw=0.2, q_mvar=0.05)
...
def add_test_ward(net):
    ...
    qs = 0.2
    # one shunt at a bus
    create_ward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs)
    # add out of service ward shuold not change the result
    create_ward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs, in_service=False, index=get_free_id(net.ward) + 1)
...
def add_test_xward(net):
    ...
    x_ohm = 70
    # one xward at a bus
    create_xward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs, vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm)
...
def add_test_xward_combination(net):
    ...
    # add out of service xward should not change the result
    create_xward(
        net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs, vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm, in_service=False
    )
...
def add_test_gen(net):
    ...
    b3 = create_bus(net, zone="test_gen", vn_kv=0.4)
    create_line_from_parameters(
...
def add_test_enforce_qlims(net):
    ...
    b3 = create_bus(net, zone="test_enforce_qlims", vn_kv=0.4)
    create_line_from_parameters(
        net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
...
def add_test_enforce_plims(net):
    ...
    b3 = create_bus(net, zone="test_enforce_plims", vn_kv=0.4)
    create_line_from_parameters(
        net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
    create_load(net, b3, p_mw=pl, q_mvar=ql)
...
def add_test_enforce_qlims_sgen(net):
    ...
    b3 = create_bus(net, zone="test_enforce_qlims_sgen", vn_kv=0.4)
    create_line_from_parameters(
        net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
    create_load(net, b3, p_mw=pl, q_mvar=ql)
...
def add_test_enforce_plims_sgen(net):
    ...
    b3 = create_bus(net, zone="test_enforce_plims_sgen", vn_kv=0.4)
    create_line_from_parameters(
        net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
    create_load(net, b3, p_mw=pl, q_mvar=ql)
...
def add_test_trafo3w(net):
    ...
    create_load(net, b4, p_mw=0.1, q_mvar=0)
...
def add_test_impedance(net):
    ...
    b3 = create_bus(net, vn_kv=220.0, zone="test_impedance")
    rij = 0.02
    xij = 0.01
    rji = 0.03
    xji = 0.005
    ...
    create_impedance(
        net, b2, b3, rft_pu=rij, xft_pu=xij, rtf_pu=rji, xtf_pu=xji, sn_mva=s, index=get_free_id(net.impedance) + 1
    )
    create_impedance(
        net,
        b2,
        ...
        xtf_pu=xji,
        sn_mva=s,
        index=get_free_id(net.impedance) + 1,
        in_service=False,
    )
    create_load(net, b3, p_mw=pl, q_mvar=ql)
...
def add_test_bus_bus_switch(net):
    ...
    b3 = create_bus(net, vn_kv=20.0, zone="test_bus_bus_switch")
    ...
    create_load(net, b2, p_mw=pl, q_mvar=ql)
    create_load(net, b3, p_mw=pl, q_mvar=ql, scaling=0.5)

    create_sgen(net, b2, p_mw=psg, q_mvar=qsg)
    create_sgen(net, b3, p_mw=psg, q_mvar=qsg, scaling=0.5)

    create_ward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs)
    create_ward(net, b3, pz_mw=0.5 * pz, qz_mvar=0.5 * qz, ps_mw=0.5 * ps, qs_mvar=0.5 * qs)
...
def add_test_oos_bus_with_is_element(net):
    ...
    # OOS buses
    b3 = create_bus(net, zone="test_oos_bus_with_is_element", vn_kv=0.4, in_service=False)
    b4 = create_bus(net, zone="test_oos_bus_with_is_element", vn_kv=0.4, in_service=False)
    b5 = create_bus(net, zone="test_oos_bus_with_is_element", vn_kv=0.4, in_service=False)

    create_line_from_parameters(
        net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
    create_line_from_parameters(
        net, b2, b4, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )
    create_line_from_parameters(
        net, b2, b5, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12, c_nf_per_km=300, max_i_ka=0.2, df=0.8
    )

    # in service elements
    create_load(net, b3, p_mw=pl, q_mvar=ql)
    create_gen(net, b4, p_mw=ps, vm_pu=vm_set_pu)
    create_sgen(net, b5, p_mw=ps, q_mvar=ql)
    create_ward(net, b3, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs)
    create_xward(
        net,
        b4,
        pz_mw=0.5 * pz,
    ...
    create_shunt(net, b5, q_mvar=-800, p_mw=0)
...
def add_test_shunt_split(net):
    ...
    qz = -1.200
    # one shunt at a bus
    create_shunt(net, b2, p_mw=pz / 2, q_mvar=qz / 2)
    create_shunt(net, b2, p_mw=pz / 2, q_mvar=qz / 2)
...
def add_test_two_open_switches_on_deactive_line(net):
    ...
    b3 = create_bus(net, vn_kv=20.0, zone="two_open_switches_on_deactive_line")
