...
def add_test_load_sgen(net):
    ...
    create_load(net, b2, p_mw=pl, q_mvar=ql, in_service=False,
                index=get_free_id(net.load) + 1)
    create_sgen(net, b2, p_mw=ps, q_mvar=qs, in_service=False,
                index=get_free_id(net.sgen) + 1)
...
def add_test_single_load_single_eg(net):
    b1 = create_bus(net, vn_kv=20., zone="test_single_load_single_eg")
...
def add_test_ward(net):
    ...
    create_ward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs, in_service=False,
                index=get_free_id(net.ward) + 1)
...
def add_test_xward(net):
    ...
    x_ohm = 70
    # one xward at a bus
    create_xward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs,
                 vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm)
    # add out of service xward should not change the result
    create_xward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs, vm_pu=vm_pu,
...
def add_test_gen(net):
    ...
    b3 = create_bus(net, zone="test_gen", vn_kv=.4)
...
def add_test_enforce_qlims(net):
    ...
    b3 = create_bus(net, zone="test_enforce_qlims", vn_kv=.4)
    create_line_from_parameters(net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12,
                                c_nf_per_km=300, max_i_ka=.2, df=.8)
...
def add_test_trafo3w(net):
    ...
    create_transformer3w_from_parameters(net, hv_bus=b2, mv_bus=b3, lv_bus=b4, vn_hv_kv=22,
...
def add_test_impedance(net):
    ...
    create_impedance(net, b2, b3, rft_pu=rij, xft_pu=xij, rtf_pu=rji, xtf_pu=xji,
                     sn_mva=s, index=get_free_id(net.impedance) + 1)
    create_impedance(net, b2, b3, rft_pu=rij, xft_pu=xij, rtf_pu=rji, xtf_pu=xji,
                     sn_mva=s, index=get_free_id(net.impedance) + 1, in_service=False)
    create_load(net, b3, p_mw=pl, q_mvar=ql)
...
def add_test_bus_bus_switch(net):
    ...
    create_xward(net, b3, pz_mw=0.5 * pz, qz_mvar=0.5 * qz, ps_mw=0.5 * ps, qs_mvar=0.5 * qs,
                 vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm)
    create_xward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs,
                 vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm)
...
def add_test_oos_bus_with_is_element(net):
    ...
    create_line_from_parameters(net, b2, b3, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12,
                                c_nf_per_km=300, max_i_ka=.2, df=.8)
    create_line_from_parameters(net, b2, b4, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12,
                                c_nf_per_km=300, max_i_ka=.2, df=.8)
    create_line_from_parameters(net, b2, b5, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12,
                                c_nf_per_km=300, max_i_ka=.2, df=.8)
    ...
    create_xward(net, b4, pz_mw=0.5 * pz, qz_mvar=0.5 * qz, ps_mw=0.5 * ps, qs_mvar=0.5 * qs,
                 vm_pu=vm_pu, x_ohm=x_ohm, r_ohm=r_ohm)
...
def add_test_shunt(net):
    ...
    qz = -1.2
    # one shunt at a bus
    create_shunt_as_capacitor(net, b2, q_mvar=1.2, loss_factor=0.1, vn_kv=22., step=2)
