...
def add_test_load_sgen(net):
    ...
    create_load(
        net, b2, p_mw=pl, q_mvar=ql, in_service=False, index=get_free_id(net.load) + 1
    )
    create_sgen(
        net, b2, p_mw=ps, q_mvar=qs, in_service=False, index=get_free_id(net.sgen) + 1
    )
...
def add_test_bus_bus_switch(net):
    ...
    create_ward(net, b2, pz_mw=pz, qz_mvar=qz, ps_mw=ps, qs_mvar=qs)
    create_ward(
        net, b3, pz_mw=0.5 * pz, qz_mvar=0.5 * qz, ps_mw=0.5 * ps, qs_mvar=0.5 * qs
    )
...
def add_test_oos_bus_with_is_element(net):
    ...
    # OOS buses
    b3 = create_bus(
        net, zone="test_oos_bus_with_is_element", vn_kv=0.4, in_service=False
    )
    b4 = create_bus(
        net, zone="test_oos_bus_with_is_element", vn_kv=0.4, in_service=False
    )
