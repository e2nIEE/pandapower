...
def test_hvdc_interconnect_with_dmr():
    ...
    create_load(net, bus=6, p_mw=100.0)
    create_load(net, bus=7, p_mw=150.0)

    # DC part
    create_bus_dc(net, 380.0, "A", geodata=(100, 10))  # 0
    create_bus_dc(net, 380.0, "B", geodata=(100, 0))  # 1
    create_bus_dc(net, 380.0, "C", geodata=(100, -10))  # 2

    create_bus_dc(net, 380.0, "D", geodata=(200, 10))  # 3
    create_bus_dc(net, 380.0, "E", geodata=(200, 0))  # 4
    create_bus_dc(net, 380.0, "F", geodata=(200, -10))  # 5

    dcp = create_line_dc_from_parameters(
        net, 0, 3, length_km=100, r_ohm_per_km=0.0212, max_i_ka=0.963
    )
    dcm = create_line_dc_from_parameters(
        net, 2, 5, length_km=100, r_ohm_per_km=0.0212, max_i_ka=0.963
    )
    # DMR Line
    dmr = create_line_dc_from_parameters(
        net, 1, 4, length_km=100, r_ohm_per_km=0.0212, max_i_ka=0.963, in_service=False
    )
...
def test_source_dc():
    ...
    create_bus(net, 380)
    create_ext_grid(net, bus=0, vm_pu=1.0)
    create_load(net, bus=1, p_mw=15.0)
    create_line_from_parameters(net, 0, 1, 1, 0.0487, 0.13823, 160, 0.664)

    create_bus_dc(net, 380.0, "A", geodata=(100, 10))  # 0
    create_bus_dc(net, 380.0, "B", geodata=(200, 10))  # 1
    create_line_dc_from_parameters(
        net, 0, 1, length_km=100, r_ohm_per_km=0.0212, max_i_ka=0.963
    )

    create_source_dc(net, bus_dc=0, vm_pu=0.5)
...
def test_b2b_vsc_shorted():
    ...
    create_line_dc_from_parameters(
        net, 2, 3, length_km=100, r_ohm_per_km=0.0212, max_i_ka=0.963
    )
