...
@pytest.mark.parametrize("vm_set_pu", [0.96, 1.0, 1.04])
def test_svc(vm_set_pu):
    ...
    create_svc(
        ...
        x_cvar_ohm=-10,
        set_vm_pu=vm_set_pu,
        thyristor_firing_angle_degree=90,
    )
    net2 = copy.deepcopy(net)
    runpp_with_consistency_checks(net)
    assert 90 <= net.res_svc.at[0, "thyristor_firing_angle_degree"] <= 180
    assert np.isclose(
        net.res_bus.at[3, "vm_pu"], net.svc.at[0, "set_vm_pu"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net.res_svc.q_mvar.at[0],
    ...
    runpp(net3)
    assert np.isclose(
        net3.res_bus.at[3, "vm_pu"], net.svc.at[0, "set_vm_pu"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net3.res_bus.at[3, "va_degree"],
        net.res_svc.at[0, "va_degree"],
        rtol=0,
        atol=1e-6,
    )
    assert np.isclose(
        net3.res_bus.at[3, "q_mvar"], net.res_bus.at[3, "q_mvar"], rtol=0, atol=1e-6
    )
    ...
    net2.svc.at[0, "thyristor_firing_angle_degree"] = (
        net.res_svc.thyristor_firing_angle_degree.at[0]
    )
    net2.svc.at[0, "controllable"] = False
    runpp(net2)
    assert np.isclose(
        net2.res_bus.at[3, "vm_pu"], net.svc.at[0, "set_vm_pu"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net2.res_bus.at[3, "q_mvar"], net.res_bus.at[3, "q_mvar"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net2.res_svc.at[0, "vm_pu"], net.res_svc.at[0, "vm_pu"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net2.res_svc.at[0, "va_degree"],
        net.res_svc.at[0, "va_degree"],
        rtol=0,
        atol=1e-6,
    )
    assert np.isclose(
        net2.res_svc.at[0, "q_mvar"], net.res_svc.at[0, "q_mvar"], rtol=0, atol=1e-6
    )
    assert np.isclose(
        net2.res_svc.at[0, "x_ohm"], net.res_svc.at[0, "x_ohm"], rtol=0, atol=1e-6
    )
...
def test_2_svcs(vm_set_pu):
    ...
    create_svc(
        ...
        x_cvar_ohm=-10,
        set_vm_pu=vm_set_pu,
        thyristor_firing_angle_degree=145,
        controllable=False,
    )
    create_svc(
        net1,
