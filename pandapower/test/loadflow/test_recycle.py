...
from pandapower.test.consistency_checks import (
    ...
    rundcpp_with_consistency_checks,
)
...
def test_recycle_pq(recycle_net):
    ...
    runpp_with_consistency_checks(
        net, recycle=dict(trafo=False, gen=False, bus_pq=True)
    )
    ...
    net["load"].at[0, "q_mvar"] = ql
    runpp_with_consistency_checks(
        net, recycle=dict(bus_pq=True, trafo=False, gen=False)
    )
...
def test_recycle_gen(recycle_net):
    ...
    runpp_with_consistency_checks(
        net, recycle=dict(trafo=False, bus_pq=False, gen=True)
    )
...
def test_recycle_trafo(recycle_net):
    ...
    runpp_with_consistency_checks(
        net, recycle=dict(trafo=True, bus_pq=False, gen=False)
    )
...
def test_result_index_unsorted():
    ...
    runpp(net_recycle)
    runpp(net_recycle, recycle=dict(trafo=True, bus_pq=True, gen=True))
    runpp(net)
...
def test_recycle_dc_trafo_shift(recycle_net):
    ...
    net["load"].at[0, "q_mvar"] = ql
    rundcpp_with_consistency_checks(
        net, recycle=dict(bus_pq=True, trafo=True, gen=False)
    )
    assert np.allclose(net.res_load.at[0, "p_mw"], pl)
    pl = 0.8
    ql = 0.55
    ...
    rundcpp(net2)
    assert np.allclose(
        net.res_bus.va_degree, net2.res_bus.va_degree, rtol=0, atol=1e-9, equal_nan=True
    )
