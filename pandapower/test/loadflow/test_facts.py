...
def compare_tcsc_impedance(net, net_ref, idx_tcsc, idx_impedance):
    ...
    net_ref.res_bus.loc[net.ssc.bus.values, "q_mvar"] += net_ref.res_impedance.loc[
        net_ref.impedance.query("name=='ssc'").index, "q_from_mvar"
    ].values
    bus_idx = net.bus.index.values
    for col in ("vm_pu", "va_degree", "p_mw", "q_mvar"):
        assert np.allclose(
            net.res_bus[col], net_ref.res_bus.loc[bus_idx, col], rtol=0, atol=1e-6
        )
    assert np.allclose(
        net.res_ext_grid.p_mw, net_ref.res_ext_grid.p_mw, rtol=0, atol=1e-6
    )
    assert np.allclose(
        net.res_ext_grid.q_mvar, net_ref.res_ext_grid.q_mvar, rtol=0, atol=1e-6
    )

    for col in "p_from_mw", "q_from_mvar", "p_to_mw", "q_to_mvar":
        assert np.allclose(
            net.res_tcsc.loc[idx_tcsc, col],
            net_ref.res_impedance.loc[idx_impedance, col],
            rtol=0,
            atol=1e-6,
        )
    assert np.allclose(
...
def compare_ssc_impedance_gen(net, net_ref, element="ssc"):
    ...
    net_ref.res_bus.loc[net[element].bus.values, "q_mvar"] += net_ref.res_impedance.loc[
        net_ref.impedance.query(f"name=='{element}'").index, "q_from_mvar"
    ].values
    assert np.allclose(
        np.abs(net._ppc["internal"]["V"][net.bus.index]),
        np.abs(net_ref._ppc["internal"]["V"][net.bus.index]),
        rtol=0,
        atol=1e-6,
    )
    ...
    for col in net.res_bus.columns:
        assert np.allclose(
            net.res_bus[col][net.bus.index],
            net_ref.res_bus[col][net.bus.index],
            rtol=0,
            atol=1e-6,
        )
    ...
    for i, j in zip(["vm_internal_pu", "va_internal_degree"], ["vm_pu", "va_degree"]):
        assert np.allclose(
            net[f"res_{element}"][i][in_service],
            net_ref.res_bus[j][len(net.bus) :].values[in_service],
            rtol=0,
            atol=1e-6,
        )

    assert np.allclose(
        np.abs(net._ppc["internal"]["V"][len(net.bus) :]),
        net_ref.res_bus.vm_pu[len(net.bus) :][in_service],
        rtol=0,
        atol=1e-6,
    )
    assert np.allclose(
        np.angle(net._ppc["internal"]["V"][len(net.bus) :], deg=True),
        net_ref.res_bus.va_degree[len(net.bus) :][in_service],
        rtol=0,
        atol=1e-6,
    )
    ...
    for col in net.res_ext_grid.columns:
        assert np.allclose(
            net.res_ext_grid[col][net.ext_grid.index],
            net_ref.res_ext_grid[col][net.ext_grid.index],
            rtol=0,
            atol=1e-6,
        )
    ...
    for col in net.res_line.columns:
        assert np.allclose(
            net.res_line[col][net.line.index],
            net_ref.res_line[col][net.line.index],
            rtol=0,
            atol=1e-6,
        )
...
def test_multiple_facts():
    ...
    net.tcsc.loc[1, "thyristor_firing_angle_degree"] = net.res_tcsc.loc[
        1, "thyristor_firing_angle_degree"
    ]
    ...
    compare_tcsc_impedance(
        net, net_ref, net.tcsc.index, net_ref.impedance.query("name=='tcsc'").index
    )

    create_svc(net, 3, 1, -10, 1.01, 90)
    runpp_with_consistency_checks(net)

    net.svc.at[0, "thyristor_firing_angle_degree"] = net.res_svc.loc[
        0, "thyristor_firing_angle_degree"
    ]
    ...
    compare_tcsc_impedance(
        net, net_ref, net.svc.index, net_ref.impedance.query("name=='svc'").index
    )
...
def test_svc_tcsc_case_study():
    ...
    create_svc(net, net.bus.loc[net.bus.name == "B7"].index.values[0], 1, -10, 1.0, 90)
