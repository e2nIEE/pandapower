...
def test_res_bus_vm():
    ...
    rundcpp(net)
    assert np.allclose(
        net.res_bus.loc[net.line.from_bus.values, "vm_pu"],
        net.res_line.vm_from_pu,
        equal_nan=True,
    )
    assert np.allclose(
        net.res_bus.loc[net.line.from_bus.values, "va_degree"],
        net.res_line.va_from_degree,
        equal_nan=True,
    )
    assert np.allclose(
        net.res_bus.loc[net.line.to_bus.values, "vm_pu"],
        net.res_line.vm_to_pu,
        equal_nan=True,
    )
    assert np.allclose(
        net.res_bus.loc[net.line.to_bus.values, "va_degree"],
        net.res_line.va_to_degree,
        equal_nan=True,
    )
...
def test_dc_after_ac():
    ...
    res_load = 1.0 * net.res_load["q_mvar"]
