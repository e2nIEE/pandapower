...
def test_group_element_index():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        # ! group_element_index()
        assert group_element_index(net, 0, "gen") == pd.Index([0, 1], dtype=np.int64)
        assert group_element_index(net, 0, "sgen") == pd.Index([2, 3], dtype=np.int64)
        assert group_element_index(net, 0, "dcline") == pd.Index([], dtype=np.int64)
...
def test_set_out_of_service():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        assert net.trafo.in_service == [False]*3 + [True]*2
