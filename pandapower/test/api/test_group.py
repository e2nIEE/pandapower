...
from pandapower import (
    ...
    drop_trafos,
    drop_buses,
    drop_elements_simple,
    drop_group_and_elements,
    return_group_as_net,
)
...
def nets_to_test_group():
    ...
    for reference_column, type_ in zip(reference_columns, types):
        ...
        idx0 = create_group_from_dict(
            net,
            {"gen": typed_list([0, 1], type_), "sgen": typed_list([2, 3], type_)},
            name="1st Group",
        ...
        idx1 = create_group(
            net,
            "trafo",
            [typed_list(net.trafo.loc[:2].index, type_)],
            name="Group of transformers",
            index=3,
...
def test_group_create():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        assert (
            len(
                net.group.loc[[idxs[0]]]
                .set_index("element_type")
                .at["gen", "element_index"]
            )
            == len(
                net.group.loc[[idxs[0]]]
                .set_index("element_type")
                .at["sgen", "element_index"]
            )
            == 2
        )
        assert (
            len(
                net.group.loc[[idxs[1]]]
                .set_index("element_type")
                .at["trafo", "element_index"]
            )
            == 3
        )
        ...
        try:
            # no xward in net
            create_group_from_dict(
                net, {"xward": typed_list([0], type_)}, reference_column=rc
            )
            assert False
        except UserWarning:
            pass
        ...
        try:
            # no sgen 100 in net
            create_group_from_dict(
                net, {"sgen": typed_list([3, 100], type_)}, reference_column=rc
            )
            assert False
        except UserWarning:
            pass
...
def test_group_element_index():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        # ! group_element_index()
        assert all(
            group_element_index(net, 0, "gen") == pd.Index([0, 1], dtype=np.int64)
        )
        assert all(
            group_element_index(net, 0, "sgen") == pd.Index([2, 3], dtype=np.int64)
        )
        assert all(
            group_element_index(net, 0, "dcline") == pd.Index([], dtype=np.int64)
        )


def test_groups_equal():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        idx_new = create_group(
            net,
            "trafo",
            [typed_list(net.trafo.loc[:2].index, type_)],
            name="Group of transformers",
...
def test_set_group_reference_column():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        assert groups_equal(
            net, 3, idx_new
        )  # ensure that we have an equal group idx_new to
...
def test_compare_group_elements():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        fail1 = create_group(
            net, ["trafo", "bus"], [net.trafo.loc[:2].index, [0]], name="xxx"
        )
...
def test_ensure_lists_in_group_element_column():
    ...
    no_nans = [1, 1]
    vals = [[np.nan, pd.Index([2, 3]), {0, 1, 2}], [(2, 3, 4), None, 3]]
...
def test_remove_not_existing_group_members():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        net.group.element_index.iat[-1] = net.group.element_index.iat[-1] + [
            8
        ]  # tafo 8 doesn't exist
        net.group = pd.concat(
            [
                net.group,
                pd.DataFrame(
                    {
                        ...
                        "element_index": [
                            typed_list([8], type_),  # impedances don't exist
                            [],  # empty list
                            typed_list([998, 999], type_),
                        ],  # gen 998, 999 don't exist
    ...
    net.group = pd.concat(
        [
            ...
            pd.DataFrame(
                ...
                index=[0, 0, 0],
                columns=["name", "element_type", "element_index"],
            ),
        ]
    )
    try:
...
def test_drop_element():
    ...
    drop_lines(net, [0])
    assert net.group.loc[[gr1]].set_index("element_type").at[
        "line", "element_index"
    ] == [1]
    drop_lines(net, [1])
    ...
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        for keep_everything_else in [False, True]:
            ...
            for elm in pp_elements():
                ...
                assert set(net2[elm].index) | set(net3[elm].index) == set(
                    net[elm].index
                )
...
def test_attach_to_group():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        try:
            # no xward in net
            attach_to_group(
                net, idxs[1], ["xward"], [typed_list([0], type_)], reference_columns=rc
            )
            assert False
        except UserWarning:
            pass
        ...
        attach_to_group(
            ...
            idxs[1],
            ["trafo", "line"],
            [typed_list([3], type_), typed_list([2], type_)],
            reference_columns=rc,
        )
        assert set(net.group.loc[[idxs[1]]].element_type.tolist()) == {
            "gen",
...
def test_detach_and_compare():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        copy_idx = create_group(
            net, et3, elm3, reference_columns=rc3, name="copy of group 3"
        )
...
def test_res_power():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        runpp(net)
        p_val = (
            net.res_trafo.pl_mw.loc[[0, 1, 2]].sum()
            - net.res_gen.p_mw.loc[[0, 1]].sum()
        ...
        for gr_idx in net.group.index:
            ...
            assert np.isclose(
                per_bus_out.sum().at["q_mvar"], group_res_q_mvar(net, gr_idx)
            )
            if gr_idx == 3:
...
def test_res_power_examples():
    ...
    expected = pd.DataFrame(
        [[2.953004, 1.328978], [0.0, 0.0], [-2.875066, -1.318864], [-0.02, 0.0]],
        index=pd.Index([1, 2, 3, 4], name="bus"),
        columns=["p_mw", "q_mvar"],
    )
...
def test_group_io():
    net = case24_ieee_rts()
    gr1 = create_group_from_dict(
        net, {"gen": [0, 1], "sgen": [2, 3], "load": [0]}, name="1st Group", index=2
    )
    gr2 = create_group_from_dict(
        net, {"trafo": net.trafo.index}, name="Group of transformers"
    )
...
def test_count_group_elements():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        pdt.assert_series_equal(
            count_group_elements(net, idxs[1]), pd.Series({"trafo": 3}, dtype=np.int64)
        )
...
def test_element_associated_groups():
    for net, type_, rc, idxs in zip(*nets_to_test_group()):
        ...
        assert (
            element_associated_groups(net, "gen", [0, 1, 2, 3], return_empties=False)
            == element_associated_groups(
                net, "gen", net.gen.index, return_empties=False
            )
            == {0: [0], 1: [0]}
        )
        assert element_associated_groups(net, "load", [0, 1]) == {0: [], 1: []}
        assert element_associated_groups(net, "trafo", [0, 1, 3]) == {
            0: [3],
            1: [3],
            3: [],
        }
...
def test_elements_connected_to_group():
    ...
    create_switches(
        net, [0, 0, 6], [0, 1, net.line.index[-1]], "l", closed=[True, False, False]
    )
    ...
    # create group
    index = create_group(
        net, ["bus", "line", "switch"], [[0], [net.line.index[-1]], [6, 7]]
    )
    ...
    assert elements_connected_to_group(
        net, index, ets_motor, include_empty_lists=True
    ) == {
        "bus": [10],
        "ext_grid": [0],
        "load": [0],
    ...
    assert elements_connected_to_group(
        ...
        index,
        element_types,
        find_buses_only_from_buses=True,
    ...
    assert elements_connected_to_group(
        net, index, element_types, respect_in_service=True
