...
def test_net_deepcopy():
    net = example_simple()
    net.line.at[0, "geo"] = geojson.LineString([(0.0, 1.0), (1.0, 2.0)])
    net.bus.at[0, "geo"] = geojson.Point((0.0, 1.0))
    ...
    if GEOPANDAS_INSTALLED:
        for tab in ("bus", "line"):
            net[f"{tab}_geodata"] = gpd.GeoDataFrame(
                net[tab].geo.dropna().apply(lambda x: x["coordinates"]),
                geometry=net[tab].geo.dropna(),
            )
        net1 = copy.deepcopy(net)
        ...
        assert isinstance(
            net1.line_geodata.geometry.iat[0], shapely.geometry.LineString
        )
...
def test_memory_leaks():
    ...
    assert (
        types_dict2[ContinuousTapControl] - types_dict1.get(ContinuousTapControl, 0)
        == 1
    )
...
def test_memory_leak_no_copy_demo():
    ...
    gc.collect()
    types_dict2 = get_gc_objects_dict()
    assert types_dict2[pandapowerNet] - types_dict1.get(pandapowerNet, 0) >= num - 1
    assert types_dict2[MemoryLeakDemo] - types_dict1.get(MemoryLeakDemo, 0) == num
...
def test_create_trafo_characteristics():
    ...
    assert all(
        col in net.trafo_characteristic_spline.columns
        for col in [
    ...
    assert isinstance(
        net.trafo_characteristic_spline.at[
    ...
    new_rows = pd.DataFrame(
        {
            "id_characteristic": [2, 2, 2, 2, 2],
            "step": [-8, -4, 0, 4, 8],
            "voltage_ratio": [1, 1, 1, 1, 1],
            "angle_deg": [0, 0, 0, 0, 0],
            "vk_hv_percent": [8.1, 9.5, 10, 11.1, 12.9],
            "vkr_hv_percent": [1.323, 1.325, 1.329, 1.331, 1.339],
            "vk_mv_percent": [8.1, 9.5, 10, 11.1, 12.9],
            "vkr_mv_percent": [1.323, 1.325, 1.329, 1.331, 1.339],
            "vk_lv_percent": [8.1, 9.5, 10, 11.1, 12.9],
    ...
    assert (
        net.trafo_characteristic_spline.loc[
            net.trafo3w["id_characteristic_table"].at[0]
        ]
        .notna()
        .sum()
        == 9
    ...
    assert isclose(
        net.trafo_characteristic_spline.at[
            net.trafo3w.id_characteristic_spline.at[0], "vk_hv_percent_characteristic"
        ](0).item(),
        10,
        rel_tol=1e-9,
    )
    assert isclose(
        net.trafo_characteristic_spline.at[
            net.trafo3w.id_characteristic_spline.at[0], "vk_lv_percent_characteristic"
        ](4).item(),
        11.1,
        rel_tol=1e-9,
    )
    ...
    assert isclose(
        net.trafo_characteristic_spline.at[
            net.trafo3w.id_characteristic_spline.at[0], "vkr_lv_percent_characteristic"
        ](8).item(),
        1.339,
        rel_tol=1e-9,
    )
...
def test_creation_of_shunt_characteristics():
    ...
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "q_mvar_characteristic"](1), -25.0
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "q_mvar_characteristic"](3), -75.0
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "q_mvar_characteristic"](5), -125.0
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](1), 1
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](3), 3
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](5), 5
    )
    ...
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](1), 6
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](3), 7
    )
    assert np.isclose(
        net.shunt_characteristic_spline.loc[0, "p_mw_characteristic"](5), 10
    )
...
def test_creation_of_q_capability_characteristics():
    ...
    net["q_capability_curve_table"] = pd.DataFrame(
        {
            ...
            "p_mw": [0.0, 50.0, 100.0, 125.0, 125.0],
            "q_min_mvar": [-100.0, -75.0, -50.0, -25.0, -10],
            "q_max_mvar": [150.0, 125.0, 75, 50.0, 10.0],
        }
    )
    ...
    assert all(
        col in net.q_capability_characteristic.columns
        for col in ["q_max_characteristic", "q_min_characteristic"]
    )
    assert isinstance(
        net.q_capability_characteristic.loc[
            net.gen.id_q_capability_characteristic.at[0], "q_max_characteristic"
        ],
        Characteristic,
    )
    assert isclose(
        net.q_capability_characteristic.at[
            net.gen.id_q_capability_characteristic.at[0], "q_max_characteristic"
        ](-2).item(),
        150,
        rel_tol=1e-9,
    ...
    new_rows = pd.DataFrame(
        {
            "id_q_capability_curve": [1, 1, 1, 1, 1],
            "p_mw": [0.0, 30.0, 50.0, 70.0, 130],
            "q_min_mvar": [-29.0, -27, -26.0, -25.0, -20.0],
            "q_max_mvar": [141.0, 141.0, 137.0, 134.0, 128.0],
        }
    )
    net["q_capability_curve_table"] = pd.concat(
        [net["q_capability_curve_table"], new_rows], ignore_index=True
    )
...
def test_geo_accessor_geojson():
    ...
    assert np.array_equal(
        net.line.geo.geojson._coords.at[l[0]], [[1, 1], [2, 2], [3, 3]]
    )
    ...
    assert set(net.line.geo.geojson.as_geo_obj.at[l[0]].keys()) == {
        "coordinates",
        "type",
    }
    assert set(net.line.geo.geojson.as_geo_obj.at[l[1]].keys()) == {
        "coordinates",
        "type",
    }
...
def test_geo_accessor_geopandas():
    ...
    radius_m = 2200
    circle_polygon = (
        gpd.GeoSeries([shapely.geometry.Point(reference_point)], crs=4326)
        .to_crs(epsg=31467)
        .buffer(radius_m)
        .to_crs(epsg=4326)
        .iloc[0]
    ...
    assert all(
        net.line[net.line.geo.geojson.within(circle_polygon)].index
        == [14, 17, 46, 47, 55, 116, 117, 118, 120, 121, 134]
    )

    line = shapely.geometry.LineString(
        [
            [7.8947079593416, 48.40549007606241],
    ...
    assert np.allclose(
        ...
        [7.74426069, 48.32845845, 7.93829196, 48.47484423],
    )
