...
def test_convert_format_pq_bus_meas():
    ...
    bus_p_meas = net.measurement.query(
        "element_type=='bus' and measurement_type=='p'"
    ).set_index("element", drop=True)
...
def test_convert_format_renaming_characteristic_table():
    ...
    net = from_json(
        os.path.join(folder, "example_3.1.2_capability.json"), convert=False
    )
    assert "q_capability_curve_table" in net
