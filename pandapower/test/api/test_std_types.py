...
def test_find_line_type():
    ...
    fitting_type = find_std_type_by_parameter(
        net, {"r_ohm_per_km": r + 0.05}, epsilon=0.06
    )
    assert len(fitting_type) == 1
    assert fitting_type[0] == name
...
def test_find_std_alternative():
    ...
    fitting_type = find_std_type_alternative(
        net, {"r_ohm_per_km": r + 0.05}, voltage_rating="LV", epsilon=0.06
    )
    assert len(fitting_type) == 1
    assert fitting_type[0] == name

    fitting_type = find_std_type_alternative(
        net, {"r_ohm_per_km": r + 0.07}, voltage_rating="LV", epsilon=0.06
    )
    assert len(fitting_type) == 0

    fitting_type = find_std_type_alternative(
        net, {"r_ohm_per_km": r + 0.07}, voltage_rating="MV", epsilon=0.06
    )
    assert len(fitting_type) == 0
...
def test_change_type_line():
    ...
    lid = create_line(net, b1, b2, 1.0, std_type=name1)
...
def test_parameter_from_std_type_line():
    ...
    b1 = create_bus(net, vn_kv=0.4)
    b2 = create_bus(net, vn_kv=0.4)
    lid1 = create_line(net, b1, b2, 1.0, std_type=name1)
    lid2 = create_line(net, b1, b2, 1.0, std_type=name2)
    lid3 = create_line_from_parameters(
        net,
        b1,
        b2,
        1.0,
    ...
    assert (
        net.line.endtemp_degree.at[lid1] == endtemp_fill
    )  # type1 one has not specified an endtemp
    assert net.line.endtemp_degree.at[lid2] == endtemp2  # type2 has specified endtemp
    assert (
        net.line.endtemp_degree.at[lid3] == endtemp_fill
    )  # line3 has no standard type

    net.line.at[lid3, "endtemp_degree"] = 10
    parameter_from_std_type(net, "endtemp_degree", fill=endtemp_fill)
    assert (
        net.line.endtemp_degree.at[lid3] == 10
    )  # check that existing values arent overwritten
...
def test_delete_std_type():
    ...
    assert trafo3w_types == set(net.std_types["trafo3w"].keys()) | {
        existing_trafo3w_std_type
    }
...
def test_rename_std_type():
    ...
    vn_kvs = [
        ...
        tr3w_std_type_params["vn_hv_kv"],
        tr3w_std_type_params["vn_mv_kv"],
        tr3w_std_type_params["vn_lv_kv"],
    ]
