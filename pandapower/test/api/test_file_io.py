...
def net_in(request):
    if request.param == 1:
        ...
        net.line.at[0, "geo"] = geojson.dumps(
            geojson.LineString([(1.1, 2.2), (3.3, 4.4)])
        )
        net.line.at[1, "geo"] = geojson.dumps(
            geojson.LineString([(5.5, 5.5), (6.6, 6.6), (7.7, 7.7)])
        )
        return net
...
def net_charactistics():
    return from_json(
        os.path.join(pp_dir, "test", "test_files", "from_excel_characteristics.json")
    )
...
@pytest.mark.skipif(
    not xlsxwriter_INSTALLED or not openpyxl_INSTALLED,
    reason=(
...
def test_excel_characteristics(net_charactistics, tmp_path):
    ...
    pd.testing.assert_frame_equal(
        net_charactistics["q_capability_characteristic"],
        net_out["q_capability_characteristic"],
        atol=1e-5,
    )
    for minmax in ["q_max_characteristic", "q_min_characteristic"]:
        net_1_ch = (
            net_charactistics["q_capability_characteristic"].loc[0, minmax].to_dict()
        )
...
def test_json_controller_none():
    try:
        from_json(
            os.path.join(
                pp_dir, "test", "test_files", "controller_containing_NoneNan.json"
            ),
            convert=False,
        )
    except:
        raise (
            UserWarning("empty net with controller containing Nan/None can't be loaded")
        )
...
def test_json(net_in, tmp_path):
    ...
    if GEOPANDAS_INSTALLED and SHAPELY_INSTALLED:
        ...
        net_geo["bus_geodata"] = gpd.GeoDataFrame(
            geometry=bus_geometry, crs="epsg:4326"
        )
        line_geometry = net_geo.line["geo"].dropna().apply(geojson.loads).apply(shape)
        net_geo["line_geodata"] = gpd.GeoDataFrame(
            geometry=line_geometry, crs="epsg:4326"
        )
...
@pytest.mark.skipif(
    ...
    reason=("cryptography is mandatory to encrypt json files, but is not installed."),
)
...
@pytest.mark.xfail(
    reason="For std_types, some dtypes are not returned correctly by sql. Therefore,"
...
def test_elements_to_deserialize_wo_keep(tmp_path):
    ...
    filename = os.path.abspath(str(tmp_path)) + "testfile.json"
    to_json(net, filename)
    net_select = from_json(
        filename,
        elements_to_deserialize=["bus", "load"],
...
def test_json_io_with_characteristics(net_in):
    ...
    assert np.isclose(
        net_out.characteristic.object.at[c1.index](0.5), c1(0.5), rtol=0, atol=1e-12
    )
    assert np.isclose(
        net_out.characteristic.object.at[c2.index](2.5), c2(2.5), rtol=0, atol=1e-12
    )
...
def test_json_generalized():
    general_net0 = pandapowerNet(
        pandapowerNet.create_dataframes(
            {
                # structure data
                "df1": {"col1": np.dtype(object), "col2": "f8"},
                "df2": {"col3": "bool", "col4": "i8"},
            }
        )
    )
    ...
    for general_in in [general_net0, general_net1]:
        out = from_json_string(
            to_json(general_in), empty_dict_like_object=pandapowerNet({})
        )
...
def test_json_multiindex_and_index_names():
    ...
    col2 = pd.MultiIndex.from_tuples(
        col_tuples, names=[7, 8]
    )  # ["7", "8"] is not possible since
    ...
    for idx, col in zip([idx1, idx2, idx3], [col1, col2, col3]):
        ...
        df_mi_mc = pd.DataFrame(
            np.arange(4 * 3).reshape((4, 3)), index=idx, columns=col
        )
        ...
        assert_frame_equal(
            input["b"], output["b"], check_dtype=False, check_column_type=False
        )
        assert_frame_equal(
            input["c"], output["c"], check_dtype=False, check_index_type=False
        )
        assert_frame_equal(
            input["d"],
            output["d"],
