    ...
    for bool_value in [True, False]:
        for check_function in report_methods.keys():
            diag_report = DiagnosticReports(
                net, diag_results, diag_errors, diag_params, compact_report=bool_value
            )
            report_check = None
            try:
...
class TestInvalidValues:
    ...
    def test_greater_equal_zero(
        self, test_net, diag_params, diag_errors, report_methods
    ):
        net = copy.deepcopy(test_net)
        check_function = "invalid_values"
    ...
    def test_pos_int(self, test_net, diag_params, diag_errors, report_methods):
        ...
        assert diag_results[check_function] == {
            ...
            "switch": [
                (0, "bus", "nan", "positive_integer"),
                (0, "element", -1.5, "positive_integer"),
            ],
            "trafo": [
                (0, "hv_bus", False, "positive_integer"),
...
def test_multiple_voltage_controlling_elements_per_bus(
    test_net, diag_params, diag_errors, report_methods
):
    net = copy.deepcopy(test_net)
...
def test_different_voltage_levels_connected(
    test_net, diag_params, diag_errors, report_methods
):
    net = copy.deepcopy(test_net)
...
def test_impedance_values_close_to_zero(
    test_net, diag_params, diag_errors, report_methods
):
    # line test
    net = copy.deepcopy(test_net)
    ...
    check_result = implausible_impedance_values(
        net, diag_params["min_r_ohm"], diag_params["min_x_ohm"], 100, 100
    )
    if check_result:
        diag_results = {check_function: check_result}
    else:
...
def test_nominal_voltages_dont_match(
    test_net, diag_params, diag_errors, report_methods
):
    net = copy.deepcopy(test_net)
    ...
    check_result = nominal_voltages_dont_match(
        net, diag_params["nom_voltage_tolerance"]
    )
    if check_result:
        diag_results = {check_function: check_result}
    else:
