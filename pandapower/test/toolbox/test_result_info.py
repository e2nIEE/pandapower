...
def test_opf_task():
    ...
    create_buses(
        net, 6, [10, 10, 10, 0.4, 7, 7], min_vm_pu=[0.9, 0.9, 0.88, 0.9, np.nan, np.nan]
    )
    idx_ext_grid = 1
    ...
    create_dcline(
        net, 4, 5, 0.3, 1e-4, 1e-2, 1.01, 1.02, min_q_from_mvar=-10, min_q_to_mvar=-10
    )
    ...
    assert sorted(out1["flexibilities"].keys()) == [
        i1 + i2 for i1 in ["P", "Q"] for i2 in ["dcline", "ext_grid", "gen", "storage"]
    ]
    ...
    out1["flexibilities"]["Pgen"].loc[0, "min"] = (
        out1["flexibilities"]["Pgen"].loc[0, "max"] - 1e-1
    )
    ...
    assert (
        sorted(out4["flexibilities_without_costs"].keys()) == expected_elm_without_cost
    )
...
def test_overloaded_lines():
    ...
    line1 = create_line(
        net, bus0, bus1, length_km=1, std_type="NA2XS2Y 1x95 RM/25 12/20 kV"
    )
...
def test_res_power_columns():
    ...
    assert (
        res_power_columns("line")
        == res_power_columns("line", side="from")
        == res_power_columns("line", side=0)
