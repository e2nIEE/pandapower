...
def test_next_bus():
    ...
    assert (
        next_bus(net, bus1, switch1, et="switch") == bus5
    )  # Switch with bus2bus connection
...
def test_get_connected_lines_at_bus():
    ...
    line0 = create_line(net, bus0, bus1, length_km=1.0, std_type="NAYY 4x50 SE")
    line1 = create_line(net, bus0, bus1, length_km=1.0, std_type="NAYY 4x50 SE")
    line2 = create_line(
        net, bus0, bus1, in_service=False, length_km=1.0, std_type="NAYY 4x50 SE"
    )
    line3 = create_line(net, bus0, bus1, length_km=1.0, std_type="NAYY 4x50 SE")

    create_switch(net, bus0, line0, "l")
    create_switch(net, bus0, line1, "l", closed=False)
    create_switch(net, bus0, line2, "l")

    lines = get_connected_elements(
        net, "line", bus0, respect_switches=False, respect_in_service=False
    )

    assert set(lines) == {line0, line1, line2, line3}

    lines = get_connected_elements(
        net, "line", bus0, respect_switches=True, respect_in_service=False
    )
    assert set(lines) == {line0, line2, line3}

    lines = get_connected_elements(
        net, "line", bus0, respect_switches=True, respect_in_service=True
    )
    assert set(lines) == {line0, line3}

    lines = get_connected_elements(
        net, "line", bus0, respect_switches=False, respect_in_service=True
    )
...
def test_get_connected_buses():
    ...
    trafo0 = create_transformer3w(
        net, hv_bus=bus0, mv_bus=bus1, lv_bus=bus2, std_type="63/25/38 MVA 110/20/10 kV"
    )
    trafo1 = create_transformer(
        net, hv_bus=bus2, lv_bus=bus3, std_type="0.4 MVA 10/0.4 kV"
    )
    line1 = create_line(
        net, from_bus=bus3, to_bus=bus4, length_km=20.1, std_type="24-AL1/4-ST1A 0.4"
    )
...
def test_get_connected_buses_at_switches():
    ...
    expected |= set(
        net.line.loc[net.switch.at[switches[1], "element"], ["from_bus", "to_bus"]]
    )
    expected |= set(
        net.trafo.loc[net.switch.at[switches[1], "element"], ["hv_bus", "lv_bus"]]
    )
...
def test_get_false_links():
    ...
    for fbus, tbus in zip([0, 1, 4, 6, 7], [1, 4, 6, 7, 3]):
        create_line(net, fbus, tbus, 2.0, "NA2XS2Y 1x185 RM/25 6/10 kV")
    # manipulate to not existing
    ...
    expected = {"gen": {1}, "line": {1, 4}, "measurement": {1, 3}, "poly_cost": {1, 2}}
...
def test_element_bus_tuples():
    ...
    assert {buses for (elm, buses) in ebts} == {
        "bus",
        "to_bus",
        "from_bus",
        "hv_bus",
    ...
    assert (
        len(element_bus_tuples(bus_elements=False, res_elements=True))
        > 1.5 * len(element_bus_tuples(bus_elements=False, res_elements=False))
        > 0
    )
...
def test_pp_elements():
    ...
    assert len(
        pp_elements(
            bus=False, other_elements=False, bus_elements=True, branch_elements=False
        )
