...
def __create_trafo3w(net, bus_sl, service: bool = True):
    ...
    bus0 = create_bus(net, vn_kv=0.4, in_service=service)
    create_switch(net, bus_sl, bus0, "b", not service)
    bus1 = create_bus(net, vn_kv=0.4, in_service=service)
    create_transformer(net, bus0, bus1, in_service=service, std_type="63 MVA 110/20 kV")
    bus2 = create_bus(net, vn_kv=0.4, in_service=service)
    create_line(
        net,
        bus1,
        bus2,
    ...
    create_load(net, bus2, p_mw=0.0, in_service=service)
    create_sgen(net, bus2, p_mw=0.0, in_service=service)
    bus3 = create_bus(net, vn_kv=0.4, in_service=service)
    bus4 = create_bus(net, vn_kv=0.4, in_service=service)
    create_transformer3w_from_parameters(
        net, bus2, bus3, bus4, 0.4, 0.4, 0.4, 100, 50, 50, 3, 3, 3, 1, 1, 1, 5, 1
    )
...
def test_drop_inactive_elements():
    for service in (False, True):
        ...
        bus_sl = create_bus(net, vn_kv=0.4, in_service=service)
        ...
        for element, table in net.items():
            ...
            try:
                if service and (
                    element == "ext_grid" or (element == "bus" and len(net.bus) == 1)
                ):
    ...
    bus0 = create_bus(net, vn_kv=0.4, in_service=True)
    create_ext_grid(net, bus0, in_service=True)
    bus1 = create_bus(net, vn_kv=0.4, in_service=False)
    create_line(
        net, bus0, bus1, length_km=1, in_service=False, std_type="149-AL1/24-ST1A 10.0"
    )
...
def test_drop_inactive_elements_with_missing_in_service_column(mock_logger):
    net = create_empty_network()
    bus_sl = create_bus(net, vn_kv=0.4)
    ...
    # drop them
    drop_inactive_elements(net)
    mock_logger.info.assert_called_with("Set 0 of 1 unsupplied buses out of service")
...
def test_merge_and_split_nets():
    ...
    assert np.array_equal(
        pd.concat(
            [
                net1.sgen.name.loc[net1.poly_cost.element],
                net2.sgen.name.loc[net2.poly_cost.element],
            ]
        ).values,
...
def test_merge_with_groups():
    ...
    assert (
        net2.bus.name.loc[group_element_index(net2, 0, "bus")].tolist()
        == net.bus.name.loc[group_element_index(net, 5, "bus")].tolist()
    )
    assert (
        net2.load.name.loc[group_element_index(net2, 0, "load")].tolist()
        == net.load.name.loc[group_element_index(net, 5, "load")].tolist()
    )
    assert (
        net2.trafo.name.loc[group_element_index(net2, 4, "trafo")].tolist()
    ...
    assert list(group_element_index(net, 5, "load")) == list(
        np.array([0, 3], dtype=np.int64) + net1.load.shape[0]
    )
    assert list(group_element_index(net, 4, "line")) == list(
        np.array([1, 3], dtype=np.int64) + net1.line.shape[0]
    )
...
def test_merge_with_characteristics():
    ...
    net2["trafo_characteristic_table"] = pd.DataFrame(
        {
            ...
            "voltage_ratio": [1, 1, 1, 1, 1],
            "angle_deg": [0, 0, 0, 0, 0],
            "vk_percent": [2.1, 3.1, 4.1, 5.1, 6.1],
            "vkr_percent": [1.3, 1.4, 1.5, 1.6, 1.7],
            "vk_hv_percent": np.nan,
            "vkr_hv_percent": np.nan,
            "vk_mv_percent": np.nan,
            "vkr_mv_percent": np.nan,
    ...
    # merge networks
    merged, lookup = merge_nets(
        net1, net2, validate=False, return_net2_reindex_lookup=True
    )
...
def test_select_subnet():
    ...
    same_net2 = select_subnet(
        net, net.bus.index, include_results=True, keep_everything_else=True
    )
...
def test_close_switch_at_line_with_two_open_switches():
    ...
    line1 = create_line(net, bus2, bus3, length_km=1.0, std_type="NAYY 4x50 SE")
    line2 = create_line(net, bus2, bus3, length_km=1.0, std_type="NAYY 4x50 SE")
    create_line(net, bus2, bus3, length_km=1.0, std_type="NAYY 4x50 SE")  # line3
...
def test_drop(net):
    replace_zero_branches_with_switches(
        net, elements=("line", "impedance"), drop_affected=True
    )
...
def test_drop_elements_at_buses():
    ...
    trafo1 = create_transformer(
        net, hv_bus=bus2, lv_bus=bus3, std_type="0.4 MVA 10/0.4 kV"
    )
    ...
    for b in net.bus.index.values:
        ...
        swt3w = set(
            net1.switch.loc[
                net1.switch.element.isin(cd.get("trafo3w", [1000]))
                & (net1.switch.et == "t3")
            ].index
        )
        swt = set(
            net1.switch.loc[
                net1.switch.element.isin(cd.get("trafo", [1000]))
                & (net1.switch.et == "t")
            ].index
        )
        swl = set(
            net1.switch.loc[
                net1.switch.element.isin(cd.get("line", [1000]))
                & (net1.switch.et == "l")
            ].index
        )
        sw = swt3w | swt | swl
...
def test_replace_ext_grid_gen():
    for i in range(2):
        ...
        if i == 0:
            ...
            replace_ext_grid_by_gen(
                net, [0], gen_indices=[4], cols_to_keep=["uuid", "max_p_mw"]
            )
    ...
    for i in range(2):
        ...
        if i == 0:
            ...
            replace_gen_by_sgen(
                ...
                [0, 1],
                sgen_indices=[4, 1],
                cols_to_keep=["max_p_mw"],
                add_cols_to_keep=["slack_weight"],
        ...
        if i == 0:
            ...
            replace_sgen_by_gen(
                net2, 1, gen_indices=[2], add_cols_to_keep=["slack_weight"]
            )
...
def test_replace_pq_elmtype():
    ...
    replace_pq_elmtype(
        net, "load", "sgen", old_indices=1, add_cols_to_keep=add_cols_to_keep
    )
    ...
    replace_pq_elmtype(
        net, "sgen", "storage", old_indices=0, add_cols_to_keep=add_cols_to_keep
    )
    ...
    net.poly_cost.element = net.poly_cost.element.astype(
        net_orig.poly_cost.dtypes["element"]
    )
...
def test_get_connected_elements_empty_in_service():
    ...
    conn = get_connected_elements_dict(
        net, [0], respect_switches=False, respect_in_service=True
    )
    assert conn == {"line": [0], "ext_grid": [0], "bus": [1]}
    conn = get_connected_elements_dict(
        net, [3, 4], respect_switches=False, respect_in_service=True
    )
...
def test_replace_ward_by_internal_elements():
    ...
    res_load_created, res_shunt_created = (
        copy.deepcopy(net.res_load),
        copy.deepcopy(net.res_shunt),
    )
    runpp(net)
    ...
    assert set(
        elm_change.loc[
            elm_change.index.difference(new_ets).difference(pd.Index(["ward"]))
        ]
    ) == {0}
...
def test_replace_xward_by_internal_elements():
    ...
    create_xward(
        net, 6, 6, 8, 10, 5, 0.009, 0.678, 1.03, in_service=False, name="xward_3"
    )
    ...
    assert set(
        elm_change.loc[
            elm_change.index.difference(new_ets).difference(pd.Index(["xward"]))
        ]
    ) == {0}
...
def test_repl_to_line_with_switch():
    ...
    for testindex in net.line.index:
        if net.line.in_service.loc[testindex]:
            ...
            REPL = create_line(
                net, from_bus=fbus, to_bus=tbus, length_km=len_, std_type=std
            )
            ...
            for bus in fbus, tbus:
                if (
                    bus
                    in net.switch[
                        ~net.switch.closed & (net.switch.element == testindex)
                    ].bus.values
                ):
                    create_switch(
                        net, bus=bus, element=REPL, closed=False, et="l", type="LBS"
                    )
            ...
            ploss_repl = (
                net.res_line.loc[REPL].p_from_mw - net.res_line.loc[REPL].p_to_mw
            )
            qloss_repl = (
                net.res_line.loc[REPL].q_from_mvar - net.res_line.loc[REPL].q_to_mvar
            )
            ...
            ploss_ne = (
                net.res_line.loc[testindex].p_from_mw
                - net.res_line.loc[testindex].p_to_mw
            ) + (
                net.res_line.loc[new_idx].p_from_mw - net.res_line.loc[new_idx].p_to_mw
            )
            qloss_ne = (
                net.res_line.loc[testindex].q_from_mvar
                - net.res_line.loc[testindex].q_to_mvar
            ) + (
                net.res_line.loc[new_idx].q_from_mvar
                - net.res_line.loc[new_idx].q_to_mvar
            )
...
def test_merge_same_bus_generation_plants():
    ...
    expected_no_of_plants = (
        sum([net[elm].shape[0] for elm in gen_elms]) - n_plants + len(dupl_buses)
    )
    ...
    assert np.isclose(
        net.ext_grid.p_disp_mw.at[0], 95.1 * 2
    )  # correct value sum (p_disp)
    assert np.isclose(net.gen.p_mw.at[0], 10 * 2 + 76 * 2)  # correct value sum (p_mw)
    assert np.isclose(
        net.gen.min_p_mw.at[0], 16 * 2 + 15.2
    )  # correct value sum (min_p_mw) (
    # 1x 15.2 has been removed above)
    assert np.isclose(
        net.gen.max_p_mw.at[0], 20 * 2 + 76 * 2
    )  # correct value sum (max_p_mw)
    assert np.isclose(
        net.gen.min_q_mvar.at[8], -10 - 16 * 5
    )  # correct value sum (min_q_mvar)
    ...
    assert np.isclose(
        net.sgen.q_mvar.at[idx_sgen22[0]], 20 + 21
    )  # correct value sum (q_mvar)
...
def test_set_isolated_areas_out_of_service():
    ...
    assert np.all(
        net.bus.loc[np.setdiff1d(net.bus.index, isolated_buses), "in_service"]
    )

    assert not np.any(net.line.loc[isolated_lines, "in_service"])
    assert np.all(
        net.line.loc[np.setdiff1d(net.line.index, isolated_lines), "in_service"]
    )
...
def test_drop_trafos_incorrect_table_names():
    ...
    bus1 = create_bus(net, vn_kv=0.4)
    create_transformer(net, bus0, bus1, std_type="63 MVA 110/20 kV")
    bus2 = create_bus(net, vn_kv=0.4)
    create_line(net, bus1, bus2, length_km=1, std_type="149-AL1/24-ST1A 10.0")
    create_load(net, bus2, p_mw=0.0)
    create_sgen(net, bus2, p_mw=0.0)
    bus3 = create_bus(net, vn_kv=0.4)
    bus4 = create_bus(net, vn_kv=0.4)
    create_transformer3w_from_parameters(
        net, bus2, bus3, bus4, 0.4, 0.4, 0.4, 100, 50, 50, 3, 3, 3, 1, 1, 1, 5, 1
...
def test_drop_elements_trafos():
    net = create_empty_network()
    bus_sl = create_bus(net, vn_kv=0.4, in_service=True)
...
def test_set_isolated_areas_out_of_service_marks_trafo_oos():
    net = create_empty_network()
    b_hv = create_bus(net, vn_kv=110.0)
    b_lv = create_bus(net, vn_kv=20.0)
