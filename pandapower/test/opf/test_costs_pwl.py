...
def test_cost_piecewise_linear_gen():
    ...
    assert net.res_cost == net.res_gen.p_kw.values/1.5
...
def test_cost_piecewise_linear_eg():
    ...
    pp.create_bus(net, max_vm_pu=vm_max, min_vm_pu=vm_min, vn_kv=10.)
    pp.create_bus(net, max_vm_pu=vm_max, min_vm_pu=vm_min, vn_kv=.4)
    pp.create_ext_grid(net, 1, max_p_kw=-5, min_p_kw=-150, max_q_kvar=50,
                  min_q_kvar=-50)
    pp.create_ext_grid(net, 0)
    pp.create_load(net, 1, p_kw=20, controllable=False)
    pp.create_line_from_parameters(net, 0, 1, 50, name="line2", r_ohm_per_km=0.876,
    ...
    with pytest.raises(ValueError):
        pp.create_piecewise_linear_cost(net, 0, "ext_grid", np.array([[0, 0], [1, 50], [2, 100]]))
    with pytest.raises(ValueError):
        pp.create_piecewise_linear_cost(net, 0, "ext_grid", np.array([[0, 0], [-1, 50], [-2, 100]]))
    with pytest.raises(ValueError):
        pp.create_piecewise_linear_cost(net, 0, "ext_grid", np.array([[-10, 0], [-200, 50], [-50, 100]]))

    pp.create_piecewise_linear_cost(net, 0, "ext_grid", np.array([[-150, 100], [-75, 50], [0, 0]]))
    # run OPF
    pp.runopp(net, cost_function="piecewise_linear", verbose=False)

    assert net["OPF_converged"]
    assert net.res_cost == net.res_ext_grid.p_kw.values / 1.5
...
def test_cost_piecewise_linear_load():
    ...
    pp.create_piecewise_linear_cost(net, 0, "load", np.array([[0, 0], [75, 50], [150, 0]]))
    # run OPF
    pp.runopp(net, cost_function="piecewise_linear",verbose=False)

    assert net["OPF_converged"]
    assert net.res_cost - net.res_load.p_kw.values/1.5 < 1e-5
