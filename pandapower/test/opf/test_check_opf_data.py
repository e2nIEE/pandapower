...
from pandapower.create import (
    create_bus, create_buses, create_transformer, create_line, create_ext_grid, create_dcline, create_load, create_gen,
    create_sgen
)
...
def _opf_net():
    ...
    create_dcline(net, 1, 3, -5, 1, 1, 1, 1.02, max_p_mw=0.01, min_q_from_mvar=-.005,
                  min_q_to_mvar=-0.005, max_q_from_mvar=.005, max_q_to_mvar=.005)
    create_load(net, bus=2, p_mw=0.015, controllable=True, max_p_mw=0.025, min_p_mw=0,
                max_q_mvar=0.005, min_q_mvar=-0.005)
    create_gen(net, bus=1, p_mw=0.002, vm_pu=1.002, controllable=True, max_p_mw=0, min_p_mw=-0.025,
               max_q_mvar=0.025, min_q_mvar=-0.025)
    create_sgen(net, bus=2, p_mw=0.001, controllable=True, max_p_mw=0.003, min_p_mw=0,
