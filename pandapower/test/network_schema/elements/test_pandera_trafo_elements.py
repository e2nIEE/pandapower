...
class TestTrafoDependencies:
    """Tests for group dependencies (tap, tap2, tdt, opf)"""
    ...
    def test_tap_group_complete_valid(self):
        ...
        net.trafo["tap_side"] = pd.Series(["lv"], dtype="string")
        net.trafo["tap_pos"] = 2.0
        net.trafo["tap_neutral"] = 0.0
        net.trafo["tap_step_percent"] = 1.25
        net.trafo["tap_step_degree"] = 0.0
    ...
    def test_tap_group_all_null_valid(self):
        ...
        net.trafo["tap_side"] = pd.Series([pd.NA], dtype=pd.StringDtype())
        net.trafo["tap_pos"] = float("nan")
        net.trafo["tap_neutral"] = float("nan")
        net.trafo["tap_step_percent"] = float("nan")
        net.trafo["tap_step_degree"] = float("nan")
    ...
    def test_tap_group_partial_missing_invalid(self, missing_column):
        ...
        if missing_column == "tap_side":
            ...
            net.trafo[missing_column] = float("nan")

        with pytest.raises(pa.errors.SchemaError):
            validate_network(net)

    @pytest.mark.parametrize("single_column", [
        "tap_pos", "tap_neutral", "tap_side", "tap_step_percent", "tap_step_degree"
    ])
    def test_tap_group_only_one_set_invalid(self, single_column):
        ...
        if single_column == "tap_side":
            net.trafo["tap_side"] = pd.Series(["hv"], dtype="string")
        elif single_column in ["tap_step_percent"]:
            net.trafo[single_column] = 1.25
        elif single_column in ["tap_step_degree"]:
            net.trafo[single_column] = 0.0
        else:
            net.trafo[single_column] = 1.0
    ...
    def test_tap_group_row_consistency_valid(self):
        ...
        # Add second row
        row2 = _create_valid_trafo_dataframe()
        net.trafo = pd.concat([net.trafo, row2], ignore_index=True)

        # Row 1: all tap values present
        # Row 2: all tap values NA
        net.trafo["tap_side"] = pd.Series(["hv", pd.NA], dtype=pd.StringDtype())
        net.trafo["tap_pos"] = [1.0, float("nan")]
        net.trafo["tap_neutral"] = [0.0, float("nan")]
        net.trafo["tap_step_percent"] = [1.25, float("nan")]
        net.trafo["tap_step_degree"] = [0.0, float("nan")]
    ...
    def test_tap_group_row_partial_invalid(self):
        ...
        net.trafo["tap_side"] = pd.Series(["hv", "lv"], dtype=pd.StringDtype())
        net.trafo["tap_pos"] = [1.0, float("nan")]
        net.trafo["tap_neutral"] = [0.0, float("nan")]
        net.trafo["tap_step_percent"] = [1.25, float("nan")]
        net.trafo["tap_step_degree"] = [0.0, float("nan")]
    ...
    def test_tap2_group_complete_valid(self):
        """Test: tap2 group with all columns present is valid"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()
    ...
    def test_tap2_group_all_null_valid(self):
        """Test: tap2 group columns can all be NA/NaN together"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()

        net.trafo["tap2_side"] = pd.Series([pd.NA], dtype=pd.StringDtype())
        net.trafo["tap2_pos"] = float("nan")
        net.trafo["tap2_neutral"] = float("nan")
        net.trafo["tap2_step_percent"] = float("nan")
        net.trafo["tap2_step_degree"] = float("nan")
    ...
    def test_tap2_group_partial_missing_invalid(self, missing_column):
        """Test: tap2 group must be complete - partial values are invalid"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()

        # Set all tap2 columns
        net.trafo["tap2_side"] = pd.Series(["lv"], dtype="string")
        net.trafo["tap2_pos"] = 0.0
        net.trafo["tap2_neutral"] = 0.0
        net.trafo["tap2_step_percent"] = 1.0
    ...
    def test_tap2_group_only_one_set_invalid(self, single_column):
        """Test: tap2 group must be complete - only one column set is invalid"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()

        # Set only one tap2 column
        if single_column == "tap2_side":
            net.trafo["tap2_side"] = pd.Series(["lv"], dtype="string")
        elif single_column == "tap2_step_percent":
            net.trafo[single_column] = 1.0
        elif single_column == "tap2_step_degree":
            net.trafo[single_column] = 0.0
        else:
            net.trafo[single_column] = 0.0
    ...
    def test_tap2_group_row_consistency_valid(self):
        """Test: tap2 group - each row must have all values or all NA"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()
        ...
        # Row 1: all tap2 values present, Row 2: all NA
        net.trafo["tap2_side"] = pd.Series(["lv", pd.NA], dtype=pd.StringDtype())
        net.trafo["tap2_pos"] = [0.0, float("nan")]
        net.trafo["tap2_neutral"] = [0.0, float("nan")]
        net.trafo["tap2_step_percent"] = [1.0, float("nan")]
        net.trafo["tap2_step_degree"] = [0.0, float("nan")]

        validate_network(net)

    def test_tap2_group_row_partial_invalid(self):
        """Test: tap2 group - partial values in a row should fail"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()
        ...
        # Row 1: complete, Row 2: partial (only tap2_side set)
        net.trafo["tap2_side"] = pd.Series(["lv", "hv"], dtype=pd.StringDtype())
        net.trafo["tap2_pos"] = [0.0, float("nan")]
        net.trafo["tap2_neutral"] = [0.0, float("nan")]
        net.trafo["tap2_step_percent"] = [1.0, float("nan")]
        net.trafo["tap2_step_degree"] = [0.0, float("nan")]
    ...
    def test_tdt_group_all_na_valid(self):
        ...
        net.trafo["tap_dependency_table"] = pd.Series([pd.NA], dtype=pd.BooleanDtype())
        net.trafo["id_characteristic_table"] = pd.Series([pd.NA], dtype="Int64")
    ...
    def test_tdt_group_only_id_characteristic_table_set_invalid(self):
        """Test: TDT group - only id_characteristic_table set is invalid"""
        net = _create_net_with_buses()
        net.trafo = _create_valid_trafo_dataframe()
    ...
    def test_tdt_group_row_consistency_valid(self):
        ...
        net.trafo["tap_dependency_table"] = pd.Series([True, pd.NA], dtype=pd.BooleanDtype())
        net.trafo["id_characteristic_table"] = pd.Series([0, pd.NA], dtype="Int64")
    ...
    def test_tdt_group_row_partial_invalid(self):
        ...
        net.trafo["tap_dependency_table"] = pd.Series([True, True], dtype=pd.BooleanDtype())
        net.trafo["id_characteristic_table"] = pd.Series([0, pd.NA], dtype="Int64")
