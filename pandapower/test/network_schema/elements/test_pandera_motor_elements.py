...
class TestMotorScGroupFields:
    ...
    def test_sc_group_all_valid_values(self):
        """Test: sc group with all valid values is accepted"""
        net = create_empty_network()
        b0 = create_bus(net, 0.4)
    ...
    def test_valid_sc_group_values(self, parameter, valid_value): #TODO sc schema marked as required?
        """Test: valid sc group values are accepted when group is complete"""
        net = create_empty_network()
        b0 = create_bus(net, 0.4)
    ...
    def test_invalid_sc_group_values(self, parameter, invalid_value):
        """Test: invalid sc group values are rejected"""
        net = create_empty_network()
        b0 = create_bus(net, 0.4)
    ...
    def test_mixed_rows_sc_group_consistency(self):
        """Test: Multiple rows where each row has consistent sc group (all or nothing)"""
        net = create_empty_network()
        b0 = create_bus(net, 0.4)
