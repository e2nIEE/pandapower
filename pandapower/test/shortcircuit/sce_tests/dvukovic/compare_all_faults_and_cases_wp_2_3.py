            ...
            for case in cases:
                ...
                try:
                    calc_sc(net, fault=fault, case=case, branch_results=branch, ip=False,
                            r_fault_ohm=r_fault_ohm, x_fault_ohm=x_fault_ohm, bus=fault_location,
                ...
                for element in compare_ids:
                    for column in result_df.columns:
                        ...
                        try:
                            ...
                            pf_val = \
                                modified_pf_results.loc[
                                    modified_pf_results[element_id_column] == element, column].values[0]
...
def get_result_dfs(net_name, fault_location, gen_active=False):
    ...
    # Note: grounding/earthing configuration removed (ignored)

    # bus
    excel_file = f"wp_2.3/{net_name}_pf_sc_results_{fault_location}_bus{gen_loc}.xlsx"
    diff_df = compare_sc_results(net, os.path.join(result_files_path, excel_file), fault_location=fault_location)

    # branch
    excel_file = f"wp_2.3/{net_name}_pf_sc_results_{fault_location}_branch{gen_loc}.xlsx"
...
def generate_summary_tables(net_names, fault_locations, detailed=False, gen_active=False):
    bus_summary = []
    branch_summary = []
    ...
    for net_name, fault_location in tqdm(combinations, desc="generate_summary", unit="grid"):
        try:
            diff_df, diff_df_branch = get_result_dfs(net_name, fault_location, gen_active=gen_active)
...
if __name__ == "__main__":
    ## all net names
    testfiles_gen_path = os.path.join('../test_grids', 'wp_2.3')
    testfiles_gen_path = os.path.join(pp_dir, 'test', 'shortcircuit', 'sce_tests', 'test_grids', 'wp_2.3')
    ...
    net_name = '4_five_bus_radial_grid_1ph_yyn_MP'  # possible net_name in net_names and net_names_gen
    ...
    diff_df, diff_df_branch, net = get_result_dfs(net_name, fault_location, gen_active=gen_active)
