...
def test_wp23(net_name, fault, case, fault_values, lv_tol_percent, fault_location_bus, is_branch):
    ...
    test_info = {
        ...
        "fault_location_bus": fault_location_bus,
        "is_branch": is_branch
    }
    ...
    try:
        ...
        test_info["error"] = None

    except Exception as e:
        test_info["status"] = "failed"
        test_info["error"] = str(e)
        test_info["error_type"] = type(e).__name__
        save_test_result(test_info)
        raise

    save_test_result(test_info)


def save_test_result(test_info):
    file_path = Path('test_results.json')
