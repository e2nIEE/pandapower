...
def iec_60909_4():
    ...
    b4 = create_bus(net, vn_kv=110.0)
    b5 = create_bus(net, vn_kv=110.0)
    b6 = create_bus(net, vn_kv=10.0)
    b7 = create_bus(net, vn_kv=10.0)
    ...
    # t1 = create_transformer_from_parameters(net, b4, HG1, sn_mva=150,
    ...
    # # Add gen 3
    # create_gen(net, b6, p_mw=0.9 * 100, vn_kv=10.5,
    ...
    create_transformer3w_from_parameters(
        ...
        vk0_mv_percent=6.299627,
        vkr0_mv_percent=0.03714286,
        vk0_lv_percent=6.299627,
        vkr0_lv_percent=0.03714286,
        vector_group="YNyd",
...
def iec_60909_4_small(with_xward=False):
    ...
    if with_xward:
        ...
        create_xward(net, b5, 1, 0, 242, -484, 10, 20, 1)
...
def iec_60909_4_2gen():
    ...
    b3 = create_bus(net, vn_kv=110.0)
    b4 = create_bus(net, vn_kv=110.0)
    HG1 = create_bus(net, vn_kv=20.0)
    HG2 = create_bus(net, vn_kv=10.0)
...
def test_iec_60909_4_3ph():
    ...
    assert np.allclose(
        net.res_bus_sc.ip_ka.values[:8], np.array(ip_standard_kappa_c), atol=1e-3
    )
...
def test_iec_60909_4_3ph_min():
    ...
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values[:10], np.array(ikss_min), atol=1e-3
    )
...
def test_iec_60909_4_1ph():
    ...
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values[:10], np.array(ikss), atol=1e-4
