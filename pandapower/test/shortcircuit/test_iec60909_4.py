...
def wind_park_grid():
    ...
    pp.create_ext_grid(net, 0, 1, s_sc_max_mva=10.5 * 110 * np.sqrt(3), rx_max=0.1)

    pp.create_transformer_from_parameters(net, 0, 1, 31.5, 110, 20, 0.6, 12, 0, 0)

    pp.create_line_from_parameters(net, 1, 2, 0.1, 0.0681, 0.102, 0, 1e3, 'L1', parallel=2)

    from_buses = np.array([3, 4, 3, 6, 7, 7, 3, 10, 11, 11, 12]) - 1
    to_buses = np.array([4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 13]) - 1
    length_km = [1.1, 0.55, 0.79, 0.17, 0.4, 0.55, 0.95, 0.24, 0.29, 0.15, 0.495]
    pp.create_lines_from_parameters(net, from_buses, to_buses, length_km, 0.211, 0.122, 0, 1e3)

    sgen_buses = np.array([4, 5, 6, 8, 9, 10, 12, 13, 3, 14]) - 1
