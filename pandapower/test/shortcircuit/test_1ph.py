...
def add_network(net, vector_group):
    ...
    create_ext_grid(
        net, b1, s_sc_max_mva=100, s_sc_min_mva=100, rx_min=0.35, rx_max=0.35
    )
    ...
    create_line(
        net, b3, b4, length_km=15, std_type="unsymmetric_line_type", in_service=False
    )
    ...
    t1 = create_transformer(
        net, b1, b2, std_type=vector_group, parallel=2, index=get_free_id(net.trafo) + 1
    )
...
def test_1ph_shortcircuit():
    ...
    for vc, result in results.items():
        ...
        try:
            ...
            raise UserWarning(
                f"{str(e)}: Did not converge after adding transformer with vector group {vc}"
            )
...
def test_1ph_shortcircuit_3w():
    ...
    for vg, result in results.items():
        ...
        for bus in net2.bus.index.values:
            ...
            assert np.allclose(
                net.res_bus_sc.ikss_ka.at[bus],
                net2.res_bus_sc.ikss_ka.at[bus],
                rtol=0,
                atol=1e-9,
            )
...
def test_iec60909_example_4():
    ...
    for inv_y in (False, True):
        ...
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "Q"].ikss_ka.values[0], 10.05957231
        )
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "T2LV"].ikss_ka.values[0], 34.467353142
        )
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "F1"].ikss_ka.values[0], 35.53066312
        )
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "F2"].ikss_ka.values[0], 34.89135137
        )
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "F3"].ikss_ka.values[0], 5.0321033105
        )
        assert np.isclose(
            net.res_bus_sc[net.bus.name == "Cable/Line IC"].ikss_ka.values[0],
            16.362586813,
        )
...
def test_iec60909_example_4_bus_selection():
    ...
    for inv_y in (False, True):
        ...
        assert np.isclose(
            net.res_bus_sc.at[net.bus[net.bus.name == "F1"].index[0], "ikss_ka"],
            35.53066312,
        )
        assert np.isclose(
            net.res_bus_sc.at[net.bus[net.bus.name == "F2"].index[0], "ikss_ka"],
            34.89135137,
        )
...
def single_3w_trafo_grid(vector_group, sn_mva=123):
    ...
    b1 = create_bus(net, vn_kv=380.0, geodata=(1, 1))
    b2 = create_bus(net, vn_kv=110.0, geodata=(0, 1))
    b3 = create_bus(net, vn_kv=30.0, geodata=(1, 0))
    create_ext_grid(
        net,
        b1,
        s_sc_max_mva=1000,
...
def iec_60909_4_small(n_t3=1, num_earth=1, with_gen=False):
    ...
    b1 = create_bus(net, vn_kv=380.0)
    b2 = create_bus(net, vn_kv=110.0)
    b3 = create_bus(net, vn_kv=110.0)
    b5 = create_bus(net, vn_kv=110.0)
    b8 = create_bus(net, vn_kv=30.0)
    ...
    if with_gen:
        t1 = create_transformer_from_parameters(
            net,
            b3,
            hg2,
            sn_mva=100,
...
def vde_232():
    ...
    create_ext_grid(
        net,
        0,
        s_sc_max_mva=13.61213 * 110 * np.sqrt(3),
        rx_max=0.20328,
        x0x_max=3.47927,
...
def test_iec60909_example_4_two_trafo3w():
    ...
    calc_sc(net, fault="1ph")
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values[:4], np.array(ikss_pf_2t3), atol=1e-4
    )
...
def test_iec60909_example_4_two_trafo3w_two_earth():
    ...
    calc_sc(net, fault="1ph", case="max")
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values[:4], np.array(ikss_pf_max), atol=1e-4
    )

    calc_sc(net, fault="1ph", case="min")
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values[:4], np.array(ikss_pf_min), atol=1e-4
    )
...
def test_1ph_sn_mva_ext_grid():
    ...
    for var in res.keys():
        assert np.allclose(
            net1.res_bus_sc[var], net2.res_bus_sc[var], atol=1e-6, rtol=0
        )
...
def test_trafo():
    ...
    for vc in results.keys():
        ...
        calc_sc(net, fault="1ph", case="max")
        assert np.allclose(net.res_bus_sc, res, rtol=0, atol=1e-6), (
            f"failed for vector group {vc}"
        )
        assert np.allclose(net.res_bus_sc.ikss_ka, results[vc], rtol=0, atol=1e-6), (
            f"{vc}: inconsistent results"
        )
...
def test_sc_1ph_impedance():
    ...
    assert np.allclose(
        net.res_bus_sc.xk0_ohm, [13.243945, 30.821973], rtol=0, atol=1e-6
    )
    assert np.allclose(
        net.res_bus_sc.rk_ohm, [1.3243945, 62.1554916], rtol=0, atol=1e-5
    )
    assert np.allclose(
        net.res_bus_sc.xk_ohm, [13.2439445, 137.5549268], rtol=0, atol=1e-5
    )
