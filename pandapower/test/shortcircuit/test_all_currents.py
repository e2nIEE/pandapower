...
def net_transformer_simple_3():
    ...
    create_ext_grid(net, 0, s_sc_max_mva=100., s_sc_min_mva=40., rx_min=0.1, rx_max=0.1)
    # 30/10
    create_transformers_from_parameters(net, [0, 1], [2, 3], sn_mva=10, vn_hv_kv=30., vn_lv_kv=10, vk_percent=6.,
                                        vkr_percent=0.5, i0_percent=0.1, pfe_kw=10, shift_degree=0.0, tap_side="hv",
                                        tap_neutral=0, tap_min=-2, tap_max=2, tap_pos=0, tap_step_percent=2.5,
                                        parallel=1)
    # 10/0.4
    create_transformer_from_parameters(net, 4, 5, sn_mva=0.4, vn_hv_kv=10., vn_lv_kv=0.4, vk_percent=6.,
...
def test_all_currents_sgen():
    ...
    calc_sc(net, case="max", ip=True, ith=True, branch_results=True, return_all_currents=True)
    assert np.allclose(net.res_line_sc.ikss_ka.values,
                       np.array([0.01259673, 0.49593036, 0.48628848, 0., 0., 0.49888962]), atol=1e-5)
    assert np.allclose(net.res_line_sc.ip_ka.values, np.array([0.01781447, 0.92787447, 0.90729584, 0., 0., 0.92511655]),
                       atol=1e-5)
    assert np.allclose(net.res_line_sc.ith_ka.values, np.array([0.01265116, 0.4981196, 0.48841266, 0., 0., 0.50106884]),
                       atol=1e-5)

    calc_sc(net, case="min", ip=True, ith=True, branch_results=True, return_all_currents=True)
    assert np.allclose(net.res_line_sc.ikss_ka.values, np.array([0., 0.3989686, 0.3919381, 0., 0., 0.3919381]),
                       atol=1e-5)
    assert np.allclose(net.res_line_sc.ip_ka.values, np.array([0., 0.74438751, 0.728265, 0., 0., 0.728265]), atol=1e-5)
...
def test_all_currents_with_oos_elements():
    ...
    assert np.allclose(net.res_line_sc.ikss_ka.loc[[(0, 0), (0, 1)]].values, np.array([0.01259673, 0.49593036]),
                       atol=1e-5)
    assert np.allclose(net.res_line_sc.ikss_ka.loc[[(0, 2), (1, 0), (1, 1), (1, 2)]].values, 0, atol=1e-10)

    calc_sc(net, case="min", branch_results=True, return_all_currents=True)
    assert np.allclose(net.res_line_sc.ikss_ka.loc[[(0, 0), (0, 1)]].values, np.array([0, 0.3989686]),
                       atol=1e-5)  # sgen does not contribute in case == "min"
...
def test_branch_all_currents_trafo_simple():
    ...
    calc_sc(net, case='max', ip=True, ith=True, lv_tol_percent=6., branch_results=False, bus=0)
...
def test_type_c_trafo_simple_other_voltage4(inverse_y):
    ...
    runpp(net)
    calc_sc(net, case='max', lv_tol_percent=6., branch_results=True, bus=6, use_pre_fault_voltage=True,
            inverse_y=inverse_y)
...
def test_sgen_type_c(inverse_y):
    ...
    runpp(net)
    calc_sc(net, case="max", use_pre_fault_voltage=True, branch_results=True, bus=2, inverse_y=inverse_y,
...
def test_trafo_impedance(trafo_impedance_case):
    ...
    create_bus(net, 20)
    create_buses(net, 2, 0.4)
    create_ext_grid(net, 0, s_sc_max_mva=346.4102, rx_max=0.1, s_sc_min_mva=346.4102, rx_min=0.1)
    v_lv = 410
    ...
    k_t = 0.95 * 1.05 / (1 + 0.6 * x_t) if case == "max" else 1.0
    z_tk = k_t * z_tlv
