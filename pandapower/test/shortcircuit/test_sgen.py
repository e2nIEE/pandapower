...
def wind_park_grid(case):
    ...
    if case == "all_async_doubly_fed":
        create_sgens(net, sgen_buses, 0, 0, 2.5, rx=0.1, current_source=False,
                     generator_type="async_doubly_fed", max_ik_ka=0.388, kappa=1.7)
    elif case == "all_full_size_converter":
        create_sgens(net, sgen_buses, 0, 0, 2.5, rx=0.1, k=1.3, current_source=True,
                     generator_type="current_source")
    elif case == "mixed":
        create_sgens(net, sgen_buses[:5], 0, 0, 2.5, rx=0.1, current_source=False,
                     generator_type="async_doubly_fed", max_ik_ka=0.388, kappa=1.7)
        create_sgens(net, sgen_buses[5:], 0, 0, 2.5, rx=0.1, k=1.3, current_source=True,
                     generator_type="current_source")
    else:
