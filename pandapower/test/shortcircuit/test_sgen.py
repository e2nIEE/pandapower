...
def simplest_test_grid(generator_type, step_up_trafo=False):
    ...
    if generator_type == "async_doubly_fed":
        ...
        raise NotImplementedError(
            f"unknown sgen generator type {generator_type}, can be one of "
...
def wind_park_grid(case):
    ...
    create_line_from_parameters(
        net, 2, 3, 13.1, 0.0681, 0.102, 0, 1e3, "L1", parallel=2
    )
    ...
    create_lines_from_parameters(
        net, from_buses, to_buses, length_km, 0.211, 0.122, 0, 1e3, names
    )
...
def wind_park_example():
    ...
    b1 = create_bus(net, vn_kv=110.0, index=1)
    b2 = create_bus(net, vn_kv=110.0, index=2)
    b3 = create_bus(net, vn_kv=110.0, index=3)
    b4 = create_bus(net, vn_kv=110.0, index=4)
    ...
    create_sgen(net, b4, p_mw=0.050e3, sn_mva=50)
    net.sgen["k"] = 1.2
    return net
...
def big_sgen_three_bus_example():
    ...
    create_sgen(net, b2, sn_mva=200.0, p_mw=0, k=1.2)
    return net
...
def test_max_3ph_branch_small_sgen():
    ...
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values, np.array([0.53746061, 0.50852707, 0.4988896])
    )
    assert np.allclose(
        net.res_line_sc.ikss_ka.values, np.array([0.49593034, 0.4988896])
    )
    assert np.allclose(net.res_line_sc.ip_ka.values, np.array([0.92787443, 0.9251165]))
    assert np.allclose(
        net.res_line_sc.ith_ka.values, np.array([0.49811957, 0.50106881])
    )
...
def test_max_3ph_branch_big_sgen():
    ...
    calc_sc(net, case="max", ip=True, ith=True, branch_results=True)
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values, np.array([1.78453722, 1.75560368, 1.72233192])
    )
    assert np.allclose(
        net.res_line_sc.ikss_ka.values, np.array([1.25967331, 1.72233192])
    )
...
def test_min_3ph_branch_results_small_sgen():
    ...
    assert np.allclose(
        net.res_bus_sc.ikss_ka.values,
        np.array([0.419891, 0.398969, 0.391938]),
        atol=1e-6,
        rtol=0,
    )
    assert np.allclose(
        net.res_line_sc.ikss_ka.values,
        np.array([0.391938, 0.391938]),
        atol=1e-6,
        rtol=0,
    )
    assert np.allclose(
        net.res_line_sc.ip_ka.values, np.array([0.744387, 0.728265]), atol=1e-6, rtol=0
    )
    assert np.allclose(
        net.res_line_sc.ith_ka.values, np.array([0.400712, 0.393625]), atol=1e-6, rtol=0
    )
...
def test_min_3ph_branch_results_big_sgen():
    ...
    # because in case "min" sgen is ignored, it doesn't matter if sgen is big or small -
...
def test_max_1ph_branch_small_sgen():
    ...
    assert np.isclose(
        i_line_with_gen.ikss_ka.at[0], i_bus_without_sgen.ikss_ka.at[1], atol=1e-4
    )
    assert np.isclose(
        i_line_with_gen.ikss_ka.at[1], i_bus_with_sgen.ikss_ka.at[2], atol=1e-4
    )
...
def test_max_1ph_branch_big_sgen():
    ...
    assert np.isclose(
        i_line_with_gen.ikss_ka.at[0], i_bus_only_sgen.ikss_ka.at[0], atol=1e-4
    )
    assert np.isclose(
        i_line_with_gen.ikss_ka.at[1], i_bus_with_sgen.ikss_ka.at[2], atol=1e-4
    )
...
def test_min_1ph_branch_small_sgen():
    ...
    assert np.isclose(
        i_line_with_gen.ikss_ka.at[1], i_bus_with_sgen.ikss_ka.at[2], atol=1e-6
    )
...
def test_min_1ph_branch_big_sgen():
    ...
    i_line_1 = i_bus_without_sgen.ikss_ka.at[2] - (
    ...
    assert np.isclose(
        i_line_with_sgen.ikss_ka.at[1], i_bus_with_sgen.ikss_ka.at[2], atol=1e-4
    )
...
def test_wind_power_station_unit():
    ...
    assert np.isclose(
        net.res_bus_sc.at[0, "ip_ka"], ikss_ka * np.sqrt(2), rtol=0, atol=1e-12
    )
    ...
    assert np.isclose(net.res_bus_sc.at[0, "ikss_ka"], ikss_ka, rtol=0, atol=1e-12)
    assert np.isclose(
        net.res_bus_sc.at[0, "ip_ka"], ikss_ka * kappa * np.sqrt(2), rtol=0, atol=1e-12
    )
    assert np.isclose(net.res_bus_sc.at[0, "skss_mw"], skss_mw, rtol=0, atol=1e-9)

    # asynchronous generator (also with a step-up trafo)
    ...
    assert np.isclose(
        net.res_bus_sc.at[1, "ip_ka"], ikss_ka * kappa * np.sqrt(2), rtol=0, atol=1e-9
    )
    assert np.isclose(net.res_bus_sc.at[1, "skss_mw"], skss_mw, rtol=0, atol=1e-9)
    assert np.isclose(
        net.res_bus_sc.at[1, "rk_ohm"], z_g_complex.real, rtol=0, atol=1e-9
    )
    assert np.isclose(
        net.res_bus_sc.at[1, "xk_ohm"], z_g_complex.imag, rtol=0, atol=1e-9
    )

    base_z_ohm = 20**2 / net.sn_mva
    z_thv = (
        net._ppc["branch"][0, BR_R] + 1j * net._ppc["branch"][0, BR_X]
    ) * base_z_ohm
    t_r = 20 / 0.4
