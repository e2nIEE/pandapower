...
def test_radial_network(radial_grid):
    ...
    calc_sc(
        net, bus=sc_bus, inverse_y=False, branch_results=True, return_all_currents=True
    )
...
def test_meshed_network(meshed_grid):
    ...
    assert np.isclose(net.res_bus_sc.ikss_ka.at[sc_bus], ik)
    line_ix = net.line[
        (net.line.to_bus == sc_bus) | (net.line.from_bus == sc_bus)
    ].index
...
def test_big_gen_network(three_bus_big_sgen_example):
    ...
    sc_bus = [0, 1, 2]
    calc_sc(
        net, bus=sc_bus, branch_results=True, return_all_currents=True, inverse_y=False
    )
    assert np.isclose(net.res_line_sc.loc[(0, 0), "ikss_ka"], 1.25967331, atol=1e-3)
    assert np.isclose(net.res_line_sc.loc[(1, 0), "ikss_ka"], 0.0, atol=2e-3)
