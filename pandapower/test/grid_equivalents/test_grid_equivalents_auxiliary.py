...
def test_drop_measurements_and_controllers():
    ...
    gen_ts = pd.DataFrame(
        index=time_series.index.tolist(), columns=net.gen.index.tolist()
    )
    for t in range(96):
    ...
    ConstControl(
        ...
        element="gen",
        variable="p_mw",
        element_index=net.gen.index.tolist(),
        profile_name=net.gen.index.tolist(),
        data_source=DFData(gen_ts),
    )
    ...
    assert net.controller.object[0].__dict__["matching_params"]["element_index"] == [
        0,
        2,
    ]
