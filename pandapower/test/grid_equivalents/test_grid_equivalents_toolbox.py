...
def boundary_testnet(which):
    if which == "case9_27":
        ...
        net = merge_nets(
            net1, net2, merge_results=False, validate=False, net2_reindex_log_level=None
        )
        ...
        expected_bbr["line"] = {
            create_line_from_parameters(
                net,
                net.bus.index[net.bus.name == 9][0],
                net.bus.index[net.bus.name == 9][1],
                1,
                0,
                65,
                0,
        ...
        expected_bbr["impedance"] = {
            create_impedance(
                net,
                net.bus.index[net.bus.name == 5][0],
                net.bus.index[net.bus.name == 5][1],
                0,
                0.06,
                250,
            )
        ...
        expected_bbr["switch"] = {
            create_switch(
                net,
                net.bus.index[net.bus.name == 7][0],
                net.bus.index[net.bus.name == 7][1],
                "b",
            )
        }
        expected_bbr["trafo"] = {
...
def test_set_bus_zone_by_boundary_branches_and_get_boundaries_by_bus_zone_with_boundary_branches3():
    ...
    for in_ext1, in_ext2 in zip(in_ext, in_ext[::-1]):
        ...
        assert (
            boundary_buses["b"][in_ext1] - trafo3w_buses
...
def test_append_set_to_dict():
    ...
    dict1 = {}
    dict2 = {2: {}, 7: "hkj"}
    dict3 = {2: {"hjk": 6, 3: dict()}}
    dict4 = {2: {"hjk": 6, 3: {5: set([8, 2])}}}
