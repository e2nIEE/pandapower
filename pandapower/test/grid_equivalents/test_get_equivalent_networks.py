    ...
    for eq_type in ["xward", "rei", "ward"]:
        ...
        for sn_mva in [1.0, 23.0, 89.0]:
            ...
            max_error, related_values = get_max_error(
                net,
                eq_type,
                boundary_buses=[18, 22, 37, 64],
                internal_buses=[68],
                return_internal=True,
                buses_out_of_service=[32],
                va_degree=va_degree,
            )
            assert max_error <= epsilon
            ...
            max_error, related_values = get_max_error(
                net,
                eq_type,
                boundary_buses=[18, 20, 25, 26, 29, 31],
                internal_buses=[68],
                return_internal=True,
                va_degree=va_degree,
            )
            ...
            max_error, related_values = get_max_error(
                net,
                eq_type,
                boundary_buses=[39, 42, 48, 65, 69, 70],
                internal_buses=[68],
                return_internal=True,
                va_degree=va_degree,
            )
    ...
    **kwargs,
    ...
    if switch_changes:
        ...
        for i in range(len(switch_changes)):
            create_switch(
                net,
                bus=switch_changes[i][1],
                element=switch_changes[i][2],
    ...
    if return_internal and len(set(reserved_buses) & set(i_buses)):
        ...
        if len(related_buses):
            for para in res_bus_parameter_to_compare:
                max_para_error = max(
                    abs(
                        net_eq.res_bus[para][related_buses].values
