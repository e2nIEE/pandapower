...
def assert_res_equal(net1, net2, **kwargs):
    ...
    if "check_only_results" in kwargs:
        if not kwargs["check_only_results"]:
            raise ValueError(
                "'check_only_results' cannot be False in assert_res_equal()."
            )
...
def create_test_network():
    ...
    b1 = create_bus(net, name="bus1", vn_kv=10.0)
    create_ext_grid(net, b1)
    b2 = create_bus(net, name="bus2", geodata=(1.0, 2.0), vn_kv=0.4)
    b3 = create_bus(net, name="bus3", geodata=(1.0, 3.0), vn_kv=0.4, index=7)
    b4 = create_bus(net, name="bus4", vn_kv=10.0)
    create_transformer_from_parameters(
        net,
        b4,
        b2,
...
def add_grid_connection(net, vn_kv=20.0, zone=None):
    ...
    b2 = (
        get_free_id(net.bus) + 2
    )  # shake up the indices so that non-consecutive indices are tested
