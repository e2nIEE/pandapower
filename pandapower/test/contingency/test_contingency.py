...
def test_with_lightsim2grid(get_net, get_case):
    ...
    if case == 0:
        ...
        nminus1_cases = {
            element: {"index": rng.choice(net[element].index.values, rng.integers(1, len(net[element])), replace=False)}
            for element in ("line", "trafo")
...
def test_case118():
    ...
    nminus1_cases = {"line": {"index": net.line.index.values}, "trafo": {"index": net.trafo.index.values}}
...
def test_lightsim2grid_distributed_slack():
    ...
    res = run_contingency(
        net, nminus1_cases, contingency_evaluation_function=run_for_from_bus_loading, distributed_slack=True
    )
...
def test_lightsim2grid_phase_shifters():
    ...
    create_lines(net, [0, 0], [1, 1], 40, "243-AL1/39-ST1A 110.0", max_loading_percent=100)
    create_transformer_from_parameters(
        net, 1, 2, 150, 110, 110, 0.5, 10, 15, 0.1, 150, "hv", 0, 10, -10, 0, 1, 5, "Ideal", max_loading_percent=100
    )
    create_lines(net, [2, 2], [3, 3], 25, "243-AL1/39-ST1A 110.0", max_loading_percent=100)
...
def check_cause_index(net, nminus1_cases):
    ...
    for check_element in elements_to_check:
        ...
        for check_element_index in net[check_element].index.values:
            ...
            assert np.isclose(
                result_table.at[check_element_index, "max_loading_percent"], element_max_loading, rtol=0, atol=1e-6
            )
