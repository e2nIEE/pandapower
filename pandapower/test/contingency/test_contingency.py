...
def test_contingency_timeseries(get_net):
    nminus1_cases = {
        element: {"index": get_net[element].index.values}
        for element in ("line", "trafo")
        if len(get_net[element]) > 0
    }
    ...
    for contingency_function in contingency_functions:
        ...
        for var in ("vm_pu", "max_vm_pu", "min_vm_pu"):
            assert np.allclose(
                res1["bus"][var], net1.res_bus[var].values, atol=1e-9, rtol=0
            ), var
            assert np.allclose(
                res0["bus"][var], net0.res_bus[var].values, atol=1e-9, rtol=0
            ), var
            assert np.allclose(
                res1["bus"][var],
                ow.output[f"res_bus.{var}"].iloc[-1, :].values,
                atol=1e-9,
                rtol=0,
            ), var
            assert np.allclose(
                res0["bus"][var],
                ow.output[f"res_bus.{var}"].iloc[0, :].values,
                atol=1e-9,
                rtol=0,
            ), var
        for var in ("loading_percent", "max_loading_percent", "min_loading_percent"):
            for element in ("line", "trafo"):
                ...
                assert np.allclose(
                    res1[element][var],
                    net1[f"res_{element}"][var].values,
                    atol=1e-6,
                    rtol=0,
                ), var
                assert np.allclose(
                    res0[element][var],
                    net0[f"res_{element}"][var].values,
                    atol=1e-6,
                    rtol=0,
                ), var
                assert np.allclose(
                    res1[element][var],
                    ow.output[f"res_{element}.{var}"].iloc[-1, :].values,
                    atol=1e-6,
                    rtol=0,
                ), var
                assert np.allclose(
                    res0[element][var],
                    ow.output[f"res_{element}.{var}"].iloc[0, :].values,
                    atol=1e-6,
                    rtol=0,
                ), var
...
@pytest.mark.skipif(
    not lightsim2grid_installed, reason="lightsim2grid package is not installed"
)
def test_with_lightsim2grid(get_net, get_case):
    ...
    if case == 0:
        ...
        nminus1_cases = {
            element: {
                "index": rng.choice(
                    net[element].index.values,
                    rng.integers(1, len(net[element])),
                    replace=False,
                )
            }
            for element in ("line", "trafo")
    ...
    res = run_contingency(
        net, nminus1_cases, contingency_evaluation_function=run_for_from_bus_loading
    )

    run_contingency_ls2g(net, nminus1_cases)

    assert np.array_equal(
        res["line"]["causes_overloading"], net.res_line.causes_overloading.values
    )
    if len(net.trafo) > 0:
        assert np.array_equal(
            res["trafo"]["causes_overloading"], net.res_trafo.causes_overloading.values
        )

    for s in ("min", "max"):
        assert np.allclose(
            res["bus"][f"{s}_vm_pu"],
            net.res_bus[f"{s}_vm_pu"].values,
            atol=1e-9,
            rtol=0,
        ), s
        assert np.allclose(
            np.nan_to_num(res["line"][f"{s}_loading_percent"]),
            net.res_line[f"{s}_loading_percent"].values,
            atol=1e-6,
            rtol=0,
        ), s
        if len(net.trafo) > 0:
            assert np.allclose(
                np.nan_to_num(res["trafo"][f"{s}_loading_percent"]),
                net.res_trafo[f"{s}_loading_percent"].values,
                atol=1e-6,
                rtol=0,
            ), s
...
@pytest.mark.xfail(
    reason="remove this xfail when new version of lightsim2grid available"
)
@pytest.mark.skipif(
    not lightsim2grid_installed, reason="lightsim2grid package is not installed"
)
def test_case118():
    ...
    res = run_contingency(
        net2, nminus1_cases, contingency_evaluation_function=run_for_from_bus_loading
    )
...
def test_unequal_trafo_hv_lv_impedances():
    ...
    create_lines(
        net, [0, 0], [1, 1], 40, "243-AL1/39-ST1A 110.0", max_loading_percent=100
    )
    create_transformer_from_parameters(
        net,
        1,
        ...
        leakage_resistance_ratio_hv=0.2,
        leakage_reactance_ratio_hv=0.4,
    )
    create_lines(
        net, [2, 2], [3, 3], 25, "243-AL1/39-ST1A 110.0", max_loading_percent=100
    )
...
def test_lightsim2grid_distributed_slack():
    ...
    res1 = run_contingency(
        net1, nminus1_cases, contingency_evaluation_function=run_for_from_bus_loading
    )
    ...
    for var in ("loading_percent", "max_loading_percent", "min_loading_percent"):
        assert np.allclose(
            res1["line"][var], net1.res_line[var].values, atol=1e-6, rtol=0
        )
        assert np.allclose(
            res["line"][var], net.res_line[var].values, atol=1e-6, rtol=0
        )
    for var in ("vm_pu", "max_vm_pu", "min_vm_pu"):
        assert np.allclose(
            res1["bus"][var], net1.res_bus[var].values, atol=1e-9, rtol=0
...
def test_lightsim2grid_phase_shifters():
    ...
    if ("tap_changer_type" in net.trafo.columns) or (
        "tap_changer_type" in net.trafo3w.columns
    ):
...
def test_cause_congestion():
    ...
    nminus1_cases = {
        "line": {"index": net.line.iloc[[4, 2, 1, 5, 7, 8]].index.values},
    ...
    for element, val in nminus1_cases.items():
        for i in val["index"]:
            ...
            idx_overloaded_tr = net.res_trafo.loc[
                net.res_trafo.loading_percent > net.trafo.max_loading_percent
            ].index
            congestion_tr = (
                (
                    net.res_trafo.loc[idx_overloaded_tr, "loading_percent"].values
                    - net.trafo.loc[idx_overloaded_tr, "max_loading_percent"].values
                )
                * net.trafo.loc[idx_overloaded_tr, "sn_mva"].values
                / 100
            )
            idx_overloaded_ln = net.res_line.loc[
                net.res_line.loading_percent > net.line.max_loading_percent
            ].index
            congestion_ln = (
                (
                    net.res_line.loc[idx_overloaded_ln, "loading_percent"].values
                    - net.line.loc[idx_overloaded_ln, "max_loading_percent"].values
                )
                * net.line.loc[idx_overloaded_ln, "max_i_ka"].values
                * net.bus.loc[
                    net.line.loc[idx_overloaded_ln, "from_bus"].values, "vn_kv"
                ].values
                * np.sqrt(3)
                / 100
            )
            if res[element].at[i, "congestion_caused_mva"] == 0:
                ...
                assert np.allclose(
                    res[element].at[i, "congestion_caused_mva"],
                    congestion_tr.sum() + congestion_ln.sum(),
                    rtol=0,
                    atol=1e-6,
                )
...
def test_cause_element_index():
    ...
    if lightsim2grid_installed:
        ...
        assert_frame_equal(
            ...
            cause_res_copy_line[columns],
            rtol=0,
            atol=1e-6,
            check_dtype=False,
        )
        assert_frame_equal(
            net.res_trafo[columns],
            cause_res_copy_trafo[columns],
            rtol=0,
            atol=1e-6,
            check_dtype=False,
        )
        ...
        for check_element_index in net[check_element].index.values:
            ...
            for nminus1_element, nminus1_element_index in nminus1_cases.items():
                for nminus1_idx in nminus1_element_index["index"]:
                    ...
                    if (
                        net[f"res_{check_element}"].at[
                            check_element_index, "loading_percent"
                        ]
                        > element_max_loading
                    ):
                        element_max_loading = net[f"res_{check_element}"].at[
                            check_element_index, "loading_percent"
                        ]
            ...
            assert (
                result_table.at[check_element_index, "cause_index"]
                == element_cause_index
            )
            assert (
                result_table.at[check_element_index, "cause_element"] == cause_element
            )
            assert np.isclose(
                result_table.at[check_element_index, "max_loading_percent"],
                element_max_loading,
                rtol=0,
                atol=1e-6,
            )
...
def run_for_from_bus_loading(net, **kwargs):
    ...
    if len(net.trafo) > 0:
        max_i_ka_limit = net.trafo.sn_mva.values / (
            net.trafo.vn_hv_kv.values * np.sqrt(3)
        )
...
def setup_timeseries(net):
    load_profiles = pd.DataFrame(
        net.load.p_mw.values * (np.random.random((4, len(net.load))) * 0.4 + 0.8),
        index=np.arange(4),
        columns=net.load.index.values,
    )
    ...
    gen_profiles = pd.DataFrame(
        net.gen.p_mw.values * (np.random.random((4, len(net.gen))) * 0.4 + 0.8),
        index=np.arange(4),
        columns=net.gen.index.values,
    )
    ...
    ConstControl(
        ...
        element="gen",
        variable="p_mw",
        element_index=net.gen.index.values,
        profile_name=net.gen.index.values,
        data_source=dsg,
    )
