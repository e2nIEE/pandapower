...
def test_test_SimBench_1_HVMVmixed_1_105_0_sw_modified_ext_grid(SimBench_1_HVMVmixed_1_105_0_sw_modified):
    ...
    assert '{"coordinates": [11.3706, 53.601], "type": "Point"}' == element_0['geo'].item()
...
def test_smallgrid_DL_line_geodata(smallgrid_DL):
    ...
    assert 176 == len(smallgrid_DL.line.loc[smallgrid_DL.line.diagram.notna()].index)
    element_0 = smallgrid_DL.line[smallgrid_DL.line['origin_id'] == '_0447c6f1-c766-11e1-8775-005056c00008']
    assert ('{"coordinates": [[162.363632, 128.4656], [162.328033, 134.391541], '
            '[181.746033, 134.43364]], "type": "LineString"}') == element_0['diagram'].item()

    element_1 = smallgrid_DL.line[smallgrid_DL.line['origin_id'] == '_044a5f09-c766-11e1-8775-005056c00008']
    assert '{"coordinates": [[12.87877, 58.5714264], [12.8923006, 69.33862]], "type": "LineString"}' == element_1[
        'diagram'].item()


def test_smallgrid_DL_bus_geodata(smallgrid_DL):
    assert "diagram" in smallgrid_DL.bus.columns
    assert 118 == len(smallgrid_DL.bus.loc[smallgrid_DL.bus.diagram.notna()].index)
    element_0 = smallgrid_DL.bus[smallgrid_DL.bus['origin_id'] == '_0471bd2a-c766-11e1-8775-005056c00008']
    assert '{"coordinates": [18.5449734, 11.8253975], "type": "Point"}' == element_0['diagram'].item()

    element_1 = smallgrid_DL.bus[smallgrid_DL.bus['origin_id'] == '_04483c26-c766-11e1-8775-005056c00008']
    assert '{"coordinates": [39.15344, 94.86773], "type": "Point"}' == element_1['diagram'].item()
...
def test_smallgrid_GL_line_geodata(smallgrid_GL):
    ...
    assert ('{"coordinates": [[-3.339864492416382, 58.50086212158203], '
            ...
            '[-4.405538082122803, 57.53498840332031]], "type": "LineString"}') == element_1['geo'].item()


def test_smallgrid_GL_bus_geodata(smallgrid_GL):
    assert "geo" in smallgrid_GL.bus.columns
    assert 115 == len(smallgrid_GL.bus.loc[smallgrid_GL.bus.geo.notna()].index)
...
def test_fullgrid_xward(fullgrid_v2):
    assert 0 == len(fullgrid_v2.xward.index)


def test_fullgrid_ward(fullgrid_v2):
    assert 5 == len(fullgrid_v2.ward.index)
    ...
    assert '_d4affe50316740bdbbf4ae9c7cbf3cfd' == fullgrid_v2.bus.iloc[element_0['bus'].item()]['origin_id']
...
def test_fullgrid_trafo3w(fullgrid_v2):
    assert 1 == len(fullgrid_v2.trafo3w.index)  # TODO: test with more elements
    ...
    assert '_ac279ca9-c4e2-0145-9f39-c7160fff094d' == fullgrid_v2.bus.iloc[element_0['hv_bus'].item()]['origin_id']
    assert '_99b219f3-4593-428b-a4da-124a54630178' == fullgrid_v2.bus.iloc[element_0['mv_bus'].item()]['origin_id']
    assert '_f96d552a-618d-4d0c-a39a-2dea3c411dee' == fullgrid_v2.bus.iloc[element_0['lv_bus'].item()]['origin_id']
...
def test_fullgrid_trafo(fullgrid_v2):
    assert 10 == len(fullgrid_v2.trafo.index)
    ...
    assert None is element_0['std_type'].item()
    assert '_c142012a-b652-4c03-9c35-aa0833e71831' == fullgrid_v2.bus.iloc[element_0['hv_bus'].item()]['origin_id']
    assert '_b01fe92f-68ab-4123-ae45-f22d3e8daad1' == fullgrid_v2.bus.iloc[element_0['lv_bus'].item()]['origin_id']
    ...
    assert '_e44141af-f1dc-44d3-bfa4-b674e5c953d7' == fullgrid_v2.bus.iloc[element_1['hv_bus'].item()]['origin_id']
    assert '_5c74cb26-ce2f-40c6-951d-89091eb781b6' == fullgrid_v2.bus.iloc[element_1['lv_bus'].item()]['origin_id']
...
def test_fullgrid_tcsc(fullgrid_v2):
    assert 0 == len(fullgrid_v2.tcsc.index)


def test_fullgrid_switch(fullgrid_v2):
    assert 4 == len(fullgrid_v2.switch.index)
    element_0 = fullgrid_v2.switch[fullgrid_v2.switch['origin_id'] == '_8a3ad6e1-6e23-b649-880e-4865217501c4']
    assert '_5c74cb26-ce2f-40c6-951d-89091eb781b6' == fullgrid_v2.bus.iloc[element_0['bus'].item()]['origin_id']
    assert '_c21be5da-d2a6-d94f-8dcb-92e4d6fa48a7' == fullgrid_v2.bus.iloc[element_0['element'].item()]['origin_id']
...
def test_fullgrid_svc(fullgrid_v2):
    assert 0 == len(fullgrid_v2.svc.index)


def test_fullgrid_storage(fullgrid_v2):
    assert 0 == len(fullgrid_v2.storage.index)


def test_fullgrid_shunt(fullgrid_v2):
    assert 6 == len(fullgrid_v2.shunt.index)
    ...
    assert '_f6ee76f7-3d28-6740-aa78-f0bf7176cdad' == fullgrid_v2.bus.iloc[element_0['bus'].item()]['origin_id']
...
def test_fullgrid_pwl_cost(fullgrid_v2):
    assert 0 == len(fullgrid_v2.pwl_cost.index)


def test_fullgrid_poly_cost(fullgrid_v2):
    assert 0 == len(fullgrid_v2.poly_cost.index)


def test_fullgrid_motor(fullgrid_v2):
    assert 1 == len(fullgrid_v2.motor.index)
    ...
    assert '_f70f6bad-eb8d-4b8f-8431-4ab93581514e' == fullgrid_v2.bus.iloc[element_0['bus'].item()]['origin_id']
...
def test_fullgrid_measurement(fullgrid_v2):
    assert 0 == len(fullgrid_v2.measurement.index)  # TODO: analogs


def test_fullgrid_load(fullgrid_v2):
    assert 5 == len(fullgrid_v2.load.index)
    ...
    assert '_4c66b132-0977-1e4c-b9bb-d8ce2e912e35' == fullgrid_v2.bus.iloc[element_0['bus'].item()]['origin_id']
    assert 0.010 == element_0['p_mw'].item()
    assert 0.010 == element_0['q_mvar'].item()
    assert 0.0 == element_0['const_z_p_percent'].item()
    assert 0.0 == element_0['const_i_p_percent'].item()
    assert 0.0 == element_0['const_z_q_percent'].item()
    assert 0.0 == element_0['const_i_q_percent'].item()
...
def test_fullgrid_line_geodata(fullgrid_v2):
    ...
    assert 0 == len(fullgrid_v2.line.loc[fullgrid_v2.line.geo.notna()].index)
    assert 'diagram' not in fullgrid_v2.line.columns


def test_fullgrid_line(fullgrid_v2):
    assert 11 == len(fullgrid_v2.line.index)
    ...
    assert None is element_0['std_type'].item()
    assert '_1fa19c281c8f4e1eaad9e1cab70f923e' == fullgrid_v2.bus.iloc[element_0['from_bus'].item()]['origin_id']
    assert '_f70f6bad-eb8d-4b8f-8431-4ab93581514e' == fullgrid_v2.bus.iloc[element_0['to_bus'].item()]['origin_id']
...
def test_fullgrid_impedance(fullgrid_v2):
    assert 1 == len(fullgrid_v2.impedance.index)  # TODO: test with more elements
    ...
    assert '_514fa0d5-a432-5743-8204-1c8518ffed76' == fullgrid_v2.bus.iloc[element_0['from_bus'].item()]['origin_id']
    assert '_4c66b132-0977-1e4c-b9bb-d8ce2e912e35' == fullgrid_v2.bus.iloc[element_0['to_bus'].item()]['origin_id']
...
def test_fullgrid_ext_grid(fullgrid_v2):
    assert 1 == len(fullgrid_v2.ext_grid.index)
...
def test_fullgrid_controller(fullgrid_v2):
    assert 8 == len(fullgrid_v2.controller.index)
    for _, obj in fullgrid_v2.controller.iterrows():
        if obj.object.matching_params.get('tid') == \
            ...
            assert '_b01fe92f-68ab-4123-ae45-f22d3e8daad1' == fullgrid_v2.bus.iloc[obj.object.trafobus]['origin_id']
        ...
        if obj.object.matching_params.get('tid') == \
            ...
            assert math.isnan(obj.object.tol)
            assert '_ac772dd8-7910-443f-8af0-a7fca0fb57f9' == fullgrid_v2.bus.iloc[obj.object.trafobus]['origin_id']
            assert 'trafo' == obj.object.trafotable
            assert '2W' == obj.object.trafotype
            assert math.isnan(obj.object.vm_delta_pu)
            assert math.isnan(obj.object.vm_lower_pu)
...
def test_fullgrid_bus_geodata(fullgrid_v2):
    ...
    assert 0 == len(fullgrid_v2.bus.loc[fullgrid_v2.bus.geo.notna()].index)
    assert 'diagram' not in fullgrid_v2.bus.columns


def test_fullgrid_bus(fullgrid_v2):
    assert 26 == len(fullgrid_v2.bus.index)
...
def test_fullgrid_asymmetric_sgen(fullgrid_v2):
    assert 0 == len(fullgrid_v2.asymmetric_sgen.index)


def test_fullgrid_asymmetric_load(fullgrid_v2):
    assert 0 == len(fullgrid_v2.asymmetric_load.index)
