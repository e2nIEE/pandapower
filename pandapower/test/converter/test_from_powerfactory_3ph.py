...
try:
    ...
    PF_INSTALLED = False
    logger.info(
        "could not import powerfactory, unbalanced load flow comparsion between pf and pp not possible"
    )

from pandapower.converter.powerfactory.pf_export_functions import (
    run_load_flow as run_powerfactory_load_flow,
)
from pandapower.converter.powerfactory.validate import (
    _get_pf_results_unbalanced,
    _set_pf_results_unbalanced,
)
...
def test_line_asym():
    ...
    runpp_3ph(net)
    read_pf_results_from_file_to_net(
        os.path.join(testfiles_path, "pf_results_line.json"), net
    )
    compare_3ph_pp_pf_results(net)
    # return net
    ...
    for loadtype in ["wye", "delta"]:
        for vector_group in ["Dyn", "YNyn", "Yzn"]:
            ...
            read_pf_results_from_file_to_net(
                os.path.join(
                    testfiles_path,
                    "pf_combinations_results_net_bus_trafo_line_load_19.10.20.json",
                ),
                net,
                vector_group + "_" + loadtype,
            )
...
def compare_3ph_pp_pf_results(net):
    ...
    for phase in ["a", "b", "c"]:
        # bus
        diff_vm_pu = abs(
            net.res_bus_3ph["vm_%s_pu" % phase] - net.res_bus_3ph["pf_vm_%s_pu" % phase]
        )
        logger.debug("vm_pu difference (phase %s):\n%s\n" % (phase, diff_vm_pu))
        assert all(diff_vm_pu < vm_tol)
        diff_va_degree = abs(
            net.res_bus_3ph["va_%s_degree" % phase]
            - net.res_bus_3ph["pf_va_%s_degree" % phase]
        )
        ...
        # trafo
        diff_i_hv_ka = abs(
            net.res_trafo_3ph["i_%s_hv_ka" % phase]
            - net.res_trafo_3ph["pf_i_%s_hv_ka" % phase]
        )
        logger.debug("i_hv_ka difference:\n%s\n" % diff_i_hv_ka)
        assert all(diff_i_hv_ka < i_trafo_tol)
        diff_i_lv_ka = abs(
            net.res_trafo_3ph["i_%s_lv_ka" % phase]
            - net.res_trafo_3ph["pf_i_%s_lv_ka" % phase]
        )
    ...
    for phase in ["a", "b", "c", "n"]:
        diff_i_from_ka = abs(
            net.res_line_3ph["i_%s_from_ka" % phase]
            - net.res_line_3ph["pf_i_%s_from_ka" % phase]
        )
        ...
        diff_i_to_ka = abs(
            net.res_line_3ph["i_%s_to_ka" % phase]
            - net.res_line_3ph["pf_i_%s_to_ka" % phase]
        )
        ...
        for idx, element in net[element_type].iterrows():
            ...
            for pp_attr, pf_attr in variables.items():
                if pf_element.HasResults(0):
                    ...
                    net["res_" + element_type + "_3ph"][pp_attr].loc[idx] = (
                        pf_element.GetAttribute(pf_attr)
                    )
