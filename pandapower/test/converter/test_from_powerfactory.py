...
@pytest.mark.xfail(
    reason="implementation of the trafo3w data model is not completely consistent with PowerFactory"
)
@pytest.mark.skipif(not PF_INSTALLED, reason="powerfactory must be installed")
...
def test_trafo_tap2_results():
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "trafo_tap_model.json"
    )
    net = from_json(path)
    all_diffs = validate_pf_conversion(net, tolerance_mva=1e-9)
    tol = 2e-7
...
def test_trafo3w_tap_dependent_imp_with_tc():
    ...
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "tap_table_from_tdi.pfd"
    )
    prj = import_project(
        path,
        app,
        "test_tap_table_from_tdi",
        import_folder="TEST_IMPORT",
        clear_import_folder=True,
    )
    prj_name = prj.GetFullName()
...
def test_pf_export_tap_changer():
    ...
    # import the tap changer test grid to powerfactory
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "test_tap_changer.pfd"
    )
    prj = import_project(
        path,
        app,
...
def test_pf_export_partial_loads():
    ...
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "test_partial_loads.pfd"
    )
    prj = import_project(
        path,
        app,
    ...
    for key, diff in all_diffs.items():
        if isinstance(diff, pd.DataFrame):
            ...
            if key == "load_p_diff_is":
                delta = (
                    abs(diff.sum()) - 0.046
                )  # sum of partial loads is not computed right in diff
            else:
                delta = diff.abs().max()
        else:
            raise UserWarning("Diff variable has wrong type!")
...
def test_pf_SC_meas_relocate():
    ...
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "test_SC_meas_relocate.pfd"
    )
    prj = import_project(
        path,
        app,
        "test_SC_meas_relocate",
        import_folder="TEST_IMPORT",
        clear_import_folder=True,
    )
    prj_name = prj.GetFullName()
...
def test_pf_export_q_capability_curve():
    ...
    app = pf.GetApplication()
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "q_capabiltiy_curve.pfd"
    )
    prj = import_project(
        path,
        app,
        "test_q_capability_curve",
        import_folder="TEST_IMPORT",
        clear_import_folder=True,
    )
    prj_name = prj.GetFullName()
    ...
    assert_array_equal(
        np.hstack(net.q_capability_curve_table.p_mw),
        np.array(
            [
                -331.01001,
                -298.0,
    ...
    assert_array_equal(
        np.hstack(net.q_capability_curve_table.q_min_mvar),
        np.array(
            [
                -0.0100,
                -134.0099,
    ...
    assert_array_equal(
        np.hstack(net.q_capability_curve_table.q_max_mvar),
        np.array(
            [
                0.01000,
                134.00900,
...
def test_shunt_tables():
    ...
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "test_shunt_table.pfd"
    )
    prj = import_project(
        path,
        app,
    ...
    assert np.isclose(
        P_shunt_with_table, net.res_shunt.loc[1, "p_mw"], rtol=0, atol=1e-5
    )
    assert np.isclose(
        Q_shunt_with_table, net.res_shunt.loc[1, "q_mvar"], rtol=0, atol=1e-5
    )
    assert np.isclose(
        P_shunt_without_table, net.res_shunt.loc[0, "p_mw"], rtol=0, atol=1e-5
    )
    assert np.isclose(
        Q_shunt_without_table, net.res_shunt.loc[0, "q_mvar"], rtol=0, atol=1e-5
    )
...
def test_vdroop_ctrl_local():
    app = pf.GetApplication()
    path = os.path.join(
        pp_dir, "test", "converter", "testfiles", "test_vdroop_local.pfd"
    )
    prj = import_project(
        path,
        app,
