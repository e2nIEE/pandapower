...
def _line_geojson_to_geodata_(_net):
    _net["line_geodata"] = (
        _net.line.geo.dropna()
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(list)
        .to_frame()
        .rename(columns={"geo": "coords"})
    )
...
def test__node_geometries_from_geodata(get_network_and_result):
    ...
    assert_index_equal(
        result.columns, expected.columns, exact="equiv", obj="GeoDataFrame.columns"
    )
    assert_frame_equal(
        left2,
        right2,
...
def test__branch_geometries_from_geodata(get_network_and_result):
    ...
    assert isinstance(result, type(expected))
    assert (
        result.geom_equals_exact(expected.geometry, tolerance=1 * 10 ** (-6))
        | (result.geometry.is_empty & expected.geometry.is_empty)
        | (result.geometry.isna() & expected.geometry.isna())
    ).all()
    left2 = result.select_dtypes(exclude="geometry")
    right2 = expected.select_dtypes(exclude="geometry")
    assert_index_equal(
...
def test_convert_gis_to_geodata():
    ...
    converted_node = pd.DataFrame(
        {
            "x": [1.0, 1.0],
            "y": [2.0, 3.0],
            "coords": [float("nan"), float("nan")],
            "geometry": [Point(1.0, 2.0), Point(1.0, 3.0)],
        }
    )
    converted_node.set_index(pd.Index([1, 7]), inplace=True)
    converted_branch = pd.DataFrame(
        {"coords": [[(1.0, 2.0), (3.0, 4.0)]], "geometry": LineString([[1, 2], [3, 4]])}
    )
...
def test_convert_geodata_to_gis():
    ...
    pdf = pd.DataFrame(
        {"x": [1.0, 1.0], "y": [2.0, 3.0], "coords": [float("nan"), float("nan")]}
    )
    pdf = pdf.astype({"coords": "object"})
    pdf.set_index(pd.Index([1, 7]), inplace=True)
    converted_node = GeoDataFrame(
        crs="epsg:31467", geometry=points_from_xy(pdf.x, pdf.y), data=pdf
    )

    pdf = pd.DataFrame(
        {"coords": [[[1, 2], [3, 4]]], "geometry": LineString([[1, 2], [3, 4]])}
    )
...
def test_dump_to_geojson():
    ...
    assert (
        dumps(result, sort_keys=True) == '{"features": [], "type": "FeatureCollection"}'
    )
    ...
    assert dumps(result, sort_keys=True) == (
        '{"features": [{"geometry": {"coordinates": [1.0, 2.0], "type": "Point"}, '
...
def test_convert_geodata_to_geojson():
    ...
    assert _net.bus.at[0, "geo"] == geojson.dumps(
        geojson.Point((10, 20)), sort_keys=True
    )
    assert _net.bus.at[1, "geo"] == geojson.dumps(
        geojson.Point((30, 40)), sort_keys=True
    )
    assert _net.line.at[0, "geo"] == geojson.dumps(
        geojson.LineString([(10, 20), (30, 40)]), sort_keys=True
    )
