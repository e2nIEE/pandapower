...
def test_batch_output_reader(simple_test_net):
    ...
    run_timeseries(
        net, time_steps, recycle=recycle, only_v_results=only_v, verbose=False
    )
    ...
    i_from_ka_normal, i_to_ka_normal = (
        ow.output["res_line.i_from_ka"],
        ow.output["res_line.i_to_ka"],
    )
    i_hv_ka_normal, i_lv_ka_normal = (
        ow.output["res_trafo.i_hv_ka"],
        ow.output["res_trafo.i_lv_ka"],
    )
    i3_hv_ka, i3_lm_ka, i3_lv_ka = (
        ow.output["res_trafo3w.i_hv_ka"],
        ow.output["res_trafo3w.i_mv_ka"],
...
def _run_recycle(net, run):
    ...
    log_variables = [
        ("res_bus", "vm_pu") if run.__name__ == "runpp" else ("res_bus", "va_degree"),
...
def test_const_gen(simple_test_net, run_function):
    ...
    c2 = add_const(
        net, ds, recycle=None, profile_name="gen", element_index=0, element="gen"
    )
...
def test_const_ext_grid(simple_test_net, run_function):
    ...
    c2 = add_const(
        ...
        variable=_v_var(run_function, False),
        element_index=0,
        element="ext_grid",
    )
    ...
    assert np.allclose(ll, ow.output["res_line.loading_percent"])
    assert np.allclose(
        ow.output[_v_var(run_function)].values[:, 0], profiles["ext_grid"]
    )
...
def test_trafo_tap(simple_test_net, run_function=runpp):
    ...
    # 1load
    c1 = add_const(net, ds, recycle=None)
    c2 = ContinuousTapControl(net, 0, 0.99, tol=1e-9)
    ...
    # calculate the same results without recycle
    c = add_const(net, ds, recycle=False)
    c2 = ContinuousTapControl(net, 0, 0.99, recycle=False, tol=1e-9)
    ow = _run_normal(net, run_function)
    assert np.allclose(vm_pu, ow.output[_v_var(run_function)])
    assert np.allclose(ll, ow.output["res_line.loading_percent"])
...
def test_const_pq_gen_trafo_tap_ideal(simple_test_net, run_function):
    # allows to use recycle = {"bus_pq", "gen", "trafo"}
    net = simple_test_net
    net.trafo.loc[0, ["tap_step_percent", "tap_step_degree", "tap_changer_type"]] = (
        0,
        5,
        "Ideal",
    )
...
def test_const_pq_out_of_service(simple_test_net, run_function):
    ...
    net = simple_test_net
    for i in range(3):
        b = create_bus(net, 20.0, in_service=False)
        create_line(
            net, 2, b, std_type="149-AL1/24-ST1A 20.0", length_km=1.0, in_service=False
        )
        create_transformer(net, 2, b, std_type="25 MVA 110/20 kV", in_service=False)
        create_transformer3w(
            net, 1, 2, b, std_type="63/25/38 MVA 110/20/10 kV", in_service=False
        )
    _, ds = create_data_source(n_timesteps)
    # 1load
    c = add_const(net, ds, recycle=None)
    ...
    assert np.allclose(
        vm_pu.loc[:, in_service], ow.output[_v_var(run_function)].loc[:, in_service]
    )
    in_service = net.line.loc[net.line.in_service].index
    assert np.allclose(
        ll.loc[:, in_service], ow.output["res_line.loading_percent"].loc[:, in_service]
    )
