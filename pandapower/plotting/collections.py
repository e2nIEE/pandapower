...
def add_cmap_to_collection(
    collection, cmap, norm, z, cbar_title, plot_colormap=True, clim=None
):
    """
...
def _create_node_element_collection(
    node_coords,
    patch_maker,
    size=1.0,
    infos=None,
    ...
    **kwargs,
    ...
    angles = (
        ...
        else [orientation] * len(node_coords)
    )
    ...
    patch_coll = PatchCollection(
        polys, match_original=True, picker=picker, linewidth=linewidths, **kwargs
    )
    color = line_color if "color" not in kwargs else kwargs.pop("color", linewidths)
    line_coll = LineCollection(
        lines, color=color, picker=picker, linewidth=linewidths, **kwargs
    )
...
def _create_complex_branch_collection(
    coords,
    patch_maker,
    size=1,
    infos=None,
    ...
    **kwargs,
    ...
    line_coll = LineCollection(
        lines, color=line_color, picker=picker, linewidths=linewidths, **kwargs
    )
    patch_coll.info = infos_pc
    line_coll.info = infos_lc
    return patch_coll, line_coll
...
def create_bus_collection(
    ...
    cmap=None,
    norm=None,
    infofunc: Optional[Callable] = None,
    picker: bool = False,
    bus_geodata: Optional[pd.DataFrame] = None,
    bus_table: str = "bus",
    ...
    **kwargs,
    ...
    if any(net[bus_table].loc[buses].geo.isna()):
        raise AttributeError(
            "net.bus.geo contains NaN values, consider dropping them beforehand."
        )
...
def create_line_collection(
    net: pandapowerNet,
    lines=None,
    line_geodata: DataFrame or None = None,
    bus_geodata: DataFrame or None = None,
    use_bus_geodata: bool = False,
    infofunc: Callable or None = None,
    cmap: "Colormap" = None,
    norm: "Normalize" = None,
    picker: bool = False,
    z=None,
    ...
    **kwargs,
    ...
    if (
        not use_bus_geodata
        and line_geodata is None
        and ("geo" not in net[line_table].columns or net[line_table].geo.empty)
    ):
        # if bus geodata is available, but no line geodata
        logger.warning(
            "use_bus_geodata is automatically set to True, since net.line.geo is empty."
        )
    ...
    line_geodata: Series[str] = (
        line_geodata.loc[lines]
        if line_geodata is not None
        else net[line_table].geo.loc[lines]
    )
    ...
    if use_bus_geodata or not lines_without_geo.empty:
        ...
        line_geodata = line_geodata.combine_first(
            pd.Series(geos, index=line_index_successful)
        )
    ...
    **kwargs,
    ...
    if use_bus_geodata:
        coords, lines_with_geo = coords_from_node_geodata(
            lines,
            net.dcline.from_bus.loc[lines].values,
            net.dcline.to_bus.loc[lines].values,
            net["bus_geodata"],
            "line",
        )
    ...
    lc = _create_line2d_collection(
        coords, lines_with_geo, infos=infos, picker=picker, **kwargs
    )

    loading_percent = (
        100
        * net.res_dcline[["p_from_mw", "p_to_mw"]].abs().max(axis=1)
        / net.dcline.p_mw.abs()
    )
...
def create_impedance_collection(
    net, impedances=None, bus_geodata=None, infofunc=None, picker=False, **kwargs
):
    """
    Creates a matplotlib line collection of pandapower lines.
    ...
    coords, impedances_with_geo = coords_from_node_geodata(
        impedances,
        net.impedance.from_bus.loc[impedances].values,
        net.impedance.to_bus.loc[impedances].values,
        bus_geodata if bus_geodata is not None else net["bus_geodata"],
        "impedance",
    )
    ...
    lc = _create_line2d_collection(
        coords, impedances_with_geo, infos=infos, picker=picker, **kwargs
    )
...
def create_trafo_connection_collection(
    ...
    **kwargs,
    ...
    in_geodata = net.trafo.hv_bus.loc[trafos].isin(
        bus_geodata.index
...
def create_trafo3w_connection_collection(
    net, trafos=None, bus_geodata=None, infofunc=None, **kwargs
):
    """
    ...
    in_geodata = (
        net.trafo3w.hv_bus.loc[trafos].isin(bus_geodata.index)
    ...
    hv_geo, mv_geo, lv_geo = (
        list(
            zip(
                *(
                    bus_geodata.loc[trafo_table[column], var].values
                    for var in ["x", "y"]
                )
            )
        )
    ...
    tg = [
        x
        for c in [list(combinations(y, 2)) for y in zip(hv_geo, mv_geo, lv_geo)]
        for x in c
    ]

    # 3 times infofunc for every trafo
    info = [
        infofunc(x) if infofunc is not None else []
...
def create_trafo_collection(
    ...
    **kwargs,
    ...
    coords, trafos_with_geo = coords_from_node_geodata(
        trafos,
        trafo_table.hv_bus.values,
        trafo_table.lv_bus.values,
        bus_geodata,
        "trafo",
    )
    ...
    infos = (
        [infofunc(i) for i in range(len(trafos_with_geo))]
    ...
    if cmap is not None:
        ...
        add_cmap_to_collection(
            lc, cmap, norm, z_duplicated, cbar_title, plot_colormap, clim
        )
    return lc, pc
...
def create_trafo3w_collection(
    ...
    **kwargs,
    ...
    in_geodata = (
        net.trafo3w.hv_bus.loc[trafo3ws].isin(bus_geodata.index)
    ...
    linewidth = kwargs.pop("linewidths", 2.0)
    ...
    if len(circles) == 0:
        return None, None
    lc = LineCollection(
        lines, color=color, picker=picker, linewidths=linewidth, **kwargs
    )
    lc.info = infos
    pc = PatchCollection(
        circles, match_original=True, picker=picker, linewidth=linewidth, **kwargs
    )
    pc.info = infos
    if cmap is not None:
...
def create_vsc_collection(
    ...
    **kwargs,
    ...
    in_geodata = net.vsc.bus.loc[vscs].isin(bus_geodata.index) & net.vsc.bus_dc.loc[
        vscs
    ].isin(bus_dc_geodata.index)
    vscs = vscs[in_geodata]
    vsc_table = net.vsc.loc[vscs]

    coords, vscs_with_geo = coords_from_node_geodata(
        vscs,
        vsc_table.bus.values,
        vsc_table.bus_dc.values,
        bus_geodata,
        "vsc",
        node_geodata_to=bus_dc_geodata,
    )
    ...
    infos = (
        [infofunc(i) for i in range(len(vscs_with_geo))] if infofunc is not None else []
    )
...
def create_vsc_connection_collection(
    ...
    **kwargs,
    ...
    ac_geo = list(
        zip(
            bus_geodata.loc[vsc_table["bus"], "x"].values,
            bus_geodata.loc[vsc_table["bus_dc"], "y"].values,
        )
    )
    dc_geo = list(
        zip(
            bus_dc_geodata.loc[vsc_table["bus_dc"], "x"].values,
...
def create_load_collection(
    net, loads=None, size=1.0, infofunc=None, orientation=np.pi, picker=False, **kwargs
):
    """
    ...
    node_coords = (
        net.bus.loc[net.load.loc[loads, "bus"].values, "geo"]
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(next)
        .to_list()
...
def create_gen_collection(
    net, gens=None, size=1.0, infofunc=None, orientation=np.pi, picker=False, **kwargs
):
    """
    ...
    node_coords = (
        net.bus.loc[net.gen.loc[gens, "bus"].values, "geo"]
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(next)
        .to_list()
...
def create_sgen_collection(
    net, sgens=None, size=1.0, infofunc=None, orientation=np.pi, picker=False, **kwargs
):
    """
    ...
    node_coords = (
        net.bus.loc[net.sgen.loc[sgens, "bus"].values, "geo"]
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(next)
        .to_list()
...
def create_storage_collection(
    ...
    **kwargs,
    ...
    node_coords = (
        net.bus.loc[net.storage.loc[storages, "bus"].values, "geo"]
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(next)
        .to_list()
...
def create_ext_grid_collection(
    ...
    **kwargs,
    ...
    infos = (
        [infofunc(ext_grid_idx) for ext_grid_idx in ext_grids]
        if infofunc is not None
        else []
    )
    ...
    node_coords = (
        net.bus.geo.loc[ext_grid_buses]
        .apply(geojson.loads)
        .apply(geojson.utils.coords)
        .apply(next)
        .values.tolist()
...
def create_line_switch_collection(
    net, switches=None, size=1, distance_to_bus=3, use_line_geodata=False, **kwargs
):
    """
    ...
    for switch in switches:
        ...
        if sb not in net.bus.index or target_bus not in net.bus.index:
            logger.warning(
                f"Bus coordinates for switch {switch} not found, skipped switch!"
            )
            continue

        # switch bus and target coordinates
        pos_sb = np.array(
            next(geojson.utils.coords(geojson.loads(net.bus.loc[sb, "geo"])))
        )
        pos_tb = np.zeros(2)
        ...
        if use_line_geodata:
            if line.name in net.line.geo.dropna().index:
                ...
                if len(line_coords) >= 2:
                    if (
                        abs(line_coords[0][0] - pos_sb[0]) < 0.01
                        and abs(line_coords[0][1] - pos_sb[1]) < 0.01
                    ):
        ...
        if not use_line_geodata or use_bus_geodata:
            pos_tb = np.array(
                next(
                    geojson.utils.coords(geojson.loads(net.bus.loc[target_bus, "geo"]))
                )
            )
        ...
        patch = Rectangle(
            (pos_sw[0] - size / 2, pos_sw[1] - size / 2),
            size,
            size,
            facecolor=col,
            edgecolor=color,
...
def create_bus_bus_switch_collection(
    ...
    **kwargs,
    ...
    lbs_switches = net.switch.index[
        (net.switch.et == "b") & (net.switch.index.isin(switches))
    ]
    color = kwargs.pop("color", "k")
    switch_patches = []
    line_patches = []
    for switch in lbs_switches:
        ...
        if (
            switch_bus not in net.bus.index
            or target_bus not in net.bus.index
            or pd.isnull(net.bus.loc[switch_bus, "geo"])
            or pd.isnull(net.bus.loc[target_bus, "geo"])
        ):
            logger.warning(
                "Bus coordinates for switch %s not found, skipped switch!" % switch
            )
            continue
        # switch bus and target coordinates
        pos_sb = np.array(
            next(geojson.utils.coords(geojson.loads(net.bus.loc[switch_bus, "geo"])))
        )
        pos_tb = np.array(
            next(geojson.utils.coords(geojson.loads(net.bus.loc[target_bus, "geo"])))
        )
        # position of switch symbol
        vec = pos_tb - pos_sb
    ...
    switches = PatchCollection(switch_patches, match_original=True, **kwargs)
    helper_lines = LineCollection(
        line_patches,
        linestyles=helper_line_style,
        linewidths=helper_line_size,
...
def create_ward_collection(
    ...
    **kwargs,
    ...
    ward_pc, ward_lc = _create_node_element_collection(
        ...
        line_color=color,
        **kwargs,
    )  # patch_facecolor=color, patch_edgecolor=color
...
def draw_collections(
    ...
    if ax is None:
        ...
        plt.subplots_adjust(
            left=0.01, right=0.99, top=0.99, bottom=0.05, wspace=0.02, hspace=0.04
        )
    ax = ax or plt.gca()

    add_collections_to_axes(
        ax,
        collections,
...
def add_collections_to_axes(
    ax, collections, plot_colorbars=True, copy_collections=True
):
    if not MATPLOTLIB_INSTALLED:
