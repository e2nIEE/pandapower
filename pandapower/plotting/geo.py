...
def set_bus_geodata_from_line_geodata(net, buses=None):
    """
    Sets bus geodata from connected line geodata.

    For each bus (default: all buses in the net), this function finds the first
    connected line (either as from_bus or to_bus) and extracts the line's
    coordinates. The bus geometry is then set to a Point with those coordinates.

    :param net: The pandapower network instance containing bus and line data
    :type net: pandapowerNet
    :param buses: Optional iterable of bus indices to process; if None, all buses
                  are processed
    :type buses: iterable of int, optional
    :return: None
    """
    buses = buses if buses is not None else net.bus.index
    for b in buses:
        if b in net.line.from_bus.values:
            lines, where = net.line.index[net.line.from_bus == b], 0
        elif b in net.line.to_bus.values:
            lines, where = net.line.index[net.line.to_bus == b], -1
        else:
            print((f"bus {b} not connected to any line"))
            continue

        for line in lines:
            c = net.line.loc[line, "geo"]["coordinates"]
            if not c:
                print((f"line {line} has no coordinates"))
                continue
            x, y = c[where]
            break
        else:
        ...
        net.bus.at[b, "geo"] = {'type': 'Point', 'coordinates': [x, y]}
