...
def simple_plot(net=None, respect_switches=False
                , line_width=1.0, bus_size=None, ext_grid_size=None
    ...
    if len(net.line_geodata) == 0 and len(net.bus_geodata) == 0:
        logger.warning("No or insufficient geodata available --> Creating artificial coordinates. This may take some time")
        plot.create_generic_coordinates(net, respect_switches=respect_switches)

    if bus_size or ext_grid_size is None:
        # if either bus_size or ext_grid size is None -> calc size from distance between min and max geocoord
        mean_distance_between_buses = sum((net['bus_geodata'].max() - net['bus_geodata'].min()) / 200)
        # set the bus / ext_grid sizes accordingly
        # Comment: This is implemented because if you would choose a fixed values (e.g. bus_size = 0.2), the size
    ...
    lc = plot.create_line_collection(net, net.line.index, color=line_color, linewidths=line_width, use_line_geodata=use_line_geodata)
    sc = plot.create_bus_collection(net, net.ext_grid.bus.values, patch_type="rect", size=ext_grid_size, color=ext_grid_color, zorder=11)
