...
def simple_plot(
    net, respect_switches=False, line_width=1.0, bus_size=1.0, ext_grid_size=1.0,
    trafo_size=1.0, plot_loads=False, plot_gens=False, plot_sgens=False, orientation=None, load_size=1.0,
...
def calculate_unique_angles(net):
    ...
    for index, bus in enumerate(net.bus_geodata.index):
        ...
        try:
            gen_count = len(get_connected_elements_dict(net, element_types=["gen"], buses=index))
        except KeyError:
            gen_count = 0

        try:
            load_count = len(get_connected_elements_dict(net, element_types=["load"], buses=index))
        except KeyError:
            load_count = 0
