...
def to_html(
    ...
    with open(filename, "w") as f:
        html_str = _net_to_html(
            net, respect_switches, include_lines, include_trafos, show_tables
        )
        f.write(html_str)
        f.close()
...
class Tag(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        contents = "".join(
            a.html if isinstance(a, Raw) else escape(str(a)) for a in args
        )
        return Raw("<%s%s>%s</%s>" % (self.name, attr.rstrip(), contents, self.name))


def _net_to_graphjs(
    net, respect_switches=True, include_lines=True, include_trafos=True, images=None
):
    """
    ...
    """
    nodes = [
        {"id": int(id), "label": str(id)}
        for id in net.bus[net.bus.in_service == 1].index
    ]
    edges = []
    ...
    if include_lines:
        ...
        edges += [
            ...
            for fb, tb, length, idx, inservice, imax in list(
                zip(
                    net.line.from_bus,
                    net.line.to_bus,
                    net.line.length_km,
        ...
        edges += [
            {"from": int(fb), "to": int(tb)}
            for fb, tb, idx, inservice in list(
                zip(
                    net.impedance.from_bus,
                    net.impedance.to_bus,
                    net.impedance.index,
        ...
        edges += [
            {"from": int(fb), "to": int(tb), "dashes": "true"}
            for fb, tb, idx, inservice in list(
                zip(
                    net.impedance.from_bus,
                    net.impedance.to_bus,
    ...
    if include_trafos:
        nogotrafos = set(
            net.switch.element[(net.switch.et == "t") & (net.switch.closed == 0)]
        )
        edges += [
            {"from": int(hvb), "to": int(lvb), "label": "trafo"}
            for hvb, lvb, idx, inservice in list(
                zip(
                    net.trafo.hv_bus,
                    net.trafo.lv_bus,
                    net.trafo.index,
        ...
        for _, t3tab in net.trafo3w.iterrows():
            edges += [
                {"from": int(bus1), "to": int(bus2), "label": "trafo3"}
                for bus1, bus2 in combinations(
                    [t3tab.hv_bus, t3tab.mv_bus, t3tab.lv_bus], 2
                )
                if t3tab.in_service
            ]
    ...
    bs = net.switch[
        (net.switch.et == "b") & ((net.switch.closed == 1) | (not respect_switches))
    ]
    edges += [
        {"from": int(b), "to": int(e), "label": "switch"}
...
def _net_to_html(
    ...
    script = _net_to_graphjs(
        net, respect_switches, include_lines, include_trafos, images
    )
    ...
    page = HTML(
        HEAD(STYLE(style)),
        BODY(
            DIV(*tables), DIV(id="net", style="border:1px solid #f1f1f1;max-width:90%")
        ),
