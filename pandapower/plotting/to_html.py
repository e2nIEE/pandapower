...
def _net_to_graphjs(net, respect_switches=True, include_lines=True, include_trafos=True, images=None):
    ...
    if include_lines:
        ...
        nogolines = (
            set(net.switch.element[(net.switch.et == "l") & (net.switch.closed == 0)]) if respect_switches else set()
        )

        edges += [
            {"from": int(fb), "to": int(tb), "label": "%.2f kA %.2f Km" % (imax, length)}
            for fb, tb, length, idx, inservice, imax in zip(
                net.line.from_bus,
                net.line.to_bus,
                net.line.length_km,
                net.line.index,
        ...
        edges += [
            ...
            for fb, tb, idx, inservice in zip(
                net.impedance.from_bus, net.impedance.to_bus, net.impedance.index, net.impedance.in_service
            )
            if inservice == 1
        ]

        edges += [
            {"from": int(fb), "to": int(tb), "dashes": "true", "label": "%.2f kA %.2f Km" % (imax, length)}
            for fb, tb, length, idx, inservice, imax in zip(
                net.line.from_bus,
                net.line.to_bus,
                net.line.length_km,
    ...
    if include_trafos:
        ...
        edges += [
            ...
            for hvb, lvb, idx, inservice in zip(
                net.trafo.hv_bus, net.trafo.lv_bus, net.trafo.index, net.trafo.in_service
            )
    ...
    bs = net.switch[(net.switch.et == "b") & ((net.switch.closed == 1) | (not respect_switches))]
    edges += [{"from": int(b), "to": int(e), "label": "switch"} for b, e, i in zip(bs.bus, bs.element, bs.index)]
