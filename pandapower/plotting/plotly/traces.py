...
def create_edge_center_trace(
    ...
    center_trace = {
        ...
        "hoverinfo": "text",
        "name": trace_name,
        "marker": {"color": color, "size": size, "symbol": patch_type},
...
def _create_node_trace(
    ...
    node_trace["x"], node_trace["y"] = zip(
        *net[node_element]
        .loc[node_plot_index, "geo"]
        .dropna()
        .apply(geojson.loads)
    ...
    if not isinstance(infofunc, pd.Series) and isinstance(infofunc, Iterable) and len(infofunc) == len(nodes):
        infofunc = pd.Series(index=nodes, data=infofunc)
    node_trace["text"] = net[node_element].loc[node_plot_index, "name"] if infofunc is None else infofunc.loc[nodes]
    if legendgroup:
    ...
    if cmap is not None:
        ...
        cmap_vals = net["res_" + node_element].loc[node_plot_index, colormap_column] if cmap_vals is None else cmap_vals
...
def _get_branch_geodata_plotly(net, branches, use_branch_geodata, branch_element="line", node_element="bus"):
    xs = []
    ys = []
    if use_branch_geodata:
        ...
        to_n = "to_" + n
        from_node_xy = (
            net[n].loc[branches[from_n], "geo"].apply(geojson.loads).apply(geojson.utils.coords).apply(next).tolist()
        )
        to_node_xy = (
            net[n].loc[branches[to_n], "geo"].apply(geojson.loads).apply(geojson.utils.coords).apply(next).tolist()
        )
...
def _create_branch_trace(
    ...
    INPUT:
        ...
        respect_separators (bool, True): whether separating elements like switches should be considered
        ...
        separator_element (str, "switch"): name of the separator element in the net. In a
            pandapower net, this is always "switch"
        node_element (str, "bus"): name of the node element in the net. In a pandapower net,
    ...
    if infofunc is not None:
        if not isinstance(infofunc, pd.Series) and isinstance(infofunc, Iterable) and len(infofunc) == len(branches):
        ...
        assert isinstance(infofunc, pd.Series), (
            f"infofunc should be a pandas series with the net.{branch_element}.index to the infofunc contents"
        )
    no_go_branches = set()
    if respect_separators:
        if separator_element == "switch":
            no_go_branches = set(branches) & set(
                net[separator_element].element[
                    (net[separator_element].et == "l") & (net[separator_element].closed == 0)
                ]
            )
        elif separator_element == "valve":
            no_go_branches = set(branches) & set(
                net[separator_element][(~net[separator_element].in_service) | (net[separator_element].opened)]
            )
        else:
    ...
    if use_branch_geodata:
        branches_to_plot = branches_to_plot.loc[np.intersect1d(branches_to_plot.index, net[branch_element].index)]
    else:
        branches_with_geodata = branches_to_plot["from_" + node_element].isin(
            net[node_element].dropna(subset=["geo"]).index
        ) & branches_to_plot["to_" + node_element].isin(net[node_element].dropna(subset=["geo"]).index)
    ...
    if cmap is not None:
        ...
        if cmap_vals is not None:
            ...
            cmap_vals = net["res_" + branch_element].loc[branches_to_plot.index, cmap_vals_category].values
    ...
    if len(no_go_branches) > 0:
        ...
        for idx, branch in no_go_branches_to_plot.iterrows():
            ...
            line_trace["x"], line_trace["y"] = _get_branch_geodata_plotly(
                net, no_go_branches_to_plot.loc[idx:idx], use_branch_geodata, branch_element, node_element
            )
    ...
    if infofunc is not None:
        ...
        assert isinstance(infofunc, pd.Series), (
            "infofunc should be a pandas series with the net.{}.index to the infofunc contents".format(branch_element)
        )
...
def create_trafo_trace(
    ...
    if trafotype == "2W":
        ...
        trafo_buses_with_geodata = net.trafo.hv_bus.isin(net.bus.geo.dropna().index) & net.trafo.lv_bus.isin(
            net.bus.geo.dropna().index
        )
        connections = [["hv_bus", "lv_bus"]]
    elif trafotype == "3W":
        trafotable = "trafo3w"
        trafo_buses_with_geodata = (
            net.trafo3w.hv_bus.isin(net.bus.dropna(subset=["geo"]).index)
...
def create_weighted_marker_trace(
    ...
    trace_name: str = "",
    infofunc: pd.Series[Any] | Callable | None = None,
    node_element: str = "bus",
    show_scale_legend: bool = True,
    scale_marker_size: float | list[float] | None = None,
    ...
    Parameters:
        net: the net where the elements are located
        elm_type: the element table in the net that holds the values to plot
        elm_ids: the element IDs of the elm_type table for which markers will be created.
            If None, all IDs in net[elm_type] that are not 0 or NaN in the
            `column_to_plot` will be used.
        column_to_plot: the column in the `net[elm_type]` table that will be plotted.
            The suffix (everything behind the last '_' in the column name) will be
            used as the unit in the infofunc.
        sizemode: whether the markers' "area" or "diameter"  will be proportional to the represented value
        color: color for the markers
        patch_type: plotly marker style that will be used (other options are triangle-up,
            triangle-down and many more; for non-filled markers, append "-open" cf. https://plotly.com/python/marker-style/)
        marker_scaling: factor to scale the size of all markers
        trace_name: trace name for the legend. If empty, elm_type will be used.
        infofunc: hover-infofuction to overwrite the internal infofunction
        node_element: the name of node elements in the net. "bus" for pandapower networks
        show_scale_legend: display a marker legend at the top right of the plot
        scale_marker_size: adjust the size of the scale, gets multiplied with `marker_scaling`.
            Default size is the average size of the respective weighted markers rounded to 5.
            A list of values will result in several scale marker legends.
        scale_marker_color: specify the color of the scale legend marker. If None, `color` will be used.
        scale_legend_unit: specifies the unit shown in the scale legend marker's string.
            It does not trigger any unit conversions! If None, the last part of `column_to_plot` will be used (all upper case).
        trace_kwargs: additional/updated entries for the marker trace that is created in this function, e.g. {"visible": "legendonly"}
    ...
    elm_ids = (
        net[elm_type].loc[~net[elm_type][column_to_plot].isna()].index.tolist() if elm_ids is None else list(elm_ids)
    )
    ...
    if any(values_by_bus < 0):
        logger.warning(
            "A marker trace cannot be created for negative values!\n"
...
def create_scale_trace(net, weighted_trace, down_shift=0):
    ...
    y_pos_original = y_max - (.2 * (y_max-y_min) * down_shift)
    ...
    **kwargs,
    ...
    if on_map:
        try:
            ...
            logger.warning(
                "Test if geo-data are in lat/long cannot be performed using geopy -> plot errors are possible."
            )
