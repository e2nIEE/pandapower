...
def _create_node_trace(
    ...
    node_plot_index = [
        b for b in nodes if b in list(set(nodes) & set(net[node_element]["geo"].index))
    ]
    ...
    if (
        ...
        and len(infofunc) == len(nodes)
    ):
        infofunc = pd.Series(index=nodes, data=infofunc)
    node_trace["text"] = (
        net[node_element].loc[node_plot_index, "name"]
        if infofunc is None
        else infofunc.loc[nodes]
    )
    if legendgroup:
    ...
    if cmap is not None:
        ...
        if cmap_vals is not None:
            ...
            cmap_vals = (
                net["res_" + node_element].loc[node_plot_index, colormap_column].values
            )

        cmap_vals = (
            net["res_" + node_element].loc[node_plot_index, colormap_column]
...
def _get_branch_geodata_plotly(
    net, branches, use_branch_geodata, branch_element="line", node_element="bus"
):
    xs = []
    ys = []
        ...
        for line_ind, _ in branches.iterrows():
            line_coords = list(
                geojson.utils.coords(
                    geojson.loads(net[branch_element].at[line_ind, "geo"])
                )
            )
...
def _create_branch_trace(
    ...
    branches = (
        net[branch_element].index.tolist() if branches is None else list(branches)
    )
    if len(branches) == 0:
        return []
        ...
        if separator_element == "switch":
            no_go_branches = set(branches) & set(
                net[separator_element].element[
                    (net[separator_element].et == "l")
                    & (net[separator_element].closed == 0)
                ]
            )
        elif separator_element == "valve":
            no_go_branches = set(branches) & set(
                net[separator_element][
                    (~net[separator_element].in_service)
                    | (net[separator_element].opened)
                ]
            )
        else:
            raise NotImplementedError(
                "respect separators is only implements for switches, "
                "not for {separator_element}s."
            )
    branches_to_plot = net[branch_element].loc[
        list(set(net[branch_element].index) & (set(branches) - no_go_branches))
    ]
    no_go_branches_to_plot = None
    use_branch_geodata = (
        use_branch_geodata
        if net[branch_element]["geo"].dropna().shape[0] > 0
        else False
    )
    if use_branch_geodata:
        branches_to_plot = branches_to_plot.loc[
            np.intersect1d(branches_to_plot.index, net[branch_element].index)
        ]
    else:
    ...
    if cmap is not None:
        ...
        if cmap_vals is not None:
            ...
            cmap_vals = (
                net["res_" + branch_element]
                .loc[branches_to_plot.index, cmap_vals_category]
                .values
            )
        ...
        if len(cmap_branches) == len(net[branch_element]):
            ...
            branch_idx_map = dict(
                zip(
                    net[branch_element].loc[branches].index.tolist(),
                    range(len(branches)),
                )
            )
            cmap_branches = [
                cmap_branches[branch_idx_map[idx]] for idx in branches_to_plot.index
            ]
        else:
    ...
    for col_i, (idx, branch) in enumerate(branches_to_plot.iterrows()):
        ...
        if cmap is not None:
            try:
                ...
                logger.warning(
                    "No color and info for {} {:d} (name: {}) available".format(
    ...
    if show_colorbar and cmap is not None:
        ...
        try:
            ...
            x, y = geojson.utils.coords(
                net[node_element].loc(
                    net[branch_element][f"from_{node_element}"][
    ...
    if infofunc is not None:
        ...
        assert isinstance(infofunc, pd.Series), (
            "infofunc should be a pandas series with the net.{}.index to the infofunc contents".format(
                branch_element
            )
        )
...
def create_trafo_trace(
    ...
    if trafotype == "2W":
        ...
        trafo_buses_with_geodata = net.trafo.hv_bus.isin(
    ...
    if cmap is not None:
        ...
        if cmap_vals is not None:
            ...
            cmap_vals = (
                net["res_" + trafotable]
                .loc[trafos_to_plot.index, "loading_percent"]
                .values
            )
    ...
    for col_i, (idx, trafo) in enumerate(trafos_to_plot.iterrows()):
        ...
        for from_bus1, to_bus1 in connections:
            ...
            trafo_trace["text"] = (
                trafo["name"] if infofunc is None else infofunc.loc[idx]
            )

            from_bus = next(
                geojson.utils.coords(
                    geojson.loads(net.bus.loc[trafo[from_bus1], "geo"])
                )
            )
            to_bus = next(
                geojson.utils.coords(geojson.loads(net.bus.loc[trafo[to_bus1], "geo"]))
            )
...
def create_weighted_marker_trace(
    ...
    elm_ids = (
        net[elm_type].loc[~net[elm_type][column_to_plot].isna()].index.tolist()
        if elm_ids is None
        else list(elm_ids)
    )
