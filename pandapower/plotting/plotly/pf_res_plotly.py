...
def pf_res_plotly(
    ...
    hoverinfo = (
        ...
        + "<br />"
        + "V_m = "
        + net.res_bus.vm_pu.round(precision).astype(str)
        + " pu"
        + "<br />"
        + "V_m = "
        + (net.res_bus.vm_pu * net.bus.vn_kv.round(2)).round(precision).astype(str)
        + " kV"
        + "<br />"
        + "V_a = "
        + net.res_bus.va_degree.round(precision).astype(str)
        + " deg"
    ).tolist()
    ...
    hoverinfo = (
        ...
        + "<br />"
        + "I = "
        + net.res_line.loading_percent.round(precision).astype(str)
        + " %"
        + "<br />"
        + "I_from = "
        + net.res_line.i_from_ka.round(precision).astype(str)
        + " kA"
        + "<br />"
        + "I_to = "
        + net.res_line.i_to_ka.round(precision).astype(str)
        + " kA"
        + "<br />"
    ).tolist()
    ...
    hoverinfo = (
        ...
        + "<br />"
        + "I = "
        + net.res_trafo.loading_percent.round(precision).astype(str)
        + " %"
        + "<br />"
        + "I_hv = "
        + net.res_trafo.i_hv_ka.round(precision).astype(str)
        + " kA"
        + "<br />"
        + "I_lv = "
        + net.res_trafo.i_lv_ka.round(precision).astype(str)
        + " kA"
        + "<br />"
    ).tolist()
    hoverinfo = pd.Series(index=net.trafo.index, data=hoverinfo)
    trafo_traces = create_trafo_trace(
        net,
    ...
    if "ext_grid" in net and len(net.ext_grid):
        ...
        ext_grid_trace = create_bus_trace(
            net,
    ...
    return draw_traces(
        line_traces + trafo_traces + ext_grid_trace + bus_trace,
        showlegend=False,
