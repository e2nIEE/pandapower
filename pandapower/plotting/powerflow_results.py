...
def plot_voltage_profile(
    ...
    **kwargs,
    ...
    for eg in sl_buses:
        ...
        for lix, line in net.line[
            net.line.in_service & net.line.index.isin(lines)
        ].iterrows():
            if line.from_bus not in d.index:
                continue
            if not (
                (net.switch.element == line.name)
                & ~net.switch.closed
                & (net.switch.et == "l")
            ).any():
                ...
                if "linewidth" in kwargs or not line_loading_weight:
                    ...
                    ax.plot(
                        x,
                        y,
                        linewidth=0.4 * np.sqrt(net.res_line.loading_percent.at[lix]),
                        color=line_color,
                        **kwargs,
                    )
        ...
        if plot_transformers:
            for trafo_table in ["trafo", "trafo3w"]:
                ...
                for tid, tr in transformers.iterrows():
                    t_buses = [
                        tr[b_col]
                        for b_col in ("lv_bus", "mv_bus", "hv_bus")
                        if b_col in tr.index
                    ]
                    if any(
                        [
                            b not in d.index.values or b not in net.res_bus.index.values
                            for b in t_buses
                        ]
                    ):
        ...
        if xlabel:
            ax.set_xlabel(xlabel, fontweight="bold", color=(0.4, 0.4, 0.4))
        if ylabel:
            ax.set_ylabel(ylabel, fontweight="bold", color=(0.4, 0.4, 0.4))
    return ax
...
def plot_loading(
    ...
    **kwargs,
    ...
    # run plotting code
    loadings = net["res_%s" % element_type].loading_percent.values[
        net["res_%s" % element_type].index.isin(index_subset)
    ]
    ...
    ax.set_ylabel(
        f"{element_type.capitalize()} Loading [%]",
        fontweight="bold",
        color=(0.4, 0.4, 0.4),
    )
    ax.set_xticks([1], [""])
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(
        ncols=2, figsize=(8, 5), gridspec_kw={"width_ratios": [4, 1]}
    )
