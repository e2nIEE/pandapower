...
def parameter_from_std_type(net, parameter, element="line", fill=None):
    ...
    for typ in net[element].std_type.unique():
        if isnull(typ) or not std_type_exists(net, typ, element):
            continue
    ...
    if fill is not None: 
        net[element].loc[isnull(net[element][parameter]).values, parameter] = fill
...
def add_basic_std_types(net):
	...
	### another recommendable references for MV cables is Werth: Netzberechnung mit Erzeugungsporfilen
   ...
   ##Overhead Lines, all from S.742f, Heuck: Elektrische Energieversorgung - Vierweg+Teubner 2013
   ...
   ##High Voltage
	...
	# another recommendable references for distribution transformers is Werth: Netzberechnung mit Erzeugungsporfilen
    "160 MVA 380/110 kV":
        {"i0_percent": 0.06,
         "pfe_kw": 60, 
         "vscr_percent": 0.25,
