...
import numpy as np
import cmath
import pandapower.harmonicanalysis.unbalancedharmonic.unbalanced_harmonic_voltage_distorion_calculation as har_vol_thd
...
bus = pd.read_excel(path + '\\CIGRE_LV.xlsx', sheet_name = 'Bus') 
line = pd.read_excel(path + '\\CIGRE_LV.xlsx', sheet_name = 'Line') 
load = pd.read_excel(path + '\\CIGRE_LV.xlsx', sheet_name = 'Load') 
...
thd_a, thd_b, thd_c, har_vol_a, har_vol_b, har_vol_c = har_vol_thd.unbalanced_thd_voltage(net, harmonics, har_a, har_a_angle, har_b, har_b_angle, har_c, har_c_angle,\
