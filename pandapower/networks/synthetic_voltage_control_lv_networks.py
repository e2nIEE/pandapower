...
def create_synthetic_voltage_control_lv_network(network_class="rural_1"):
    ...
    line_types = {
        "rural_1": ["NAYY 4x150 SE"] * 2 + [("NAYY 4x150 SE", "NAYY 4x120 SE")],
        "rural_2": ["NAYY 4x35"] + ["NF 4x70"] * 3,
        "village_1": ["NAYY 4x150 SE"] * 4,
        "village_2": [
            ("NAYY 4x70", "NF 4x50"),
            ("NAYY 4x120 SE", "NF 4x95"),
            ("NAYY 4x95", "NF 4x70"),
            ("NAYY 4x150 SE", "NF 4x120"),
        ],
    ...
    for i in idx_feeder:
        # buses
        lv_buses[i] = create_buses(
            net, n_feeder[network_class][i], 0.4, zone="Feeder" + str(i + 1), type="m"
        )
        house_buses[i] = create_buses(
            net, n_feeder[network_class][i], 0.4, zone="Feeder" + str(i + 1), type="n"
        )
        # lines
        lines = pd.DataFrame()
        ...
        if line_type_change_at[network_class][i] is nan:
            lines["std_type"] = [line_types[network_class][i]] * n_feeder[
                network_class
            ][i] + [house_connection_type[network_class]] * n_feeder[network_class][i]
        else:
            lines["std_type"] = (
                [line_types[network_class][i][0]]
                * line_type_change_at[network_class][i]
                + [line_types[network_class][i][1]]
                * (n_feeder[network_class][i] - line_type_change_at[network_class][i])
        ...
        lines["length"] = [l_lines[network_class][i]] * n_feeder[network_class][i] + [
            house_connection_length[network_class]
    ...
    net.bus.geo = net.bus.apply(
        lambda row: dumps(Point(bus_geo[network_class][row.name])), axis=1
    )
    return net
