...
from pandapower.results_bus import (
    _get_bus_results,
    _get_bus_dc_results,
    _set_buses_out_of_service,
    _get_shunt_results,
    _get_p_q_results,
    _get_bus_v_results,
    _get_bus_v_results_3ph,
    _get_p_q_results_3ph,
    _get_bus_results_3ph,
    _get_bus_dc_v_results,
    _get_p_dc_results,
    _set_dc_buses_out_of_service,
)
from pandapower.results_gen import (
    _get_gen_results,
    _get_gen_results_3ph,
    _get_dc_slack_results,
)
...
def _extract_results(net, ppc):
    ...
    _set_dc_buses_out_of_service(
        ppc
    )  # for NaN results in net.res_bus_dc for inactive buses
    ...
    if len(net["b2b_vsc"]) > 0:
        ...
        net.res_b2b_vsc = res_vsc.groupby("group").agg(
            ...
            vm_internal_pu=(
                "vm_internal_pu",
                "mean",
            ),  # The internal vm_pu is the average of both vsc
            ...
            vm_internal_dc_pu_p=(
                "vm_internal_dc_pu",
                "first",
            ),  # Internal dc set point for the plus bus
...
def _get_aranged_lookup(net, bus_table="bus"):
    ...
    bus_lookup_aranged[net[bus_table].index.values] = np.arange(
        len(net[bus_table].index.values)
    )
...
def verify_results(net, mode="pf"):
    ...
    for element in elements:
        ...
        index_equal = (
            False
            if res_element not in net
            else net[element].index.equals(net[res_element].index)
        )
        if not index_equal:
            if net["_options"]["init_results"] and element == "bus":
                ...
                raise UserWarning(
                    "index of result table '{}' is not equal to the element table '{}'. The init result"
                    " option may lead to a non-converged power flow.".format(
                        res_element, element
                    )
                )
...
def get_result_tables(element, suffix=None):
    ...
    res_element_with_suffix = (
        res_element if suffix is None else res_element + "_%s" % suffix
    )
...
def init_element(net, element, suffix=None):
    ...
    if len(index):
        ...
        if res_empty_element in net:
            ...
            net[res_element] = pd.DataFrame(
                np.nan, index=index, columns=columns, dtype="float"
            )
        else:
...
def _ppci_branch_to_ppc(result, ppc):
    ...
    ppc["branch"][result["internal"]["branch_is"], :branch_cols] = result["branch"][
        :, :branch_cols
    ]
...
def _ppci_internal_to_ppc(result, ppc):
    for key, value in result["internal"].items():
        ...
        if key in BRANCH_RESULTS_KEYS:
            ...
            update_matrix[result["internal"]["branch_is"], : value.shape[1]] = result[
                "internal"
            ][key]
            ppc["internal"][key] = update_matrix
            if "br_res_ks_ppci_bus" in result["internal"]:
                br_res_ks_ppci_bus = np.r_[
                    result["internal"]["br_res_ks_ppci_bus"], [-1]
                ]
            else:
                br_res_ks_ppci_bus = np.r_[np.arange(value.shape[1]), [-1]]
            ppc["internal"][key] = pd.DataFrame(
                data=update_matrix, columns=br_res_ks_ppci_bus
            )
        else:
