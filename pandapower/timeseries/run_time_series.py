...
def output_writer_routine(
    net, time_step, pf_converged, ctrl_converged, recycle_options
):
    output_writer = net["output_writer"].iat[0, 0]
    ...
    output_writer.save_results(
        ...
        ctrl_converged=ctrl_converged,
        recycle_options=recycle_options,
    )


def _call_output_writer(net, time_step, pf_converged, ctrl_converged, ts_variables):
    output_writer_routine(
        net, time_step, pf_converged, ctrl_converged, ts_variables["recycle_options"]
    )
...
def run_time_step(
    ...
    output_writer_fct=_call_output_writer,
    **kwargs,
):
    """
...
def _check_output_writer_recyclability(net, recycle, run):
    ...
    for output in ow.log_variables:
        ...
        if (
            table not in ["res_bus", "res_line", "res_trafo", "res_trafo3w"]
            or recycle["trafo"]
            or len(output) > 2
        ):
...
def get_recycle_settings(net, **kwargs):
    ...
    if recycle is not False:
        ...
        if recycle is not False:
            recycle = _check_output_writer_recyclability(
                net, recycle, kwargs.get("run", kwargs.get("run_control_fct"))
            )
...
def init_time_steps(net, time_steps, **kwargs):
    ...
    if not isinstance(time_steps, Iterable):
        if isinstance(time_steps, tuple):
            ...
            logger.warning(
                "start_step and stop_step are depricated. "
            ...
            logger.warning(
                "No time steps to calculate are specified. "
                "I'll check the datasource of the first controller for avaiable time steps"
            )
            ds = net.controller.object.at[0].data_source
            if ds is None:
                raise UserWarning(
                    "No time steps are specified and the first controller doesn't have a data source"
...
def init_time_series(
    net, time_steps, continue_on_divergence=False, verbose=True, **kwargs
):
    """
    ...
    if hasattr(run, "__name__") and (
        run.__name__ == "runpp" or run.__name__ == "rundcpp"
    ):
    ...
    **kwargs,
    ...
    for i, time_step in enumerate(ts_variables["time_steps"]):
        print_progress(
            i,
            time_step,
            ts_variables["time_steps"],
            ts_variables["verbose"],
            ts_variables=ts_variables,
            **kwargs,
        )
        run_time_step(
            net, time_step, ts_variables, run_control_fct, output_writer_fct, **kwargs
        )
...
def run_timeseries(
    ...
    **Controllers**. Optionally other functions than the pp power flow can be called by setting the run function in
    ...
    ts_variables = init_time_series(
        net, time_steps, continue_on_divergence, verbose, **kwargs
    )
    ...
    if check_controllers:
        control_diagnostic(
            net
        )  # produces significant overhead if you run many timeseries of short duration
