...
# set the parameters
def doc_parameters(net,relay_configuration,tripping_time_auto=None,tripping_time_manual=None,sc_fraction=0.95,
                   overload_factor=1.2, ct_current_factor=1.25, safety_factor=1, pickup_current_manual=None):  # all the factors can be re-considered
    
    """
     The main function to create relay settings using doc parameters
        ...
        INPUT:
            ...
            **relay_configuration** (Dict,None) - For directional relay, additional informations are needed
                ...
                - relay_configuration = User defined relay configurations are given as a dataframe with columns as'Switch_name': [switch_id,'type', 'direction', OSA, RCA]}
         ...
         OUTPUT:
             **return** (DataFrame,float) - Return relay setting as a dataframe with columns as 'switch_id','line_id','bus_id','I_g[kA]','I_gg[kA]', 't_g[s]','t_gg[s]', 'MTA','direction' 'OSA','RCA'
"""
     
    # get the user input confiuration of directional relay switch direction     
    doc_relay_settings = pd.DataFrame(columns = ["switch_id","line_id","bus_id","I_g[kA]","I_gg[kA]", "t_g[s]","t_gg[s]","MTA","direction","OSA","RCA"])
    ...
    for switch_id in net.switch.index:
        if (net.switch.closed.at[switch_id]) & (net.switch.type.at[switch_id] == "CB_dir") & (net.switch.et.at[switch_id] == "l"):
            ...
            for index, configuration in relay_configuration.iterrows() :
                if configuration[0]==switch_id:
                    
                    # biasing of the relay(forward or reverse)
                    direction=configuration[2]
            ...
            settings = pd.DataFrame([{"switch_id": switch_id, "line_id": line_idx, "bus_id": bus_idx,
                 "I_g[kA]": I_g, "I_gg[kA]": I_gg, "t_g[s]": t_g, "t_gg[s]": t_gg, "MTA": MTA,
                 "direction":direction,"OSA":OSA, "RCA": RCA }])
            doc_relay_settings = pd.concat([ doc_relay_settings,settings],ignore_index=True)
    return doc_relay_settings
...
def  run_fault_scenario_doc(net, sc_line_id, sc_location,relay_settings):
    ...
    for index, settings in relay_settings.iterrows():
        ...
        i_ka,vi_angle = doc_get_measurement_at_relay_location(net_sc, settings)
        
        trip_decision_doc = doc_get_trip_decision(net_sc, settings, i_ka,vi_angle)
...
if __name__ == "__main__":
    ...
    tripping_time_manual= pd.DataFrame({'switch_id': [0,1,2,3,4],
                                    't_gg': [100,100,100,100,100],
                                    't_g':[1400,1300,1200,1100,1000]})
    
    pickup_current_manual= pd.DataFrame({'switch_id': [0,1,2,3,4],
                                       'I_gg': [1.7,1.6,1.5,1.2,1.3],
                                       'I_g':[0.8,0.7,0.7,0.8,0.82]})
    
        #idx,type,direction,MSA RCA  # MCA:Max Sector Angle, RCA: Relay characteristics angle- shoulb be betwen 30 and 60
    relay_configuration = pd.DataFrame({'switch_id':[0,1,2,3,4],
                                        'switch_type':['CB_dir','CB_dir','CB_dir','CB_dir','CB_dir' ],
                                        'direction':['forward','forward','forward','forward','forward'],
                                        'OSA':[86,86,86,86,86],
                                        'RCA':[45,45,45,45,45]})

    
    doc_relay_settings=  doc_parameters(net,relay_configuration=relay_configuration,tripping_time_auto=[0.07,0.5,0.3])
    
    # here user can define the time grade according to their choice # by default  Tgg is 0.07 and Tg=0.5 Tdelta=0.3
    trip_decisions,net_sc= run_fault_scenario_doc(net,sc_line_id=4,sc_location =0.5,relay_settings=doc_relay_settings)
