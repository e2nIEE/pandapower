...
class OCRelay(ProtectionDevice):
    ...
    def create_protection_function(self, net):
        ...
        if (net.switch.closed.at[self.switch_index] == True) and (
            net.switch.et.at[self.switch_index] == "l"
        ):
        ...
        elif (net.switch.closed.at[self.switch_index] == True) and (
            net.switch.et.at[self.switch_index] == "t" or "b"
        ):
            bus_idx = net.switch.bus.at[self.switch_index]
            line_idx = None
        else:
            raise ValueError(
                "OC Relay must be connected to line or transformer element"
            )
        ...
        if self.oc_relay_type == "DTOC":
            if self.pickup_current_manual is None:
                self.I_g = (
                    float(net_sc.line.max_i_ka.iloc[line_idx])
                    * self.overload_factor
                    * self.ct_current_factor
                )
                self.I_gg = (
                    float(net_sc.res_line_sc.ikss_ka.iloc[line_idx])
                    * self.safety_factor
                )
            else:
                self.I_g = float(self.pickup_current_manual.I_g.iloc[self.switch_index])
                self.I_gg = float(
                    self.pickup_current_manual.I_g.iloc[self.switch_index]
                )
        ...
        if self.oc_relay_type == "IDTOC":
            ...
            self.time_grading = time_grading(
                net,
                [self.time_settings[0], self.time_settings[1], self.time_settings[2]],
            )
            self.t_g = float(self.time_grading.t_g[self.switch_index])
            self.t_gg = float(self.time_grading.t_gg[self.switch_index])
            # calculate time grading again for IDMT part to obtain t_grade and tms
            self.time_grading = time_grading(
                net, [self.time_settings[3], self.time_settings[4]]
            )
            self.tms = float(self.time_grading.t_gg[self.switch_index])
            self.t_grade = float(self.time_grading.t_g[self.switch_index])
    ...
    def protection_function(self, net, scenario):
        ...
        if self.oc_relay_type == "IDMT":
            if i_ka > self.I_s:
                self.tripped = True
                act_time_s = (self.tms * self.k) / (
                    ((i_ka / self.I_s) ** self.alpha) - 1
                ) + self.t_grade
            else:
                self.tripped = False
                act_time_s = np.inf
    ...
    def plot_protection_characteristic(
        ...
        if self.oc_relay_type == "DTOC":
            plt.loglog(0, 0)
            plt.step(
                np.array([self.I_g * 1000, self.I_gg * 1000, xmax]),
                np.array([ymax, self.t_g, self.t_gg]),
            )
            ...
            plt.loglog(
                x,
                (self.tms * self.k) / (((x / (1000 * self.I_s)) ** self.alpha) - 1)
                + self.t_grade,
            )

        elif self.oc_relay_type == "IDTOC":
            x = np.logspace(
                np.log10((1000 * self.I_s) + 0.001), np.log10(1000 * self.I_g), num=num
            )
            plt.loglog(
                x,
                (self.tms * self.k) / (((x / (1000 * self.I_s)) ** self.alpha) - 1)
            ...
            plt.step(
                ...
                np.array(
                    [
                        (self.tms * self.k)
                        / (((self.I_g / self.I_s) ** self.alpha) - 1)
                        + self.t_grade,
                        self.t_g,
                        self.t_gg,
                    ]
...
def time_grading(net, time_settings):
    ...
    if isinstance(time_settings, list):
        ...
        protection_time_settings = protection_time_settings.sort_values(
            by=["switch_id"]
        )
    ...
    if isinstance(time_settings, pd.DataFrame):
        ...
        if time_settings.columns.values.tolist() == [
            "switch_id",
            "t_gg",
            "t_gg",
            "tms",
