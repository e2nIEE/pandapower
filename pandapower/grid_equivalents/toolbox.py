...
def create_eq_loads(net, buses, branches=None, idx_start=None, sign=1,
    ...
    if len(buses - set(net.res_bus.index)) or net.res_bus.loc[
        buses, ["p_mw", "q_mvar"]].isnull().any().any():
        logger.warning("No loads could be added to 'net_ib_eq_load' since bus results " +
                       "are missing.")
        return pd.Index()
    ...
    for no, (bus, name) in enumerate(zip(buses, names)):
        ...
        p = get_branch_power(net, bus, "p_mw", bra)
        q = get_branch_power(net, bus, "q_mvar", bra)
        new = pp.create_load(net, bus, p, q, name=name, index=idx, **kwargs)
        if "origin_id" in net.load.columns:
            net.load.origin_id.loc[new] = eq_name("load", other_zone, zone, number=no)
        new_idx.append(new)
...
def nets_ib_by_bus_zone_with_boundary_branches(
    ...
    if eq_type is not None and eq_type not in ["load", "rei", "ward", "xward"]:
    ...
    for zone in boundary_buses.keys():
        ...
        elif eq_type == "load" and not separate_eqs:
            eq_loads = create_eq_loads(nets_ib[zone], boundary_buses[zone]["external"],
                                       idx_start=net.load.index.values.max() + n_add_load1,
                                       zone=zone)
            n_add_load1 += len(eq_loads)
            if group_imported:
                elm_col = kwargs.get("elm_col", None)
                if elm_col is not None:
                    eq_loads = nets_ib[zone]["load"][elm_col].loc[eq_loads]
                Group(nets_ib[zone], {"load": eq_loads}, name=eq_name("load", zone=zone),
                      elm_col=elm_col)
            continue
        ...
        for other_zone, bb in boundary_buses[zone].items():
            ...
            if eq_type == "load" and separate_eqs:
                eq_loads = create_eq_loads(nets_ib[zone], bb,
                                           idx_start=net.load.index.values.max() + n_add_load1,
                ...
                if group_imported:
                    ...
                    if elm_col is not None:
                        eq_loads = nets_ib[zone]["load"][elm_col].loc[eq_loads]
                    Group(nets_ib[zone], {"load": eq_loads},
                          name=eq_name("load", other_zone, zone), elm_col=elm_col)
...
def split_grid_by_bus_zone_with_boundary_buses(
    ...
    for zone, buses in nets_ib_buses.items():
        ...
        if eq_type is None:
            ...
            eq_loads = create_eq_loads(nets_ib[zone], boundary_buses[zone]["all"], zone=zone,
                                       idx_start=net.load.index.values.max() + n_add_load1)
            n_add_load1 += len(eq_loads)
            if group_imported:
                elm_col = kwargs.get("elm_col", None)
        ...
        for other_zone, bb in boundary_buses[zone].items():
            ...
            if eq_type == "load" and separate_eqs:
                eq_loads = create_eq_loads(nets_ib[zone], bb,
                                           idx_start=net.load.index.values.max() + n_add_load1,
                                           name="equivalent load", zone=zone, other_zone=other_zone)
                n_add_load1 += len(eq_loads)
                if group_imported:
                    elm_col = kwargs.get("elm_col", None)
                    if elm_col is not None:
...
if __name__ == "__main__":
    ...
    if 1:  # split and merge by bus zones - both directions
        import RPC2

        net1 = RPC2.TSO_DSO_net()
        _, _, nets_i, _, _, _, nets_b = split_grid_by_bus_zone_with_boundary_branches(net1)
        net2 = merge_splitted_grid_by_bus_zone(nets_i, nets_b)
        net2["OPF_converged"] = net1["OPF_converged"]
        assert pp.nets_equal(net1, net2)
    if 1:  # split and merge at hv mv connection - both directions
        import RPC2

        net1 = RPC2.TSO_DSO_net()
        mv_nets, hv_net = split_grid_at_hvmv_connections(net1)
        net2 = merge_grid_at_hvmv_connections(hv_net, mv_nets)
        assert pp.nets_equal(net1, net2)
