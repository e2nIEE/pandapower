...
def _runpp_except_voltage_angles(net, **kwargs):
    if (
        "calculate_voltage_angles" not in kwargs
        or not kwargs["calculate_voltage_angles"]
    ):
        runpp(net, **kwargs)
    else:
        try:
            ...
            runpp(net, **kwargs1)
            logger.warning(
                "In grid equivalent generation, the power flow did converge only without"
...
def add_ext_grids_to_boundaries(
    ...
    **kwargs,
    ...
    buses_to_add_ext_grids = (
        set(boundary_buses)
        - set(net.ext_grid.bus[net.ext_grid.in_service])
    ...
    add_eg = []
    vms = pd.Series(np.ones(len(buses_to_add_ext_grids)), index=buses_to_add_ext_grids)
    vas = pd.Series(np.zeros(len(buses_to_add_ext_grids)), index=buses_to_add_ext_grids)
    vms.loc[btaegwr] = net.res_bus.vm_pu.loc[btaegwr]
    vms.loc[pd.Index(net.gen.bus.loc[net.gen.in_service]).intersection(vms.index)] = (
        net.gen.vm_pu.loc[
            net.gen.in_service & net.gen.bus.isin(vms.index) & ~net.gen.bus.duplicated()
        ].values
    )  # avoid
    ...
    for ext_bus, vm, va in zip(buses_to_add_ext_grids, vms, vas):
        add_eg += [create_ext_grid(net, ext_bus, vm, va, name="assist_ext_grid")]
        new_bus = create_bus(net, net.bus.vn_kv[ext_bus], name="assist_bus")
        create_impedance(
            net, ext_bus, new_bus, 1e6, 1e6, net.sn_mva, name="assist_impedance"
        )
    ...
    if adapt_va_degree and net.gen.slack.any() and net.ext_grid.shape[0]:
        ...
        try:
            ...
            if allow_net_change_for_convergence:
                ...
                imp_neg = (
                    net.impedance[["xft_pu"]].loc[imp_neg].sort_values("xft_pu").index
                )
                for no, idx in enumerate(imp_neg):
                    net.impedance.loc[
                        idx, ["rft_pu", "rtf_pu", "xft_pu", "xtf_pu"]
                    ] *= -1
                    try:
                        runpp_fct(
                            net,
                        ...
                        logger.warning(
                            "The sign of these impedances were changed to enable a power"
                            f" flow: {imp_neg[:no]}"
                        )
                        break
                    except LoadflowNotConverged:
                ...
                if not net.converged:
                    net.impedance.loc[
                        imp_neg, ["rft_pu", "rtf_pu", "xft_pu", "xtf_pu"]
                    ] *= -1
                    ...
                    if changes:
                        try:
                            ...
                            logger.warning(
                                "Reactances of these impedances has been increased to "
        ...
        runpp_fct(
            net, calculate_voltage_angles=calc_volt_angles, max_iteration=100, **kwargs
        )
...
def drop_internal_branch_elements(net, internal_buses, branch_elements=None):
    ...
    if branch_elements is not None:
        bebd = {
            elm: bus_types for elm, bus_types in bebd.items() if elm in branch_elements
        }
...
def calc_zpbn_parameters(
    net, boundary_buses, all_external_buses, slack_as="gen", existing_shift_degree=False
):
    """
    ...
    if ((net.trafo.hv_bus.isin(be_buses)) & (net.trafo.shift_degree != 0)).any() or (
        (net.trafo3w.hv_bus.isin(be_buses))
        & ((net.trafo3w.shift_mv_degree != 0) | (net.trafo3w.shift_lv_degree != 0))
    ).any():
        existing_shift_degree = True
        logger.info(
            "Transformers with non-zero shift-degree are existed,"
    ...
    if slack_as == "gen":
        elements = set(
            [
                ("load", "res_load", "load_separate", "sn_load_separate", -1),
    ...
    for i in all_external_buses:
        for ele, res_ele, power, sn, sign in elements:
            if (
                i in net[ele].bus.values
                and net[ele].in_service[net[ele].bus == i].values.any()
            ):
                ind = list(net[ele].index[net[ele].bus == i].values)
                # act. values --> ref. values:
                S.loc[k, power] += (
                    sum(net[res_ele].p_mw[ind].values * sign) / net.sn_mva
                    + 1j * sum(net[res_ele].q_mvar[ind].values * sign) / net.sn_mva
                )
                S.loc[k, sn] = (
                    sum(net[ele].sn_mva[ind].values) + 1j * 0
                    if ele != "ext_grid"
                    else 1e6 + 1j * 0
                )
        ...
        S.loc[k, "v_cpx"] = S.v_m[k] * np.exp(
            1j * net.res_bus.va_degree[i] * np.pi / 180
        )
        k = k + 1
    ...
    Y.columns = [
        "ext_bus",
        "load_ground",
        "load_integrated_total",
        "load_separate_total",
        "gen_ground",
        "gen_integrated_total",
        "gen_separate_total",
        "sgen_ground",
        "sgen_integrated_total",
        "sgen_separate_total",
    ]
    ...
    for elm in ["load", "gen", "sgen"]:
        if existing_shift_degree:
            Y[elm + "_ground"] = (
                S[elm + "_separate"].values / S.v_cpx.values
            ).conjugate() / S.v_cpx.values
        else:
            Y[elm + "_ground"] = S[elm + "_separate"].values.conjugate() / np.square(
                S.v_m
            )
        I_elm_integrated_total = sum(
            (S[elm + "_separate"].values / S.v_cpx.values).conjugate()
        )
        if I_elm_integrated_total == 0:
            Y[elm + "_integrated_total"] = float("nan")
        else:
            vm_elm_integrated_total = (
                S[elm + "_integrated"][0] / I_elm_integrated_total.conjugate()
            )
            if existing_shift_degree:
                Y[elm + "_integrated_total"] = (
                    -S[elm + "_integrated"][0] / vm_elm_integrated_total
                ).conjugate() / vm_elm_integrated_total
            else:
                Y[elm + "_integrated_total"] = -S[elm + "_integrated"][
                    0
                ].conjugate() / np.square(abs(vm_elm_integrated_total))
    ...
    limits = pd.DataFrame(
        [], index=range(nb_ext_buses), columns=["min_vm_pu", "max_vm_pu", "ext_bus"]
    )
...
def _ensure_unique_boundary_bus_names(net, boundary_buses):
    ...
    net.bus.loc[boundary_buses, "name_equivalent"] = [
        "Boundary bus " + str(uuid.uuid1()) for _ in boundary_buses
    ]
...
def drop_assist_elms_by_creating_ext_net(net, elms=None):
    ...
    for elm in elms:
        target_elm_idx = net[elm].index[
            net[elm]
            .name.astype(str)
            .str.contains("assist_" + elm, na=False, regex=False)
        ]
...
def drop_measurements_and_controllers(net, buses, skip_controller=False):
    ...
    if len(net.measurement):
        ...
        lines = net.line.index[
            (net.line.from_bus.isin(buses)) & (net.line.from_bus.isin(buses))
        ]
        drop_measurements_at_elements(net, "line", idx=lines)
        trafos = net.trafo3w.index[
...
def ensure_origin_id(net, elms=None):
    ...
    for elm in elms:
        ...
        net[elm].loc[idxs, "origin_id"] = [
            "%s_%i_%s" % (elm, idx, str(uuid.uuid4())) for idx in idxs
        ]


def drop_and_edit_cost_functions(
    net, buses, drop_cost, add_origin_id, check_unique_elms_name=True
):
    """
        ...
        if len(net[cost_elm]):
            ...
            if drop_cost:
                ...
                for elm in set(net[cost_elm].et.values):
                    idx = net[cost_elm].element.index[
                        (net[cost_elm].et == elm)
                        & (net[cost_elm].element.isin(net[elm].index))
                    ]
                    net[cost_elm].loc[idx, "bus"] = (
                        net[elm].bus.loc[net[cost_elm].element.loc[idx]].values
                    )
                to_drop = net[cost_elm].index[
                    net[cost_elm].bus.isin(buses) | net[cost_elm].bus.isnull()
                ]
            ...
            if add_origin_id:
                ...
                for elm in set(net[cost_elm].et.values):
                    ...
                    net[cost_elm].loc[idx, "et_origin_id"] = (
                        net[elm].origin_id.loc[net[cost_elm].element.loc[idx]].values
                    )
                    net[cost_elm].loc[idx, "origin_idx"] = idx
                    net[cost_elm].loc[idx, "origin_seq"] = [
                        cost_backup.index.tolist().index(t) for t in idx
                    ]
        ...
        if len(net[cost_elm]):
            if "ward" not in eq_type:
                ...
                for pc in net[cost_elm].itertuples():
                    new_idx = (
                        net[pc.et].index[net[pc.et].origin_id == pc.et_origin_id].values
                    )
                    net[cost_elm].loc[pc.Index, "element"] = new_idx[0]
            net[cost_elm] = net[cost_elm].drop(
                columns=["bus", "et_origin_id", "origin_idx", "origin_seq"]
            )
...
def _check_network(net):
    ...
    if net.res_bus.vm_pu.isnull().any():
        logger.info(
            "There are some inactive buses. It is suggested to remove "
    ...
    if len(net.controller):
        for i in net.controller.index:
            ...
            if et is not None and len(net[et]) != len(set(net[et].name.values)):
                raise ValueError(
                    "if controllers are used, please give a name for every "
...
def get_boundary_vp(net_eq, bus_lookups):
    ...
    p_boundary = (
        net_eq.res_ext_grid.values
        + net_eq.res_bus[["p_mw", "q_mvar"]]
        .loc[bus_lookups["bus_lookup_pd"]["b_area_buses"]]
        .values
    )
    p_boundary = pd.DataFrame(
        p_boundary,
        index=bus_lookups["bus_lookup_pd"]["b_area_buses"],
        columns=["p_mw", "q_mvar"],
    )
...
def adaptation_phase_shifter(net, v_boundary, p_boundary):
    ...
    phase_errors = (
        v_boundary.va_degree.values - net.res_bus.va_degree[target_buses].values
    )
    vm_errors = v_boundary.vm_pu.values - net.res_bus.vm_pu[target_buses].values
    ...
    for idx, lb in enumerate(target_buses):
        if abs(vm_errors[idx]) > 1e-6 and abs(vm_errors[idx]) > 1e-6:
            hb = create_bus(
                net,
                net.bus.vn_kv[lb] * (1 - vm_errors[idx]),
            ...
            for e, e_list in elm_dict.items():
                for i in e_list:
                    ...
                    if (
                        "eq_" not in name
                        and "_integrated_" not in name
                        and "_separate_" not in name
                    ):
...
def replace_motor_by_load(net, all_external_buses):
    ...
    for mi, m in net.motor.loc[motors].iterrows():
        ...
        li = create_load(
            ...
            name="equivalent_" + str(m["name"]) + "_" + str(mi),
        )
