...
def _calculate_equivalent_Ybus(
    net_zpbn, bus_lookups, eq_type, show_computing_time=False, **kwargs
):
    """
    ...
    Ybus_ee = Ybus_sorted[
        -(nb_dict["nb_e"] + nb_dict["nb_g"]) :, -(nb_dict["nb_e"] + nb_dict["nb_g"]) :
    ]
    Ybus_eb = Ybus_sorted[
    ...
    try:
        ...
        if "Singular matrix" in str(err):
            logger.debug(
                "Ymat_ee is a singular martix, now try to compute the \
    ...
    Ybus_eq[
        -(nb_dict["nb_b"] + nb_dict["nb_t"]) :, -(nb_dict["nb_b"] + nb_dict["nb_t"]) :
    ] = Ybus_eq_boundary

    t_end = time.perf_counter()
    if show_computing_time:
        logger.info(
            '"calculate_equivalent_Ybus" finished in %s seconds:'
...
def _create_net_zpbn(
    ...
    **kwargs,
    ...
    net_internal, net_external = _get_internal_and_external_nets(
        ...
        all_external_buses,
        show_computing_time,
        calc_volt_angles=calc_volt_angles,
        runpp_fct=runpp_fct,
        **kwargs,
    )
    if net_internal is not None and "trafo_characteristic_table" in net_internal:
        net_internal.trafo_characteristic_table = (
            net_internal.trafo_characteristic_table[
                ~net_internal.trafo_characteristic_table.id_characteristic.isin(
                    net_external.trafo.id_characteristic_table
                )
            ]
        )
        net_external.trafo_characteristic_table = (
            net_external.trafo_characteristic_table[
                ~net_external.trafo_characteristic_table.id_characteristic.isin(
                    net_internal.trafo.id_characteristic_table
                )
            ]
        )
    ...
    for elm, separate in [
        ...
        net_zpbn[elm].drop(
            net_zpbn[elm].index[net_zpbn[elm].bus.isin(all_external_buses)],
            inplace=True,
        )
        if elm == "ext_grid":
            continue
        ...
        if not np.isnan(Z[elm + "_ground"].values).all():
            if separate:
                ...
                new_g_buses = create_buses(
                    net_zpbn,
                    len(idxs),
                    vn_kvs,
                    name=[
                        "%s_separate-ground %s" % (elm, str(Z.ext_bus.loc[i]))
                        for i in idxs
                    ],
                )
                new_t_buses = create_buses(
                    net_zpbn,
                    len(idxs),
                    vn_kvs,
                    name=[
                        "%s_separate-total %s" % (elm, str(Z.ext_bus.loc[i]))
                        for i in idxs
                    ],
                    max_vm_pu=limits.max_vm_pu.loc[idxs],
                    min_vm_pu=limits.min_vm_pu.loc[idxs],
                )

                # add impedances
                rft_pu_g, xft_pu_g = adapt_impedance_params(
                    Z[elm + "_ground"].loc[idxs].values
                )
                max_idx = (
                    net_zpbn.impedance.index.max() if net_zpbn.impedance.shape[0] else 0
                )
                new_imps_g = pd.DataFrame(
                    {
                        "from_bus": Z.ext_bus.loc[idxs].astype(np.int64).values,
                        "to_bus": new_g_buses,
                        "rft_pu": rft_pu_g,
                        "xft_pu": xft_pu_g,
                        "rtf_pu": rft_pu_g,
                ...
                rft_pu_t, xft_pu_t = adapt_impedance_params(
                    Z[elm + "_separate_total"].loc[idxs].values
                )
                new_imps_t = pd.DataFrame(
                    {
                        "from_bus": new_g_buses,
                        "to_bus": new_t_buses,
                        "rft_pu": rft_pu_t,
                        "xft_pu": xft_pu_t,
                        "rtf_pu": rft_pu_t,
                ...
                net_zpbn["impedance"] = pd.concat(
                    [net_zpbn["impedance"], new_imps_g, new_imps_t]
                )
                ...
                new_g_bus = create_bus(
                    net_zpbn, vn_kv, name=elm + "_integrated-ground "
                )
                i_all_integrated = []
                for i in Z.index[~np.isnan(Z[elm + "_ground"].values)]:
                    ...
                    create_impedance(
                        ...
                        xft_pu,
                        sn_mva,
                        name="eq_impedance_ext_to_ground",
                    )
                ...
                create_impedance(
                    ...
                    xft_pu,
                    sn_mva,
                    name="eq_impedance_ground_to_total",
                )
                g_buses += [new_g_bus.tolist()]
                t_buses += [new_t_bus.tolist()]
        else:
            Z.drop(
                [elm + "_ground", elm + "_separate_total", elm + "_integrated_total"],
                axis=1,
                inplace=True,
            )
    ...
    max_load_idx = max(
        -1,
        net.load.index[~net.load.bus.isin(all_external_buses)].max()
        - len(net_zpbn.load),
    )
    max_sgen_idx = max(
        -1,
        net.sgen.index[~net.sgen.bus.isin(all_external_buses)].max()
        - len(net_zpbn.sgen),
    )
    max_gen_idx = max(
        -1,
        net.gen.index[~net.gen.bus.isin(all_external_buses)].max() - len(net_zpbn.gen),
    )
    for i in t_buses:
        ...
        if elm == "load":
            elm_idx = create_load(
                ...
                -float(P),
                -float(Q),
                name=key + "_rei_" + busstr,
                sn_mva=Sn,
                index=max_load_idx + len(net_zpbn.load) + 1,
            )
        elif elm == "sgen":
            elm_idx = create_sgen(
                net_zpbn,
                ...
                name=key + "_rei_" + busstr,
                sn_mva=Sn,
                index=max_sgen_idx + len(net_zpbn.sgen) + 1,
            )
        elif elm == "gen":
            vm_pu = v[key + "_vm_total"][
                v.ext_bus == int(re.findall(r"\d+", busstr)[0])
            ].values.real
            elm_idx = create_gen(
                net_zpbn,
                i,
                ...
                name=key + "_rei_" + busstr,
                sn_mva=Sn,
                index=max_gen_idx + len(net_zpbn.gen) + 1,
            )
        ...
        if elm_old is None or elm_old != elm:
            ...
            other_cols_bool = set(
                net[elm][list(other_cols)].columns[
                    net[elm][list(other_cols)].apply(is_bool_dtype)
                ]
            )
            ...
            for c in other_cols.copy():
                value_types = (
                    net[elm][c][elm_org.bus.isin(all_external_buses)]
                    .apply(type)
                    .unique()
                )
        ...
        if "integrated" in key:
            if "voltLvl" in other_cols_number:
                net_zpbn[elm].loc[elm_idx, "voltLvl"] = net_zpbn.bus.voltLvl[
                    boundary_buses
                ].max()
                other_cols_number -= {"voltLvl"}
            net_zpbn[elm].loc[elm_idx, list(other_cols_number)] = elm_org[
                list(other_cols_number)
            ][elm_org.bus.isin(all_external_buses)].sum(axis=0)
            net_zpbn[elm].loc[elm_idx, list(other_cols_bool)] = (
                elm_org[list(other_cols_bool)][
                    elm_org.bus.isin(all_external_buses)
                ].values.sum(axis=0)
                > 0
            )
            all_str_values = list(
                zip(
                    *elm_org[list(other_cols_str)][
                        elm_org.bus.isin(all_external_buses)
                    ].values[::-1]
                )
            )
            ...
            if (
                elm == "gen"
                and bus in net.ext_grid.bus.values
                and net.ext_grid.in_service[net.ext_grid.bus == bus].values[0]
            ):
                net_zpbn[elm].loc[elm_idx, "name"] = (
                    str(net.ext_grid.name[net.ext_grid.bus == bus].values[0])
                    + "-"
                    + net_zpbn[elm].name[elm_idx]
                )
                ext_grid_cols = list(
                    set(elm_org.columns)
                    & set(net.ext_grid.columns)
                    - {"name", "bus", "p_mw", "sn_mva", "in_service", "scaling"}
                )
                net_zpbn[elm].loc[elm_idx, ext_grid_cols] = net.ext_grid[ext_grid_cols][
                    net.ext_grid.bus == bus
                ].values[0]
            else:
                names = elm_org.name[elm_org.bus == bus].values
                names = [str(n) for n in names]
                net_zpbn[elm].loc[elm_idx, "name"] = (
                    "//".join(names) + "-" + net_zpbn[elm].name[elm_idx]
                )
                if len(names) > 1:
                    net_zpbn[elm].loc[elm_idx, list(other_cols_number)] = elm_org[
                        list(other_cols_number)
                    ][elm_org.bus == bus].sum(axis=0)
                    if "voltLvl" in other_cols_number:
                        net_zpbn[elm].loc[elm_idx, "voltLvl"] = net_zpbn.bus.voltLvl[
                            boundary_buses
                        ].max()
                    net_zpbn[elm].loc[elm_idx, list(other_cols_bool)] = (
                        elm_org[list(other_cols_bool)][elm_org.bus == bus].values.sum(
                            axis=0
                        )
                        > 0
                    )
                    ...
                    all_str_values = list(
                        zip(
                            *elm_org[list(other_cols_str)][elm_org.bus == bus].values[
                                ::-1
                            ]
                        )
                    )
                    ...
                    for ocm in other_cols_mixed:
                        net_zpbn[elm][ocm] = net_zpbn[elm][ocm].astype("object")
                    net_zpbn[elm].loc[elm_idx, list(other_cols_mixed)] = (
                        "mixed data type"
                    )
                else:
                    net_zpbn[elm].loc[
                    ...
                    net_zpbn[elm].loc[elm_idx, list(other_cols)] = elm_org[
                        list(other_cols)
                    ][elm_org.bus == bus].values[0]
    ...
    for cost_elm in ["poly_cost", "pwl_cost"]:
        if len(net[cost_elm]):
            ...
            df.loc[
                (df.et.isin(["storage", "dcline"]) & (~df.bus.isin(boundary_buses))),
                "et",
            ] = "load"

            logger.debug(
                "During the equivalencing, also in polt_cost, "
            ...
            for elm in ["load", "gen", "sgen"]:
                for idx in net_zpbn[elm].index:
                    if net_zpbn[elm].bus[idx] in boundary_buses:
                        ...
                        if net_zpbn[elm].name.str.contains("integrated").any() and len(
                            pc_idx
                        ):
                            logger.debug(
                                "Attention! After equivalencing, "
                                + elm
                                + "s are modeled as "
                                + "an aggregated "
                                + elm
                            ...
                            related_bus = int(
                                str(net_zpbn[elm].name[idx]).split("_")[-1]
                            )
                            pc_idx = df.index[(df.bus == related_bus) & (df.et == elm)]
                            if len(pc_idx) > 1:
                                logger.debug(
                                    "Attention! There are at least two "
                                    + elm
                                    + "s connected to a "
                                    + "common bus. The "
                                    + elm
                                    + "s with commen bus are modeled as an "
                                    + "aggreated "
                                    + elm
                                    + " during the equivalencing. "
                                    + "The "
                                    + cost_elm
                                    + " data of the first "
                                    + elm
                                    + " is used as the "
                                    + cost_elm
                                    + " data of the aggregated "
                                    + elm
                                    + ". "
                                    + "It is NOT correct at present."
                                )
                                pc_idx = df.index[
                                    (df.bus == related_bus) & (df.et == elm)
                                ]
    ...
    runpp_fct(
        net_zpbn,
        calculate_voltage_angles=calc_volt_angles,
        tolerance_mva=1e-3,
        max_iteration=100,
        **kwargs,
    )
...
def _create_bus_lookups(
    ...
    bus_lookup_pd = {
        ...
        "t_area_buses": net_zpbn.bus.index[
            net_zpbn.bus.name.str.contains("-total", na=False)
        ].tolist(),
        "g_area_buses": net_zpbn.bus.index[
            net_zpbn.bus.name.str.contains("-ground", na=False)
        ].tolist(),
    ...
    for key in ["b", "i", "e", "g", "t"]:
        bus_lookup_ppc[key + "_area_buses"] = pd2ppc_bus_lookup[
            list(bus_lookup_pd[key + "_area_buses"])
        ].tolist()
        origin_sequence = bus_lookup_ppc[key + "_area_buses"].copy()
        # remove repeated "neg." (e.g. bus-bus-switch) ppc buses
        bus_lookup_ppc[key + "_area_buses"] = sorted(
            set(bus_lookup_ppc[key + "_area_buses"]), key=origin_sequence.index
        )
        if key == "b":
            ...
            bus_lookup_pd["b_area_buses_no_switch"] = bus_lookup_pd[
                key + "_area_buses"
            ].copy()
            val_ppc = []
        ...
        if key != "b":
            common_ppc_busese = set(bus_lookup_ppc["b_area_buses"]) & set(
                bus_lookup_ppc[key + "_area_buses"]
            )
            if common_ppc_busese:
                bus_lookup_ppc[key + "_area_buses"].remove(list(common_ppc_busese))
        if key == "b" and len(bus_lookup_ppc["b_area_buses"]) != len(
            bus_lookup_pd["b_area_buses"]
        ):
    ...
    bus_lookups = {
        "bus_lookup_pd": bus_lookup_pd,
        "bus_lookup_ppc": bus_lookup_ppc,
        "bus_lookup_ppc_origin": bus_lookup_ppc_origin,
        "pos_aux_bus_ppc": pos_aux_buses_ppc,
        "boundary_buses_inclusive_bswitch": boundary_buses_inclusive_bswitch,
        "origin_all_internal_buses": all_internal_buses,
    }
    t_end = time.perf_counter()
    if show_computing_time:
        logger.info(
            '"create_bus_lookup" finished in %s seconds:' % round((t_end - t_start), 2)
        )
    return bus_lookups


def _get_internal_and_external_nets(
    net,
    boundary_buses,
    all_internal_buses,
    all_external_buses,
    ...
    **kwargs,
    ...
    if not all_internal_buses:
        ...
        drop_and_edit_cost_functions(
            net_internal, all_external_buses + boundary_buses, True, True
        )
    ...
    _integrate_power_elements_connected_with_switch_buses(
        net, net_external, all_external_buses
    )  # for sgens, gens, and loads
    runpp_fct(net_external, calculate_voltage_angles=calc_volt_angles, **kwargs)
    t_end = time.perf_counter()
    if show_computing_time:
        logger.info(
            '"get_int_and_ext_nets" '
...
def _calclate_equivalent_element_params(
    net_zpbn,
    Ybus_eq,
    bus_lookups,
    show_computing_time=False,
    max_allowed_impedance=1e8,
    **kwargs,
):
    ...
    bt_buses_ppc = list(bus_lookups["bus_lookup_ppc"]["b_area_buses"]) + list(
        bus_lookups["bus_lookup_ppc"]["t_area_buses"]
    )
    bt_buses_pd = list(bus_lookups["bus_lookup_pd"]["b_area_buses_no_switch"]) + list(
    ...
    rows = (np.arange(params.shape[0]).reshape(-1, 1) * np.ones(params.shape)).astype(
        np.int64
    )[non_zero]
    cols = (np.arange(params.shape[1]) * np.ones(params.shape)).astype(np.int64)[
        non_zero
    ]

    impedance_params = pd.DataFrame(
        columns=impedance_columns, index=range(len(rows)), data=0
    )
    ...
    impedance_params.loc[non_zero_cr, "rtf_pu"] = (-1 / params[cols, rows]).real[
        non_zero_cr
    ]
    impedance_params.loc[non_zero_cr, "xtf_pu"] = (-1 / params[cols, rows]).imag[
        non_zero_cr
    ]
    ...
    if show_computing_time:
        logger.info(
            '"calclate_equivalent_element_params" finished in %s seconds:'
            % round((t_end - t_start), 2)
        )
...
def _replace_ext_area_by_impedances_and_shunts(
    ...
    **kwargs,
    ...
    drop_internal_branch_elements(
        net_eq, bus_lookups["boundary_buses_inclusive_bswitch"]
    )
    # --- drop shunt elements attached to boundary buses
    traget_shunt_idx = net_eq.shunt.index[
        net_eq.shunt.bus.isin(bus_lookups["boundary_buses_inclusive_bswitch"])
    ]
    net_eq.shunt = net_eq.shunt.drop(traget_shunt_idx)

    # --- create impedance
    not_very_low_imp = (
        (impedance_params.rft_pu.abs() > imp_threshold)
        ...
        | (
            impedance_params.from_bus.isin(
                set(net_eq.gen.bus) | set(net_eq.ext_grid.bus)
            )
            & impedance_params.to_bus.isin(
                set(net_eq.gen.bus) | set(net_eq.ext_grid.bus)
            )
        )
    )
    ...
    new_sws = (
        impedance_params[["from_bus", "to_bus"]].loc[~not_very_low_imp].astype(np.int64)
    )
    ...
    if not not_very_low_imp.all():
        ...
        shunt_params.drop(
            shunt_params.index[shunt_params.bus_pd.isin(fb)], inplace=True
        )
        shunt_params.drop(
            shunt_params.index[shunt_params.bus_pd.isin(tb)], inplace=True
        )

    # --- create shunts
    max_idx = net_eq.shunt.index.max() if net_eq.shunt.shape[0] else 0
    shunt_buses = shunt_params.bus_pd.values.astype(np.int64)
    new_shunts = pd.DataFrame(
        {
            "bus": shunt_buses,
    ...
    new_shunts.loc[isin_sh, "vn_kv"] = net_eq.bus.vn_kv.loc[
        new_shunts.bus.loc[isin_sh]
    ].values
    new_shunts["step"] = 1
...
def _integrate_power_elements_connected_with_switch_buses(
    net, net_external, all_external_buses
):
    _, bus_dict = get_connected_switch_buses_groups(net_external, all_external_buses)
    active_sgen = net.sgen[
        (net.sgen.in_service) & ~((net["sgen"].p_mw == 0) & (net["sgen"].q_mvar == 0))
    ]
    active_load = net.load[
        (net.load.in_service) & ~((net["load"].p_mw == 0) & (net["load"].q_mvar == 0))
    ]
    ...
    for elm in ["sgen", "load", "gen"]:
        for bd in bus_dict:
            ...
            if len(connected_elms) <= 1:
                ...
                net_external[elm].loc[connected_elms, "bus"] = net_external[elm].bus[
                    elm1
                ]
