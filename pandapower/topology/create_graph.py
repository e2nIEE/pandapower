...
def create_nxgraph(net, respect_switches=True, include_lines=True, include_trafos=True,
    ...
    if include_lines:
        ...
        mg.add_edges_from((int(fb), int(tb), {"weight": float(l), "key": int(idx), "type": "l",
                                              "capacity": float(imax), "path": 1, 'R': float(r_ohm), 'Z': float(z_ohm)})
    ...
    if include_impedances:
        ...
        mg.add_edges_from((int(fb), int(tb), {"weight": 0, "key": int(idx), "type": "i", "path": 1, 'R': float(r_ohm), 'Z': float(z_ohm)})
    ...
    if include_trafos:
        ...
        mg.add_edges_from((int(hvb), int(lvb), {"weight": 0, "key": int(idx), "type": "t", 'R': float(r_ohm), 'Z': float(z_ohm)})
        ...
        #hv-mv
        mg.add_edges_from((int(bus1), int(bus2), {"weight": 0, "key": int(idx),"type": "t3", 'R': float(r_ohm), 'Z': float(z_ohm)})
