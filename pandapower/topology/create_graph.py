...
def create_nxgraph(
    ...
    trafo_length_km=None,
    switch_length_km=None,
):
    """
    ...
    if multi:
        if graph_tool_available and library == "graph_tool":
            ...
            raise UserWarning(
                "graph_tool selected as the library for topological analysis but it is not installed"
            )
        else:
            mg = nx.MultiGraph()
    else:
    ...
    if line is not None:
        indices, parameter, in_service = init_par(
            line, calc_branch_impedances, include_out_of_service
        )
        ...
        if calc_branch_impedances:
            baseR = (
                get_baseR(net, ppc, line.from_bus.values)
                if branch_impedance_unit == "pu"
                else 1
            )
    ...
    if impedance is not None:
        indices, parameter, in_service = init_par(
            impedance, calc_branch_impedances, include_out_of_service
        )
        ...
        if calc_branch_impedances:
            baseR = (
                get_baseR(net, ppc, impedance.from_bus.values)
    ...
    if tcsc is not None:
        indices, parameter, in_service = init_par(
            tcsc, calc_branch_impedances, include_out_of_service
        )
        ...
        if calc_branch_impedances:
            ...
            x = 1 / calc_y_svc_pu(
                net.tcsc.thyristor_firing_angle_degree,
                net.tcsc.x_l_ohm / baseR,
    ...
    if dclines is not None:
        indices, parameter, in_service = init_par(
            dclines, calc_branch_impedances, include_out_of_service
        )
    ...
    if trafo is not None:
        indices, parameter, in_service = init_par(
            trafo, calc_branch_impedances, include_out_of_service
        )
        ...
        if calc_branch_impedances:
            baseR = (
                get_baseR(net, ppc, trafo.hv_bus.values)
                if branch_impedance_unit == "ohm"
                else 1
            )
    ...
    if trafo3w is not None:
        ...
        if calc_branch_impedances:
            ...
            baseR = (
                get_baseR(net, ppc, trafo3w.hv_bus.values)
                if branch_impedance_unit == "ohm"
                else 1
            )
        ...
        for f, t in combinations(sides, 2):
            indices, parameter, in_service = init_par(
                trafo3w, calc_branch_impedances, include_out_of_service
            )
...
def init_par(tab, calc_branch_impedances=False, include_out_of_service=False):
    ...
    if "in_service" in tab:
        return (
            ...
            else tab.in_service.values.copy(),
        )
    else:
