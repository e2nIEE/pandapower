...
from pandapower.build_opf import _pd2ppc_opf
...
def runopp(net, cost_function="linear", verbose=False, suppress_warnings=True, **kwargs):
    ...
    """
    _rundcopp(net, verbose, suppress_warnings, cost_function, False, **kwargs)
...
def rundcopp(net, cost_function="linear", verbose=False, suppress_warnings=True, **kwargs):
    ...
    OPTIONAL:
        ...
        **suppress_warnings** (bool, True) - suppress warnings in pypower
            ...
            warnings are suppressed, too.
    """
    _rundcopp(net, verbose, suppress_warnings, cost_function, True, **kwargs)


def _rundcopp(net, verbose, suppress_warnings, cost_function, dc=False, **kwargs):
    ppopt = ppoption(VERBOSE=verbose, OPF_FLOW_LIM=2, PF_DC=dc, **kwargs)
    ...
    _extract_results_opf(net, result, is_elems, bus_lookup, "current", True, not dc)