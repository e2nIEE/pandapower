    ...
    **INPUT**:
    ...
    if line_end > 0:
        ppc["branch"][:line_end, [F_BUS, T_BUS, BR_R, BR_X, BR_B, BR_STATUS, RATE_A]] = \
            _calc_line_parameter(net, ppc, bus_lookup, copy_constraints_from_ppc)
    if trafo_end > line_end:
        ppc["branch"][line_end:trafo_end,
            ...
            _calc_trafo_parameter(net, ppc, bus_lookup, calculate_voltage_angles,
                                  trafo_model, copy_constraints_from_ppc)
...
def _calc_line_parameter(net, ppc, bus_lookup, copy_constraints_from_ppc=False):
    """
...
def _update_trafo_trafo3w_ppc(net, ppc, bus_lookup, calculate_voltage_angles, trafo_model,copy_constraints_from_ppc=False):
    """
    ...
    :param bus_lookup: lookup pandapower -> pypower
