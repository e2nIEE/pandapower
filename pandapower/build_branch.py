...
from pandapower.pypower.idx_brch_dc import (
    branch_dc_cols,
    DC_RATE_A,
    DC_RATE_B,
    DC_RATE_C,
...
from pandapower.pypower.idx_brch_tdpf import (
    BR_R_REF_OHM_PER_KM,
    BR_LENGTH_KM,
    RATE_I_KA,
    T_START_C,
...
from pandapower.pypower.idx_bus_dc import (
    DC_BUS_AREA,
    DC_VM,
    DC_ZONE,
    DC_VMAX,
...
from pandapower.pypower.idx_tcsc import (
    TCSC_F_BUS,
    TCSC_T_BUS,
    TCSC_X_L,
    TCSC_X_CVAR,
    TCSC_SET_P,
    TCSC_THYRISTOR_FIRING_ANGLE,
    TCSC_STATUS,
    TCSC_CONTROLLABLE,
    tcsc_cols,
    TCSC_MIN_FIRING_ANGLE,
    TCSC_MAX_FIRING_ANGLE,
)
    ...
    **INPUT**:
    ...
    if mode == "sc" and tdpf:
        raise NotImplementedError(
            "indexing for ppc branch columns not implemented for tdpf and sc together"
        )
...
def _build_branch_dc_ppc(net, ppc):
    ...
    **INPUT**:
    ...
    ppc["branch_dc"][:, [DC_RATE_A, DC_RATE_B, DC_RATE_C, DC_BR_STATUS]] = np.array(
        [250, 250, 250, 1]
    )
    if mode != "pf":
        return
...
def _initialize_branch_lookup(net, dc=False):
    ...
    elements = (
        ["line", "trafo", "trafo3w", "impedance", "xward"] if not dc else ["line_dc"]
    )
...
def _calc_trafo3w_parameter(net, ppc):
    ...
    if "max_loading_percent" in trafo_df:
        ...
        branch[f:t, RATE_A] = max_load / 100.0 * sn_mva
    else:
        ...
        branch[f:t, RATE_A] = 0.0 if net["_options"]["mode"] == "opf" else 100.0
...
def _calc_line_parameter(net, ppc, elm="line", ppc_elm="branch"):
    ...
    **RETURN**:
    ...
    baseR = (
        np.square(base_kv) / (3 * net.sn_mva)
        if mode == "pf_3ph"
        else np.square(base_kv) / net.sn_mva
    )
    ...
    if net._options["tdpf"]:
        branch[f:t, TDPF] = line["in_service"].values & line["tdpf"].fillna(
            False
        ).values.astype(bool)
        ...
        branch[f:t, SOLAR_RADIATION_W_PER_SQ_M] = line.get(
            "solar_radiation_w_per_sq_m", default=np.nan
        )
        ...
        branch[f:t, OUTER_DIAMETER_M] = line.get(
            "conductor_outer_diameter_m", default=np.nan
        )
    ...
    if mode == "sc" and not net._options.get("use_pre_fault_voltage", False):
        ...
        if net["_options"]["case"] == "min":
            branch[f:t, BR_R] *= _end_temperature_correction_factor(
                net, short_circuit=True
            )
    else:
        # temperature correction
    ...
    if "max_loading_percent" in line:
        ...
        vr = net.bus.loc[line["from_bus"].values, "vn_kv"].values * np.sqrt(3.0)
        max_i_ka = line.max_i_ka.values
        df = line.df.values
        branch[f:t, RATE_A] = max_load / 100.0 * max_i_ka * df * parallel * vr
    else:
        # PowerModels considers "0" as "no limit"
        # todo: inf and convert only when using PowerModels to 0., pypower opf converts the zero to inf
        branch[f:t, RATE_A] = 0.0 if mode == "opf" else 100.0
...
def _calc_line_dc_parameter(net, ppc, elm="line_dc", ppc_elm="branch_dc"):
    ...
    **RETURN**:
    ...
    branch_dc[f:t, DC_BR_R] = (
        line_dc["r_ohm_per_km"].values * length_km / baseR / parallel
    )

    if net._options["tdpf"]:
        # todo implement idx_brch_dc_tdpf
        raise NotImplementedError(
            "temperature-related calculation for line_dc not implemented"
        )
    ...
    # RATE_A is considered by the (PowerModels) OPF. If zero -> unlimited
    max_load = (
        line_dc.max_loading_percent.values if "max_loading_percent" in line_dc else 0.0
    )
    vr = net.bus_dc.loc[line_dc["from_bus_dc"].values, "vn_kv"].values * np.sqrt(3.0)
    max_i_ka = line_dc.max_i_ka.values
    df = line_dc.df.values
    # This calculates the maximum apparent power at 1.0 p.u.
    branch_dc[f:t, DC_RATE_A] = max_load / 100.0 * max_i_ka * df * parallel * vr
    # RATE_A is considered by the (PowerModels) OPF. If zero -> unlimited
    if "max_loading_percent" in line_dc:
        max_load = line_dc.max_loading_percent.values
        vr = net.bus_dc.loc[line_dc["from_bus_dc"].values, "vn_kv"].values * np.sqrt(
            3.0
        )
        max_i_ka = line_dc.max_i_ka.values
        df = line_dc.df.values
        ...
        # PowerModels considers "0" as "no limit"
        # todo: inf and convert only when using PowerModels to 0., pypower opf converts the zero to inf
        branch_dc[f:t, DC_RATE_A] = 0.0 if mode == "opf" else 100.0
...
def _calc_trafo_parameter(net, ppc, sequence=1):
    ...
    **RETURN**:
    ...
    if any(trafo.df.values <= 0):
        raise UserWarning(
            "Rating factor df must be positive. Transformers with false "
    ...
    if "max_loading_percent" in trafo:
        ...
        df = trafo.df.values
        branch[f:t, RATE_A] = max_load / 100.0 * sn_mva * df * parallel
    else:
        # PowerModels considers "0" as "no limit"
        # todo: inf and convert only when using PowerModels to 0., pypower opf converts the zero to inf
        branch[f:t, RATE_A] = 0.0 if net["_options"]["mode"] == "opf" else 100.0
...
def _calc_branch_values_from_trafo_df(net, ppc, trafo_df=None, sequence=1):
    ...
    **RETURN**:
    ...
    ratio = _calc_nominal_ratio_from_dataframe(
        ppc, trafo_df, vn_trafo_hv, vn_trafo_lv, bus_lookup
    )
    r, x, g, b, g_asym, b_asym = _calc_r_x_y_from_dataframe(
        net, trafo_df, vn_trafo_lv, vn_lv, ppc, sequence=sequence
    )
...
def _calc_r_x_y_from_dataframe(net, trafo_df, vn_trafo_lv, vn_lv, ppc, sequence=1):
    ...
    if "tap_dependency_table" in trafo_df:
        ...
        warnings.warn(
            DeprecationWarning(
                "tap_dependency_table is missing in net, which is most probably due to "
    ...
    if mode == "sc":
        if net._options.get("use_pre_fault_voltage", False):
            g, b = _calc_y_from_dataframe(
                mode, trafo_df, vn_lv, vn_trafo_lv, net.sn_mva
            )
        else:
            g, b = 0, 0  # why for sc are we assigning y directly as 0?
        if isinstance(
            trafo_df, pd.DataFrame
        ):  # 2w trafo is dataframe, 3w trafo is dict
            bus_lookup = net._pd2ppc_lookups["bus"]
    ...
    if trafo_model == "pi":
        ...
        r_ratio = (
            get_trafo_values(trafo_df, "leakage_resistance_ratio_hv")
            if "leakage_resistance_ratio_hv" in trafo_df
            else np.full_like(r, fill_value=0.5, dtype=np.float64)
        )
        x_ratio = (
            get_trafo_values(trafo_df, "leakage_reactance_ratio_hv")
        ...
        raise ValueError(
            "Unknown Transformer Model %s - valid values ar 'pi' or 't'" % trafo_model
        )
...
def _calc_y_from_dataframe(mode, trafo_df, vn_lv, vn_trafo_lv, net_sn_mva):
    ...
    baseZ = (
        np.square(vn_lv) / (3 * net_sn_mva)
        if mode == "pf_3ph"
        else np.square(vn_lv) / net_sn_mva
    )
    vn_lv_kv = get_trafo_values(trafo_df, "vn_lv_kv")
    pfe_mw = (
        (get_trafo_values(trafo_df, "pfe_kw") * 1e-3) / 3
        if mode == "pf_3ph"
    ...
    g_mva = pfe_mw
    i0 = (
        get_trafo_values(trafo_df, "i0_percent") / 3
        if mode == "pf_3ph"
...
def _calc_tap_from_dataframe(net, trafo_df):
    ...
    trafo_shift = (
        get_trafo_values(trafo_df, "shift_degree").astype(float)
        if calculate_voltage_angles
        else np.zeros(len(vnh))
    )
    if mode == "sc" and not net._options.get(
    ...
    for t in ("", "2"):
        ...
        if f"tap{t}_changer_type" in trafo_df:
            ...
            tap_table = np.logical_and(
                tap_dependency_table, tap_changer_type is not None
            )
            tap_no_table = np.logical_and(
                ~tap_dependency_table, tap_changer_type is not None
            )
            if any(tap_table):
                id_characteristic_table = get_trafo_values(
                    trafo_df, "id_characteristic_table"
                )
                ...
                for side, vn, direction in [("hv", vnh, 1), ("lv", vnl, -1)]:
                    ...
                    filtered_df = net.trafo_characteristic_table.merge(
                        filter_df[filter_df["mask"]], on=["id_characteristic", "step"]
                    )

                    cleaned_id_characteristic = id_characteristic_table[
                        (~pd.isna(id_characteristic_table)) & mask
                    ]

                    voltage_mapping = dict(
                        zip(
                            filtered_df["id_characteristic"],
                            filtered_df["voltage_ratio"],
                        )
                    )
                    shift_mapping = dict(
                        zip(filtered_df["id_characteristic"], filtered_df["angle_deg"])
                    )

                    if direction == 1:
                        ratio = [
                            voltage_mapping.get(id_val, 1)
                            for id_val in cleaned_id_characteristic
                        ]
                        shift = [
                            shift_mapping.get(id_val, 1)
                            for id_val in cleaned_id_characteristic
                        ]
                    else:
                        ratio = [
                    ...
                    if isinstance(trafo_df, dict):
                        ...
                        relevant_tap_at_star_point = (
                            mask & trafo_df["tap_at_star_point"]
                        )
                        count_index = 0
            ...
            if any(tap_no_table):
                ...
                tap_complex = np.logical_and(
                    np.logical_or(
                        tap_changer_type == "Ratio", tap_changer_type == "Symmetrical"
                    ),
                    tap_no_table,
                )
                for side, vn, direction in [("hv", vnh, 1), ("lv", vnl, -1)]:
                    ...
                    if any(mask_complex):
                        tap_steps = (
                            tap_step_percent[mask_complex]
                            * tap_diff[mask_complex]
                            / 100
                        )
                        tap_angles = _replace_nan(tap_step_degree[mask_complex])
                        u1 = vn[mask_complex]
                        du = u1 * _replace_nan(tap_steps)
                        vn[mask_complex] = np.sqrt(
                            (u1 + du * cos(tap_angles)) ** 2
                            + (du * sin(tap_angles)) ** 2
                        )
                        trafo_shift[mask_complex] += arctan(
                            direction
                            * du
                            * sin(tap_angles)
            ...
            warnings.warn(
                DeprecationWarning(
                    "tap{t}_phase_shifter was removed with pandapower 3.0 and replaced by "
            ...
            for side, vn, direction in [("hv", vnh, 1), ("lv", vnl, -1)]:
                ...
                if tap_complex.any():
                    ...
                    vn[tap_complex] = np.sqrt(
                        (u1 + du * cos(tap_angles)) ** 2 + (du * sin(tap_angles)) ** 2
                    )
                    trafo_shift[tap_complex] += arctan(
                        direction * du * sin(tap_angles) / (u1 + du * cos(tap_angles))
                    )
                if tap_ideal.any():
...
def _get_vk_values_from_table(trafo_df, trafo_characteristic_table, trafotype="2W"):
    ...
    tap_dependency_table = np.array(
        [
            False if isinstance(x, float) and np.isnan(x) else x
            for x in tap_dependency_table
        ]
    )
    if np.any(np.isnan(tap_dependency_table)):
        raise UserWarning(
            "tap_dependent_impedance has NaN values, but must be of type "
    ...
    for _, vk_var in enumerate(vk_variables):
        ...
        if any(tap_dependency_table):
            ...
            filtered_df = trafo_characteristic_table.merge(
                filter_df[filter_df["mask"]], on=["id_characteristic", "step"]
            )
            cleaned_id_characteristic = id_characteristic_table[
                (~pd.isna(id_characteristic_table)) & mask
            ...
            vk_mapping = dict(
                zip(filtered_df["id_characteristic"], filtered_df[vk_var])
            )
...
def _get_vk_values(trafo_df, characteristic, trafotype="2W"):
    ...
    if use_tap_dependent_impedance:
        ...
        if characteristic is None:
            raise UserWarning(
                "tap_dependent_impedance of transformers requires net.characteristic"
            )
        ...
        all_columns = (
            trafo_df.keys() if isinstance(trafo_df, dict) else trafo_df.columns.values
        )
        char_columns = [v for v in vk_variables if f"{v}_characteristic" in all_columns]
        if len(char_columns) == 0:
            raise UserWarning(
                f"At least one of the columns for characteristics "
        ...
        all_characteristic_idx = np.vstack(
            [
                get_trafo_values(trafo_df, f"{c}_characteristic").astype(np.float64)
        ...
        all_missing = (
            np.isnan(all_characteristic_idx).all(axis=1) & tap_dependent_impedance
        )
        if np.any(all_missing):
            trafo_index = (
                trafo_df["index"]
                if isinstance(trafo_df, dict)
                else trafo_df.index.values
            )
            raise UserWarning(
                f"At least one characteristic must be defined for {trafotype} "
...
def _calc_tap_dependent_value(
    tap_pos, value, tap_dependent_impedance, characteristic, characteristic_idx
):
    ...
    vk_characteristic[relevant_idx] = characteristic.loc[
        characteristic_idx[relevant_idx], "object"
    ].values
    ...
    return np.where(
        relevant_idx, custom_func_vec(relevant_idx, tap_pos, vk_characteristic), value
    )


def _calc_r_x_from_dataframe(
    mode,
    trafo_df,
    vn_lv,
    vn_trafo_lv,
    ...
    if sequence == 1:
        if "tap_dependency_table" in trafo_df:
            ...
            tap_dependency = np.array(
                [
                    False if isinstance(x, float) and np.isnan(x) else x
                    for x in tap_dependency
                ]
            )
            if any(tap_dependency) and not isinstance(trafo_df, dict):
                if np.any(tap_dependency) and trafo_characteristic_table is None:
                    raise UserWarning(
                        "Trafo with tap_dependency_table True, but no trafo_characteristic_table found."
                    )
                vk_percent, vkr_percent = _get_vk_values_from_table(
                    trafo_df, trafo_characteristic_table
                )
...
def _calc_nominal_ratio_from_dataframe(ppc, trafo_df, vn_hv_kv, vn_lv_kv, bus_lookup):
    ...
    lv_bus = get_trafo_values(trafo_df, "lv_bus")
    nom_rat = get_values(ppc["bus"][:, BASE_KV], hv_bus, bus_lookup) / get_values(
        ppc["bus"][:, BASE_KV], lv_bus, bus_lookup
    )
...
def _calc_impedance_parameter(net, ppc):
    ...
    branch = ppc["branch"]
    rij, xij, r_asym, x_asym, gi, bi, g_asym, b_asym = (
        _calc_impedance_parameters_from_dataframe(net)
    )
    branch[f:t, BR_R] = rij
    branch[f:t, BR_X] = xij
...
def _calc_tcsc_parameter(net, ppc):
    ...
    tcsc[f:t, TCSC_THYRISTOR_FIRING_ANGLE] = np.deg2rad(
        net["tcsc"]["thyristor_firing_angle_degree"].values
    )
    tcsc[f:t, TCSC_MIN_FIRING_ANGLE] = np.deg2rad(
        net["tcsc"]["min_angle_degree"].values
    )
    tcsc[f:t, TCSC_MAX_FIRING_ANGLE] = np.deg2rad(
        net["tcsc"]["max_angle_degree"].values
    )

    tcsc[f:t, TCSC_STATUS] = net["tcsc"]["in_service"].values
    tcsc[f:t, TCSC_CONTROLLABLE] = net["tcsc"]["controllable"].values.astype(
        bool
...
def _calc_impedance_parameters_from_dataframe(net, zero_sequence=False):
    ...
    mode = net["_options"]["mode"]
    sn_factor = 3.0 if mode == "pf_3ph" else 1.0
...
def _calc_xward_parameter(net, ppc):
    ...
    baseR = (
        np.square(
            get_values(ppc["bus"][:, BASE_KV], net["xward"]["bus"].values, bus_lookup)
        )
        / net.sn_mva
    )
...
def _gather_branch_switch_info(bus, branch_id, branch_type, net):
    ...
    if branch_type == "l":
        ...
        if net["trafo3w"]["hv_bus"].at[branch_id] == bus:
            ...
            offset = (t - f) / 3 * 2
        branch_idx = (
            lookup["trafo3w"][0] + net["trafo3w"].index.get_loc(branch_id) + offset
        )
        return side, int(bus), int(branch_idx)
    ...
    if n_oos_buses > 0:
        ...
        if n_oos_buses_at_lines > 0:
            ...
            ls_info[:, 2] = np.nonzero(
                np.isin(net[line_table].index, line_is_idx[mask_or])
            )[0]
            ...
            # build new buses
            new_ls_buses = np.zeros(
                shape=(n_oos_buses_at_lines, ppc[bus_table].shape[1]), dtype=np.float64
            )
            ...
            new_ls_buses[:, DC_BASE_KV if dc else BASE_KV] = get_values(
                ppc[bus_table][:, DC_BASE_KV if dc else BASE_KV],
                ls_info[:, 1],
                bus_lookup,
            )
            ...
            ppc[branch_table][ls_info[ls_info[:, 0].astype(bool), 2], 1] = new_indices[
                ls_info[:, 0].astype(bool)
            ]
...
def _end_temperature_correction_factor(net, short_circuit=False, dc=False):
    ...
    if short_circuit:
        ...
        if "endtemp_degree" not in net[element].columns:
            raise UserWarning(
                f"Specify end temperature for {element}s in net.{element}.endtemp_degree"
            )

        delta_t_degree_celsius = (
            net[element].endtemp_degree.values.astype(np.float64) - 20
        )
        ...
        if "temperature_degree_celsius" not in net[element].columns:
            raise UserWarning(
                f"Specify {element} temperature in net.{element}.temperature_degree_celsius"
            )

        delta_t_degree_celsius = (
            net[element].temperature_degree_celsius.values.astype(np.float64) - 20
        )
...
def _trafo_df_from_trafo3w(net, sequence=1):
    ...
    loss_side = (
        t3.loss_side.values
        if "loss_side" in t3.columns
        else np.full(len(t3), net._options["trafo3w_losses"].lower())
    )
    ...
    if sequence == 1:
        if "tap_dependency_table" in t3:
            mode_tmp = (
                "type_c"
                if mode == "sc" and net._options.get("use_pre_fault_voltage", False)
                else mode
            )
            _calculate_sc_voltages_of_equivalent_transformers(
                t3, trafo2, mode_tmp, net=net
            )
        else:
            mode_tmp = (
                "type_c"
            ...
            _calculate_sc_voltages_of_equivalent_transformers(
                t3, trafo2, mode_tmp, characteristic=net.get("characteristic")
            )
    elif sequence == 0:
        if mode != "sc":
            raise NotImplementedError(
        ...
        _calculate_sc_voltages_of_equivalent_transformers_zero_sequence(
            t3,
            trafo2,
        )
    else:
    ...
    trafo2["i0_percent"] = {
        side: np.where(loss_side == side, t3.i0_percent.values, zeros) for side in sides
    }
    trafo2["pfe_kw"] = {
        side: np.where(loss_side == side, t3.pfe_kw.values, zeros) for side in sides
    }
    trafo2["vn_hv_kv"] = {side: t3.vn_hv_kv.values for side in sides}
    trafo2["vn_lv_kv"] = {side: t3["vn_%s_kv" % side].values for side in sides}
    trafo2["shift_degree"] = {
        "hv": np.zeros(nr_trafos),
        "mv": t3.shift_mv_degree.values,
    ...
    if "max_loading_percent" in net.trafo3w:
        trafo2["max_loading_percent"] = {
            side: net.trafo3w.max_loading_percent.values for side in sides
        }
    return {
        var: np.concatenate([trafo2[var][side] for side in sides])
        for var in trafo2.keys()
    }
...
def _calculate_sc_voltages_of_equivalent_transformers(
    ...
    if "tap_dependency_table" in t3:
        ...
        vk_hv, vkr_hv, vk_mv, vkr_mv, vk_lv, vkr_lv = _get_vk_values(
            t3, characteristic, "3W"
        )
...
def _calculate_sc_voltages_of_equivalent_transformers_zero_sequence(t3, t2):
    vk_3w = np.stack(
        [t3.vk_hv_percent.values, t3.vk_mv_percent.values, t3.vk_lv_percent.values]
    )
    vkr_3w = np.stack(
        [t3.vkr_hv_percent.values, t3.vkr_mv_percent.values, t3.vkr_lv_percent.values]
    )
    vk0_3w = np.stack(
        [t3.vk0_hv_percent.values, t3.vk0_mv_percent.values, t3.vk0_lv_percent.values]
    )
    vkr0_3w = np.stack(
        [
            t3.vkr0_hv_percent.values,
            t3.vkr0_mv_percent.values,
            t3.vkr0_lv_percent.values,
        ]
    )
    sn = np.stack([t3.sn_hv_mva.values, t3.sn_mv_mva.values, t3.sn_lv_mva.values])
...
def z_br_to_bus_vector(z, sn):
    return sn[0, :] * np.array(
        [
            z[0, :] / sn[[0, 1], :].min(axis=0),
            z[1, :] / sn[[1, 2], :].min(axis=0),
            z[2, :] / sn[[0, 2], :].min(axis=0),
        ]
    )
...
def _calculate_3w_tap_changers(t3, t2, sides):
    ...
    for side in sides:
        ...
        if any_at_star_point & np.any(mask_star_point := (tap_mask & at_star_point)):
            t = tap_arrays["tap_step_percent"][side][mask_star_point] * np.exp(
                1j * np.deg2rad(tap_arrays["tap_step_degree"][side][mask_star_point])
            )
            ...
            tap_arrays["tap_side"][side][mask_star_point] = (
                "lv" if side == "hv" else "hv"
            )
            tap_arrays["tap_step_degree"][side][mask_star_point] = np.rad2deg(
                np.angle(t_corrected)
            )
