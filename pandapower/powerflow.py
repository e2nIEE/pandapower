...
def _powerflow(net, **kwargs):
    ...
    _add_auxiliary_elements(
        net
    )  # create gen elements for start and end buses of dcline
    ...
    if (
        net["_options"]["voltage_depend_loads"]
        and algorithm not in ["nr", "bfsw"]
        and not (
            allclose(
                concatenate(
                    (
                        net.load.const_z_p_percent.values,
                        net.load.const_z_q_percent.values,
                    )
                ),
                0,
            )
    ...
    # clear lookups
    net._pd2ppc_lookups = {
        "bus": array([], dtype=int64),
        "bus_dc": array([], dtype=int64),
        "ext_grid": array([], dtype=int64),
        "gen": array([], dtype=int64),
        "branch": array([], dtype=int64),
        "branch_dc": array([], dtype=int64),
    }
...
def _recycled_powerflow(net, **kwargs):
    ...
    if algorithm not in ["nr", "iwamoto_nr"] and ac:
        raise ValueError(
            "recycle is only available with Newton-Raphson power flow. Choose "
            "algorithm='nr'"
        )
...
def _ppci_to_net(result, net):
    ...
    if result["success"] != 1:
        ...
        raise LoadflowNotConverged(
            "Power Flow {0} did not converge after {1} iterations!".format(
...
def _bypass_pf_and_set_results(ppci, options):
    ...
    baseMVA, bus, gen, branch, svc, tcsc, ssc, vsc, ref, _, pq, *_, V0, ref_gens = (
        _get_pf_variables_from_ppci(ppci)
    )
    V = ppci["bus"][:, VM]
    bus, gen, branch = pfsoln_pypower(
