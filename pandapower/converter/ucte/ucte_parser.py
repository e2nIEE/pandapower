...
class UCTEParser:
    ...
    def parse_file(self, path_ucte_file: str = None) -> bool:
        ...
        if self.path_ucte_file is None:
            self.logger.error("The variable 'path_ucte_file' is None, set it before parsing it!")
            return False
        ...
        if 'custom' in self.config.keys() and isinstance(self.config['custom'], dict) and \
                'date' in self.config['custom'].keys():
        ...
        self.logger.info("Finished parsing file %s in %ss." % (
    ...
    def _create_df_from_raw(self, raw_input_dict):
        ...
        i_t = pd.Int64Dtype()
        dtypes['##N'] = dict({
            'status': i_t, 'voltage': float, 'p_load': float, 'q_load': float, 'p_gen': float,
            'q_gen': float, 'min_p_gen': float, 'max_p_gen': float, 'min_q_gen': float,
            'max_q_gen': float, 'static_primary_control': float, 'p_primary_control': float,
            'three_ph_short_circuit_power': float, 'x_r_ratio': float, 'node_type': i_t})
        dtypes['##L'] = dict({'status': i_t, 'r': float, 'x': float, 'b': float, 'i': float})
        dtypes['##T'] = dict({
            'status': i_t, 'voltage1': float, 'voltage2': float, 'p': float, 'r': float, 'x': float,
            'b': float, 'g': float, 'i': float})
        # dtypes['##R'] = dict({
        #     'phase_reg_delta_u': float, 'phase_reg_n': i_t, 'phase_reg_n2': i_t, 'phase_reg_u': float,
        #     'angle_reg_delta_u': float, 'angle_reg_theta': float, 'angle_reg_n': i_t,
        #     'angle_reg_n2': i_t, 'angle_reg_p': float}) todo remove
        dtypes['##R'] = dict({
            'phase_reg_delta_u': float, 'phase_reg_n': float, 'phase_reg_n2': float,
            'phase_reg_u': float, 'angle_reg_delta_u': float, 'angle_reg_theta': float,
            'angle_reg_n': float, 'angle_reg_n2': float, 'angle_reg_p': float})
        dtypes['##TT'] = dict({
            'tap_position': float, 'r': float, 'x': float, 'delta_u': float, 'alpha': float})
        dtypes['##E'] = dict({'p': float})
        for ucte_element, one_dtypes in dtypes.items():
            for field, field_type in one_dtypes.items():
                self.data[ucte_element][field].loc[self.data[ucte_element][field] == ''] = np.nan
    ...
    def _split_connections_from_raw(self, element_type: str):
        ...
        self.data[element_type]['node1'] = self.data[element_type][0].str[0:8].str.strip()
        self.data[element_type]['node2'] = self.data[element_type][0].str[9:17].str.strip()
    ...
    def _split_trafos_from_raw(self):
        element_type = '##T'
        if element_type not in self.data.keys():
            self.logger.warning("No transformers in 'self.data' available! Didn't split them.")
            return
        df = self.data[element_type]
        if 0 not in df.columns:
            df[0] = ''
    ...
    def _split_trafos_regulation_from_raw(self):
        element_type = '##R'
        if element_type not in self.data.keys():
            self.logger.warning("No tap changers in 'self.data' available! Didn't split them.")
            return
        df = self.data[element_type]
        if 0 not in df.columns:
            df[0] = ''
    ...
    def _split_trafos_specified_parameters_from_raw(self):
        element_type = '##TT'
        if element_type not in self.data.keys():
            self.logger.warning("No specified transformer parameters in 'self.data' available! Didn't split them.")
            return
        df = self.data[element_type]
        if 0 not in df.columns:
            df[0] = ''
    ...
    def _split_exchange_powers_from_raw(self):
        element_type = '##E'
        if element_type not in self.data.keys():
            self.logger.warning("No exchange powers in 'self.data' available! Didn't split them.")
            return
        df = self.data[element_type]
        if 0 not in df.columns:
            df[0] = ''
    ...
    def get_fields_dict(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        list of dtypes.
        """
        self.logger.info("Creating a dictionary containing the UCTE element types as keys and for each UCTE element "
