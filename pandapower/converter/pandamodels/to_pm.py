...
def convert_pp_to_pm(
    ...
    **kwargs,
    ...
    net, pm, ppc, ppci = convert_to_pm_structure(
        net, from_time_step=from_time_step, to_time_step=to_time_step
    )
...
def convert_to_pm_structure(
    net, opf_flow_lim="S", from_time_step=None, to_time_step=None, **kwargs
):
    if net["_options"]["voltage_depend_loads"] and not (
        np.allclose(net.load.const_z_p_percent.values, 0)
        and np.allclose(net.load.const_i_p_percent.values, 0)
        and np.allclose(net.load.const_z_q_percent.values, 0)
        and np.allclose(net.load.const_i_q_percent.values, 0)
    ):
        logger.error(
...
def dump_pm_json(pm, buffer_file=None):
    ...
    if buffer_file is None:
        ...
        buffer_file = os.path.join(
            tempfile.gettempdir(), "pp_to_pm_" + temp_name + ".json"
        )
...
def _pp_element_to_pm(net, pm, element, pd_bus, qd_bus, load_idx):
    ...
    pm_lookup = (
        np.ones(max(net[element].index) + 1, dtype=np.int64) * -1
        if len(net[element].index)
    ...
    for idx in net[element].index:
        ...
        pm["load"][str(load_idx)] = {
            "pd": pd.item(),
            "qd": qd.item(),
            "load_bus": pm_bus.item(),
            "status": int(in_service),
            "index": load_idx,
        }
...
def get_branch_angles(row, correct_pm_network_data):
    ...
    if correct_pm_network_data:
        if angmin < -60.0:
            logger.debug(
                "changed voltage angle minimum of branch {}, "
                "to -60 from {} degrees".format(int(row[0].real), angmin)
            )
            angmin = -60.0
        if angmax > 60.0:
            logger.debug(
                "changed voltage angle maximum of branch {} to 60. "
...
def ppc_to_pm(net, ppci):
    ...
    load_idx, storage_lookup = _pp_element_to_pm(
        net, pm, "storage", pd_bus, qd_bus, load_idx
    )
    ...
    for row in ppci["bus"]:
        ...
        pq_mismatch = not np.allclose(pd_value, 0.0) or not np.allclose(qd_value, 0.0)
        if pq_mismatch:
        ...
        if not np.allclose(bs, 0.0) or not np.allclose(gs, 0.0):
    ...
    for idx, row in enumerate(ppci["branch"], start=1):
        ...
        if net._options["opf_flow_lim"] == "S":  # or branch["transformer"]:
            ...
            net._options["opf_flow_lim"] == "I"
        ):  # need to call _run_opf_cl from PowerModels
            ...
            logger.error(
                "Branch flow limit %s not understood", net._options["opf_flow_lim"]
            )

        branch["f_bus"] = int(row[F_BUS].real) + 1
        branch["t_bus"] = int(row[T_BUS].real) + 1
        branch["br_status"] = int(row[BR_STATUS].real)
        branch["angmin"], branch["angmax"] = get_branch_angles(
            row, correct_pm_network_data
        )
    ...
    if "ne_branch" in ppci:
        for idx, row in enumerate(ppci["ne_branch"], start=1):
            ...
            if (
                net._options["opf_flow_lim"] == "S"
            ):  # --> Rate_a is always needed for the TNEP problem, right?
                branch["rate_a"] = (
                    row[RATE_A].real if row[RATE_A] > 0 else row[RATE_B].real
                )
                branch["rate_b"] = row[RATE_B].real
...
def add_pm_options(pm, net):
    ...
    pm["pm_solver"] = (
        net._options["pm_solver"] if "pm_solver" in net._options else "ipopt"
    )
    pm["pm_mip_solver"] = (
        net._options["pm_mip_solver"] if "pm_mip_solver" in net._options else "cbc"
    )
    pm["pm_nl_solver"] = (
        net._options["pm_nl_solver"] if "pm_nl_solver" in net._options else "ipopt"
    )
    pm["pm_model"] = (
        net._options["pm_model"] if "pm_model" in net._options else "DCPPowerModel"
    )
    pm["pm_log_level"] = (
        net._options["pm_log_level"] if "pm_log_level" in net._options else 0
    )
    pm["pm_tol"] = net._options["pm_tol"] if "pm_tol" in net._options else 1e-8
    if "pm_time_limits" in net._options and isinstance(
        net._options["pm_time_limits"], dict
    ):
...
def build_ne_branch(net, ppc):
    ...
    if "ne_line" in net:
        ...
        ppc["ne_branch"] = np.zeros(
            shape=(length, branch_cols + 1), dtype=np.complex128
        )
        ppc["ne_branch"][:, :13] = np.array(
            [0, 0, 0, 0, 0, 250, 250, 250, 1, 0, 1, -60, 60]
        )
        ...
        ppc["ne_branch"][:, CONSTRUCTION_COST] = (
            net["ne_line"].loc[:, "construction_cost"].values
        )
    return ppc
...
def init_ne_line(net, new_line_index, construction_costs=None):
    ...
    construction_costs = (
        np.zeros(len(new_line_index))
        if construction_costs is None
        else construction_costs
    )
    ...
    net["res_ne_line"] = pd.DataFrame(
        data=0, index=new_line_index, columns=["built"], dtype=np.int64
    )
    ...
    for elm in ["bus", "line", "gen", "load", "trafo", "sgen"]:
        ...
        for param, param_col in zip(params, param_cols):
            ...
            if elm in ["line", "trafo"]:
                ...
                pm_idxs = [
                    int(v) for v in net._pd2pm_lookups[elm + "_controllable"][pd_idxs]
                ]
                elm = "gen"
            else:
                pm_idxs = [int(v) for v in net._pd2pm_lookups[elm][pd_idxs]]
            df = (
                pd.DataFrame(index=pm_idxs)
                if elm not in ["line", "trafo"]
                else pd.DataFrame(index=pm_idxs_br)
            )
            df["element_index"] = pm_idxs
            df["element_pp_index"] = (
                pd_idxs if elm not in ["line", "trafo"] else pd_idxs_br
            )
            df["value"] = target_values
            df["element"] = elm
            pm["user_defined_params"][param] = df.to_dict(
    ...
    if len(net.controller):
        ...
        for idx, content in net.controller.iterrows():
            if not type(content["object"]) == ConstControl:
                ...
                elm_idxs = content["object"].__dict__["matching_params"][
                    "element_index"
                ]
                ...
                for pd_ei in elm_idxs:
                    if (
                        element == "sgen"
                        and "controllable" in net[element].columns
                        and net[element].controllable[pd_ei]
                    ):
                    ...
                    pm["time_series"][pm_elm][str(pm_ei)][variable] = (
                        {str(tp_list[m]): n for m, n in enumerate(list(-target_ts))}
                        if (element != "load" and pm_elm not in element)
