...
def set_pp_col_types(
    net: Union[pandapowerNet, Dict], ignore_errors: bool = False
) -> pandapowerNet:
    """
    ...
    if hasattr(net, "sgen"):
        _set_column_to_type(
            net["sgen"], "current_source", structure_dict["sgen"]["current_source"]
        )
        _set_column_to_type(
            net["sgen"],
    ...
    if hasattr(net, "shunt"):
        ...
        _set_column_to_type(
            net["shunt"], "max_step", structure_dict["shunt"]["max_step"]
        )
        _set_column_to_type(
            net["shunt"],
            "id_characteristic_table",
...
def add_slack_and_lines_to_boundary_nodes(
    net: pandapowerNet, voltage_levels: List[int] = None
):
    """
    ...
    busses = net.bus.loc[
        (net.bus[sc["o_prf"]] == "eq_bd") | (net.bus[sc["o_prf"]] == "tp_bd")
    ]
    ...
    for one_voltage_level in voltage_levels:
        ...
        create_lines(
            net,
            from_buses=busses_t.index.values,
            to_buses=new_bus_id_array,
            std_type="low_impedance_line",
            name="virtual line to slack node with voltage level "
            + str(one_voltage_level),
            length_km=1,
        )
