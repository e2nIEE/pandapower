...
class PandapowerDiagnostic:
    ...
    def _rec_replace_pp_diagnostic_with_cim_ids(
        self, input_obj, element_type: str = None
    ):
        ...
        element_mapping = {
            ...
            "asymmetric_loads": "asymmetric_load",
            "asymmetric_sgen": "asymmetric_sgen",
            "asymmetric_sgens": "asymmetric_sgen",
            "storage": "storage",
            "storages": "storage",
            "gen": "gen",
            "gens": "gen",
        ...
        if isinstance(input_obj, list):
            ...
            for one_input_obj in input_obj:
                if isinstance(one_input_obj, list) or isinstance(one_input_obj, dict):
                    return_obj.append(
                        self._rec_replace_pp_diagnostic_with_cim_ids(
                            one_input_obj, element_type
                        )
                    )
                elif element_type is not None and isinstance(one_input_obj, int):
                    # get the RDF ID direct
                    return_obj.append(
                        self.net[element_type][sc["o_id"]].at[one_input_obj]
                    )
                    ...
                    if one_input_obj[0] in self.net[element_type].index.values:
                        one_input_obj[0] = self.net[element_type][sc["o_id"]].at[
                            one_input_obj[0]
                        ]
            ...
            for key, item in input_obj.items():
                if (
                    ...
                    and key in element_mapping
                ):
                    element_type = element_mapping[key]
                    return_obj[key] = self._rec_replace_pp_diagnostic_with_cim_ids(
                        item, element_type
                    )
                else:
                    # default
