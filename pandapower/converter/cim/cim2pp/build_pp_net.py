...
class CimConverter:
    def __init__(
        self, cim_parser: cim_classes.CimParser, converter_classes: Dict, **kwargs
    ):
        ...
        if pp_type not in self.net:
            ...
            self.report_container.add_log(
                Report(
                    ...
                    message="Missing pandapower type %s in the pandapower network!"
                    % pp_type,
                )
            )
            return
    ...
    def convert_to_pp(
        ...
        **kwargs,
        ...
        for one_prf, one_profile_dict in self.cim.items():
            if (
                "FullModel" in one_profile_dict
                and one_profile_dict["FullModel"].index.size > 0
            ):
                self.net["CGMES"][one_prf] = (
                    one_profile_dict["FullModel"]
                    .set_index("rdfId")
                    .to_dict(orient="index")
                )
        # store the BaseVoltage IDs
        self.net["CGMES"]["BaseVoltage"] = pd.concat(
            [self.cim["eq"]["BaseVoltage"], self.cim["eq_bd"]["BaseVoltage"]],
            sort=False,
            ignore_index=True,
        )[["rdfId", "nominalVoltage"]]

        # --------- convert busses ---------
        self.classes_dict["ConnectivityNodesCim16"](
            cimConverter=self
        ).convert_connectivity_nodes_cim16()
        # --------- convert external networks ---------
        self.classes_dict["externalNetworkInjectionsCim16"](
            cimConverter=self
        ).convert_external_network_injections_cim16()
        # --------- convert lines ---------
        self.classes_dict["acLineSegmentsCim16"](
            cimConverter=self
        ).convert_ac_line_segments_cim16(
            convert_line_to_switch, line_r_limit, line_x_limit
        )
        self.classes_dict["dcLineSegmentsCim16"](
            cimConverter=self
        ).convert_dc_line_segments_cim16()
        # --------- convert switches ---------
        self.classes_dict["switchesCim16"](cimConverter=self).convert_switches_cim16()
        # --------- convert loads ---------
        self.classes_dict["energyConsumersCim16"](
            cimConverter=self
        ).convert_energy_consumers_cim16()
        self.classes_dict["conformLoadsCim16"](
            cimConverter=self
        ).convert_conform_loads_cim16()
        self.classes_dict["nonConformLoadsCim16"](
            cimConverter=self
        ).convert_non_conform_loads_cim16()
        self.classes_dict["stationSuppliesCim16"](
            cimConverter=self
        ).convert_station_supplies_cim16()
        # --------- convert generators ---------
        self.classes_dict["synchronousMachinesCim16"](
            cimConverter=self
        ).convert_synchronous_machines_cim16()
        self.classes_dict["asynchronousMachinesCim16"](
            cimConverter=self
        ).convert_asynchronous_machines_cim16()
        self.classes_dict["energySourcesCim16"](
            cimConverter=self
        ).convert_energy_sources_cim16()
        # --------- convert shunt elements ---------
        self.classes_dict["linearShuntCompensatorCim16"](
            cimConverter=self
        ).convert_linear_shunt_compensator_cim16()
        self.classes_dict["nonLinearShuntCompensatorCim16"](
            cimConverter=self
        ).convert_nonlinear_shunt_compensator_cim16()
        self.classes_dict["staticVarCompensatorCim16"](
            cimConverter=self
        ).convert_static_var_compensator_cim16()
        # --------- convert impedance elements ---------
        self.classes_dict["equivalentBranchesCim16"](
            cimConverter=self
        ).convert_equivalent_branches_cim16()
        self.classes_dict["seriesCompensatorsCim16"](
            cimConverter=self
        ).convert_series_compensators_cim16()
        # --------- convert extended ward and ward elements ---------
        self.classes_dict["equivalentInjectionsCim16"](
            cimConverter=self
        ).convert_equivalent_injections_cim16()
        # --------- convert transformers ---------
        self.classes_dict["powerTransformersCim16"](
            cimConverter=self
        ).convert_power_transformers_cim16()
        ...
        if (
            self.cim["gl"]["Location"].index.size > 0
            and self.cim["gl"]["PositionPoint"].index.size > 0
        ):
            try:
                self.classes_dict["geoCoordinatesFromGLCim16"](
                    cimConverter=self
                ).add_geo_coordinates_from_gl_cim16()
            except Exception as e:
                self.logger.warning(
                    "Creating the geo coordinates failed, returning the net without geo coordinates!"
                )
                ...
                self.report_container.add_log(
                    Report(
                        ...
                        code=ReportCode.EXCEPTION_CONVERTING,
                        message=traceback.format_exc(),
                    )
                )
        if (
            self.cim["dl"]["Diagram"].index.size > 0
            and self.cim["dl"]["DiagramObject"].index.size > 0
            and self.cim["dl"]["DiagramObjectPoint"].index.size > 0
        ):
            try:
                self.classes_dict["coordinatesFromDLCim16"](
                    cimConverter=self
                ).add_coordinates_from_dl_cim16(
                    diagram_name=kwargs.get("diagram_name", None)
                )
            except Exception as e:
                self.logger.warning(
                    "Creating the coordinates failed, returning the net without coordinates!"
                )
                self.logger.exception(e)
                self.report_container.add_log(
                    Report(
        ...
        self.classes_dict["tapController"](
            cimConverter=self
        ).create_tap_controller_for_power_transformers()

        self.logger.info("Running a power flow.")
        self.report_container.add_log(
            Report(
                level=LogLevel.INFO,
                code=ReportCode.INFO,
                message="Running a power flow.",
            )
        )
            ...
            try:
                ...
                self.report_container.add_log(
                    Report(
                        ...
                        code=ReportCode.ERROR,
                        message="Failed running a powerflow.",
                    )
                )
                self.report_container.add_log(
                    Report(
                        level=LogLevel.EXCEPTION,
                        code=ReportCode.EXCEPTION,
                        message=traceback.format_exc(),
                    )
                )
                ...
                self.report_container.add_log(
                    Report(
                        level=LogLevel.INFO,
                        code=ReportCode.INFO,
                        message="Power flow solved normal.",
                    )
                )
        try:
            ...
            if create_measurements is not None and create_measurements.lower() == "sv":
                ...
                create_measurements is not None
                and create_measurements.lower() == "analog"
            ):
                ...
                self.report_container.add_log(
                    Report(
                        level=LogLevel.ERROR,
                        code=ReportCode.ERROR_CONVERTING,
                        message="Not supported value for argument 'create_measurements', check method signature for"
                        "valid values!",
                    )
                )
                raise ValueError(
                    "Not supported value for argument 'create_measurements', check method signature for"
                    "valid values!"
                )
        except Exception as e:
            self.logger.error(
                "Creating the measurements failed, returning the net without measurements!"
            )
            self.logger.exception(e)
            self.report_container.add_log(
                Report(
        ...
        bus_drop = bus_t.loc[
            bus_t[sc["o_prf"]] == "eq_bd", ["index", sc["o_id"], "cim_topnode"]
        ]
        bus_drop = bus_drop.rename(columns={"index": "b1"})
        bus_drop = pd.merge(
            bus_drop,
            bus_t[["index", sc["o_id"]]].rename(
                columns={"index": "b2", sc["o_id"]: "o_id2"}
            ),
            how="inner",
            left_on="cim_topnode",
            right_on="o_id2",
        ...
        ward_t["bus_prf"] = ward_t["bus"].map(
            self.net.bus[[sc["o_prf"]]].to_dict().get(sc["o_prf"])
        )
        self.net.ward.loc[
            (
                self.net.ward.bus.duplicated(keep=False)
                & ((ward_t["bus_prf"] == "eq_bd") | (ward_t["bus_prf"] == "tp_bd"))
            ),
            "in_service",
        ] = False
        xward_t = self.net.xward.copy()
        xward_t["bus_prf"] = xward_t["bus"].map(
            self.net.bus[[sc["o_prf"]]].to_dict().get(sc["o_prf"])
        )
        self.net.xward.loc[
            (
                self.net.xward.bus.duplicated(keep=False)
                & ((xward_t["bus_prf"] == "eq_bd") | (xward_t["bus_prf"] == "tp_bd"))
            ),
            "in_service",
        ] = False
