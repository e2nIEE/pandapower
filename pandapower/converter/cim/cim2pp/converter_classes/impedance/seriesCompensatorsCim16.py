...
class SeriesCompensatorsCim16:
    ...
    def convert_series_compensators_cim16(self):
        ...
        self.logger.info(
            "Created %s impedance elements in %ss."
    ...
    def _prepare_series_compensators_cim16(self) -> pd.DataFrame:
        ...
        ser_comp = pd.merge(
            ...
            pd.concat(
                [
                    self.cimConverter.cim["eq"]["BaseVoltage"],
                    self.cimConverter.cim["eq_bd"]["BaseVoltage"],
                ],
                ignore_index=True,
            )[["rdfId", "nominalVoltage"]].rename(columns={"rdfId": "BaseVoltage"}),
            how="left",
            on="BaseVoltage",
        )
        ...
        # add the buses
        eqs_length_before_merge = self.cimConverter.cim["eq"][
            "SeriesCompensator"
        ].index.size
        ...
        ser_comp = pd.merge(
            ser_comp, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        if ser_comp.index.size != eqs_length_before_merge * 2:
            self.logger.error(
                "There is a problem at the SeriesCompensators source data: Not each SeriesCompensator "
            ...
            for rdfId, count in dups.items():
                self.logger.warning(
                    "The SeriesCompensator with RDF ID %s has %s Terminals!"
                    % (rdfId, count)
                )
                self.logger.warning(
                    "The SeriesCompensator data: \n%s"
                    % ser_comp[ser_comp["rdfId"] == rdfId]
                )
                self.cimConverter.report_container.add_log(
                    Report(
                        level=LogLevel.ERROR,
                        code=ReportCode.ERROR_CONVERTING,
                        message="The SeriesCompensator with RDF ID %s has %s Terminals!"
                        % (rdfId, count),
                    )
                )
        ...
        ser_comp.rdfId_Terminal2 = (
            ser_comp.rdfId_Terminal2.iloc[1:].reset_index().rdfId_Terminal2
        )
