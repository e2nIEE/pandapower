...
class EquivalentBranchesCim16:
    ...
    def _prepare_equivalent_branches_cim16(self) -> pd.DataFrame:
        ...
        eqb = pd.merge(
            ...
            pd.concat(
                [
                    ...
                    ].rename(columns={"rdfId": "BaseVoltage"}),
        ...
        # add the buses
        eqb_length_before_merge = self.cimConverter.cim["eq"][
            "EquivalentBranch"
        ].index.size
        ...
        if eqb.index.size != eqb_length_before_merge * 2:
            self.logger.error(
                "There is a problem at the EquivalentBranches source data: Not each EquivalentBranch "
            ...
            for rdfId, count in dups.items():
                self.logger.warning(
                    "The EquivalentBranch with RDF ID %s has %s Terminals!"
                    % (rdfId, count)
                )
                self.logger.warning(
                    "The EquivalentBranch data: \n%s" % eqb[eqb["rdfId"] == rdfId]
                )
                self.cimConverter.report_container.add_log(
                    Report(
                        level=LogLevel.WARNING,
                        code=ReportCode.WARNING_CONVERTING,
                        message="The EquivalentBranch with RDF ID %s has %s Terminals!"
                        % (rdfId, count),
                    )
                )
            eqb = eqb[0:0]
