...
logger = logging.getLogger(
    "cim.cim2pp.converter_classes.nonLinearShuntCompensatorCim16"
)
...
class NonLinearShuntCompensatorCim16:
    ...
    def _prepare_nonlinear_shunt_compensator_cim16(self) -> pd.DataFrame:
        eqssh_shunts = self.cimConverter.merge_eq_ssh_profile(
            "NonlinearShuntCompensator", add_cim_type_column=True
        )
        eqssh_shunts = pd.merge(
            eqssh_shunts, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        nscp = self.cimConverter.cim["eq"]["NonlinearShuntCompensatorPoint"][
            ["NonlinearShuntCompensator", "sectionNumber", "b", "g"]
        ].rename(columns={"NonlinearShuntCompensator": "rdfId"})
        ...
        for i in range(1, int(nscp["sectionNumber"].max()) + 1):
            ...
            if i == 1:
                ...
                eqssh_shunts.loc[
                    eqssh_shunts["sections"] >= eqssh_shunts["sectionNumber"], "p"
                ] = eqssh_shunts["p"] + eqssh_shunts["p_temp"]
                eqssh_shunts.loc[
                    eqssh_shunts["sections"] >= eqssh_shunts["sectionNumber"], "q"
                ] = eqssh_shunts["q"] + eqssh_shunts["q_temp"]
            eqssh_shunts = eqssh_shunts[eqssh_shunts_cols]
        if "inService" in eqssh_shunts.columns:
            eqssh_shunts["connected"] = (
                eqssh_shunts["connected"] & eqssh_shunts["inService"]
            )
        ...
        eqssh_shunts = eqssh_shunts.rename(
            columns={
                "rdfId": sc["o_id"],
                "rdfId_Terminal": sc["t"],
    ...
    def _create_shunt_characteristic_table(self, eqssh_shunts):
        ...
        nscp = self.cimConverter.cim["eq"]["NonlinearShuntCompensatorPoint"][
            ["NonlinearShuntCompensator", "sectionNumber", "b", "g"]
        ].rename(columns={"NonlinearShuntCompensator": sc["o_id"]})
        ...
        char_temp = char_temp.rename(
            columns={"p": "p_mw", "q": "q_mvar", "sectionNumber": "step"}
        )
        ...
        id_char_dict = (
            char_temp.drop_duplicates(sc["o_id"])
            .set_index(sc["o_id"])["id_characteristic"]
            .to_dict()
        )
        eqssh_shunts["id_characteristic_table"] = (
            eqssh_shunts[sc["o_id"]].map(id_char_dict).astype("Int64")
        )
        ...
        eqssh_shunts.loc[
            eqssh_shunts["id_characteristic_table"].notna(), "step_dependency_table"
        ] = True

        # populate shunt_characteristic_temp table
        self.cimConverter.net["shunt_characteristic_table"] = char_temp[
