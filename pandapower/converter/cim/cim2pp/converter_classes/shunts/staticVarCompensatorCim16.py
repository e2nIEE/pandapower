...
class StaticVarCompensatorCim16:
    ...
    def _prepare_static_var_compensator_cim16(self) -> pd.DataFrame:
        eq_stat_coms = self.cimConverter.merge_eq_ssh_profile(
            "StaticVarCompensator", True
        )
        eq_stat_coms = pd.merge(
            eq_stat_coms, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        eq_stat_coms.loc[
            eq_stat_coms["sVCControlMode"] == "reactivePower", "voltageSetPoint"
        ] = float("NaN")
        if "inService" in eq_stat_coms.columns:
            eq_stat_coms["connected"] = (
                eq_stat_coms["connected"] & eq_stat_coms["inService"]
            )
        eq_stat_coms = eq_stat_coms.rename(
            columns={
                "rdfId_Terminal": sc["t"],
                "rdfId": sc["o_id"],
                "voltageSetPoint": "vn_kv",
                "index_bus": "bus",
