...
class CoordinatesFromDLCim16:
    ...
    def add_coordinates_from_dl_cim16(self, diagram_name: str = None):
        ...
        if diagram_name is None:
            diagram_name = (
                self.cimConverter.cim["dl"]["Diagram"]
        ...
        if diagram_name != "all":
            ...
            diagram_rdf_id = self.cimConverter.cim["dl"]["Diagram"]["rdfId"][
                self.cimConverter.cim["dl"]["Diagram"]["name"] == diagram_name
            ].values[0]
            dl_do = self.cimConverter.cim["dl"]["DiagramObject"][
                self.cimConverter.cim["dl"]["DiagramObject"]["Diagram"]
                == diagram_rdf_id
            ]
        ...
        dl_data = pd.merge(
            dl_do,
            self.cimConverter.cim["dl"]["DiagramObjectPoint"],
            how="left",
            on="DiagramObject",
        )
        ...
        dl_data["coords_str"] = (
            "["
            + dl_data["xPosition"].astype(str)
            + ", "
            + dl_data["yPosition"].astype(str)
            + "]"
        )
        ...
        for one_ele in [
            ...
            if one_ele != "line" and one_ele != "dcline" and one_ele != "impedance":
                ...
                one_ele_df["diagram"] = (
                    '{"coordinates": ' + one_ele_df["coords_str"] + ', "type": "Point"}'
                )
            else:
                # line strings
            ...
            self.cimConverter.net[one_ele]["diagram"] = self.cimConverter.net[one_ele][
                sc["o_id"]
            ].map(
        ...
        self.logger.info(
            "Finished creating the DL coordinates, needed time: %ss"
        ...
        self.cimConverter.report_container.add_log(
            Report(
                ...
                message="Created the DL coordinates, needed time: %ss"
                % (time.time() - time_start),
            )
        )
