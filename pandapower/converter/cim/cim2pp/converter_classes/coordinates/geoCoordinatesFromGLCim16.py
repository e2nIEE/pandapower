...
class GeoCoordinatesFromGLCim16:
    ...
    def add_geo_coordinates_from_gl_cim16(self):
        self.logger.info(
            "Creating the geo coordinates from CGMES GeographicalLocation."
        )
        time_start = time.time()
        gl_data = pd.merge(
            self.cimConverter.cim["gl"]["PositionPoint"][
                ["Location", "xPosition", "yPosition", "sequenceNumber"]
            ],
            self.cimConverter.cim["gl"]["Location"][["rdfId", "PowerSystemResources"]],
            how="left",
            left_on="Location",
            right_on="rdfId",
        )
        ...
        gl_data["coords_str"] = (
            gl_data["xPosition"].astype(str) + ", " + gl_data["yPosition"].astype(str)
        )
        ...
        cn = self.cimConverter.cim["eq"]["ConnectivityNode"][
            ["rdfId", "ConnectivityNodeContainer"]
        ]
        cn = pd.concat(
            [
                cn,
                self.cimConverter.cim["eq_bd"]["ConnectivityNode"][
                    ["rdfId", "ConnectivityNodeContainer"]
                ],
            ]
        )
        cn = pd.concat(
            [
                cn,
                self.cimConverter.cim["tp"]["TopologicalNode"][
                    ["rdfId", "ConnectivityNodeContainer"]
                ],
            ]
        )
        ...
        cn = cn.rename(
            columns={"rdfId": sc["o_id"], "ConnectivityNodeContainer": "rdfId"}
        )
        cn = pd.merge(
            cn,
            self.cimConverter.cim["eq"]["VoltageLevel"][["rdfId", "Substation"]],
            how="left",
            on="rdfId",
        )
        cn = cn.drop(columns=["rdfId"])
        buses = pd.merge(
            self.cimConverter.net.bus[[sc["o_id"]]], cn, how="left", on=sc["o_id"]
        )
        ...
        bus_geo["geo"] = (
            '{"coordinates": [' + bus_geo["coords_str"] + '], "type": "Point"}'
        )
        self.cimConverter.net["bus"]["geo"] = self.cimConverter.net["bus"][
            sc["o_id"]
        ].map(bus_geo.set_index(sc["o_id"]).to_dict(orient="dict").get("geo"))
        ...
        for _, df_group in line_geo.groupby(by=sc["o_id"]):
            line_geo.at[df_group.index.values[0], "coords"] = df_group[
                ["xPosition", "yPosition"]
            ].values.tolist()
        ...
        line_geo["geo"] = (
            '{"coordinates": '
            + line_geo["coords"].astype(str)
            + ', "type": "LineString"}'
        )
        # now add the line coordinates
        self.cimConverter.net["line"]["geo"] = self.cimConverter.net["line"][
            sc["o_id"]
        ].map(line_geo.set_index(sc["o_id"]).to_dict(orient="dict").get("geo"))
        ...
        for one_ele in [
            ...
            if one_ele not in ["dcline", "impedance"]:
                ...
                one_ele_df["geo"] = (
                    '{"coordinates": ['
                    + one_ele_df["coords_str"]
                    + '], "type": "Point"}'
                )
            else:
                # line strings
                one_ele_df["coords"] = one_ele_df[
                    ["xPosition", "yPosition"]
                ].values.tolist()
                ...
                one_ele_df["geo"] = (
                    '{"coordinates": '
                    + one_ele_df["coords"].astype(str)
                    + ', "type": "LineString"}'
                )
            # now add the coordinates
            self.cimConverter.net[one_ele]["geo"] = self.cimConverter.net[one_ele][
                sc["o_id"]
            ].map(one_ele_df.set_index(sc["o_id"]).to_dict(orient="dict").get("geo"))

        self.logger.info(
            "Finished creating the GL coordinates, needed time: %ss"
        ...
        self.cimConverter.report_container.add_log(
            Report(
                ...
                message="Created the GL coordinates, needed time: %ss"
                % (time.time() - time_start),
            )
        )
