...
class SwitchesCim16:
    ...
    def convert_switches_cim16(self):
        ...
        self.logger.info(
            "Start converting Breakers, Disconnectors, LoadBreakSwitches and Switches."
        )
        eqssh_switches = self._prepare_switches_cim16()
        self.cimConverter.copy_to_pp("switch", eqssh_switches)
        self.logger.info(
            "Created %s switches in %ss."
    ...
    def _prepare_switches_cim16(self) -> pd.DataFrame:
        eqssh_switches = self.cimConverter.merge_eq_ssh_profile(
            "Breaker", add_cim_type_column=True
        )
        ...
        eqssh_switches = pd.concat(
            [
                eqssh_switches,
                self.cimConverter.merge_eq_ssh_profile(
                    "Disconnector", add_cim_type_column=True
                ),
            ],
            ignore_index=True,
            sort=False,
        ...
        eqssh_switches = pd.concat(
            [
                eqssh_switches,
                self.cimConverter.merge_eq_ssh_profile(
                    "LoadBreakSwitch", add_cim_type_column=True
                ),
            ],
            ignore_index=True,
            sort=False,
        ...
        eqssh_switches = pd.concat(
            [
                eqssh_switches,
                self.cimConverter.merge_eq_ssh_profile(
                    "Switch", add_cim_type_column=True
                ),
            ],
            ignore_index=True,
            sort=False,
        ...
        eqssh_switches = pd.merge(
            eqssh_switches, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        if eqssh_switches.index.size == switch_length_before_merge * 2:
            ...
            eqssh_switches.rdfId_Terminal2 = (
                eqssh_switches.rdfId_Terminal2.iloc[1:].reset_index().rdfId_Terminal2
            )
            eqssh_switches.connected2 = (
                eqssh_switches.connected2.iloc[1:].reset_index().connected2
            )
            eqssh_switches.index_bus2 = (
                eqssh_switches.index_bus2.iloc[1:].reset_index().index_bus2
            )
            eqssh_switches = eqssh_switches.drop_duplicates(
                subset=["rdfId"], keep="first"
            )
        else:
            self.logger.error(
                "Something went wrong at switches, seems like that terminals for connection with "
            ...
            for rdfId, count in dups.items():
                self.logger.warning(
                    "The switch with RDF ID %s has %s Terminals!" % (rdfId, count)
                )
                self.logger.warning(
                    "The switch data: \n%s"
                    % eqssh_switches[eqssh_switches["rdfId"] == rdfId]
                )
                self.cimConverter.report_container.add_log(
                    Report(
                        level=LogLevel.WARNING,
                        code=ReportCode.WARNING_CONVERTING,
                        message="The switch with RDF ID %s has %s Terminals!"
                        % (rdfId, count),
                    )
                )
            eqssh_switches = eqssh_switches[0:0]
        eqssh_switches = eqssh_switches.rename(
            columns={
                "rdfId": sc["o_id"],
                "index_bus": "bus",
                "index_bus2": "element",
        ...
        if eqssh_switches.index.size > 0:
            eqssh_switches["closed"] = (
                ~eqssh_switches.open
                & eqssh_switches.connected
                & eqssh_switches.connected2
