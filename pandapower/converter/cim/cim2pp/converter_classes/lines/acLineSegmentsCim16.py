...
class AcLineSegmentsCim16:
    ...
    def convert_ac_line_segments_cim16(
        self, convert_line_to_switch, line_r_limit, line_x_limit
    ):
        time_start = time.time()
        self.logger.info("Start converting ACLineSegments.")
        eq_ac_line_segments = self._prepare_ac_line_segments_cim16(
        ...
        if "line" in eq_ac_line_segments["kindOfType"].values:
            line_df = eq_ac_line_segments.loc[
                eq_ac_line_segments["kindOfType"] == "line"
            ]
            line_df = line_df.rename(
                columns={
                    "rdfId": sc["o_id"],
                    "rdfId_Terminal": sc["t_from"],
                    "rdfId_Terminal2": sc["t_to"],
                    "index_bus": "from_bus",
                    "index_bus2": "to_bus",
                    "length": "length_km",
            ...
            line_df["c_nf_per_km"] = (
                abs(line_df.bch) / (2 * 50 * np.pi * line_df.length_km) * 1e9
            )
            ...
            line_df["c0_nf_per_km"] = (
                abs(line_df.b0ch) / (2 * 50 * np.pi * line_df.length_km) * 1e9
            )
        ...
        if "switch" in eq_ac_line_segments["kindOfType"].values:
            switch_df = eq_ac_line_segments.loc[
                eq_ac_line_segments["kindOfType"] == "switch"
            ]
        ...
        self.logger.info(
            "Created %s lines and %s switches in %ss"
    ...
    def _prepare_ac_line_segments_cim16(
        self, convert_line_to_switch, line_r_limit, line_x_limit
    ) -> pd.DataFrame:
        ...
        ac_line_segments = pd.merge(
            ac_line_segments, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        if ac_line_segments.index.size != line_length_before_merge * 2:
            self.logger.error(
                "Error processing the ACLineSegments, there is a problem with Terminals in the source "
                "data!"
            )
            self.cimConverter.report_container.add_log(
                Report(
            ...
            for rdfId, count in dups.items():
                self.logger.warning(
                    "The ACLineSegment with RDF ID %s has %s Terminals!"
                    % (rdfId, count)
                )
                self.logger.warning(
                    "The ACLineSegment data: \n%s"
                    % ac_line_segments[ac_line_segments["rdfId"] == rdfId]
                )
                self.cimConverter.report_container.add_log(
                    Report(
                        level=LogLevel.WARNING,
                        code=ReportCode.WARNING_CONVERTING,
                        message="The ACLineSegment with RDF ID %s has %s Terminals!"
                        % (rdfId, count),
                    )
                )
        ...
        eq_operational_limit_sets = self.cimConverter.cim["eq"]["OperationalLimitSet"][
            ["rdfId", "Terminal"]
        ]
        eq_operational_limit_sets = eq_operational_limit_sets.rename(
            columns={"rdfId": "rdfId_OperationalLimitSet", "Terminal": "rdfId_Terminal"}
        )
        ac_line_segments = pd.merge(
            ac_line_segments, eq_operational_limit_sets, how="left", on="rdfId_Terminal"
        )
        if "CurrentLimit" in self.cimConverter.cim["ssh"]:
            current_limits = self.cimConverter.merge_eq_ssh_profile("CurrentLimit")[
                ["rdfId", "OperationalLimitSet", "value"]
            ]
        else:
            current_limits = self.cimConverter.cim["eq"]["CurrentLimit"][
                ["rdfId", "OperationalLimitSet", "value"]
            ]
        current_limits = current_limits.rename(
            columns={
                "rdfId": "rdfId_CurrentLimit",
                "OperationalLimitSet": "rdfId_OperationalLimitSet",
            }
        )
        ac_line_segments = pd.merge(
            ac_line_segments, current_limits, how="left", on="rdfId_OperationalLimitSet"
        )
        ...
        ac_line_segments = ac_line_segments.sort_values(
            by=["rdfId", "sequenceNumber", "value"]
        )
        ac_line_segments = ac_line_segments.drop_duplicates(
            ["rdfId", "rdfId_Terminal"], keep="first"
        )

        # copy the columns which are needed to reduce the ac_line_segments to one row per line
        ac_line_segments["rdfId_Terminal2"] = ac_line_segments["rdfId_Terminal"].copy()
        ac_line_segments["connected2"] = ac_line_segments["connected"].copy()
        ac_line_segments["index_bus2"] = ac_line_segments["index_bus"].copy()
        ac_line_segments["value2"] = ac_line_segments["value"].copy()
        ac_line_segments = ac_line_segments.reset_index()
        ...
        ac_line_segments.rdfId_Terminal2 = (
            ac_line_segments.rdfId_Terminal2.iloc[1:].reset_index().rdfId_Terminal2
        )
        ac_line_segments.connected2 = (
            ac_line_segments.connected2.iloc[1:].reset_index().connected2
        )
        ac_line_segments.index_bus2 = (
            ac_line_segments.index_bus2.iloc[1:].reset_index().index_bus2
        )
        ...
        # get the max_i_ka
        ac_line_segments["max_i_ka"] = (
            ac_line_segments["value"].fillna(ac_line_segments["value2"]) * 1e-3
        )

        # filter if line or switches will be added
        ac_line_segments["kindOfType"] = "line"
        if convert_line_to_switch:
            ac_line_segments.loc[
                (abs(ac_line_segments["r"]) <= line_r_limit)
                | (abs(ac_line_segments["x"]) <= line_x_limit),
                "kindOfType",
            ] = "switch"
