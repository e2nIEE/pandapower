...
class DcLineSegmentsCim16:
    ...
    def _prepare_dc_line_segments_cim16(self) -> pd.DataFrame:
        line_length_before_merge = self.cimConverter.cim["eq"][
            "DCLineSegment"
        ].index.size
        ...
        dc_line_segments = pd.merge(
            self.cimConverter.cim["eq"]["DCLineSegment"],
            self.cimConverter.bus_merge,
            how="left",
            on="rdfId",
        )
        dc_line_segments = dc_line_segments[
            ["rdfId", "name", "description", "ConnectivityNode", "sequenceNumber"]
        ]
        ...
        if dc_line_segments.index.size != line_length_before_merge * 2:
            self.logger.error(
                "Error processing the DCLineSegments, there is a problem with Terminals in the source "
                "data!"
            )
            self.cimConverter.report_container.add_log(
                Report(
        ...
        converters = pd.merge(
            pd.concat(
                [
                    self.cimConverter.cim["eq"]["CsConverter"],
                    self.cimConverter.cim["eq"]["VsConverter"],
                ],
                ignore_index=True,
                sort=False,
            ),
            pd.concat(
                [
                    self.cimConverter.cim["ssh"]["CsConverter"],
                    self.cimConverter.cim["ssh"]["VsConverter"],
                ],
                ignore_index=True,
                sort=False,
            ),
        ...
        converters = pd.merge(
            converters, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        converter_terminals = converter_terminals[["rdfId"]].rename(
            columns={"rdfId": "rdfId_Terminal"}
        )
        converters_t = pd.merge(
            converters, converter_terminals, how="inner", on="rdfId_Terminal"
        )

        dc_line_segments = pd.merge(
            dc_line_segments,
            converters[["converters", "ConnectivityNode"]],
            how="left",
            on="ConnectivityNode",
        )
        # get the missing converters (maybe there is a switch or something else between the line and the converter)
        t = self.cimConverter.cim["eq"]["DCTerminal"][
            ["DCNode", "DCConductingEquipment", "sequenceNumber"]
        ]
        t = t.rename(
            columns={
            ...
            for one_cn, from_dev in cn_ids.items():
                ...
                for _, one_t in t_temp.iterrows():
                    ...
                    if one_t["ConductingEquipment"] == from_dev:
                        continue
                    ids_temp = t.loc[
                        (t["ConductingEquipment"] == one_t["ConductingEquipment"])
                        & (t["sequenceNumber"] != one_t["sequenceNumber"])
                    ]["ConnectivityNode"].values
                    for id_temp in ids_temp:
                        # check if the ConnectivityNode has a converter
                        if id_temp in converters["ConnectivityNode"].values:
                            # found the converter
                            return converters.loc[
                                converters["ConnectivityNode"] == id_temp, "converters"
                            ].values[0]
        ...
        for row_index, row in dc_line_segments[
            dc_line_segments["converters"].isna()
        ].iterrows():
            conv = search_converter(
                cn_ids={row["ConnectivityNode"]: row["rdfId"]},
                visited_cns=[row["ConnectivityNode"]],
            )
            dc_line_segments.loc[row_index, "converters"] = conv
            if conv is None:
                self.logger.warning(
                    "Problem with converting tht DC line %s: No ACDC converter found, maybe the DC "
        ...
        dc_line_segments = pd.merge(
            dc_line_segments, converters_t, how="left", on="converters"
        )
        dc_line_segments["targetUpcc"] = dc_line_segments["targetUpcc"].fillna(
            dc_line_segments["base_voltage_bus"]
        )
        ...
        for one_item in copy_list:
            ...
            dc_line_segments[one_item + "2"] = (
                dc_line_segments[one_item + "2"].iloc[1:].reset_index()[one_item + "2"]
            )
        ...
        dc_line_segments = pd.merge(
            dc_line_segments,
            pd.DataFrame(
                dc_line_segments.pivot_table(index=["converters"], aggfunc="size"),
                columns=["converter_dups"],
            ),
            how="left",
            on="converters",
        )
        dc_line_segments["loss_mw"] = (
            abs(abs(dc_line_segments["p"]) - abs(dc_line_segments["p2"]))
            / dc_line_segments["converter_dups"]
        )
        dc_line_segments["p_mw"] = (
            dc_line_segments["p"] / dc_line_segments["converter_dups"]
        )
        dc_line_segments["loss_percent"] = 0
        dc_line_segments["vm_from_pu"] = (
            dc_line_segments["targetUpcc"] / dc_line_segments["base_voltage_bus"]
        )
        dc_line_segments["vm_to_pu"] = (
            dc_line_segments["targetUpcc2"] / dc_line_segments["base_voltage_bus2"]
        )
        if "inService" not in dc_line_segments.columns:
            dc_line_segments["inService"] = True
        dc_line_segments["in_service"] = (
            dc_line_segments.connected
            & dc_line_segments.connected2
            & dc_line_segments.inService
        )
        dc_line_segments = dc_line_segments.rename(
            columns={
                "rdfId": sc["o_id"],
