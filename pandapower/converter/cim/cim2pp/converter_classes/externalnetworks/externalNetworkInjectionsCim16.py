...
logger = logging.getLogger(
    "cim.cim2pp.converter_classes.externalNetworkInjectionsCim16"
)
...
class ExternalNetworkInjectionsCim16:
    ...
    def convert_external_network_injections_cim16(self):
        ...
        # choose the slack
        eni_ref_prio_min = eqssh_eni.loc[
            (eqssh_eni["enabled"]) & (eqssh_eni["slack_weight"] > 0), "slack_weight"
        ].min()
        ...
        sync_ref_prio_min = sync_machines.loc[
            (sync_machines["referencePriority"] > 0) & (sync_machines["enabled"]),
            "referencePriority",
        ].min()
        ...
        eni_slacks = eqssh_eni.loc[
            (eqssh_eni["slack_weight"] == ref_prio_min) & (eqssh_eni["controllable"])
        ]
        eni_gens = eqssh_eni.loc[
            (eqssh_eni["slack_weight"] != ref_prio_min) & (eqssh_eni["controllable"])
        ]
        ...
        self.logger.info(
            "Created %s external networks, %s generators and %s static generators in %ss"
            % (
                eni_slacks.index.size,
                eni_gens.index.size,
                eni_sgens.index.size,
    ...
    def _prepare_external_network_injections_cim16(self) -> pd.DataFrame:
        if "sc" in self.cimConverter.cim:
            eni = self.cimConverter.merge_eq_other_profiles(
                ["ssh", "sc"], "ExternalNetworkInjection", add_cim_type_column=True
            )
        else:
            eni = self.cimConverter.merge_eq_ssh_profile(
                "ExternalNetworkInjection", add_cim_type_column=True
            )
        ...
        eni = pd.merge(
            eni,
            self.cimConverter.net.bus[["vn_kv", sc["ct"]]],
            how="left",
            left_on="index_bus",
            right_index=True,
        )
        eni = pd.merge(
            eni,
            self.cimConverter.cim["sv"]["SvVoltage"][["TopologicalNode", "v", "angle"]],
            how="left",
            left_on=sc["ct"],
        ...
        eni["vm_pu"] = eni["vm_pu"].fillna(
            eni["v"] / eni["vn_kv"]
        )  # voltage from measurement
        eni["vm_pu"] = eni["vm_pu"].fillna(1.0)  # default voltage
        eni["angle"] = eni["angle"].fillna(0.0)  # default angle
        ...
        eni["s_sc_max_mva"] = (
            3**0.5 * eni["ratedU"] * (eni["maxInitialSymShCCurrent"] / 1e3)
        )
        eni["s_sc_min_mva"] = (
            3**0.5 * eni["ratedU"] * (eni["minInitialSymShCCurrent"] / 1e3)
        )
        # get the substations
        eni = pd.merge(
            eni,
            self.cimConverter.net.bus[[sc["o_id"], "zone"]].rename(
                {sc["o_id"]: "b_id"}, axis=1
            ),
            how="left",
            left_on="ConnectivityNode",
            right_on="b_id",
        )
        ...
        eni["q"] = -eni["q"]
        eni["x0x_max"] = (
            (eni["maxR1ToX1Ratio"] + 1j) / (eni["maxR0ToX0Ratio"] + 1j)
        ).abs() * eni["maxZ0ToZ1Ratio"]
        ...
        eni = eni.rename(
            columns={
                ...
                "minR1ToX1Ratio": "rx_min",
                "maxR0ToX0Ratio": "r0x0_max",
    ...
    def get_voltage_from_controllers(self, eqssh_eni):
        ...
        eqssh_eni = pd.merge(
            eqssh_eni, regulation_controllers, how="left", on="RegulatingControl"
        )
        return eqssh_eni
