...
class EquivalentInjectionsCim16:
    ...
    def convert_equivalent_injections_cim16(self):
        ...
        self.logger.info(
            "Created %s wards and %s extended ward elements in %ss."
            % (
                eqssh_ei_wards.index.size,
                eqssh_ei_xwards.index.size,
    ...
    def _prepare_equivalent_injections_cim16(self) -> pd.DataFrame:
        if "sc" in self.cimConverter.cim:
            equivalent_injection = self.cimConverter.merge_eq_other_profiles(
                ["ssh", "sc"], "EquivalentInjection", add_cim_type_column=True
            )
        else:
            equivalent_injection = self.cimConverter.merge_eq_ssh_profile(
                "EquivalentInjection", add_cim_type_column=True
            )
        ...
        equivalent_injection = pd.merge(
            equivalent_injection, eq_base_voltages, how="left", on="BaseVoltage"
        )
        equivalent_injection = pd.merge(
            equivalent_injection, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        # maybe the BaseVoltage is not given, also get the nominalVoltage from the buses
        equivalent_injection = pd.merge(
            equivalent_injection,
            self.cimConverter.net.bus[["vn_kv"]],
            how="left",
            left_on="index_bus",
            right_index=True,
        )
        equivalent_injection.nominalVoltage = (
            equivalent_injection.nominalVoltage.fillna(equivalent_injection.vn_kv)
        )
        equivalent_injection["regulationStatus"] = equivalent_injection[
            "regulationStatus"
        ].fillna(False)
        equivalent_injection["vm_pu"] = (
            equivalent_injection.regulationTarget / equivalent_injection.nominalVoltage
        )
        if "inService" in equivalent_injection.columns:
            equivalent_injection["connected"] = (
                equivalent_injection["connected"] & equivalent_injection["inService"]
            )
        ...
        equivalent_injection["pz_mw"] = 0.0
        equivalent_injection["qz_mvar"] = 0.0
        equivalent_injection["r_ohm"] = 0.0
        equivalent_injection["x_ohm"] = 0.1
