...
class ConformLoadsCim16:
    ...
    def _prepare_conform_loads_cim16(self) -> pd.DataFrame:
        eqssh_conform_loads = self.cimConverter.merge_eq_ssh_profile(
            "ConformLoad", add_cim_type_column=True
        )
        eqssh_conform_loads = pd.merge(
            eqssh_conform_loads, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        if "inService" in eqssh_conform_loads.columns:
            eqssh_conform_loads["connected"] = (
                eqssh_conform_loads["connected"] & eqssh_conform_loads["inService"]
            )
        eqssh_conform_loads = eqssh_conform_loads.rename(
            columns={
                "rdfId": sc["o_id"],
                "rdfId_Terminal": sc["t"],
                "index_bus": "bus",
        ...
        eqssh_conform_loads["const_i_p_percent"] = 0.0
        eqssh_conform_loads["const_z_p_percent"] = 0.0
        eqssh_conform_loads["const_i_q_percent"] = 0.0
        eqssh_conform_loads["const_z_q_percent"] = 0.0
        eqssh_conform_loads["scaling"] = 1.0
