...
class NonConformLoadsCim16:
    ...
    def _prepare_non_conform_loads_cim16(self) -> pd.DataFrame:
        ...
        eqssh_non_conform_loads = eqssh_non_conform_loads.rename(columns={'rdfId': sc['o_id'], 'rdfId_Terminal': sc['t'],
                                                                          'index_bus': 'bus','p': 'p_mw', 'q': 'q_mvar'})
        if 'inService' not in eqssh_non_conform_loads.columns:
            eqssh_non_conform_loads['inService'] = True
