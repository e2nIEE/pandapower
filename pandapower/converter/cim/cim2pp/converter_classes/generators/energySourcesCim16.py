...
class EnergySourceCim16:
    ...
    def _prepare_energy_sources_cim16(self) -> pd.DataFrame:
        eq_energy_scheduling_type = pd.concat(
            [
                self.cimConverter.cim["eq"]["EnergySchedulingType"],
        ...
        eqssh_energy_sources = self.cimConverter.merge_eq_ssh_profile(
            "EnergySource", add_cim_type_column=True
        )
        eqssh_energy_sources = pd.merge(
        ...
        eqssh_energy_sources = pd.merge(
            eqssh_energy_sources, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        eqssh_energy_sources = eqssh_energy_sources.drop_duplicates(
            ["rdfId"], keep="first"
        )
        sgen_type = {
            "WP": "WP",
        ...
        eqssh_energy_sources["va_degree"] = (
            eqssh_energy_sources["voltageAngle"] * 180 / math.pi
        )
        eqssh_energy_sources["vm_pu"] = (
            eqssh_energy_sources["voltageMagnitude"]
            / eqssh_energy_sources["base_voltage_bus"]
        )
        eqssh_energy_sources["scaling"] = 1.0
        ...
        if "inService" in eqssh_energy_sources.columns:
            eqssh_energy_sources["connected"] = (
                eqssh_energy_sources["connected"] & eqssh_energy_sources["inService"]
            )
        eqssh_energy_sources = eqssh_energy_sources.rename(
            columns={
                "rdfId_Terminal": sc["t"],
                "rdfId": sc["o_id"],
