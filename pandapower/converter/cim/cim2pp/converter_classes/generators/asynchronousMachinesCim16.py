...
class AsynchronousMachinesCim16:
    ...
    def _prepare_asynchronous_machines_cim16(self) -> pd.DataFrame:
        eqssh_generating_units = self.cimConverter.merge_eq_ssh_profile(
            "WindGeneratingUnit"
        )
        # a column for the type of the static generator in pandapower
        eqssh_generating_units["type"] = "WP"
        eqssh_generating_units = pd.concat(
            [
                eqssh_generating_units,
                self.cimConverter.merge_eq_ssh_profile("GeneratingUnit"),
            ],
            sort=False,
        )
        eqssh_generating_units["type"] = eqssh_generating_units["type"].fillna(
            "GeneratingUnit"
        )
        eqssh_generating_units = pd.concat(
            [
        ...
        eqssh_generating_units = eqssh_generating_units.rename(
            columns={"rdfId": "GeneratingUnit"}
        )
        if "sc" in self.cimConverter.cim:
            asynchronous_machines = self.cimConverter.merge_eq_other_profiles(
                ["ssh", "sc"], "AsynchronousMachine", add_cim_type_column=True
            )
        else:
            asynchronous_machines = self.cimConverter.merge_eq_ssh_profile(
                "AsynchronousMachine", add_cim_type_column=True
            )
        ...
        asynchronous_machines = pd.merge(
            asynchronous_machines,
            eqssh_generating_units,
            how="left",
            suffixes=("_x", "_y"),
            on="GeneratingUnit",
        ...
        asynchronous_machines = pd.merge(
            asynchronous_machines, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        ...
        asynchronous_machines["sn_mva"] = asynchronous_machines["ratedS"].fillna(
            asynchronous_machines["nominalP"]
        )
        asynchronous_machines["generator_type"] = "async"
        asynchronous_machines["loading_percent"] = (
            100
            * asynchronous_machines["p_mw"]
            / asynchronous_machines["ratedMechanicalPower"]
        )
        if "inService_x" in asynchronous_machines.columns:
            asynchronous_machines["connected"] = (
                asynchronous_machines["connected"]
                & asynchronous_machines["inService_x"]
                & asynchronous_machines["inService_y"]
            )
        asynchronous_machines = asynchronous_machines.rename(
            columns={
                "rdfId_Terminal": sc["t"],
                "rdfId": sc["o_id"],
                "connected": "in_service",
                "index_bus": "bus",
                "rxLockedRotorRatio": "rx",
                "iaIrRatio": "lrc_pu",
