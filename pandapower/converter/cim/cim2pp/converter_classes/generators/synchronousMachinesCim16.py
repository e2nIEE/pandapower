...
class SynchronousMachinesCim16:
    ...
    def convert_synchronous_machines_cim16(self):
        ...
        eqssh_synchronous_machines = self._create_gen_characteristics_table(
            eqssh_synchronous_machines
        )

        # convert the SynchronousMachines with voltage control to gens
        eqssh_sm_gens = eqssh_synchronous_machines.loc[
            (eqssh_synchronous_machines["mode"] == "voltage")
            & (eqssh_synchronous_machines["enabled"])
        ]
        self.cimConverter.copy_to_pp("gen", eqssh_sm_gens)
        # now deal with the pq generators
        eqssh_synchronous_machines = eqssh_synchronous_machines.loc[
            (eqssh_synchronous_machines["mode"] != "voltage")
            | (~eqssh_synchronous_machines["enabled"])
        ]
        self.cimConverter.copy_to_pp("sgen", eqssh_synchronous_machines)
        self.logger.info(
            "Created %s gens and %s sgens in %ss."
            % (
                eqssh_sm_gens.index.size,
                eqssh_synchronous_machines.index.size,
    ...
    def _prepare_synchronous_machines_cim16(self) -> pd.DataFrame:
        ...
        eq_generating_units = pd.concat(
            [eq_generating_units, self.cimConverter.cim["eq"]["WindGeneratingUnit"]],
            sort=False,
        )
        eq_generating_units["type"] = eq_generating_units["type"].fillna("WP")
        eq_generating_units = pd.concat(
            [eq_generating_units, self.cimConverter.cim["eq"]["HydroGeneratingUnit"]],
            sort=False,
        )
        eq_generating_units["type"] = eq_generating_units["type"].fillna("Hydro")
        eq_generating_units = pd.concat(
            [eq_generating_units, self.cimConverter.cim["eq"]["SolarGeneratingUnit"]],
            sort=False,
        )
        eq_generating_units["type"] = eq_generating_units["type"].fillna("PV")
        eq_generating_units = pd.concat(
            [eq_generating_units, self.cimConverter.cim["eq"]["ThermalGeneratingUnit"]],
            sort=False,
        )
        eq_generating_units["type"] = eq_generating_units["type"].fillna("Thermal")
        eq_generating_units = pd.concat(
            [eq_generating_units, self.cimConverter.cim["eq"]["NuclearGeneratingUnit"]],
            sort=False,
        )
        eq_generating_units["type"] = eq_generating_units["type"].fillna("Nuclear")
        eq_generating_units = eq_generating_units.rename(
            columns={"rdfId": "GeneratingUnit"}
        )
        ...
        if "sc" in self.cimConverter.cim:
            ...
            synchronous_machines = self.cimConverter.merge_eq_ssh_profile(
        ...
        if "EquipmentContainer" in synchronous_machines.columns:
            synchronous_machines = synchronous_machines.drop(
                columns=["EquipmentContainer"]
            )
        synchronous_machines = pd.merge(
            synchronous_machines, eq_generating_units, how="left", on="GeneratingUnit"
        )
        ...
        eqtp_terminals = pd.merge(
            self.cimConverter.cim["eq"]["Terminal"],
            self.cimConverter.cim["tp"]["Terminal"],
            how="left",
            on="rdfId",
        )
        eqtp_terminals.ConnectivityNode = eqtp_terminals.ConnectivityNode.fillna(
            eqtp_terminals.TopologicalNode
        )
        eqtp_terminals = eqtp_terminals[["rdfId", "ConnectivityNode"]].rename(
            columns={"rdfId": "Terminal", "ConnectivityNode": "reg_control_cnode"}
        )
        eqssh_reg_control = pd.merge(
            eqssh_reg_control, eqtp_terminals, how="left", on="Terminal"
        )
        eqssh_reg_control = eqssh_reg_control.drop(columns=["Terminal"])
        # add the voltage from the bus
        eqssh_reg_control = pd.merge(
            eqssh_reg_control,
            self.cimConverter.net.bus[["vn_kv", sc["o_id"]]].rename(
                columns={sc["o_id"]: "reg_control_cnode"}
            ),
            how="left",
            on="reg_control_cnode",
        )
        # merge with RegulatingControl to check if it is a voltage controlled generator
        eqssh_reg_control = eqssh_reg_control.loc[
            eqssh_reg_control["mode"] == "voltage"
        ]
        synchronous_machines = pd.merge(
            synchronous_machines,
            eqssh_reg_control.rename(columns={"rdfId": "RegulatingControl"}),
            how="left",
            on="RegulatingControl",
        )
        synchronous_machines["controllable"] = (
            synchronous_machines["controlEnabled"] & synchronous_machines["enabled"]
        )
        synchronous_machines = pd.merge(
            synchronous_machines, self.cimConverter.bus_merge, how="left", on="rdfId"
        )
        synchronous_machines = synchronous_machines.drop_duplicates(
            ["rdfId"], keep="first"
        )
        synchronous_machines["vm_pu"] = (
            synchronous_machines.targetValue / synchronous_machines.vn_kv
        )
        synchronous_machines["vm_pu"] = synchronous_machines["vm_pu"].fillna(1.0)
        synchronous_machines = synchronous_machines.rename(
            columns={"vn_kv": "bus_voltage"}
        )
        synchronous_machines["slack"] = False
        synchronous_machines["controllable"] = synchronous_machines[
            "controllable"
        ].fillna(False)
        ...
        regulation_controllers = self.cimConverter.merge_eq_ssh_profile(
            "RegulatingControl"
        )
        regulation_controllers = regulation_controllers.loc[
            regulation_controllers["mode"] == "voltage"
        ]
        regulation_controllers = regulation_controllers[
            ["rdfId", "targetValue", "enabled"]
        ]
        regulation_controllers = regulation_controllers.rename(
            columns={"rdfId": "RegulatingControl"}
        )
        enis = pd.merge(
            enis, regulation_controllers, how="left", on="RegulatingControl"
        )

        eni_ref_prio_min = enis.loc[
            (enis["referencePriority"] > 0) & (enis["enabled"]), "referencePriority"
        ].min()
        ...
        synchronous_machines.loc[
            synchronous_machines["referencePriority"] == ref_prio_min, "slack"
        ] = True
        ...
        synchronous_machines["sn_mva"] = synchronous_machines["ratedS"].fillna(
            synchronous_machines["nominalP"]
        )
        # Convert governorSCD unit from percent to MW/Hz
        synchronous_machines["governorSCD"] = (
            synchronous_machines["governorSCD"]
            * synchronous_machines["nominalP"]
            / 50
            / 100
        ...
        synchronous_machines["rdss_ohm"] = synchronous_machines["r2"] * (
            synchronous_machines["ratedU"] ** 2 / synchronous_machines["ratedS"]
        )
        synchronous_machines["xdss_pu"] = synchronous_machines["x2"][:]
        synchronous_machines["voltageRegulationRange"] = synchronous_machines[
            "voltageRegulationRange"
        ].fillna(0.0)
        synchronous_machines["pg_percent"] = synchronous_machines[
            "voltageRegulationRange"
        ]
        synchronous_machines["k"] = (
            synchronous_machines["ratedS"] * 1e3 / synchronous_machines["ratedU"]
        ) / (
            synchronous_machines["ratedU"]
            / (
                3**0.5
                * (synchronous_machines["r2"] ** 2 + synchronous_machines["x2"] ** 2)
            )
        )
        synchronous_machines["rx"] = (
            synchronous_machines["r2"] / synchronous_machines["x2"]
        )
        synchronous_machines["scaling"] = 1.0
        synchronous_machines["generator_type"] = "current_source"
        synchronous_machines.loc[
            synchronous_machines["referencePriority"] == 0, "referencePriority"
        ] = float("NaN")
        synchronous_machines["referencePriority"] = synchronous_machines[
            "referencePriority"
        ].astype(float)
        if "inService" in synchronous_machines.columns:
            synchronous_machines["connected"] = (
                synchronous_machines["connected"] & synchronous_machines["inService"]
            )
        synchronous_machines = synchronous_machines.rename(
            columns={
                "rdfId_Terminal": sc["t"],
                ...
                "index_bus": "bus",
                "minOperatingP": "min_p_mw",
                "maxOperatingP": "max_p_mw",
                "minQ": "min_q_mvar",
                "maxQ": "max_q_mvar",
    ...
    def _create_gen_characteristics_table(self, syn_gen_df_origin) -> pd.DataFrame:
        ...
        if not eq["ReactiveCapabilityCurve"].empty and not eq["CurveData"].empty:
            ...
            # get the curve data
            curve_data = self.cimConverter.cim["eq"]["ReactiveCapabilityCurve"][
                ["rdfId", "curveStyle"]
            ].rename(columns={"rdfId": "Curve", "curveStyle": "curve_style"})
            curve_points = pd.merge(
                curve_data,
                self.cimConverter.cim["eq"]["CurveData"][
                    ["rdfId", "Curve", "xvalue", "y1value", "y2value"]
                ],
                how="left",
                on="Curve",
            )

            curve_points["id_q_capability_characteristic"] = pd.factorize(
                curve_points["Curve"]
            )[0]
            curve_points = curve_points.drop(columns=["rdfId"]).rename(
                columns={
                    "Curve": "rdfId",
                    "xvalue": "p_mw",
                    "y1value": "q_min_mvar",
            ...
            curve_points = curve_points[
                ...
                + [
                    col
                    for col in curve_points.columns
                    if col != "id_q_capability_characteristic"
                ]
            ]
            self.cimConverter.net["q_capability_curve_table"] = curve_points
            self.cimConverter.net["q_capability_curve_table"] = (
                self.cimConverter.net["q_capability_curve_table"]
                .drop(columns=["rdfId", "curve_style"])
                .rename(
            ...
            curve_points = (
                curve_points.drop(columns=["p_mw", "q_min_mvar", "q_max_mvar"])
                .drop_duplicates(subset=["rdfId"])
                .rename(columns={"rdfId": "InitialReactiveCapabilityCurve"})
            )

            syn_gen_df_origin = syn_gen_df_origin.drop(
                columns=["id_q_capability_characteristic"]
            )
            syn_gen_df_origin = pd.merge(
                syn_gen_df_origin,
                curve_points,
                how="left",
