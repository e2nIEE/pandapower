...
class PowerTransformersCim16:
    ...
    def convert_power_transformers_cim16(self):
        ...
        self.logger.info(
            "Created %s 2w trafos and %s 3w trafos in %ss."
            % (
                power_trafo2w.index.size,
                power_trafo3w.index.size,
                time.time() - time_start,
            )
        )
        self.cimConverter.report_container.add_log(
            Report(
                level=LogLevel.INFO,
                code=ReportCode.INFO_CONVERTING,
    ...
    def _create_trafo_characteristic_table(self, trafo_type, trafo_df_origin):
        ...
        # get the TablePoints
        ptct = self.cimConverter.cim["eq"]["PhaseTapChangerTabular"][
            ["TransformerEnd", "PhaseTapChangerTable"]
        ]
        ptct = pd.merge(
            ptct,
            self.cimConverter.cim["eq"]["PhaseTapChangerTablePoint"][
                ["PhaseTapChangerTable", "step", "r", "x", "ratio", "angle"]
            ],
            how="left",
            on="PhaseTapChangerTable",
        )
        # append the ratio tap changers
        ptct_ratio = self.cimConverter.cim["eq"]["RatioTapChanger"][
            ["TransformerEnd", "RatioTapChangerTable"]
        ]
        ptct_ratio = pd.merge(
            ptct_ratio,
            self.cimConverter.cim["eq"]["RatioTapChangerTablePoint"][
                ["RatioTapChangerTable", "step", "r", "x", "ratio"]
            ],
            how="left",
            on="RatioTapChangerTable",
        )
        ...
        ptct = ptct.rename(
            columns={
                ...
                "r": "r_dev",
                "x": "x_dev",
                "TransformerEnd": sc["pte_id"],
                "ratio": "ratio_dev",
                "angle": "angle_dev",
            }
        )
        if trafo_type == "trafo":
            trafo_df = trafo_df_origin.sort_values(
                ["PowerTransformer", "endNumber"]
            ).reset_index()
            ...
            for one_item in copy_list:
                ...
                trafo_df[one_item + "_lv"] = (
                    trafo_df[one_item + "_lv"].iloc[1:].reset_index()[one_item + "_lv"]
                )
            del copy_list, one_item
            fillna_list = ["neutralStep", "lowStep", "highStep", "step"]
            for one_item in fillna_list:
                trafo_df[one_item] = trafo_df[one_item].fillna(
                    trafo_df[one_item + "_lv"]
                )
            del fillna_list, one_item
            # just keep one transformer
            trafo_df = trafo_df.drop_duplicates(
                subset=["PowerTransformer"], keep="first"
            )
            ...
            trafo_df = pd.merge(
                ...
                ptct.rename(
                    columns={
                        "tabular_step": "tabular_step_lv",
                        "r_dev": "r_dev_lv",
                        "x_dev": "x_dev_lv",
                        "ratio_dev": "ratio_dev_lv",
            ...
            trafo_df["ratio_dev"] = trafo_df["ratio_dev"].fillna(1.0)
            trafo_df["angle_dev"] = trafo_df["angle_dev"].fillna(0)
            trafo_df = trafo_df.rename(
                columns={"ratio_dev": "voltage_ratio", "angle_dev": "angle_deg"}
            )
            ...
            trafo_df["vk_percent"] = (
                abs(trafo_df.r) ** 2 + abs(trafo_df.x) ** 2
            ) ** 0.5 * (trafo_df.ratedS * 1e3) / (10.0 * trafo_df.ratedU**2) + (
                abs(trafo_df.r_lv) ** 2 + abs(trafo_df.x_lv) ** 2
            ) ** 0.5 * (trafo_df.ratedS * 1e3) / (10.0 * trafo_df.ratedU_lv**2)
            ...
            for one_item in copy_list:
                ...
                trafo_df[one_item + "_mv"] = (
                    trafo_df[one_item + "_mv"].iloc[1:].reset_index()[one_item + "_mv"]
                )
                trafo_df[one_item + "_lv"] = (
                    trafo_df[one_item + "_lv"].iloc[2:].reset_index()[one_item + "_lv"]
                )
            del copy_list, one_item
            fillna_list = ["neutralStep", "lowStep", "highStep", "step"]
            for one_item in fillna_list:
            ...
            trafo_df = pd.concat(
                [
                    pd.merge(trafo_df, ptct, how="left", on=sc["pte_id"]),
                    pd.merge(
                        trafo_df,
                        ptct.rename(
                            columns={
                                "tabular_step": "tabular_step_mv",
                                "r_dev": "r_dev_mv",
                                "x_dev": "x_dev_mv",
                                "ratio_dev": "ratio_dev_mv",
            ...
            trafo_df = trafo_df.loc[
                (
                    ~trafo_df.duplicated(
                        subset=["PowerTransformer", "tabular_step"], keep=False
                    )
                )
                | (~trafo_df.RatioTapChangerTable.isna())
                | (~trafo_df.PhaseTapChangerTable.isna())
            ]
            fillna_list = ["tabular_step"]
            for one_item in fillna_list:
                trafo_df[one_item] = trafo_df[one_item].fillna(
            ...
            del fillna_list, one_item
            trafo_df = trafo_df.dropna(
                subset=["r_dev", "r_dev_mv", "r_dev_lv"], how="all"
            )
            fillna_list = [
                "r_dev",
                "r_dev_mv",
            ...
            trafo_df["vkr_hv_percent"] = (
                (
                    trafo_df.r
                    + trafo_df.r_mv * (trafo_df.ratedU / trafo_df.ratedU_mv) ** 2
                )
                * trafo_df.min_s_hvmv
                * 100
                / trafo_df.ratedU**2
            ...
            trafo_df["vkr_mv_percent"] = (
                (
                    trafo_df.r_mv
                    + trafo_df.r_lv * (trafo_df.ratedU_mv / trafo_df.ratedU_lv) ** 2
                )
                * trafo_df.min_s_mvlv
                * 100
                / trafo_df.ratedU_mv**2
            )
            trafo_df["vkr_lv_percent"] = (
                (
                    trafo_df.r_lv
                    + trafo_df.r * (trafo_df.ratedU_lv / trafo_df.ratedU) ** 2
                )
                * trafo_df.min_s_lvhv
                * 100
                / trafo_df.ratedU_lv**2
            )
            trafo_df["vk_hv_percent"] = (
                (
                    ...
                    ** 2
                    + (
                        trafo_df.x
                        + trafo_df.x_mv * (trafo_df.ratedU / trafo_df.ratedU_mv) ** 2
                    )
                    ** 2
                )
                ** 0.5
            ...
            trafo_df["vk_mv_percent"] = (
                (
                    ...
                    ** 2
                    + (
                        trafo_df.x_mv
                        + trafo_df.x_lv * (trafo_df.ratedU_mv / trafo_df.ratedU_lv) ** 2
                    )
                    ** 2
                )
                ** 0.5
            ...
            trafo_df["vk_lv_percent"] = (
                (
                    ...
                    ** 2
                    + (
                        trafo_df.x_lv
                        + trafo_df.x * (trafo_df.ratedU_lv / trafo_df.ratedU) ** 2
                    )
                    ** 2
                )
                ** 0.5
        ...
        for one_id, one_df in trafo_df.groupby(sc["pte_id"]):
            ...
            trafo_df_origin.loc[
                trafo_df_origin["PowerTransformer"]
                == trafo_df_origin.loc[
                    trafo_df_origin[sc["pte_id"]] == one_id, "PowerTransformer"
                ].values[0],
        ...
        trafo_df_origin.loc[
            trafo_df_origin["id_characteristic_table"].notna(), "tap_dependency_table"
        ] = True

        self.cimConverter.net["trafo_characteristic_table"] = pd.concat(
            [
                self.cimConverter.net["trafo_characteristic_table"],
                pd.DataFrame(append_dict),
            ],
            ignore_index=True,
            sort=False,
        )
    ...
    def _prepare_power_transformers_cim16(self) -> pd.DataFrame:
        if "sc" in self.cimConverter.cim:
            power_transformers = self.cimConverter.merge_eq_sc_profile(
                "PowerTransformer"
            )
        else:
        ...
        if "sc" in self.cimConverter.cim:
            power_transformer_ends = self.cimConverter.merge_eq_sc_profile(
                "PowerTransformerEnd"
            )
        else:
        ...
        eqssh_tap_changers = pd.merge(
            self.cimConverter.cim["eq"]["RatioTapChanger"][
                [
                    "rdfId",
                    "TransformerEnd",
                    "neutralStep",
            ...
            self.cimConverter.cim["ssh"]["RatioTapChanger"][["rdfId", "step"]],
            how="left",
            on="rdfId",
        )
        eqssh_tap_changers[sc["tc"]] = "RatioTapChanger"
        eqssh_tap_changers["tap_changer_type"] = (
            "Ratio"  # Ratio/Asymmetrical phase shifter
        )
        ...
        eqssh_tap_changers_linear = pd.merge(
            self.cimConverter.cim["eq"]["PhaseTapChangerLinear"],
            self.cimConverter.cim["ssh"]["PhaseTapChangerLinear"],
            how="left",
            on="rdfId",
        )
        eqssh_tap_changers_linear["stepVoltageIncrement"] = 0.001
        ...
        eqssh_tap_changers_linear[sc["tc_id"]] = eqssh_tap_changers_linear[
            "rdfId"
        ].copy()
        eqssh_tap_changers = pd.concat(
            [eqssh_tap_changers, eqssh_tap_changers_linear],
            ignore_index=True,
            sort=False,
        )
        # todo: check correct implementation for PhaseTapChangerAsymmetrical tap changers
        eqssh_tap_changers_async = pd.merge(
            self.cimConverter.cim["eq"]["PhaseTapChangerAsymmetrical"],
            self.cimConverter.cim["ssh"]["PhaseTapChangerAsymmetrical"],
            how="left",
            on="rdfId",
        )
        eqssh_tap_changers_async["stepVoltageIncrement"] = eqssh_tap_changers_async[
            "voltageStepIncrement"
        ][:]
        eqssh_tap_changers_async["stepPhaseShiftIncrement"] = eqssh_tap_changers_async[
            "windingConnectionAngle"
        ][:]
        eqssh_tap_changers_async = eqssh_tap_changers_async.drop(
            columns=["voltageStepIncrement", "windingConnectionAngle"]
        )
        eqssh_tap_changers_async[sc["tc"]] = "PhaseTapChangerAsymmetrical"
        eqssh_tap_changers_async["tap_changer_type"] = (
            "Ratio"  # Ratio/Asymmetrical phase shifter
        )
        eqssh_tap_changers_async[sc["tc_id"]] = eqssh_tap_changers_async["rdfId"].copy()
        eqssh_tap_changers = pd.concat(
            [eqssh_tap_changers, eqssh_tap_changers_async],
            ignore_index=True,
            sort=False,
        )
        # todo: check correct implementation for PhaseTapChangerSymmetrical tap changers
        eqssh_ratio_tap_changers_sync = pd.merge(
            self.cimConverter.cim["eq"]["PhaseTapChangerSymmetrical"],
            self.cimConverter.cim["ssh"]["PhaseTapChangerSymmetrical"],
            how="left",
            on="rdfId",
        )
        eqssh_ratio_tap_changers_sync["stepVoltageIncrement"] = (
            eqssh_ratio_tap_changers_sync["voltageStepIncrement"]
        )
        eqssh_ratio_tap_changers_sync = eqssh_ratio_tap_changers_sync.drop(
            columns=["voltageStepIncrement"]
        )
        eqssh_ratio_tap_changers_sync[sc["tc"]] = "PhaseTapChangerSymmetrical"
        eqssh_ratio_tap_changers_sync["tap_changer_type"] = (
            "Symmetrical"  # Symmetrical phase shifter
        )
        eqssh_ratio_tap_changers_sync[sc["tc_id"]] = eqssh_ratio_tap_changers_sync[
            "rdfId"
        ].copy()
        eqssh_tap_changers = pd.concat(
            [eqssh_tap_changers, eqssh_ratio_tap_changers_sync],
            ignore_index=True,
            sort=False,
        )
        # convert the PhaseTapChangerTabular to one tap changer
        ptct = pd.merge(
            self.cimConverter.cim["eq"]["PhaseTapChangerTabular"][
                [
                    "rdfId",
                    "TransformerEnd",
            ...
            self.cimConverter.cim["ssh"]["PhaseTapChangerTabular"][["rdfId", "step"]],
            how="left",
            on="rdfId",
        )
        ...
        ptct = pd.merge(
            ptct,
            self.cimConverter.cim["eq"]["PhaseTapChangerTablePoint"][
                ["PhaseTapChangerTable", "step", "angle", "ratio"]
            ],
            how="left",
            on="PhaseTapChangerTable",
        )
        for one_id, one_df in ptct.groupby("TransformerEnd"):
            ...
            if keep_index.size > 0:
                ...
                self.logger.warning(
                    "Ignoring PhaseTapChangerTabular with ID: %s. The current tap position is missing "
            ...
            ptct.loc[keep_index, "ratio"] = (
                one_df.loc[neutral_step, "ratio"] - 1
            ) * 100
        ...
        eqssh_tap_changers = pd.concat(
            [eqssh_tap_changers, ptct], ignore_index=True, sort=False
        )
        del (
            eqssh_tap_changers_linear,
            eqssh_tap_changers_async,
        ...
        eqssh_tap_changers = eqssh_tap_changers.drop_duplicates(
            subset=["TransformerEnd"]
        )
        # prepare the controllers
        eq_ssh_tap_controllers = self.cimConverter.merge_eq_ssh_profile(
            "TapChangerControl"
        )
        eq_ssh_tap_controllers = eq_ssh_tap_controllers[
            [
                "rdfId",
                "Terminal",
                "discrete",
        ...
        eq_ssh_tap_controllers = eq_ssh_tap_controllers.rename(
            columns={"rdfId": "TapChangerControl"}
        )
        # first merge with the VoltageLimits
        if "VoltageLimit" in self.cimConverter.cim["ssh"]:
            vl = self.cimConverter.merge_eq_ssh_profile("VoltageLimit")[
                ["OperationalLimitSet", "OperationalLimitType", "value"]
            ]
        else:
            vl = self.cimConverter.cim["eq"]["VoltageLimit"][
                ["OperationalLimitSet", "OperationalLimitType", "value"]
            ]
        opl_kind = (
            "kind"
            if "kind" in self.cimConverter.cim["eq"]["OperationalLimitType"].columns
            else "limitType"
        )
        vl = pd.merge(
            vl,
            self.cimConverter.cim["eq"]["OperationalLimitType"][
                ["rdfId", opl_kind]
            ].rename(columns={"rdfId": "OperationalLimitType"}),
            how="left",
            on="OperationalLimitType",
        )
        vl = pd.merge(
            vl,
            self.cimConverter.cim["eq"]["OperationalLimitSet"][
                ["rdfId", "Terminal"]
            ].rename(columns={"rdfId": "OperationalLimitSet"}),
            how="left",
            on="OperationalLimitSet",
        )
        ...
        eq_ssh_tap_controllers = pd.merge(
            eq_ssh_tap_controllers, vl, how="left", on="Terminal"
        )
        eq_ssh_tap_controllers["c_Terminal"] = eq_ssh_tap_controllers["Terminal"][:]
        eq_ssh_tap_controllers = eq_ssh_tap_controllers.rename(
            columns={
                "Terminal": "rdfId",
                "enabled": "c_in_service",
                "targetValue": "c_vm_set_pu",
                "targetDeadband": "c_tol",
            }
        )
        ...
        eq_ssh_tap_controllers = pd.merge(
            ...
            pd.concat(
                [
                    ...
                    self.cimConverter.cim["eq_bd"]["Terminal"],
                ],
                ignore_index=True,
                sort=False,
            )[["rdfId", "ConnectivityNode"]],
            how="left",
            on="rdfId",
        )
        eq_ssh_tap_controllers = eq_ssh_tap_controllers.drop(columns=["rdfId"])
        eq_ssh_tap_controllers = eq_ssh_tap_controllers.rename(
            columns={"ConnectivityNode": sc["o_id"]}
        )
        eq_ssh_tap_controllers = pd.merge(
            eq_ssh_tap_controllers,
        ...
        eq_ssh_tap_controllers = eq_ssh_tap_controllers.rename(
            columns={"index": "c_bus_id"}
        )
        eq_ssh_tap_controllers["c_vm_set_pu"] = (
            eq_ssh_tap_controllers["c_vm_set_pu"] / eq_ssh_tap_controllers["vn_kv"]
        )
        eq_ssh_tap_controllers["c_tol"] = (
            eq_ssh_tap_controllers["c_tol"] / eq_ssh_tap_controllers["vn_kv"]
        )
        ...
        eqssh_tap_changers = pd.merge(
            ...
            eq_ssh_tap_controllers,
            how="left",
            on="TapChangerControl",
        )
        eqssh_tap_changers = eqssh_tap_changers.rename(
            columns={"TransformerEnd": sc["pte_id"]}
        )

        power_transformers = power_transformers.rename(
            columns={"rdfId": "PowerTransformer"}
        )
        power_transformer_ends = power_transformer_ends.rename(
            columns={"rdfId": sc["pte_id"]}
        )
        # add the PowerTransformerEnds
        power_transformers = pd.merge(
            power_transformers,
        ...
        power_transformers = pd.merge(
            power_transformers,
            self.cimConverter.bus_merge.drop("rdfId", axis=1),
            how="left",
            left_on="Terminal",
            right_on="rdfId_Terminal",
        )
        # add the TapChangers
        power_transformers = pd.merge(
            power_transformers, eqssh_tap_changers, how="left", on=sc["pte_id"]
        )
        return power_transformers

    def _prepare_trafos_cim16(self, power_trafo2w: pd.DataFrame) -> pd.DataFrame:
        power_trafo2w = power_trafo2w.sort_values(
            ["PowerTransformer", "endNumber"]
        ).reset_index()
        ...
        for one_item in copy_list:
            ...
            # cut the first element from the copied columns
            power_trafo2w[one_item + "_lv"] = (
                power_trafo2w[one_item + "_lv"].iloc[1:].reset_index()[one_item + "_lv"]
            )
        del copy_list, one_item
        ...
        # just keep one transformer
        power_trafo2w = power_trafo2w.drop_duplicates(
            subset=["PowerTransformer"], keep="first"
        )

        power_trafo2w["pfe_kw"] = (
            power_trafo2w.g * power_trafo2w.ratedU**2
            + power_trafo2w.g_lv * power_trafo2w.ratedU_lv**2
        ) * 1000
        power_trafo2w["vkr_percent"] = (
            abs(power_trafo2w.r) * power_trafo2w.ratedS * 100 / power_trafo2w.ratedU**2
            + abs(power_trafo2w.r_lv)
            * power_trafo2w.ratedS
            * 100
            / power_trafo2w.ratedU_lv**2
        )
        ...
        power_trafo2w["vk_percent"] = (
            power_trafo2w.r**2 + power_trafo2w.x**2
        ) ** 0.5 * (power_trafo2w.ratedS * 1e3) / (10.0 * power_trafo2w.ratedU**2) + (
            power_trafo2w.r_lv**2 + power_trafo2w.x_lv**2
        ) ** 0.5 * (power_trafo2w.ratedS * 1e3) / (10.0 * power_trafo2w.ratedU_lv**2)
        power_trafo2w["vk_percent"] = (
            power_trafo2w["x_lv_sign"] * power_trafo2w["vk_percent"]
        )
        power_trafo2w["vk_percent"] = (
            power_trafo2w["x_sign"] * power_trafo2w["vk_percent"]
        )
        power_trafo2w["i0_percent"] = (
            (
                (
                    (power_trafo2w.b * power_trafo2w.ratedU**2) ** 2
                    + (power_trafo2w.g * power_trafo2w.ratedU**2) ** 2
                )
                ** 0.5
                + (
                    (power_trafo2w.b_lv * power_trafo2w.ratedU_lv**2) ** 2
                    + (power_trafo2w.g_lv * power_trafo2w.ratedU_lv**2) ** 2
                )
                ** 0.5
            )
            * 100
        ...
        power_trafo2w["vk0_percent"] = (
            power_trafo2w.r0**2 + power_trafo2w.x0**2
        ) ** 0.5 * (power_trafo2w.ratedS * 1e3) / (10.0 * power_trafo2w.ratedU**2) + (
            power_trafo2w.r0_lv**2 + power_trafo2w.x0_lv**2
        ) ** 0.5 * (power_trafo2w.ratedS * 1e3) / (10.0 * power_trafo2w.ratedU_lv**2)
        power_trafo2w["vk0_percent"] = (
            power_trafo2w["x0_lv_sign"] * power_trafo2w["vk0_percent"]
        )
        power_trafo2w["vk0_percent"] = (
            power_trafo2w["x0_sign"] * power_trafo2w["vk0_percent"]
        )
        power_trafo2w["std_type"] = None
        power_trafo2w["df"] = 1.0
        power_trafo2w.loc[power_trafo2w["phaseAngleClock"] == 0, "phaseAngleClock"] = (
            np.nan
        )
        power_trafo2w["phaseAngleClock_lv"] = power_trafo2w[
            "phaseAngleClock_lv"
        ].fillna(0)
        power_trafo2w["shift_degree"] = (
            power_trafo2w["phaseAngleClock"]
            .astype(float)
            .fillna(power_trafo2w["phaseAngleClock_lv"].astype(float))
            * 30
        )
        power_trafo2w["parallel"] = 1
        power_trafo2w["in_service"] = (
            power_trafo2w.connected & power_trafo2w.connected_lv
        )
        ...
        power_trafo2w["vector_group"] = (
            power_trafo2w.connectionKind.str.upper()
            + power_trafo2w.connectionKind_lv.str.lower()
        )
        power_trafo2w.loc[power_trafo2w["vector_group"] == "", "vector_group"] = None
        power_trafo2w = power_trafo2w.rename(
            columns={
                "PowerTransformer": sc["o_id"],
                "Terminal": sc["t_hv"],
                "Terminal_lv": sc["t_lv"],
                sc["pte_id"]: sc["pte_id_hv"],
                sc["pte_id"] + "_lv": sc["pte_id_lv"],
                "index_bus": "hv_bus",
                "index_bus_lv": "lv_bus",
                "neutralStep": "tap_neutral",
                "lowStep": "tap_min",
                "highStep": "tap_max",
                "step": "tap_pos",
                "stepVoltageIncrement": "tap_step_percent",
                "stepPhaseShiftIncrement": "tap_step_degree",
                "isPartOfGeneratorUnit": "power_station_unit",
                "ratedU": "vn_hv_kv",
                "ratedU_lv": "vn_lv_kv",
                "ratedS": "sn_mva",
                "xground": "xn_ohm",
                "grounded": "oltc",
                "neutralStep_lv": "tap2_neutral",
                "lowStep_lv": "tap2_min",
                "highStep_lv": "tap2_max",
                "step_lv": "tap2_pos",
                "stepVoltageIncrement_lv": "tap2_step_percent",
                "stepPhaseShiftIncrement_lv": "tap2_step_degree",
                "tap_changer_type_lv": "tap2_changer_type",
                "tapchanger_class_lv": sc["tc2"],
                "tapchanger_id_lv": sc["tc2_id"],
            }
        )
        return power_trafo2w

    def _prepare_trafo3w_cim16(self, power_trafo3w: pd.DataFrame) -> pd.DataFrame:
        power_trafo3w = power_trafo3w.sort_values(
            ["PowerTransformer", "endNumber"]
        ).reset_index()
        # precessing the transformer data
        # a list of transformer parameters which are used for each transformer winding
        ...
        for one_item in copy_list:
            ...
            # cut the first (or first two) element(s) from the copied columns
            power_trafo3w[one_item + "_mv"] = (
                power_trafo3w[one_item + "_mv"].iloc[1:].reset_index()[one_item + "_mv"]
            )
            power_trafo3w[one_item + "_lv"] = (
                power_trafo3w[one_item + "_lv"].iloc[2:].reset_index()[one_item + "_lv"]
            )
        del copy_list, one_item
        ...
        for one_item in fillna_list:
            power_trafo3w[one_item] = power_trafo3w[one_item].fillna(
                power_trafo3w[one_item + "_mv"]
            )
            power_trafo3w[one_item] = power_trafo3w[one_item].fillna(
                power_trafo3w[one_item + "_lv"]
        ...
        del fillna_list, one_item
        # just keep one transformer
        power_trafo3w = power_trafo3w.drop_duplicates(
            subset=["PowerTransformer"], keep="first"
        )

        power_trafo3w["min_s_hvmv"] = power_trafo3w[["ratedS", "ratedS_mv"]].min(axis=1)
        power_trafo3w["min_s_mvlv"] = power_trafo3w[["ratedS_mv", "ratedS_lv"]].min(
            axis=1
        )
        power_trafo3w["min_s_lvhv"] = power_trafo3w[["ratedS_lv", "ratedS"]].min(axis=1)
        power_trafo3w["pfe_kw"] = (
            power_trafo3w.g * power_trafo3w.ratedU**2
            + power_trafo3w.g_mv * power_trafo3w.ratedU_mv**2
            + power_trafo3w.g_lv * power_trafo3w.ratedU_lv**2
        ) * 1000
        power_trafo3w["vkr_hv_percent"] = (
            (
                power_trafo3w.r
                + power_trafo3w.r_mv
                * (power_trafo3w.ratedU / power_trafo3w.ratedU_mv) ** 2
            )
            * power_trafo3w.min_s_hvmv
            * 100
            / power_trafo3w.ratedU**2
        )
        power_trafo3w["vkr_mv_percent"] = (
            (
                power_trafo3w.r_mv
                + power_trafo3w.r_lv
                * (power_trafo3w.ratedU_mv / power_trafo3w.ratedU_lv) ** 2
            )
            * power_trafo3w.min_s_mvlv
            * 100
            / power_trafo3w.ratedU_mv**2
        )
        power_trafo3w["vkr_lv_percent"] = (
            (
                power_trafo3w.r_lv
                + power_trafo3w.r
                * (power_trafo3w.ratedU_lv / power_trafo3w.ratedU) ** 2
            )
            * power_trafo3w.min_s_lvhv
            * 100
            / power_trafo3w.ratedU_lv**2
        )
        ...
        power_trafo3w["i0_percent"] = (
            (
                (
                    ...
                    + (power_trafo3w.g * power_trafo3w.ratedU**2) ** 2
                )
                ** 0.5
                + (
                    (power_trafo3w.b_mv * power_trafo3w.ratedU_mv**2) ** 2
                    + (power_trafo3w.g_mv * power_trafo3w.ratedU_mv**2) ** 2
                )
                ** 0.5
                + (
                    (power_trafo3w.b_lv * power_trafo3w.ratedU_lv**2) ** 2
                    + (power_trafo3w.g_lv * power_trafo3w.ratedU_lv**2) ** 2
                )
                ** 0.5
            )
            * 100
            / power_trafo3w.ratedS
        )
        ...
        power_trafo3w["phaseAngleClock_mv"] = power_trafo3w[
            "phaseAngleClock_mv"
        ].fillna(0)
        power_trafo3w["phaseAngleClock_lv"] = power_trafo3w[
            "phaseAngleClock_lv"
        ].fillna(0)
        power_trafo3w["shift_mv_degree"] = (
            power_trafo3w["phaseAngleClock_mv"].astype(float) * 30
        )
        power_trafo3w["shift_lv_degree"] = (
            power_trafo3w["phaseAngleClock_mv"].astype(float) * 30
        )
        power_trafo3w["tap_at_star_point"] = False
        power_trafo3w["in_service"] = (
            power_trafo3w.connected
            & power_trafo3w.connected_mv
        ...
        power_trafo3w["vector_group"] = (
            power_trafo3w.connectionKind.str.upper()
            + power_trafo3w.connectionKind_mv.str.lower()
            + power_trafo3w.connectionKind_lv.str.lower()
        )
        power_trafo3w.loc[power_trafo3w["vector_group"] == "", "vector_group"] = None
        power_trafo3w = power_trafo3w.rename(
            columns={
                "PowerTransformer": sc["o_id"],
                "Terminal": sc["t_hv"],
                "Terminal_mv": sc["t_mv"],
