...
class TapController:
    ...
    def _create_tap_controller(self, input_df: pd.DataFrame, trafo_type: str):
        if not self.cimConverter.kwargs.get("create_tap_controller", True):
            self.logger.info(
                "Skip creating transformer tap changer controller for transformer type %s."
                % trafo_type
            )
            return
        for _, row in input_df.loc[input_df["TapChangerControl"].notna()].iterrows():
            trafo_id = (
                self.cimConverter.net[trafo_type]
                .loc[self.cimConverter.net[trafo_type][sc["o_id"]] == row[sc["o_id"]]]
                .index.values[0]
            )
            # get the controlled bus (side), assume "lv" as default
            side = "lv"
            if (
                sc["t_hv"] in self.cimConverter.net[trafo_type].columns
                and row["c_Terminal"]
                in self.cimConverter.net[trafo_type][sc["t_hv"]].values
            ):
                side = "hv"
            if (
                sc["t_mv"] in self.cimConverter.net[trafo_type].columns
                and row["c_Terminal"]
                in self.cimConverter.net[trafo_type][sc["t_mv"]].values
            ):
                side = "mv"
            if row["discrete"]:
                self.logger.info(
                    "Creating DiscreteTapControl for transformer %s." % row[sc["o_id"]]
                )
                DiscreteTapControl(
                    self.cimConverter.net,
                    element=trafo_type,
                    element_index=trafo_id,
                    side=side,
                    tol=row["c_tol"],
                    in_service=row["c_in_service"],
                    vm_lower_pu=row["c_vm_lower_pu"],
                    vm_upper_pu=row["c_vm_upper_pu"],
                )
            else:
                self.logger.info(
                    "Creating ContinuousTapControl for transformer %s."
                    % row[sc["o_id"]]
                )
                ContinuousTapControl(
                    self.cimConverter.net,
