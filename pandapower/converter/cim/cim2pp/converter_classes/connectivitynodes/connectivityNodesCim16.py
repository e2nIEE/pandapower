...
class ConnectivityNodesCim16:
    ...
    def convert_connectivity_nodes_cim16(self):
        ...
        eqssh_terminals = eqssh_terminals.rename(
            columns={"ConductingEquipment": "rdfId"}
        )
        # buses for merging with assets:
        bus_merge = pd.DataFrame(
            data=self.cimConverter.net["bus"].loc[:, [sc["o_id"], "vn_kv"]]
        )
        bus_merge = bus_merge.rename(columns={"vn_kv": "base_voltage_bus"})
        bus_merge = bus_merge.reset_index(level=0)
        bus_merge = bus_merge.rename(
            columns={"index": "index_bus", sc["o_id"]: "ConnectivityNode"}
        )
        bus_merge = pd.merge(
            eqssh_terminals, bus_merge, how="left", on="ConnectivityNode"
        )
        self.cimConverter.bus_merge = bus_merge

        self.logger.info(
            "Created %s busses in %ss"
    ...
    def _prepare_connectivity_nodes_cim16(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        node_breaker = (
            True
            if self.cimConverter.cim["eq"]["ConnectivityNode"].index.size > 0
            else False
        )
        # use this dictionary to store the source profile from the element (normal or boundary profile)
        cn_dict = {
            "eq": {sc["o_prf"]: "eq"},
            "eq_bd": {sc["o_prf"]: "eq_bd"},
            "tp": {sc["o_prf"]: "tp"},
        }
        if (
            "tp_bd" in self.cimConverter.cim
        ):  # check because tp_bd was removed in cgmes 3.0
            cn_dict["tp_bd"] = {sc["o_prf"]: "tp_bd"}
        if node_breaker:
            # Node-Breaker model
            connectivity_nodes = pd.concat(
                [
                    self.cimConverter.cim["eq"]["ConnectivityNode"].assign(
                        **cn_dict["eq"]
                    ),
            ...
            connectivity_nodes[sc["cnc_id"]] = connectivity_nodes[
            ...
            eq_bay = pd.merge(
                self.cimConverter.cim["eq"]["ConnectivityNode"][
                    ["ConnectivityNodeContainer"]
                ],
                eq_bay,
                how="inner",
            ...
            eq_bay = pd.merge(
                eq_bay,
                self.cimConverter.cim["eq"]["VoltageLevel"][
                    ["rdfId", "BaseVoltage", "Substation"]
                ],
                how="left",
            ...
            eq_subs = pd.merge(
                self.cimConverter.cim["eq"]["ConnectivityNode"][
                    ["ConnectivityNodeContainer"]
                ].rename(columns={"ConnectivityNodeContainer": "Substation"}),
                self.cimConverter.cim["eq"]["Substation"][["rdfId"]].rename(
                    columns={"rdfId": "Substation"}
                ),
                how="inner",
                on="Substation",
            )
            ...
            eq_subs_duplicates = eq_subs[
                eq_subs.duplicated(["Substation"], keep="first")
            ]
            ...
            if eq_subs_duplicates.index.size > 0:
                ...
                self.cimConverter.report_container.add_log(
                    Report(
                        ...
                        code=ReportCode.WARNING_CONVERTING,
            ...
            eq_voltage_levels = self.cimConverter.cim["eq"]["VoltageLevel"][
                ["rdfId", "BaseVoltage", "Substation"]
            ]
            eq_voltage_levels = pd.concat(
                [eq_voltage_levels, eq_bay, eq_subs], ignore_index=True, sort=False
            )
            eq_voltage_levels = eq_voltage_levels.drop_duplicates(
                ["rdfId"], keep="first"
            )
            ...
            eq_substations = eq_substations.rename(
                columns={"rdfId": "Substation", "name": "name_substation"}
            )
            eq_voltage_levels = pd.merge(
                eq_voltage_levels, eq_substations, how="left", on="Substation"
            )
            eq_voltage_levels = eq_voltage_levels.drop_duplicates(subset=["rdfId"])
            eq_voltage_levels = eq_voltage_levels.rename(
                columns={"rdfId": "ConnectivityNodeContainer"}
            )

            connectivity_nodes = pd.merge(
                connectivity_nodes,
                eq_voltage_levels,
                how="left",
                on="ConnectivityNodeContainer",
            ...
            if "tp_bd" in self.cimConverter.cim:
                ...
                eq_bd_cns = pd.merge(
                    self.cimConverter.cim["eq_bd"]["ConnectivityNode"][["rdfId"]],
                    self.cimConverter.cim["tp_bd"]["ConnectivityNode"][
                        ["rdfId", "TopologicalNode"]
                    ],
                    how="inner",
                    on="rdfId",
                )
                ...
                eq_bd_cns = pd.merge(
                    eq_bd_cns,
                    self.cimConverter.cim["tp_bd"]["TopologicalNode"][
                        ["rdfId", "BaseVoltage"]
                    ].rename(columns={"rdfId": "TopologicalNode"}),
                    how="inner",
                    on="TopologicalNode",
                )
            else:
                eq_bd_cns = self.cimConverter.cim["eq_bd"]["ConnectivityNode"][
                    ["rdfId"]
                ]
                eq_bd_cns["BaseVoltage"] = float("NaN")
                eq_bd_cns["TopologicalNode"] = float("NaN")
            # eq_bd_cns = eq_bd_cns.drop(columns=['TopologicalNode'])
            eq_bd_cns = eq_bd_cns.rename(
                columns={
            ...
            connectivity_nodes = pd.merge(
                connectivity_nodes, eq_bd_cns, how="left", on="rdfId"
            )
            connectivity_nodes["BaseVoltage"] = connectivity_nodes[
                "BaseVoltage"
            ].fillna(connectivity_nodes["BaseVoltage_2"])
            ...
            if terminals_temp.index.size > 0:
                ...
                terminals_temp = terminals_temp.rename(
                    columns={"TopologicalNode": "rdfId"}
                )
                ...
                if (
                    "tp_bd" in self.cimConverter.cim
                ):  # check because tp_bd has been removed in cgmes 3.0
                    tp_temp = pd.concat(
                        [
                            tp_temp,
                            self.cimConverter.cim["tp_bd"]["TopologicalNode"][
                                ["rdfId", "name", "BaseVoltage"]
                            ],
                        ],
                        sort=False,
                    )
                ...
                connectivity_nodes = pd.concat(
                    [connectivity_nodes, tp_temp], ignore_index=True, sort=False
                )
        else:
            # Bus-Branch model
            ...
            connectivity_nodes = self.cimConverter.cim["tp"]["TopologicalNode"].assign(
                **cn_dict["tp"]
            )
            if (
                "tp_bd" in self.cimConverter.cim
            ...
            connectivity_nodes = connectivity_nodes.rename(
                columns={"ConnectivityNodeContainer": sc["cnc_id"]}
            )
            eq_voltage_levels = self.cimConverter.cim["eq"]["VoltageLevel"][
                ["rdfId", "Substation"]
            ]
            ...
            eq_voltage_levels = eq_voltage_levels.drop_duplicates(subset=["rdfId"])
            eq_voltage_levels = eq_voltage_levels.rename(
                columns={"rdfId": sc["cnc_id"]}
            )
            connectivity_nodes = pd.merge(
                connectivity_nodes, eq_voltage_levels, how="left", on=sc["cnc_id"]
            )

        connectivity_nodes[sc["sub_id"]] = connectivity_nodes["Substation"][:]
        # prepare the voltages from the buses
        eq_base_voltages = pd.concat(
            [
                self.cimConverter.cim["eq"]["BaseVoltage"][["rdfId", "nominalVoltage"]],
                self.cimConverter.cim["eq_bd"]["BaseVoltage"][
                    ["rdfId", "nominalVoltage"]
                ],
            ],
            ignore_index=True,
            sort=False,
        ...
        connectivity_nodes["BaseVoltage"] = connectivity_nodes["BaseVoltage"].astype(
            str
        )
        connectivity_nodes = pd.merge(
            connectivity_nodes, eq_base_voltages, how="left", on="BaseVoltage"
        )
        connectivity_nodes = connectivity_nodes.drop(columns=["BaseVoltage"])
        eqssh_terminals = self.cimConverter.cim["eq"]["Terminal"][
            ["rdfId", "ConnectivityNode", "ConductingEquipment", "sequenceNumber"]
        ]
        eqssh_terminals = pd.concat(
            [
                eqssh_terminals,
        ...
        eqssh_terminals = pd.merge(
            eqssh_terminals,
            self.cimConverter.cim["ssh"]["Terminal"],
            how="left",
            on="rdfId",
        )
        eqssh_terminals = pd.merge(
        ...
        eqssh_terminals["ConnectivityNode"] = eqssh_terminals[
            "ConnectivityNode"
        ].fillna(eqssh_terminals["TopologicalNode"])
        # concat the DC terminals
        dc_terminals = pd.merge(
            pd.concat(
                [
                    self.cimConverter.cim["eq"]["DCTerminal"],
                    self.cimConverter.cim["eq"]["ACDCConverterDCTerminal"],
                ],
                ignore_index=True,
                sort=False,
            ),
            pd.concat(
                [
                    self.cimConverter.cim["ssh"]["DCTerminal"],
        ...
        dc_terminals = pd.merge(
            ...
            pd.concat(
                [
                    self.cimConverter.cim["tp"]["DCTerminal"],
        ...
        dc_terminals = dc_terminals.rename(
            columns={
                "DCNode": "ConnectivityNode",
                "DCConductingEquipment": "ConductingEquipment",
                "DCTopologicalNode": "TopologicalNode",
            }
        )
        eqssh_terminals = pd.concat(
            [eqssh_terminals, dc_terminals], ignore_index=True, sort=False
        )
        # special fix for concat tp profiles
        eqssh_terminals = eqssh_terminals.drop_duplicates(
            subset=["rdfId", "TopologicalNode"]
        )
        ...
        if "TopologicalNode_2" in connectivity_nodes.columns:
            connectivity_nodes["TopologicalNode"] = connectivity_nodes[
                "TopologicalNode"
            ].fillna(connectivity_nodes["TopologicalNode_2"])
            connectivity_nodes = connectivity_nodes.drop(columns=["TopologicalNode_2"])
        if (
            connectivity_nodes.index.size != connectivity_nodes_size
            and not self.cimConverter.kwargs.get("ignore_errors", True)
        ):
            ...
            for rdfId, count in dups.items():
                self.logger.warning(
                    "The ConnectivityNode with RDF ID %s has %s TopologicalNodes!"
                    % (rdfId, count)
                )
                self.logger.warning(
                    "The ConnectivityNode data: \n%s"
                ...
                self.cimConverter.report_container.add_log(
                    Report(
                        level=LogLevel.WARNING,
                        code=ReportCode.WARNING_CONVERTING,
                        message="The ConnectivityNode with RDF ID %s has %s TopologicalNodes!"
                        % (rdfId, count),
                    )
                )
            connectivity_nodes = connectivity_nodes.drop_duplicates(
                subset=["rdfId"], keep="first"
            )
        ...
        t = t.rename(
            columns={"ConnectivityNode": "rdfId", "ConductingEquipment": sc["bb_id"]}
        )
        ...
        sgr = self.cimConverter.cim["eq"]["SubGeographicalRegion"][
            ["rdfId", "name", "Region"]
        ]
        regions = pd.merge(
            self.cimConverter.cim["eq"]["Substation"],
            sgr,
            left_on="Region",
            right_on="rdfId",
        ...
        regions = pd.merge(
            self.cimConverter.cim["eq"]["GeographicalRegion"],
            regions,
            left_on="rdfId",
            right_on="Region_SubGeographicalRegion",
        )
        regions = regions.rename(
            columns={
                "name": "GeographicalRegion_name",
                "name_SubGeographicalRegion": "SubGeographicalRegion_name",
                "rdfId": "GeographicalRegion_id",
                "rdfId_SubGeographicalRegion": "SubGeographicalRegion_id",
            }
        )
