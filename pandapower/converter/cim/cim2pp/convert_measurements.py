...
class CreateMeasurements:
    ...
    def _set_measurement_element_datatype(self):
        self.net.measurement.element = self.net.measurement.element.astype(
            np.sctypeDict.get("UInt32")
        )
    ...
    def create_measurements_from_analog(self):
        self.logger.info(
            "------------------------- Creating measurements from Analog -------------------------"
        )
        ...
        analogs = pd.merge(
            ...
            self.cim[analogs_prf]["AnalogValue"][
                ["rdfId", "sensorAccuracy", "MeasurementValueSource", "Analog", "value"]
            ],
            how="inner",
            left_on="rdfId",
            right_on="Analog",
            suffixes=("_Analog", "_AnalogValue"),
        )
        analogs = analogs.drop(columns=["Analog"])
        analogs = analogs.rename(columns={"name": "name_analog"})
        analogs = pd.merge(
            analogs,
            self.cim[analogs_prf]["MeasurementValueSource"],
            how="left",
        ...
        for element_type, sides in append_dict.items():
            for side_name, side in sides.items():
                temp = (
                    self.net[element_type][[sc["o_id"], side_name, sc[side_name]]]
                    .reset_index()
                    .rename(columns={side_name: "bus", sc[side_name]: "terminal_asset"})
                )
        ...
        psr["measurement_type"] = psr.unitSymbol.map(
            {"W": "p", "VAr": "q", "A": "i", "V": "v"}
        )
        # change the sign if needed
        psr["value"].loc[~psr["positiveFlowIn"]] = psr.loc[~psr["positiveFlowIn"]][
            "value"
        ] * (-1)
        # move the voltage measurements to the buses
        psr = pd.merge(
            psr, self.net.bus[["vn_kv"]], how="inner", left_on="bus", right_index=True
        )
        ...
        self.logger.info(
            "Needed time for creating the measurements: %ss"
            % (time.time() - time_start)
        )

    def create_measurements_from_sv(self):
        self.logger.info(
            "--------------------------- Creating measurements from SV ---------------------------"
        )
        time_start = time.time()
        sc = cim_tools.get_pp_net_special_columns_dict()
        ...
        busses_temp = busses_temp.rename(
            columns={"index": "element", sc["ct"]: "TopologicalNode"}
        )
        sv_sv_voltages = pd.merge(
            self.cim["sv"]["SvVoltage"][["rdfId", "TopologicalNode", "v"]],
            busses_temp,
            how="left",
            on="TopologicalNode",
        )
        ...
        sv_sv_voltages["std_dev"] = sv_sv_voltages[
            ["value_stddev", "vn_kv_stddev"]
        ].max(axis=1)
        ...
        # ---------------------------------------measure: line---------------------------------------------------
        sigma_line = 0.03
        line_temp = self.net.line[
            ["name", "from_bus", "to_bus", sc["t_from"], sc["t_to"]]
        ].copy()
        line_temp["p_from"] = pd.merge(
            line_temp[sc["t_from"]],
            sv_powerflow["p"],
            left_on=sc["t_from"],
            right_index=True,
        )["p"]
        line_temp["p_to"] = pd.merge(
            line_temp[sc["t_to"]],
            sv_powerflow["p"],
            left_on=sc["t_to"],
            right_index=True,
        )["p"]
        line_temp["q_from"] = pd.merge(
            line_temp[sc["t_from"]],
            sv_powerflow["q"],
            left_on=sc["t_from"],
            right_index=True,
        )["q"]
        line_temp["q_to"] = pd.merge(
            line_temp[sc["t_to"]],
            sv_powerflow["q"],
            left_on=sc["t_to"],
            right_index=True,
        ...
        line_temp = line_temp.dropna(
            subset=["p_from", "p_to", "q_from", "q_to"], thresh=4
        )

        line_temp["stddev_line_from_p"] = abs(line_temp.p_from) * sigma_line + 1.0
        line_temp["stddev_line_to_p"] = abs(line_temp.p_to) * sigma_line + 1.0
        line_temp["stddev_line_from_q"] = abs(line_temp.q_from) * sigma_line + 1.0
        line_temp["stddev_line_to_q"] = abs(line_temp.q_to) * sigma_line + 1.0
        ...
        line_temp = pd.merge(
            line_temp, sv_powerflow, left_on=sc["t_from"], right_index=True
        )
        ...
        line_temp = pd.merge(
            line_temp, sv_powerflow, left_on=sc["t_to"], right_index=True
        )
        self._copy_to_measurement(line_temp)
        line_temp = line_temp.drop(columns=sv_powerflow.columns)
        # ---------------------------------------measure: line q from---------------------------------------------------
        ...
        # ---------------------------------------measure: trafo---------------------------------------------------
        sigma_trafo = 0.03
        trafo_temp = self.net.trafo[
            ["name", "hv_bus", "lv_bus", sc["t_hv"], sc["t_lv"]]
        ].copy()
        # if trafo_temp.index.size > 0:
        trafo_temp["p_hv"] = pd.merge(
            trafo_temp[sc["t_hv"]],
            sv_powerflow["p"],
            left_on=sc["t_hv"],
            right_index=True,
        )["p"]
        trafo_temp["p_lv"] = pd.merge(
            trafo_temp[sc["t_lv"]],
            sv_powerflow["p"],
            left_on=sc["t_lv"],
            right_index=True,
        )["p"]
        trafo_temp["q_hv"] = pd.merge(
            trafo_temp[sc["t_hv"]],
            sv_powerflow["q"],
            left_on=sc["t_hv"],
            right_index=True,
        )["q"]
        trafo_temp["q_lv"] = pd.merge(
            trafo_temp[sc["t_lv"]],
            sv_powerflow["q"],
            left_on=sc["t_lv"],
            right_index=True,
        ...
        trafo_temp = trafo_temp.dropna(
            subset=["p_hv", "p_lv", "q_hv", "q_lv"], thresh=4
        )

        trafo_temp["stddev_trafo_hv_p"] = abs(trafo_temp.p_hv) * sigma_trafo + 1.0
        trafo_temp["stddev_trafo_lv_p"] = abs(trafo_temp.p_lv) * sigma_trafo + 1.0
        trafo_temp["stddev_trafo_hv_q"] = abs(trafo_temp.q_hv) * sigma_trafo + 1.0
        trafo_temp["stddev_trafo_lv_q"] = abs(trafo_temp.q_lv) * sigma_trafo + 1.0
        ...
        trafo_temp = pd.merge(
            trafo_temp, sv_powerflow, left_on=sc["t_hv"], right_index=True
        )
        ...
        trafo_temp = pd.merge(
            trafo_temp, sv_powerflow, left_on=sc["t_lv"], right_index=True
        )
        self._copy_to_measurement(trafo_temp)
        trafo_temp = trafo_temp.drop(columns=sv_powerflow.columns)
        # ---------------------------------------measure: trafo q hv---------------------------------------------------
        ...
        trafo3w_temp = self.net.trafo3w[
            ["name", "hv_bus", "mv_bus", "lv_bus", sc["t_hv"], sc["t_mv"], sc["t_lv"]]
        ].copy()
        trafo3w_temp["p_hv"] = pd.merge(
            trafo3w_temp[sc["t_hv"]],
            sv_powerflow["p"],
            left_on=sc["t_hv"],
            right_index=True,
        )["p"]
        trafo3w_temp["p_mv"] = pd.merge(
            trafo3w_temp[sc["t_mv"]],
            sv_powerflow["p"],
            left_on=sc["t_mv"],
            right_index=True,
        )["p"]
        trafo3w_temp["p_lv"] = pd.merge(
            trafo3w_temp[sc["t_lv"]],
            sv_powerflow["p"],
            left_on=sc["t_lv"],
            right_index=True,
        )["p"]
        trafo3w_temp["q_hv"] = pd.merge(
            trafo3w_temp[sc["t_hv"]],
            sv_powerflow["q"],
            left_on=sc["t_hv"],
            right_index=True,
        )["q"]
        trafo3w_temp["q_mv"] = pd.merge(
            trafo3w_temp[sc["t_mv"]],
            sv_powerflow["q"],
            left_on=sc["t_mv"],
            right_index=True,
        )["q"]
        trafo3w_temp["q_lv"] = pd.merge(
            trafo3w_temp[sc["t_lv"]],
            sv_powerflow["q"],
            left_on=sc["t_lv"],
            right_index=True,
        ...
        trafo3w_temp = trafo3w_temp.dropna(
            subset=["p_hv", "p_mv", "p_lv", "q_hv", "q_mv", "q_lv"], thresh=6
        )

        trafo3w_temp["stddev_trafo_hv_p"] = abs(trafo3w_temp.p_hv) * sigma_trafo3w + 1.0
        trafo3w_temp["stddev_trafo_mv_p"] = abs(trafo3w_temp.p_mv) * sigma_trafo3w + 1.0
        trafo3w_temp["stddev_trafo_lv_p"] = abs(trafo3w_temp.p_lv) * sigma_trafo3w + 1.0
        trafo3w_temp["stddev_trafo_hv_q"] = abs(trafo3w_temp.q_hv) * sigma_trafo3w + 1.0
        trafo3w_temp["stddev_trafo_mv_q"] = abs(trafo3w_temp.q_mv) * sigma_trafo3w + 1.0
        trafo3w_temp["stddev_trafo_lv_q"] = abs(trafo3w_temp.q_lv) * sigma_trafo3w + 1.0
        ...
        trafo3w_temp = pd.merge(
            trafo3w_temp, sv_powerflow, left_on=sc["t_hv"], right_index=True
        )
        ...
        trafo3w_temp = pd.merge(
            trafo3w_temp, sv_powerflow, left_on=sc["t_mv"], right_index=True
        )
        self._copy_to_measurement(trafo3w_temp)
        trafo3w_temp = trafo3w_temp.drop(columns=sv_powerflow.columns)
        ...
        trafo3w_temp = pd.merge(
            trafo3w_temp, sv_powerflow, left_on=sc["t_lv"], right_index=True
        )
        self._copy_to_measurement(trafo3w_temp)
        trafo3w_temp = trafo3w_temp.drop(columns=sv_powerflow.columns)
