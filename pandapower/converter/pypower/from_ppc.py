...
def from_ppc(ppc, f_hz=50, validate_conversion=False, **kwargs):
    ...
    _from_ppc_gencost(
        net, ppc, net._from_ppc_lookups["gen"], check=kwargs.get("check_costs", True)
    )
...
def _from_ppc_bus(net, ppc):
    ...
    idx_buses = create_buses(
        net,
        ppc["bus"].shape[0],
        name=ppc.get("bus_name", None),
        vn_kv=ppc["bus"][:, BASE_KV],
        type="b",
        zone=ppc["bus"][:, ZONE],
        in_service=(ppc["bus"][:, BUS_TYPE] != 4).astype(bool),
        max_vm_pu=ppc["bus"][:, VMAX],
        min_vm_pu=ppc["bus"][:, VMIN],
        index=ppc["bus"][:, BUS_I].astype(np.int64),
    )

    # create loads
    is_load = (ppc["bus"][:, PD] > 0) | (
        (ppc["bus"][:, PD] == 0) & (ppc["bus"][:, QD] != 0)
    )
    create_loads(
        net,
        idx_buses[is_load],
        p_mw=ppc["bus"][is_load, PD],
        q_mvar=ppc["bus"][is_load, QD],
        controllable=False,
    )
    ...
    create_sgens(
        net,
        idx_buses[is_sgen],
        p_mw=-ppc["bus"][is_sgen, PD],
        q_mvar=-ppc["bus"][is_sgen, QD],
        type="",
        controllable=False,
    )
...
def _from_ppc_gen(net, ppc):
    ...
    for i in np.arange(n_gen, dtype=np.int64)[is_ext_grid]:
        idx_eg.append(
            create_ext_grid(
                net,
                bus=net.bus.index[bus_pos[i]],
                vm_pu=vg_bus_lookup.at[bus_pos[i]],
    ...
    idx_gen = create_gens(
        net,
        buses=net.bus.index[bus_pos[is_gen]],
        vm_pu=vg_bus_lookup.loc[bus_pos[is_gen]].values,
        p_mw=ppc["gen"][is_gen, PG],
        sn_mva=ppc["gen"][is_gen, MBASE],
        in_service=(ppc["gen"][is_gen, GEN_STATUS] > 0),
        controllable=True,
        max_p_mw=ppc["gen"][is_gen, PMAX],
        min_p_mw=ppc["gen"][is_gen, PMIN],
        max_q_mvar=ppc["gen"][is_gen, QMAX],
        min_q_mvar=ppc["gen"][is_gen, QMIN],
    ...
    idx_sgen = create_sgens(
        net,
        buses=net.bus.index[bus_pos[is_sgen]],
        p_mw=ppc["gen"][is_sgen, PG],
        q_mvar=ppc["gen"][is_sgen, QG],
        sn_mva=ppc["gen"][is_sgen, MBASE],
        type="",
        in_service=(ppc["gen"][is_sgen, GEN_STATUS] > 0),
        max_p_mw=ppc["gen"][is_sgen, PMAX],
        min_p_mw=ppc["gen"][is_sgen, PMIN],
        max_q_mvar=ppc["gen"][is_sgen, QMAX],
        min_q_mvar=ppc["gen"][is_sgen, QMIN],
        controllable=True,
        name=gen_name[is_sgen],
    )

    neg_p_gens = np.arange(n_gen, dtype=np.int64)[
        (ppc["gen"][:, PG] < 0) & (is_gen | is_sgen)
    ]
    neg_p_lim_false = np.arange(n_gen, dtype=np.int64)[
        ppc["gen"][:, PMIN] > ppc["gen"][:, PMAX]
    ]
    neg_q_lim_false = np.arange(n_gen, dtype=np.int64)[
        ppc["gen"][:, QMIN] > ppc["gen"][:, QMAX]
    ]
    if len(neg_p_gens):
        logger.info(
            f"These gen have PG < 0 and are not converted to ext_grid: {neg_p_gens}."
        )
    ...
    for is_, idx, et in zip(
        [is_ext_grid, is_gen, is_sgen],
...
def _from_ppc_branch(net, ppc, f_hz, **kwargs):
    ...
    is_line, is_trafo, is_impedance, to_vn_is_leq = _branch_to_which(
        ppc, from_vn_kv=from_vn_kv, to_vn_kv=to_vn_kv
    )
    ...
    if np.any(is_trafo):
        ...
        idx_trafo = create_transformers_from_parameters(
            net,
            hv_buses=net.bus.index[hv_bus],
            lv_buses=net.bus.index[lv_bus],
            sn_mva=sn,
            vn_hv_kv=vn_hv_kv,
            vn_lv_kv=vn_lv_kv,
            name=bra_name[is_trafo],
    ...
    if np.any(is_impedance):
        ...
        idx_impedance = create_impedances(
            ...
            rft_pu=rft_pu,
            xft_pu=xft_pu,
            rtf_pu=rft_pu + r_asym_pu,
            xtf_pu=xft_pu + x_asym_pu,
            bf_pu=bf_pu,
            gf_pu=gf_pu,
            gt_pu=gf_pu + g_asym_pu,
            bt_pu=bf_pu + b_asym_pu,
            sn_mva=sn_mva,
        )
    else:
        idx_impedance = []
    ...
    branch_lookup = pd.DataFrame(
        {"element": [-1] * n_bra, "element_type": [""] * n_bra}
    )
...
def _get_bus_pos(ppc, bus_names):
    try:
        return (
            pd.Series(
                np.arange(ppc["bus"].shape[0], dtype=np.int64),
                index=ppc["bus"][:, BUS_I],
            )
            .loc[bus_names]
            .values
...
def _gen_to_which(ppc, bus_pos=None, flattened=True):
    if bus_pos is None:
        bus_pos = _get_bus_pos(ppc, ppc["gen"][:, GEN_BUS])
    bus_type_df = pd.DataFrame(
        {"bus_type": ppc["bus"][bus_pos, BUS_TYPE], "bus": ppc["gen"][:, GEN_BUS]}
    ).astype(np.int64)
    bus_type_df = pd.concat(
        [
            bus_type_df.loc[bus_type_df.bus_type == 3],
    ...
    is_ext_grid = (
        ((bus_type_df["bus_type"] == 3) & ~bus_type_df.duplicated(subset=["bus"]))
        .sort_index()
        .values
    )
    is_gen = (
        ((bus_type_df["bus_type"] == 2) & ~bus_type_df.duplicated(subset=["bus"]))
        .sort_index()
        .values
    )
    is_sgen = (
        ~(is_ext_grid | is_gen)
        & bus_type_df["bus_type"].sort_index().isin([3, 2, 1]).values
    )
...
def _calc_pp_pwl_points(ppc_pwl_points):
    ...
    def construct_list_of_list(row):
        arr = pts[row, ::2]
        arr = np.concatenate((arr[:1], np.repeat(arr[1:-1], 2), arr[-1:])).reshape(
            (-1, 2)
        )
        arr = np.c_[arr, c[row, :]]
    ...
    if not (pts.shape[1] % 2) == 0:
        raise ValueError(
            "_calc_pp_pwl_points() expects ppc_pwl_points with shape[1] is "
...
def _from_ppc_gencost(net, ppc, gen_lookup, check=True):
    ...
    # check gencost type
    if not all(
        np.isclose(gc2consider[:, MODEL], 1) | np.isclose(gc2consider[:, MODEL], 2)
    ):
    ...
    for is_, power_type in zip([is_p, ~is_p], ["p", "q"]):
        ...
        if not np.allclose(2 * gc4pwl[is_i, NCOST], gc4pwl[is_i, COST:].shape[1]):
            raise ValueError(
                "In pwl gencost, the number n does not fit to the number of values"
            )
        pp_pwl_points = _calc_pp_pwl_points(gc4pwl[is_i, 4:])
        create_pwl_costs(
            net,
            glu2.element.values[is_i],
            glu2.element_type.values[is_i],
            pp_pwl_points,
    ...
    # --- create poly costs
    is_poly = np.any(
        np.isclose(np.c_[gc4poly[:n_glu, MODEL], gc4poly[n_glu:, MODEL]], 2), axis=1
    )
    ...
    if sum(is_poly):
        ...
        if any(is_poly2 & (ncost > 3)):
            logger.warning(
                "The pandapower poly_cost table only supports up to 2nd order "
...
def _validate_diff_res(diff_res, max_diff_values):
    ...
    for et_val in [
        ...
        if max_diff_values[et_val] is not None:
            et = et_val.split("_")[0]
            log_key = (
                et if et != "gen" else "gen_p" if "p" in et_val else "gen_q_sum_per_bus"
            )
            i_col = _log_dict(log_key)[0]
            val &= bool(
                np.max(abs(diff_res[log_key][:, i_col])) < max_diff_values[et_val]
            )
    return val
...
def _log_dict(key=None):
    log_dict = {
        ...
        "branch": [
            [[0, 2], [1, 3]],
            ["branch flow active power", "branch flow reactive power"],
            ["MW", "Mvar"],
        ],
...
def validate_from_ppc(
    ...
    **pf_match** - Whether the power flow results matches.
    ...
    """
    if (
        "_from_ppc_lookups" not in net.keys()
        or ("gen" not in net._from_ppc_lookups.keys() and len(ppc["gen"]) > 0)
        or ("branch" not in net._from_ppc_lookups.keys() and len(ppc["branch"]) > 0)
    ):
        raise ValueError(
    ...
    if net.res_bus.shape[0] == 0 and net.bus.shape[0] > 0:
        logger.debug(
            "runpp() is performed by validate_from_ppc() since res_bus is empty."
        )
    ...
    # --- bus
    pp_res["bus"] = net.res_bus.loc[
        ppc["bus"][:, BUS_I].astype(np.int64), ["vm_pu", "va_degree"]
    ].values
    ...
    for et in net._from_ppc_lookups["branch"].element_type.unique():
        ...
        pp_res["branch"][is_et] += (
            net[f"res_{et}"]
            .loc[net._from_ppc_lookups["branch"].element.loc[is_et], res_cols]
            .values
        )
        from_to_buses[is_et] = (
            net[et]
            .loc[net._from_ppc_lookups["branch"].element.loc[is_et], from_to_cols]
            .values
        )

    # switch direction as in ppc
    correct_from_to = np.all(
        from_to_buses == ppc["branch"][:, F_BUS : T_BUS + 1].astype(np.int64), axis=1
    )
    switch_from_to = np.all(
        from_to_buses[:, ::-1] == ppc["branch"][:, F_BUS : T_BUS + 1].astype(np.int64),
        axis=1,
    )
    if not np.all(correct_from_to | switch_from_to):
        raise ValueError(
            "ppc branch from and to buses don't fit to pandapower from and to + "
            "hv and lv buses."
        )
    if np.any(switch_from_to):
        pp_res["branch"][switch_from_to, :] = pp_res["branch"][switch_from_to, :][
            :, [2, 3, 0, 1]
        ]
    ...
    for et in net._from_ppc_lookups["gen"].element_type.unique():
        ...
        pp_res["gen"][is_et] += (
            net[f"res_{et}"]
            .loc[net._from_ppc_lookups["gen"].element.loc[is_et], res_cols]
            .values
        )
    ...
    for comp_key in comp_keys:
        ...
        for i_col, var_str, unit in zip(*_log_dict(comp_key)):
            ...
            logger.debug(
                f"Maximum {var_str} difference between pandapower and pypower: "
    ...
    if not pf_match:
        ...
        if (
            _validate_diff_res(diff_res, other_max_diff)
            and (np.max(abs(pp_res["gen"] - ppc_res["gen"])) > 1e-1).any()
        ):
            logger.debug(
