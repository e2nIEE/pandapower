...
def conf_var_graphics(app, opt_grf, var_graphics):
    ...
    for option in menu_options:
        opt_grf["menu"].add_command(
            label=option, command=tk._setit(var_graphics, option)
        )
...
def make_gui(app, project_name, browse_dst, calc):
    ...
    # row 0
    label_path_dst = tk.Label(
        input_panel,
        anchor="w",
        text="Path to the destination directory for pandapower export: ",
    )
    label_path_dst.grid(row=0, column=0, columnspan=3, sticky="w", padx=2, pady=4)

    entry_path_dst = tk.Entry(input_panel, width=70)
    entry_path_dst.delete(0, tk.END)
    ...
    label_fname = tk.Label(
        input_panel, anchor="w", text="Provide file name for the network: "
    )
    label_fname.grid(row=1, column=0, columnspan=3, sticky="w", padx=2, pady=2)

    entry_fname = tk.Entry(input_panel, width=70)
    ...
    tk.Label(input_panel, anchor="w", text="Loads P variable:").grid(
        row=2, column=0, sticky=tk.W, pady=0
    )
    ...
    tk.Label(input_panel, anchor="w", text="Generators P variable:").grid(
        row=3, column=0, sticky=tk.W, pady=0
    )
    ...
    tk.Label(input_panel, anchor="w", text="Collect coordinates from:").grid(
        row=4, column=0, sticky=tk.W, pady=0
    )
    ...
    var_us.set(US_OPTIONS[2])
    tk.Label(input_panel, anchor="w", text="Unsupplied Elements:").grid(
        row=5, column=0, sticky=tk.W, pady=0
    )
    ...
    tk.Label(input_panel, anchor="w", text="Save As:").grid(
        row=6, column=0, sticky=tk.W, pady=0
    )
    ...
    tk.Label(input_panel, anchor="w", text="Tap:").grid(
        row=7, column=0, sticky=tk.W, pady=0
    )
    ...
    max_iter.set(30)
    tk.Label(input_panel, anchor="w", text="Max. iterations:").grid(
        row=8, column=0, sticky=tk.W, pady=0
    )
    ...
    PV_SL = tk.IntVar()
    tk.Checkbutton(
        input_panel, text="Export 'PV' buses as slack buses", variable=PV_SL
    ...
    tk.Checkbutton(
        input_panel, text="Export controllers", variable=EXPORT_CONTROLLER
    ).grid(row=3, column=4, sticky=tk.W)
    params.export_controller = EXPORT_CONTROLLER.get

    CV_VERIFY = tk.IntVar()
    CV_VERIFY.set(1)
    tk.Checkbutton(input_panel, text="Verify conversion", variable=CV_VERIFY).grid(
        row=4, column=4, sticky=tk.W
    )
    params.is_to_verify = CV_VERIFY.get

    RUN_DIAGNOSTIC = tk.IntVar()
    RUN_DIAGNOSTIC.set(0)
    tk.Checkbutton(input_panel, text="Diagnostic report", variable=RUN_DIAGNOSTIC).grid(
    ...
    tk.Checkbutton(
        input_panel, text="Logger in debug mode", variable=LOGGER_DEBUG
    ).grid(row=6, column=4, sticky=tk.W)
    ...
    tk.Checkbutton(
        input_panel,
        text="Create line sections as separate lines",
        variable=CREATE_SECTIONS,
    ).grid(row=7, column=4, sticky=tk.W)
    params.create_sections_entry = CREATE_SECTIONS.get

    REPLACE_ZERO_BRANCHES = tk.IntVar()
    REPLACE_ZERO_BRANCHES.set(0)
    tk.Checkbutton(
        input_panel,
        text="Replace low-impedance branches with switches",
        variable=REPLACE_ZERO_BRANCHES,
    ).grid(row=8, column=4, sticky=tk.W)
    params.replace_zero_branches = REPLACE_ZERO_BRANCHES.get

    tk.Label(input_panel, anchor="w", text="Cut-off Z:").grid(
        row=8, column=5, sticky=tk.E, pady=0
    )
    tk.Label(input_panel, anchor="w", text="Ohm").grid(
        row=8, column=7, sticky=tk.W, pady=0
    )
    ...
    REPLACE_INF_BRANCHES = tk.IntVar()
    REPLACE_INF_BRANCHES.set(0)
    tk.Checkbutton(
        input_panel,
        text="Deactivate high-impedance branches",
        variable=REPLACE_INF_BRANCHES,
    ).grid(row=9, column=4, sticky=tk.W)
    params.replace_inf_branches = REPLACE_INF_BRANCHES.get

    tk.Label(input_panel, anchor="w", text="Cut-off Z:").grid(
        row=9, column=5, sticky=tk.E, pady=0
    )
    tk.Label(input_panel, anchor="w", text="Ohm").grid(
        row=9, column=7, sticky=tk.W, pady=0
    )

    max_ohm = tk.Entry(input_panel, width=14)
    max_ohm.delete(0, tk.END)
    max_ohm.grid(row=9, column=6, padx=2, pady=2, sticky=tk.W)
    max_ohm.insert(0, "1000")
    # entry_fname.insert(0, 'test') ##for testing
    params.max_ohm_entry = max_ohm

    # row 2 col 4
    stop_button = tk.Button(
        input_panel, text="Cancel", width=8, command=lambda: cancel(app, input_panel)
    )
    ...
    start_button = tk.Button(
        input_panel, text="Export", width=8, command=lambda: calc(app, **params)
    )
