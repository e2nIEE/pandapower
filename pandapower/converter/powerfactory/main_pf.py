...
def browse_dst(input_panel, entry_path_dst):
    ...
    path = askdirectory(
        parent=input_panel,
        title="Provide path to the target directory "
...
def save_net(net, filepath, save_as):
    if save_as == "JSON":
        ...
        raise ValueError(
            "tried to save grid as %s to %s and failed :(" % (save_as, filepath)
        )
...
def calc(
    ...
    try:
        ...
        net = run_export(
            ...
            export_controller=export_controller(),
            max_iter=max_iter,
            create_sections=create_sections_entry(),
        )
        if replace_zero_branches():
            ...
            to_replace = (
                np.abs(net.line.x_ohm_per_km * net.line.length_km) <= min_ohm
            ) & net.line.in_service
            ...
            xward = net.xward[
                (np.abs(net.xward.x_ohm) <= min_ohm) & net.xward.in_service
            ].index.values
            if len(xward) > 0:
            ...
            impedance = (
                (np.abs(net.impedance.xft_pu) <= min_ohm / zb_f_ohm)
                | (np.abs(net.impedance.xtf_pu) <= min_ohm / zb_t_ohm)
            ) & net.impedance.in_service
            ...
            if any(impedance):
                logger.info(
                    f"replaced {sum(impedance)} impedance elements with switches"
                )
            ...
            if any(trafos):
                logger.info(
                    f"adjusted {sum(trafos)} 2W transformers by setting vk_percent to {min_vk_percent}"
                )
            ...
            net.trafo3w.loc[
                trafo3w, ["vk_hv_percent", "vk_mv_percent", "vk_lv_percent"]
            ] = min_vk_percent
            if any(trafo3w):
                logger.info(
                    f"adjusted {sum(trafo3w)} 3W transformers by setting vk_percent to {min_vk_percent}"
                )
        ...
        if replace_inf_branches():
            ...
            to_replace = (
                np.abs(
                    ...
                    + 1j * net.line.x_ohm_per_km * net.line.length_km
                )
                >= max_ohm
            ) & net.line.in_service
            ...
            xward = (
                np.abs(net.xward.r_ohm + 1j * net.xward.x_ohm) >= max_ohm
            ...
            impedance = (
                (
                    np.abs(net.impedance.rft_pu + 1j * net.impedance.xft_pu)
                    >= max_ohm / zb_f_ohm
                )
                | (
                    np.abs(net.impedance.rtf_pu + 1j * net.impedance.xtf_pu)
                    >= max_ohm / zb_t_ohm
                )
            ) & net.impedance.in_service
            if np.any(impedance):
            ...
            if np.any(trafos):
                ...
                logger.info(
                    f"adjusted {sum(trafos)} 2W transformers with vk_percent of {max_vk_percent}"
                )
            ...
            if np.any(trafo3w):
                ...
                net.trafo3w.loc[trafo3w, "vk_hv_percent"] = np.fmin(
                    max_vk_percent, net.trafo3w.loc[trafo3w, "vk_hv_percent"]
                )
                net.trafo3w.loc[trafo3w, "vk_mv_percent"] = np.fmin(
                    max_vk_percent, net.trafo3w.loc[trafo3w, "vk_mv_percent"]
                )
                net.trafo3w.loc[trafo3w, "vk_lv_percent"] = np.fmin(
                    max_vk_percent, net.trafo3w.loc[trafo3w, "vk_lv_percent"]
                )
                logger.info(
                    f"adjusted {sum(trafo3w)} 3W transformers with {max_vk_percent}"
                )
        ...
        if is_to_diagnostic():
            try:
                diagnostic(
                    net, report_style="compact", warnings_only=True, min_x_ohm=0.01
                )
            except Exception as err:
