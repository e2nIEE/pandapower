...
def create_lines(net, dict_net, flag_graphics, is_unbalanced):
    ...
    for n, item in enumerate(dict_net['ElmLne']):
        ...
        if len(line_sections) == 0:
            ...
            lines_to_create.append(create_line_normal(net=net, item=item, geodata=coords,
            ...
            lines_to_create.extend(create_line_sections(net=net, item_list=line_sections, line=item,
                                            geodata=coords, is_unbalanced=is_unbalanced, **params))
...
#        logger.debug('line <%s> created' % params['name'])
    ...
    # net.line_geodata = net.line[["coords"]] #TODO: adjust to new format
...
# TODO check how create_lines function is beneficial
...
def create_line_normal(net, item, bus1, bus2, name, line_idx, parallel, is_unbalanced, geodata=None):
    pf_type = item.typ_id
    ...
    params = {
        ...
        'df': item.fline,
        'parallel': parallel,
        # 'alpha': pf_type.alpha if pf_type is not None else None, #TODO: make this possible again
        # 'temperature_degree_celsius': pf_type.tmax if pf_type is not None else None, #TODO: make this possible again
        'description': ' \n '.join(item.desc) if len(item.desc) > 0 else '',
        'line_idx': line_idx,
