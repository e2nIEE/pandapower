...
def from_pf(
    ...
    flag_graphics = (
        flag_graphics if flag_graphics in ["GPS", "no geodata"] else "graphic objects"
    )
    ...
    if "res_switch" not in net.keys():
        net["res_switch"] = DataFrame(
            columns=["pf_closed", "pf_in_service"], dtype="bool"
        )
    ...
    for n, bus in enumerate(dict_net["ElmTerm"], 1):
        create_pp_bus(
            net=net, item=bus, flag_graphics=flag_graphics, is_unbalanced=is_unbalanced
        )
    if n > 0:
        logger.info("imported %d buses" % n)
    ...
    for n, ext_net in enumerate(dict_net["ElmXnet"], 1):
        create_ext_net(
            net=net, item=ext_net, pv_as_slack=pv_as_slack, is_unbalanced=is_unbalanced
        )
    if n > 0:
        logger.info("imported %d external grids" % n)
    ...
    if n > 0:
        logger.info("imported %d loads" % n)
    ...
    if n > 0:
        logger.info("imported %d mv loads" % n)
    ...
    if n > 0:
        logger.info("imported %d static generators" % n)
    ...
    if n > 0:
        logger.info("imported %d pv generators" % n)
    ...
    for n, asm in enumerate(dict_net["ElmAsm"], n):
        create_sgen_asm(
            net=net, item=asm, pf_variable_p_gen=pf_variable_p_gen, dict_net=dict_net
        )
    if n > 0:
        logger.info("imported %d asynchronous machines" % n)
    ...
    if n > 0:
        logger.info("imported %d synchronous machines" % n)
    ...
    for n, trafo in enumerate(dict_net["ElmTr2"], 1):
        create_trafo(
            ...
            tap_opt=tap_opt,
            is_unbalanced=is_unbalanced,
            hunting_limit=dict_net["lvp_params"]["hunting_limit"],
        )
    if n > 0:
        logger.info("imported %d trafos" % n)
    ...
    if n > 0:
        logger.info("imported %d coups" % n)
    ...
    if n > 0:
        logger.info("imported %d fuses" % n)
    ...
    if n > 0:
        logger.info("imported %d shunts" % n)
    ...
    if n > 0:
        logger.info("imported %d impedances" % n)
    ...
    if n > 0:
        logger.info("imported %d VSC (mono)" % n)
    ...
    logger.info(
        "imported %d lines" % (len(net.line.line_idx.unique())) if len(net.line) else 0
    )
    ...
    if len(dict_net["ElmLodlvp"]) > 0:
        ...
        if len(lvp_lne_dict) > 0:
            logger.info(
                "imported %d partial loads on line, seperated line" % len(lvp_lne_dict)
            )

        # ElmLodLvp within load (ElmLodlv)
        lvp_lod_items = [
            lvp
            for lvp in dict_net["ElmLodlvp"]
            if lvp.fold_id.GetClassName() == "ElmLodlv"
        ]
    ...
    if export_controller:
        ...
        if n > 0:
            logger.info("imported %d station controllers" % n)
    ...
    if handle_us == "Deactivate":
        ...
        raise ValueError(
            "handle_us should be 'Deactivate', 'Drop' or 'Nothing', "
...
def add_additional_attributes(
    item, net, element, element_id, attr_list=None, attr_dict=None
):
    """
                ...
                if isinstance(chr_name, (str, numbers.Number)):
                    ...
                    if len(chr_name) > 1:
                        raise NotImplementedError(
                            f"attribute {attr} is a list with more than 1 items - not supported."
                        )
                    elif len(chr_name) == 0:
                        continue
...
def create_pp_bus(net, item, flag_graphics, is_unbalanced):
    ...
    if system_type == "ac":
        ...
        raise NotImplementedError(
            f"Only buses with system type AC or DC are supported, "
    ...
    if item.HasAttribute("cpSubstat"):
        ...
        if substat is not None:
            logger.debug(
                "adding substat %s to descr of bus %s (#%d)"
        ...
        logger.debug("bus %s is not part of any substation" % params["name"])
    ...
    add_additional_attributes(
        item, net, table, bid, attr_dict=attr_dict, attr_list=["sernum", "chr_name"]
    )
...
def get_pf_bus_results(net, item, bid, is_unbalanced, system_type):
    if is_unbalanced:
        ...
        bus_type = "res_bus"
        result_variables = {"pf_vm_pu": "m:u", "pf_va_degree": "m:phiu"}
    else:
        bus_type = "res_bus_dc"
    ...
    for res_var_pp, res_var_pf in result_variables.items():
        ...
        net[bus_type].at[bid, res_var_pp] = (
            np.abs(res) if "vm_pu" in res_var_pp else res
        )
...
def get_connection_nodes(net, item, num_nodes):
    ...
    if all([b is None for b in buses]):
        logger.error(
            "Element %s is Disconnected: buses are %s" % (item.loc_name, buses)
        )
        raise IndexError
    elif None in buses:
        ...
        logger.warning(
            "object %s of class %s is not properly connected - creating auxiliary buses."
...
def import_switch(item, idx_cubicle):
    ...
    if len(switches) != 0:
        switch = switches[0]
        switch_in_service = (
            not bool(switch.outserv) if switch.HasAttribute("outserv") else True
        )
        ...
        switch_is_closed = (
            bool(switch.on_off) and bool(switch.isclosed) and switch_in_service
        )
...
def create_connection_switches(net, item, number_switches, et, buses, elements):
    ...
    for i in range(number_switches):
        ...
        logger.debug(
            "switch closed: %s, switch_usage: %s" % (switch_is_closed, switch_usage)
        )
        if switch_is_closed is not None:
            ...
            net.res_switch.loc[cd, ["pf_closed", "pf_in_service"]] = (
                switch_is_closed,
                True,
            )
...
def get_coords_from_buses(net, from_bus, to_bus, **kwargs):
    ...
    if from_geo and to_geo:
        coords = [
            geojson.utils.coords(geojson.loads(from_geo)),
            geojson.utils.coords(geojson.loads(to_geo)),
        ]
...
def create_pp_line(net, item, flag_graphics, create_sections, is_unbalanced):
    ...
    logger.debug(
        "line <%s> has <%d> parallel lines" % (params["name"], params["parallel"])
    )
    ...
    if len(line_sections) == 0:
        ...
        lid = create_line_normal(
            net=net, item=item, is_unbalanced=is_unbalanced, ac=ac, **params
        )
        sid_list = [lid]
        ...
        if create_sections:
            if not ac:
                raise NotImplementedError(
                    f"Export of lines with line sections only implemented for AC lines ({item})"
                )
            sid_list = create_line_sections(
                net=net,
                item_list=line_sections,
                line=item,
                coords=coords,
                is_unbalanced=is_unbalanced,
                **params,
            ...
            sid_list = [lidx]
        logger.debug(
            "created <%d> line sections for line <%s>" % (len(sid_list), params["name"])
        )
    ...
    if ac:
        ...
        new_switch_idx, new_switch_closed = create_connection_switches(
            net, item, 2, "l", (params["bus1"], params["bus2"]), new_elements
        )
...
def cut_coords_segment(p1, p2, split_len):
    ...
    logger.debug(
        "cut coords segment: p1=%s, p2=%s, split_len=%.3f, x_k = %.3f, y_k = %.3f"
...
def get_section_coords(coords, sec_len, start_len, scale_factor):
    ...
    logger.debug(
        "calculating section coords: sec_len=%.3f, start_len=%.3f, scale_factor=%.3f"
    ...
    for i in range(len(coords) - 1):
        ...
        if (
            ...
            or abs(len_i * scale_factor - start_len) <= tol
        ):
            ...
            if start_len == 0:
                ...
                x_k, y_k = cut_coords_segment(
                    coords[i],
                    coords[i + 1],
                    start_len / scale_factor
                    + point_len(coords[i], coords[i + 1])
                    - len_i,
                )
                sec_coords = [[x_k, y_k]]
    ...
    for j in range(i + 1, len(coords)):
        ...
        if len_j <= sec_len / scale_factor:
            ...
            x_k, y_k = cut_coords_segment(
                sec_coords[k],
                coords[j],
                sec_len / scale_factor + point_len(sec_coords[k], coords[j]) - len_j,
            )
...
def segment_buses(
    net, bus1, bus2, num_sections, line_name
):  # , sec_len, start_len, coords):
    """
...
def create_line_sections(
    net, item_list, line, bus1, bus2, coords, parallel, is_unbalanced, **kwargs
):
    sid_list = []
    ...
    buses_gen = segment_buses(
        net, bus1=bus1, bus2=bus2, num_sections=len(item_list), line_name=line_name
    )
    ...
    for item in item_list:
        ...
        if coords:
            try:
                ...
                sec_coords = get_section_coords(
                    coords,
                    sec_len=item.dline,
                    start_len=item.rellen,
                    scale_factor=scaling_factor,
                )
                net.line.loc[sid, "geo"] = geojson.dumps(geojson.LineString(sec_coords))
                # p1 = sec_coords[0]
                # p2 = sec_coords[-1]
                net.bus.loc[bus2, ["geo"]] = geojson.dumps(
                    geojson.Point(sec_coords[-1])
                )
...
def create_line_no_sections(
    net, main_item, item_list, bus1, bus2, coords, is_unbalanced, ac, **kwargs
):
    if not ac:
        raise NotImplementedError(
            "Creating DC lines with sections as DC lines without sections not implemented"
        )
    ...
    max_i_ka = min(
        [
            item.Inom * p * d if item.Inom != 0 else 1e-3
            for item, p, d in zip(item_list, parallel, df)
        ]
    )
    r_ohm_per_km = sum([item.R1 for item in item_list]) / total_len
    x_ohm_per_km = sum([item.X1 for item in item_list]) / total_len
    c_nf_per_km = (
        sum([item.C1 * 1e3 for item in item_list]) / total_len
    )  # internal unit for C in PF is uF
    ...
    c0_nf_per_km = (
        sum([item.C0 * 1e3 for item in item_list]) / total_len
    )  # internal unit for C in PF is uF
    ...
    alpha = (
        sum([item.alpha / p * w for item, p, w in zip(item_list, parallel, weights)])
        / total_len
    )
    ...
    lid = create_line_from_parameters(
        net=net,
        from_bus=bus1,
        to_bus=bus2,
        length_km=total_len,
...
def create_line_normal(
    net, item, bus1, bus2, name, parallel, is_unbalanced, ac, geodata=None
):
    pf_type = item.typ_id
    std_type, type_created = create_line_type(
        net=net,
        item=pf_type,
        cable_in_air=item.inAir if item.HasAttribute("inAir") else False,
    )
    ...
    if std_type is not None:  # and not is_unbalanced:delete later
        ...
        if ac:
            ...
            if r_ohm == 0 and x_ohm == 0 and c_nf == 0:
                logger.error(
                    'Incomplete data for line "%s": missing type and '
            ...
            params.update(
                {
                    ...
                    "c_nf_per_km": c_nf
                    / params["length_km"]
                    * 1e3,  # internal unit for C in PF is uF
            ...
            lid = create_line_from_parameters(
                net=net, from_bus=bus1, to_bus=bus2, **params
            )
        else:
            lid = create_line_dc_from_parameters(
                net, from_bus_dc=bus1, to_bus_dc=bus2, **params
            )
...
def create_line_type(net, item, cable_in_air=False):
    ...
    if item is None:
        logger.warning(
            "create_line_type: no item given! Be sure you can deal with None!"
        )
        return None, False
    ...
    name = (
        "%s\\%s" % (pf_folder, item.loc_name)
        if not cable_in_air
        else "%s\\%s_%s" % (pf_folder, item.loc_name, "air")
    )
...
def monopolar_in_service(item):
    ...
    if switch_is_closed is not None:
        logger.debug(
            "element in service: <%s>, switch is closed: <%s>"
...
def create_ext_net(net, item, pv_as_slack, is_unbalanced):
    ...
    s_max = (
        item.snss
        if item.HasAttribute("snss")
        else item.Pmax_uc
        if item.HasAttribute("Pmax_uc")
        else np.nan
    )
    # needed change in create.py: line 570 - added sk_min_mva in list of params
    s_min = (
        item.snssmin
        if item.HasAttribute("snssmin")
        else item.Pmin_uc
        if item.HasAttribute("Pmin_uc")
        else np.nan
    )
    ...
    if node_type == "PQ":
        ...
        xid = create_sgen(
            net, bus1, p_mw=p_mw, q_mvar=q_mvar, name=name, in_service=in_service
        )
        elm = "sgen"
    elif node_type == "PV" and not pv_as_slack:
        logger.debug('node type is "PV" and pv_as_slack is False, creating gen')
        xid = create_gen(
            net, bus1, p_mw=p_mw, vm_pu=vm_set_pu, name=name, in_service=in_service
        )
        elm = "gen"
    else:
        logger.debug(
            "node type is <%s>, pv_as_slack=%s, creating ext_grid"
            % (node_type, pv_as_slack)
        )
        xid = create_ext_grid(
            net,
            bus=bus1,
...
def get_pf_ext_grid_results(net, item, xid, is_unbalanced):
    ...
    if is_unbalanced:
        ...
        result_variables = {"pf_p": "m:P:bus1", "pf_q": "m:Q:bus1"}
...
def map_type_var(pf_load_type):
    load_type = {"3PH PH-E": "wye", "3PH-'YN'": "wye", "3PH-'D'": "delta"}
    return load_type[pf_load_type]


def map_sgen_type_var(pf_sgen_type):
    sgen_type = {0: "wye", 1: "wye", 2: "wye"}
...
def ask_load_params(item, pf_variable_p_loads, dict_net, variables):
    ...
    if pf_variable_p_loads == "m:P:bus1" and not item.HasResults(0):
        raise RuntimeError(
            "load %s does not have results and is ignored" % item.loc_name
        )
    if "p_mw" in variables:
        params.p_mw = item.GetAttribute(pf_variable_p_loads) * multiplier
    if "q_mvar" in variables:
        params.q_mvar = (
            item.GetAttribute(map_power_var(pf_variable_p_loads, "q")) * multiplier
        )
    if "sn_mva" in variables:
        params.sn_mva = (
            item.GetAttribute(map_power_var(pf_variable_p_loads, "s")) * multiplier
        )
    ...
    if item.GetClassName() == "ElmLodlvp":
        ...
        scale = item.scale0
    params.scaling = global_scaling * scale if pf_variable_p_loads == "plini" else 1
...
def ask_unbalanced_load_params(item, pf_variable_p_loads, dict_net, variables):
    ...
    if "q_mvar" in variables:
        params.q_a_mvar = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "q") + "r"
        )
        params.q_b_mvar = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "q") + "s"
        )
        params.q_c_mvar = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "q") + "t"
        )
    if "sn_mva" in variables:
        params.sn_a_mva = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "s") + "r"
        )
        params.sn_b_mva = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "s") + "s"
        )
        params.sn_c_mva = item.GetAttribute(
            map_power_var(pf_variable_p_loads, "s") + "t"
        )
    ...
    global_scaling = dict_net["global_parameters"]["global_load_scaling"]
    params.scaling = (
        global_scaling * item.scale0 if pf_variable_p_loads == "plini" else 1
    )
    if item.HasAttribute("zonefact"):
        params.scaling *= item.zonefact
...
def split_line_add_bus(net, split_dict):
    for line, loads in split_dict.items():
        ...
        for i in range(len(len_sections)):
            ...
            net.line.at[lix, "length_km"] = (
                net.line.at[lix, "length_km"] - len_sections[i]
            )
            pass
...
def split_line_add_bus_old(net, item, parent):
    ...
    if len(sections) == 0:
        ...
        for s in sections:
            logger.debug(
                "section start: %s, load pos: %s, section end: %s"
            ...
            raise RuntimeError(
                "could not find section where ElmLodlvp %s belongs" % item.loc_name
            )
    ...
    if has_sections:
        sid = net.line.loc[
            (net.line.name == parent.loc_name) & (net.line.section == sec.loc_name)
        ].index
    ...
    if len(sid) > 1:
        ...
        for m in range(len(sid)):
            ...
            temp_sec_len = float(
                temp_lines.loc[temp_lines.section_idx == m, "length_km"]
            )
    ...
    logger.debug(
        "total length: %.3f, a: %.3f, b:%.3f" % (tot_len, sec_len_a, sec_len_b)
    )
    if sec_len_b < 0:
    ...
    if sec_len_b > 0:
        ...
        net.bus.loc[bus, "description"] = "Partial load %s = %.3f kW" % (
            item.loc_name,
            item.plini,
        )
...
def create_pp_load(net, item, pf_variable_p_loads, dict_net, is_unbalanced):
    ...
    if load_class == "ElmLodlv":
        try:
            ...
            if bool(
                item.GetAttribute("e:cHasPartLod")
            ):  # item.HasAttribute('e:cHasPartLod'):
            ...
            logger.error(
                "While creating load %s, error occurred for "
        ...
        if load_type is None:
            ...
            used_e_p = set()
            used_e_q = set()
            for cc_p, ee_p, cc_q, ee_q in zip(
                ("aP", "bP", "cP"),
                ("kpu0", "kpu1", "kpu"),
                ("aQ", "bQ", "cQ"),
                ("kqu0", "kqu1", "kqu"),
    ...
    ### added
        ...
        if parent_class == "ElmLodlv":
            ...
            params["parent_load_index"] = net.load.loc[
                net.load.name == parent.loc_name
            ].index.tolist()[0]
            bus_is_known = True
            params["bus"] = net.load.loc[
                net.load.name == parent.loc_name, "bus"
            ].values[0]
            ...
            if item.pnight != 0:
                if item.plini == 0:
                    ...
                    params["p_mw"] = (
                        item.pnight * scale_p_night / 1000
                    )  # is given in kW, we need MW
                else:
                    logger.warning(
        ...
        #     logger.debug('created bus <%d> in net and changed lines' % params.bus)
            ...
            raise NotImplementedError(
                "ElmLodlvp as part of %s not implemented" % parent
            )

    else:
        logger.warning(
            "item <%s> not imported - <%s> not implemented yet!"
            % (item.loc_name, load_class)
        )
        raise RuntimeError(
            "Load <%s> of type <%s> not implemented!" % (item.loc_name, load_class)
        )
    ...
    try:
        ...
        logger.error(
            "While creating %s.%s, error occured: %s" % (params.name, load_class, err)
        )
        raise err
    ...
    if load_class == "ElmLodlv" or load_class == "ElmLodlvp":
        attr_list.extend(
            ["pnight", "cNrCust", "cPrCust", "UtilFactor", "cSmax", "cSav", "ccosphi"]
        )
        attr_dict["cpGrid.loc_name"] = "grid"
    add_additional_attributes(
        item, net, load_type, ld, attr_dict=attr_dict, attr_list=attr_list
    )
...
def ask_gen_params(item, pf_variable_p_gen, *vars):
    ...
    if pf_variable_p_gen == "m:P:bus1" and not item.HasResults(0):
        raise RuntimeError(
            "generator %s does not have results and is ignored" % item.loc_name
        )
    if "p_mw" in vars:
        params.p_mw = item.GetAttribute(pf_variable_p_gen) * multiplier
    if "q_mvar" in vars:
        params.q_mvar = (
            item.GetAttribute(map_power_var(pf_variable_p_gen, "q")) * multiplier
        )
    if "sn_mva" in vars:
        params.sn_mva = (
            item.GetAttribute(map_power_var(pf_variable_p_gen, "sn")) * multiplier
        )
...
def ask_unbalanced_sgen_params(item, pf_variable_p_sgen, *vars):
    ...
    if pf_variable_p_sgen == "m:P:bus1" and not item.HasResults(0):
        raise RuntimeError(
            "sgen %s does not have results and is ignored" % item.loc_name
        )
    ...
    if technology in [0, 1]:  # (0-1: 3PH)
        ...
        if "q_mvar" in vars:
            params.q_a_mvar = (
                item.GetAttribute(map_power_var(pf_variable_p_sgen, "q")) / 3
            )
            params.q_b_mvar = (
                item.GetAttribute(map_power_var(pf_variable_p_sgen, "q")) / 3
            )
...
def create_sgen_genstat(
    net, item, pv_as_slack, pf_variable_p_gen, dict_net, is_unbalanced, export_ctrl
):
    params = ADict()
    ...
    if is_reference_machine or (av_mode == "constv" and pv_as_slack):
        ...
        sg = create_ext_net(
            net, item=item, pv_as_slack=pv_as_slack, is_unbalanced=is_unbalanced
        )
        element = "ext_grid"
    else:
        try:
        ...
        if item.GetClassName() == "ElmPvsys":
            ...
            try:
                ...
                cat = None
                logger.debug(
                    "sgen <%s> with category <%s> imported as <%s>"
            ...
            if av_mode == "vdroop":
                ...
                if not item.ddroop == 0:
                    bsc = BinarySearchControl(
                        net,
                        name=item.loc_name + "_ctrl",
                        ctrl_in_service=not item.outserv,
                        output_element="gen",
                        output_variable="vm_pu",
                        output_element_index=[next_index],
                        output_element_in_service=[not item.outserv],
                        output_values_distribution=[1],
                        input_element="res_gen",
                        input_variable="q_mvar",
                        input_inverted=[False],
                        gen_Q_response=[1],
                        input_element_index=[next_index],
                        set_point=item.usetp,
                        voltage_ctrl=True,
                        bus_idx=bus,
                        tol=1e-5,
                    )
                    VDroopControl_local(
                        net,
                        name=item.loc_name + "_ctrl",
                        q_droop_mvar=item.sgn * 100 / ddroop,
                        q_set_mvar=item.qgini,
                        vm_set_pu_bsc=item.usetp,
                        bus_idx=bus,
    ...
    net[element].at[sg, "scaling"] = (
        dict_net["global_parameters"]["global_generation_scaling"] * item.scale0
    )
    ...
    logger.debug(
        "created genstat <%s> as element <%s> at index <%d>"
        % (params.name, element, sg)
    )
    ...
    for res_var_pp, res_var_pf in result_variables.items():
        ...
        if item.HasResults(0):
            if res_var_pf is not None:
                res = item.GetAttribute(res_var_pf) * get_power_multiplier(
                    item, res_var_pf
                )
            else:
                res = np.nan
...
def create_sgen_sym(net, item, pv_as_slack, pf_variable_p_gen, dict_net, export_ctrl):
    ...
    global_scaling = (
        dict_net["global_parameters"]["global_motor_scaling"]
        if is_motor
        else dict_net["global_parameters"]["global_generation_scaling"]
    )
    multiplier = get_power_multiplier(item, pf_variable_p_gen)
    ...
    if is_reference_machine or (av_mode == "constv" and pv_as_slack):
        ...
        logger.debug(
            "ref. machine: %d, av_mode: %s, pv as slack: %s"
            % (is_reference_machine, av_mode, pv_as_slack)
        )
        sid = create_ext_net(
            net, item=item, pv_as_slack=pv_as_slack, is_unbalanced=False
        )
        ...
        logger.debug(
            "sgen <%s> is a %s" % (name, {True: "motor", False: "generator"}[is_motor])
        )
        ...
        if av_mode == "constv":
            ...
            if item.iqtype == 1:
                ...
                sid = create_gen(
                    ...
                    p_mw=p_mw,
                    vm_pu=vm_pu,
                    min_q_mvar=type.Q_min,
                    max_q_mvar=type.Q_max,
                    min_p_mw=item.Pmin_uc,
                    max_p_mw=item.Pmax_uc,
                    name=name,
                    type=cat,
                    in_service=in_service,
                ...
                sid = create_gen(
                    ...
                    p_mw=p_mw,
                    vm_pu=vm_pu,
                    min_q_mvar=item.cQ_min,
                    max_q_mvar=item.cQ_max,
                    min_p_mw=item.Pmin_uc,
                    max_p_mw=item.Pmax_uc,
                    name=name,
                    type=cat,
    ...
    if item.HasResults(0):  # 'm' results...
        logger.debug("<%s> has results" % name)
        net["res_" + element].at[sid, "pf_p"] = (
            item.GetAttribute("m:P:bus1") * multiplier
        )
        net["res_" + element].at[sid, "pf_q"] = (
            item.GetAttribute("m:Q:bus1") * multiplier
        )
    else:
...
def create_sgen_asm(net, item, pf_variable_p_gen, dict_net):
    ...
    if item.HasResults(0):
        ...
        logger.warning(
            "reactive power for asynchronous generator is not exported properly "
    ...
    add_additional_attributes(
        item, net, "sgen", sid, attr_dict=attr_dict, attr_list=["sernum", "chr_name"]
    )
...
def create_trafo_type(net, item):
    ...
    if item.itapch:
        ...
        if item.tapchtype == 2:
            logger.warning(
                "trafo %s has symmetrical tap changer (tap changer at both hv and "
    ...
    if item.itapch and item.itapch2:
        ...
        type_data.update(
            {
                ...
                "tap2_step_degree": item.dphitap2
                if item.tapchtype2 == 1
                else item.phitr2,
        ...
        if item.tapchtype2 == 2:
            logger.warning(
                "trafo %s has symmetrical tap2 changer (tap2 changer at both hv and "
                "lv side) - not implemented, importing as asymmetrical tap2 changer at "
                "side %s. Results will differ."
                % (item.loc_name, type_data["tap2_side"])
            )

    if "tap_side" in type_data.keys() and (
        type_data.get("tap_side") == "ext" or type_data.get("tap_side") == "ext"
    ):
        logger.warning(
            'controlled node of trafo "EXT" not implemented (type <%s>)' % name
        )
...
def create_trafo(
    ...
    if pf_type.itapch:
        if tap_opt == "nntap":
            ...
            raise ValueError(
                "could not read current tap position: tap_opt = %s" % tap_opt
            )
    ...
    if std_type is not None:
        if use_tap_table == 1:
            (
                ...
                tap_side,
            ) = create_trafo_characteristics_from_measurement_protocol(
                item, net, pf_type
            )
        else:
        ...
        tid = create_transformer(
            ...
            df=item.ratfac,
            tap2_pos=tap_pos2,
            leakage_resistance_ratio_hv=pf_type.itrdr,
            leakage_reactance_ratio_hv=pf_type.itrdl,
        )
        trafo_dict[item] = tid
    ...
    new_switch_idx, new_switch_closed = create_connection_switches(
        net, item, 2, "t", (bus1, bus2), new_elements
    )
    ...
    if (
        export_controller
        and pf_type.itapch
        and item.HasAttribute("ntrcn")
        and item.HasAttribute("i_cont")
        and item.ntrcn == 1
    ):
        if item.t2ldc == 0:
        ...
        if item.i_cont == 1:
            vm_set_pu = item.usetp
            logger.debug(
                "trafo <%s> has continuous tap controller with vm_set_pu = %.3f, side = %s"
                % (name, vm_set_pu, side)
            )
            try:
                ...
                logger.error(
                    "error while creating continuous tap controller at trafo <%s>"
                    % name
                )
            ...
            logger.debug(
                "trafo <%s> has discrete tap controller with "
                "u_low = %.3f, u_up = %.3f, side = %s"
                % (name, vm_lower_pu, vm_upper_pu, side)
            )
            try:
                DiscreteTapControl(
                    net,
                ...
                logger.error(
                    "error while creating discrete tap controller at trafo <%s>" % name
                )
                logger.error("Error: %s" % err)
            else:
                logger.debug("created discrete tap controller at trafo <%s>" % name)
...
def add_tap_dependent_impedance_for_trafo(item, net, pf_type, tid):
    ...
    vk_points = SplineCharacteristic(
        net, x_points, vk_values, table="temporary_characteristics"
    )(steps)
    vkr_points = SplineCharacteristic(
        net, x_points, vkr_values, table="temporary_characteristics"
    )(steps)
    ...
    index = (
        int(net.trafo_characteristic_table.iat[-1, 0] + 1)
        if not net["trafo_characteristic_table"].empty
        else int(0)
    )
    ...
    if pf_type.tapchtype == 1:  # if tapchanger is Ideal
        ...
        voltage_ratio = (
            np.sqrt(
                (u1 + du * cos(tap_step_degree)) ** 2 + (du * sin(tap_step_degree)) ** 2
            )
            / u1
        )
        angle_deg = arctan(
            direction * du * sin(tap_step_degree) / (u1 + du * cos(tap_step_degree))
        )
    ...
    net["trafo_characteristic_table"] = pd.concat(
        [net["trafo_characteristic_table"], new_tap_table], ignore_index=True
    )

    # Update transformer attributes
    net.trafo.loc[
        tid, ["tap_dependency_table", "id_characteristic_table", "tap_changer_type"]
    ] = [True, index, "Tabular"]
...
def create_trafo_characteristics_from_measurement_protocol(item, net, pf_type):
    last_index = (
        net["trafo_characteristic_table"]["id_characteristic"].max()
        if not net["trafo_characteristic_table"].empty
        else -1
    )
    ...
    new_tap_table = pd.DataFrame(
        item.GetAttribute("mTaps"),
        columns=["voltage_ratio", "angle_deg", "vk_percent", "vkr_percent", "ignore"],
    )
    new_tap_table = new_tap_table.drop(columns="ignore")
    if meas_side == 0:
        if tap_side == 0:
            new_tap_table["voltage_ratio"] = (
                new_tap_table["voltage_ratio"] / pf_type.utrn_h
            )
        else:
            new_tap_table["voltage_ratio"] = (
                new_tap_table["voltage_ratio"] / pf_type.utrn_l
            )
    elif meas_side == 1:
        new_tap_table["angle_deg"] = -new_tap_table["angle_deg"]
        if tap_side == 0:
            new_tap_table["voltage_ratio"] = (
                pf_type.utrn_l / new_tap_table["voltage_ratio"]
            )
        else:
            new_tap_table["voltage_ratio"] = (
                pf_type.utrn_h / new_tap_table["voltage_ratio"]
            )
    else:
        raise ValueError("Measurement location for tap table not given.")
    new_tap_table["vkr_percent"] = (
        new_tap_table["vkr_percent"] / pf_type.strn / 1000 * 100
    )
    ...
    if len(new_tap_table) == len(steps):
        ...
        raise ValueError(
            "The number of steps differs from the number of rows in new_tap_table."
        )
    new_tap_table["id_characteristic"] = new_id_characteristic_table
    missing_columns = set(net["trafo_characteristic_table"].columns) - set(
        new_tap_table.columns
    )
...
def create_trafo3w(net, item, tap_opt="nntap"):
    ...
    if item.nt3nm != 1:
        logger.warning(
            "trafo3w %s has parallel=%d, this is not implemented. "
    ...
    if use_tap_table == 1:
        ...
        if table_side == 0:
            ...
            if meas_side == 0:
                new_tap_table["voltage_ratio"] = (
                    new_tap_table["voltage_ratio"] / pf_type.utrn3_h
                )
            elif meas_side == 1:
                new_tap_table["voltage_ratio"] = (
                    new_tap_table["voltage_ratio"] / pf_type.utrn3_m
                )
            elif meas_side == 2:
                new_tap_table["voltage_ratio"] = (
                    new_tap_table["voltage_ratio"] / pf_type.utrn3_l
                )
        elif table_side == 1:
        ...
        new_tap_table["vkr_hv_percent"] = (
            new_tap_table["vkr_hv_percent"]
            / (np.min([float(strn3_h), float(strn3_m)]) * 1000)
            * 100
        )
        new_tap_table["vkr_mv_percent"] = (
            new_tap_table["vkr_mv_percent"]
            / (np.min([float(strn3_m), float(strn3_l)]) * 1000)
            * 100
        )
        new_tap_table["vkr_lv_percent"] = (
            new_tap_table["vkr_lv_percent"]
            / (np.min([float(strn3_h), float(strn3_l)]) * 1000)
            * 100
        )
    ...
    if item.HasAttribute("t:du3tp_h"):
        ...
        if len(side) > 1:
            logger.warning(
                "pandapower currently doesn't support 3w transformer with"
            ...
            # ideal phase shifters
                ...
                tap_step_degree is not None
                and tap_step_degree != 0
                and (tap_step_percent is None or tap_step_percent == 0)
            ):
                tap_changer_type = "Ideal"
    ...
    new_switch_idx, new_switch_closed = create_connection_switches(
        net, item, 3, "t3", (bus1, bus2, bus3), new_elements
    )
...
def add_tap_dependant_impedance_for_trafo3W(net, pf_type, tid):
    ...
    side, vn, tap_step_percent, tap_step_degree, tap_neutral, tap_min, tap_max = (
        tp_map.get(pf_type.itapzside, tp_map[2])
    )
    ...
    voltage_ratio = (
        np.sqrt(
            (vn + du * cos(tap_step_degree)) ** 2 + (du * sin(tap_step_degree)) ** 2
        )
        / vn
    )
    angle_deg = arctan(
        direction * du * sin(tap_step_degree) / (vn + du * cos(tap_step_degree))
    )
    angle_deg[angle_deg == -0] = 0.0
    ...
    for side in ("h", "m", "l"):
        vk_points = SplineCharacteristic(
            ...
            table="temporary_characteristics",
        )(steps)
...
def create_coup(net, item, is_fuse=False):
    ...
    switch_is_closed = bool(item.on_off) and (
        bool(item.isclosed) if item.HasAttribute("isclosed") else True
    )
    ...
    logger.debug(
        "created switch at index <%d>, closed = %s, usage = %s"
...
def create_pp_shunt(net, item):
    ...
    if use_tap_table == 1:
        ...
        last_index = (
            net["shunt_characteristic_table"]["id_characteristic"].max()
            if not net["shunt_characteristic_table"].empty
            else -1
        )
        ...
        new_tap_table = pd.DataFrame(
            item.GetAttribute("mTaps"), columns=["q_mvar", "p_mw"]
        )
        ...
        new_tap_table["p_mw"] = np.where(
            new_tap_table["p_mw"] == 0,
            0,
            new_tap_table["q_mvar"] / new_tap_table["p_mw"],
        )
        new_tap_table["id_characteristic"] = id_characteristic_table

        net["shunt_characteristic_table"] = pd.concat(
            [net["shunt_characteristic_table"], new_tap_table], ignore_index=True
        )
    else:
        use_tap_table = 0
        id_characteristic_table = None
    ...
    def calc_p_mw_and_q_mvar(r: float, x: float) -> tuple[float, float]:
        ...
        return (item.ushnm**2 * r) / divisor * multiplier, (
            item.ushnm**2 * x
        ) / divisor * multiplier
    ...
    if 0 <= item.shtype <= 4:
        if not use_tap_table:
            ...
            p_mw = (
                new_tap_table.loc[new_tap_table["step"] == item.ncapa, "p_mw"].values[0]
                / item.ncapa
            )
            params["q_mvar"] = (
                new_tap_table.loc[new_tap_table["step"] == item.ncapa, "q_mvar"].values[
                    0
                ]
                / item.ncapa
            )
...
def create_zpu(net, item):
    ...
    aux_bus1 = create_bus(
        net,
        vn_kv=net.bus.vn_kv.at[bus1],
        name=net.bus.name.at[bus1] + "_aux",
        type="b",
        zone=net.bus.zone.at[bus1],
        in_service=True,
    )
    ...
    aux_bus2 = create_bus(
        net,
        vn_kv=net.bus.vn_kv.at[bus2],
        name=net.bus.name.at[bus2] + "_aux",
        type="b",
        zone=net.bus.zone.at[bus2],
        in_service=True,
    )
    ...
    new_switch_idx, new_switch_closed = create_connection_switches(
        net, item, 2, "b", (bus1, bus2), new_elements
    )
    ...
    logger.debug(
        "created ZPU %s as impedance at index %d" % (net.impedance.at[xid, "name"], xid)
    )
...
def create_vac(net, item):
    ...
    if item.itype == 3:
        # extended ward
        params.update({"r_ohm": item.Rext, "x_ohm": item.Xext, "vm_pu": item.usetp})

        if params["x_ohm"] == 0:
            params["x_ohm"] = 1e-6
            logger.warning(
                "Element %s has x_ohm == 0, setting to 1e-6. Check impedance of the "
    ...
    logger.debug(
        "added pf_p and pf_q to {} {}: {}".format(
            elm, xid, net["res_" + elm].loc[xid, ["pf_p", "pf_q"]].values
        )
    )


def update_in_service_depending_station_switch(
    net, element_type, new_elements, new_switch_idx, new_switch_closed
):
    ### fcn is not used!
    if len(new_switch_idx) != 0:
        for i in range(len(new_switch_idx)):
            if new_switch_closed[i] == 0:
                if net[element_type].loc[new_elements[i], "in_service"] == False:
                    ...
                    logger.debug(
                        "element of element_type %s with index %d is set\
                                 out of service because station switch is open "
                        % (
                            net[element_type].at[new_elements[i], "name"],
                            new_elements[i],
                        )
                    )
    else:
...
def create_sind(net, item):
    ...
    logger.debug(
        "created series reactor %s as per unit impedance at index %d"
...
def create_scap(net, item):
    ...
    if (item.gcap == 0) and (item.bcap == 0):
        ...
        scap = create_series_reactor_as_impedance(
            net,
            from_bus=aux_bus1,
            to_bus=aux_bus2,
            r_ohm=r_ohm,
        ...
        logger.debug(
            "created series capacitor %s as per unit impedance at index %d"
...
def create_svc(net, item, pv_as_slack, pf_variable_p_gen, dict_net):
    ...
    if item.i_ctrl == 1:  # 0: no control, 1: voltage control, 2: reactive power control
        ...
        if item.HasResults(0):  # 'm' results...
            logger.debug("<%s> has results" % name)
            net["res_" + element].at[svc, "pf_p"] = item.GetAttribute(
                "m:P:bus1"
            )  # * multiplier
            net["res_" + element].at[svc, "pf_q"] = item.GetAttribute(
                "m:Q:bus1"
            )  # * multiplier
        else:
...
def _get_vsc_control_modes(item, mono=True):
    ...
    if c_m not in [0, 3, 4, 5, 6]:
        raise NotImplementedError(
            f"control mode for vscmono {item.loc_name} not implemented: {c_m}"
        )
    ...
    if item.HasResults(0):
        ...
        p_set_dc = (
            -item.psetp * scaling
        )  # does not work - in PowerFactory, the P set-point relates to AC side
        q_set_ac = -item.qsetp * scaling

    control_mode_ac, control_mode_dc, control_value_ac, control_value_dc = {
        0: ("slack", "p_mw", item.usetp, 0.0),  # Vac-phi
...
def create_vscmono(net, item):
    ...
    logger.debug(
        f"VSCmono: {item.loc_name=}, {sn_mva=}, {v_ac=}, {p_cu_kw=}, {vk=}, {r_ohm=}, {x_ohm=}"
    )

    control_mode_ac, control_mode_dc, control_value_ac, control_value_dc = (
        _get_vsc_control_modes(item)
    )
...
def create_vsc(net, item):
    ...
    logger.debug(
        f"VSC: {item.loc_name=}, {sn_mva=}, {v_ac=}, {p_cu_kw=}, {vk=}, {r_ohm=}, {x_ohm=}"
    )

    control_mode_ac, control_mode_dc, control_value_ac, control_value_dc = (
        _get_vsc_control_modes(item, mono=False)
    )
    ...
    if params["r_dc_ohm"] == 0:
        logger.warning(
            f"VSC element {params['name']} has no DC resistive loss factor - power flow will not converge!"
        )
    ...
    result_variables = {"pf_p_mw": "m:P:busac", "pf_q_mvar": "m:Q:busac"}
...
def create_stactrl(net, item):
    ...
    if len(machines) == 0 or np.all([s is None for s in machines]):
        logger.error(
            f"No machines controlled by station controller {item.loc_name} - skipping"
        )
        return
    ...
    if duplicated_sgen_names == False:
        for s in machines:
            gen_element_index.append(
                net[gen_element]
                .loc[net[gen_element].name == s.loc_name]
                .index.values[0]
            )
    else:
        # check if gen_element has set controller
        for s in machines:
            gen_element_index_try = (
                net[gen_element].loc[net[gen_element].name == s.loc_name].index.values
            )
            if len(gen_element_index_try) == 1:
                gen_element_index.append(gen_element_index_try[0])
            else:
                gen_element_index_try_again = (
                    net[gen_element]
                    .loc[
                        (net[gen_element].name == s.loc_name)
                        & (net[gen_element].sta_ctrl == s.c_pstac.loc_name)
                    ]
                    .index.values
                )
    ...
    if len(gen_element_index) != len(machines):
        raise UserWarning(
            "station controller: could not properly identify the machines"
        )

    gen_element_in_service = [
        net[gen_element].loc[net[gen_element].name == s.loc_name].in_service
        for s in machines
    ]
    ...
    if item.imode > 2:
        raise NotImplementedError(
            f"{item}: reactive power distribution {item.imode=} not implemented"
        )
    ...
    if item.selBus != 0:
        raise NotImplementedError(
            f"{item}: controlled node selection {item.selBus=} not implemented"
        )
    ...
    if control_mode == 1 or item.i_droop:
        ...
        if q_control_cubicle is None:
            logger.info(
                f"Input Element of Controller {item.loc_name} is missing, skipping"
            )
            return
        q_control_element = []
        q_control_side = []
        ...
        if q_control_cubicle.GetClassName() == "StaCubic":
            ...
            q_control_side.append(
                q_control_cubicle.obj_bus
            )  # 0=from, 1=to // trafo3w 0=HV, 1 = MV, 2=LV
        ...
        if element_class[0] == "ElmLne":
            ...
            for element in q_control_element:
                ...
                gen_dist_lv_bus = nx.shortest_path_length(
                    top_all,
                    get_element_bus(net, gen_element, gen_element_index[0]),
                    net.trafo.loc[res_element_index[-1]].lv_bus,
                )
                gen_dist_hv_bus = nx.shortest_path_length(
                    top_all,
                    get_element_bus(net, gen_element, gen_element_index[0]),
            ...
            for element in q_control_element:
                ...
                gen_dist_t3w_lv_bus = nx.shortest_path_length(
                    top_all,
                    get_element_bus(net, gen_element, gen_element_index[0]),
                    net.trafo3w.loc[res_element_index[-1]].lv_bus,
                )
                gen_dist_t3w_mv_bus = nx.shortest_path_length(
                    top_all,
                    get_element_bus(net, gen_element, gen_element_index[0]),
                    net.trafo3w.loc[res_element_index[-1]].mv_bus,
                )
                gen_dist_t3w_hv_bus = nx.shortest_path_length(
                    top_all,
                    get_element_bus(net, gen_element, gen_element_index[0]),
            ...
            for element in q_control_element:
                variable.append(
                    "q_from_mvar" if q_control_side[0] == 0 else "q_to_mvar"
                )
                if q_control_side[i] == 0:
            ...
            for element in q_control_element:
                if q_control_side[0] == 0:
                    gen_dist_bus = nx.shortest_path_length(
                        top_all,
                        get_element_bus(net, gen_element, gen_element_index[0]),
                        net.switch.loc[switch_dict[element], "bus"],
                    )
                    gen_dist_element = nx.shortest_path_length(
                        top_all,
                        get_element_bus(net, gen_element, gen_element_index[0]),
                ...
                if res[switch_dict[element]] is not None:
                ...
                if element_type == "trafo":
                    ...
                    variable.append(
                        {"hv_bus": "q_hv_mvar", "lv_bus": "q_lv_mvar"}[connection_side]
                    )
                    ...
                    input_inverted.append(direction == q_control_side[0])
                    variable.append(
                        "q_from_mvar" if connection_side == "from_bus" else "q_to_mvar"
                    )
    ...
    if control_mode == 0:  # VOLTAGE CONTROL
        ...
        raise NotImplementedError(
            f"{item}: control mode {item.i_ctrl=} not implemented"
        )
    ...
    def find_branch(graph, start_bus, blocked_bus):
        ...
        while queue:
            ...
            neighbors = [
                ...
                if n != blocked_bus and n not in visited
            ]
            ...
            if len(neighbors) == 1 or valid_t3w_neighbors:
                ...
                par_imp = net.impedance[
                    ...
                    | (
                        (net.impedance.from_bus == nb)
                        & (net.impedance.to_bus == current)
                    )
                ]
                ...
                # line match
                line_match = net.line[
                    (net.line.from_bus == current) | (net.line.to_bus == current)
                ]
                ...
                # transformer match
                trafo_match = net.trafo[
                    (net.trafo.hv_bus == current) | (net.trafo.lv_bus == current)
                ]
                ...
                # impedance match
                impedance_match = net.impedance[
                    (net.impedance.from_bus == current)
                    | (net.impedance.to_bus == current)
                ]
                    ...
                    if elm in net:
                        ...
                        if isinstance(df, pd.DataFrame):
                            if "bus" in df.columns and current in df.bus.values:
                                ...
                                if (
                                    current in df.get("from_bus", pd.Series()).values
                                    or current in df.get("to_bus", pd.Series()).values
                                ):
                                    elements_at_bus.append(elm)
                                    break
                if elements_at_bus:
                    logger.warning(
                        f"Bus {current} is connected with {elements_at_bus} – Stopping."
                    )
                    return None
...
def split_line_at_length(net, line, length_pos):
    ...
    if length_pos == net.line.at[line, "length_km"]:
        ...
        if "max_loading_percent" in net.line.columns:
            net.line.loc[new_line, "max_loading_percent"] = net.line.at[
        ...
        if net.line.loc[line, "geo"].notna():
            ...
            sec_coords_a = get_section_coords(
                coords, sec_len=length_pos, start_len=0.0, scale_factor=scaling_factor
            )
            sec_coords_b = get_section_coords(
                coords,
                sec_len=new_length,
                start_len=length_pos,
                scale_factor=scaling_factor,
            )

            net.line.loc[line, "geo"] = geojson.dumps(geojson.LineString(sec_coords_a))
            net.line.loc[new_line, "geo"] = geojson.dumps(
                geojson.LineString(sec_coords_b)
            )
...
def get_next_line(net, line):
    ...
    next_line = net.line.loc[
        (net.line.name == name) & (net.line.from_bus == to_bus)
    ].index
    ...
    if pos_at_line < tol:
        ...
        raise ValueError(
            ...
            % (
                ...
                line_length,
                pos_at_line,
                net.line.loc[line_dict[line_item]],
            )
        )
    else:
        ...
        net.res_line.at[new_line, "pf_loading"] = net.res_line.at[
            line_idx, "pf_loading"
        ]
...
def set_new_coords(net, bus_id, line_idx, new_line_idx, line_length, pos_at_line):
    ...
    if np.isnan(scale_factor_length):
        logger.warning(
            "Could not generate geodata for line sections (partial loads on line)!"
        )
    else:
        section_coords, new_coords = break_coords_sections(
            line_coords, pos_at_line, scale_factor_length
        )
...
def get_lvp_for_lines(dict_net):
    ...
    lvp_items = [
        lvp for lvp in dict_net["ElmLodlvp"] if lvp.fold_id.GetClassName() == "ElmLne"
    ]
    ...
    for line in line_items:
        ...
        lvp_dict[line] = [
            (lvp, lvp.lneposkm, calc_p_q(lvp, dict_net["lvp_params"]))
...
def get_pos_at_sec(net, lvp_dict, line_item, load_item):
    ...
    for section in line_sections.index:
        ...
        logger.debug(
            "section: %s, sec_length: %s, tot_length: %s"
            % (section, sec_length, tot_length)
        )
    ...
    for line_item, val in lvp_dict.items():
        ...
        for load_item, pos_at_line, (p, q) in val:
            ...
            if not net.line.at[section, "in_service"]:
                print(
                    "line %s skipped because it is not in service"
                    % net.line.at[section, "name"]
                )
                continue
            ...
            if p >= 0 or True:
                new_load = create_load(
                    net,
                    new_bus,
                    name=load_item.loc_name,
                    p_mw=p,
                    q_mvar=q,
                    const_i_p_percent=0,
                    const_i_q_percent=0,
                ...
                new_load = create_sgen(
                    net, new_bus, name=load_item.loc_name, p_mw=p, q_mvar=q
                )
...
def remove_folder_of_std_types(net):
    ...
    for element in ["line", "trafo", "trafo3w"]:
        ...
        for std_type in reduced_std_types:
            ...
            if len(all_types) > 1:
                types_equal = [
                    load_std_type(net, type1, element)
                    == load_std_type(net, type2, element)
...
def create_q_capability_curve(net, item):
    ...
    index = (
        net.q_capability_curve_table.iat[-1, 0] + 1
    ...
    net["q_capability_curve_table"] = pd.concat(
        [net["q_capability_curve_table"], new_data], ignore_index=True
    )
    return index
