...
def run_load_flow(
    ...
    if com_ldf.iopt_sim == 1:
        logger.warning(
            "Calculation method probabilistic loadflow of lv-loads is activated!"
    ...
    logger.info(
        "Calculation method (AC balanced, AC unbalanced): %s" % com_ldf.iopt_net
    )
    logger.info(
        f"Calculation method probabilistic loadflow of lv-loads: {com_ldf.iopt_sim}"
    )
    ...
    if res != 0:
        logger.error(
            "Load flow failed due to divergence of %s loops"
            % ["inner", "outer"][res - 1]
        )
    #        raise RuntimeError(
...
def setup_unit_exponents(prj, elm_class, unit, exponent):
    ...
    logger.debug(
        "setting unit exponents: %s, %s, %s, %s" % (elm_class, unit, cdigexp, exponent)
    )
    ...
    try:  # version-specific expectation at this point
        unit_setting.filtclass = (
            elm_class if isinstance(elm_class, list) else [elm_class]
        )
    except TypeError:
        unit_setting.filtclass = (
            elm_class[0] if isinstance(elm_class, list) else elm_class
        )
