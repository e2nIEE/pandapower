# este arquivo é diferente do pandapower original
import json
import math
import os
import tempfile
...
def _pp_element_to_pm(net, pm, element, pd_bus, qd_bus, load_idx, max_bus_id_on_ppci):
    ...
    for idx in net[element].index:
        ...
        if pm_bus <= max_bus_id_on_pm:
            ...
            if element == "sgen":
                pd = -net[element].at[idx, "p_mw"] * scaling # / sn_mva testar
                qd = -net[element].at[idx, "q_mvar"] * scaling # / sn_mva  testar
            else:
                pd = net[element].at[idx, "p_mw"] * scaling # / sn_mva  testar
                qd = net[element].at[idx, "q_mvar"] * scaling # / sn_mva  testar
...
def ppc_to_pm(net, ppci):
    ...
    for row in ppci["bus"]:
        ...
        bs = row[BS] / baseMVA # to be validated # (ou / sn_mva)  # testar
        gs = row[GS] / baseMVA # to be validated # (ou / sn_mva)  # testar
    ...
    for idx, row in enumerate(ppci["branch"], start=1):
        ...
        if net._options["opf_flow_lim"] == "S":  # or branch["transformer"]:
            branch["rate_a"] = (row[RATE_A].real if row[RATE_A] > 0 else row[RATE_B].real) # / sn_mva  testar
            branch["rate_b"] = row[RATE_B].real # / sn_mva  testar
            branch["rate_c"] = row[RATE_C].real # / sn_mva  testar
            ...
            # branch["c_rating_a"] = row[RATE_A].real / sn_mva if row[RATE_A] > 0 else row[RATE_B].real / sn_mva  testar
            branch["c_rating_a"] = row[RATE_A].real if row[RATE_A] > 0 else row[RATE_B].real
            branch["c_rating_a"] = branch["c_rating_a"]
            branch["c_rating_b"] = row[RATE_B].real # / sn_mva testar
            branch["c_rating_c"] = row[RATE_C].real # / sn_mva testar
        else:
