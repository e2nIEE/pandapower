...
def _extract_result_ppci_to_pp(net, ppc, ppci):
    ...
    net.res_bus_est.p_mw = get_values(
        ppc["bus"][:, 2], net.bus.index.values, mapping_table
    )
    net.res_bus_est.q_mvar = get_values(
        ppc["bus"][:, 3], net.bus.index.values, mapping_table
    )
    ...
    for element in ["shunt", "ward", "xward"]:
        if ~net[element].empty:
            ...
            if element == "shunt":
                ...
                Sn += net[element].p_mw.values
                Sn *= net[element].step.values
                Ysh = Sn / ((net[element].vn_kv.values) ** 2)
            else:
                Sn = 1j * (net[element].qz_mvar.values)
                Sn += net[element].pz_mw.values
                Ysh = Sn / ((net.bus.loc[bus, "vn_kv"].values) ** 2)
            V = (
                net["res_bus_est"].loc[bus, "vm_pu"].values
                * net["bus"].loc[bus, "vn_kv"].values
            )
            Sinj = Ysh * (V**2)
