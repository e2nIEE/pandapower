...
class WLSAlgebraNew:
    ...
    def create_hx(self, v, delta, meas_mask):
        hx_valid_meas_mask = meas_mask ## TODO: ADD
        ...
        hx = np.concatenate([np.real(Sbuse),
                     np.real(Sfe), 
                     np.real(Ste),
                     np.imag(Sbuse),
                     np.imag(Sfe),
                     ...
                     Ife,
                     Ite])
        return hx[hx_valid_meas_mask]
    
    def _dSbus_dv(self, V, diagV, diagVnorm):
        diagIbus = np.diag(self.Ybus * V)
        dSbus_dth = diagV * np.conj(np.dot(self.Ybus, diagVnorm)) + np.dot(np.conj(diagIbus), diagVnorm)
        dSbus_dv = 1j * diagV * np.conj(diagIbus - np.dot(self.Ybus, diagV))
        return dSbus_dth, dSbus_dv
        
    def _dSbr_dv(self, V, diagV, diagVnorm):
        # TODO! 
        n_bus = self.ppci['bus'].shape[0]
        f_bus, t_bus = self.fb, self.tb
    ...
    def create_h_jacobian(self, v, delta, meas_mask):
        ...
        h_jac_th = np.concatenate([np.real(dSbus_dth),
                                   np.real(dSf_dth),
                                   np.real(dSt_dth),
                                   np.imag(dSbus_dth),
                                   np.imag(dSf_dth),
                                   ...
                                   dif_dth,
                                   dit_dth])
        h_jac_v = np.concatenate([np.real(dSbus_dv),
                                  np.real(dSf_dv),
                                  np.real(dSt_dv),
                                  np.imag(dSbus_dv),
                                  np.imag(dSf_dv),
