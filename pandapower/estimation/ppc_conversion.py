...
# Constant Lookup
BR_SIDE = {"line": {"f": "from", "t": "to"}, "trafo": {"f": "hv", "t": "lv"}}
BR_MEAS_PPCI_IX = {
    ("p", "f"): {"VALUE": P_FROM, "IDX": P_FROM_IDX, "STD": P_FROM_STD},
...
BUS_MEAS_PPCI_IX = {
    "v": {"VALUE": VM, "IDX": VM_IDX, "STD": VM_STD},
...
def _add_measurements_to_bus(meas_bus, bus_append, map_bus):
    ...
    for meas_type in ("v", "va"):
        ...
        bus_append[meas_merged.index, BUS_MEAS_PPCI_IX[meas_type]["VALUE"]] = (
            meas_merged.weighted_measurement
        )
        bus_append[meas_merged.index, BUS_MEAS_PPCI_IX[meas_type]["STD"]] = (
            meas_merged.merged_weight
        )
        bus_append[meas_merged.index, BUS_MEAS_PPCI_IX[meas_type]["IDX"]] = meas_merged[
            "index"
        ]
    ...
    for meas_type in ("p", "q"):
        ...
        bus_append[merged_value.index, BUS_MEAS_PPCI_IX[meas_type]["VALUE"]] = (
            merged_value.sum_values
        )
        bus_append[merged_value.index, BUS_MEAS_PPCI_IX[meas_type]["STD"]] = (
            merged_value.sum_std_dev
        )
        bus_append[merged_value.index, BUS_MEAS_PPCI_IX[meas_type]["IDX"]] = (
            merged_value["index"]
        )
...
def _add_rated_power_information_af_wls(net, ppci):
    ...
    for var in ["load", "sgen"]:
        ...
        if (bus >= ppci["bus"].shape[0]).any():
            std_logger.warning(
                "Loads or sgen defined in pp-grid do not exist in ppci, will be deleted!"
            )
        ...
        for i in range(len(P)):
            ...
            ppci["rated_power_clusters"][bus_i, cluster_i + 2 * num_clusters] += abs(
                0.03 * P_i
            )  # std dev cluster variability hardcoded, think how to change it
...
def _add_measurements_to_ppci(net, ppci, zero_injection, algorithm):
    ...
    if meas.empty:
        raise Exception(
            "No measurements are available in pandapower Network! Abort estimation!"
        )
    ...
    if not i_meas.empty:
        ...
        i_meas["side"] = i_meas.apply(
            lambda row: net["line"].at[row["element"], row["side"] + "_bus"]
            if row["side"] in ("from", "to")
            else net[row["element_type"]].at[row["element"], row["side"] + "_bus"]
    ...
    if not meas[meas_dg_mask].empty:
        ...
        meas.loc[meas_dg_mask, "std_dev"] = np.deg2rad(
            meas.loc[meas_dg_mask, "std_dev"]
        )
    ...
    if (
        map_bus[meas_bus["element"].values.astype(np.int64)] >= ppci["bus"].shape[0]
    ).any():
        std_logger.warning(
            "Measurement defined in pp-grid does not exist in ppci, will be deleted!"
        )
        meas_bus = meas_bus[
            map_bus[meas_bus["element"].values.astype(np.int64)] < ppci["bus"].shape[0]
        ]
    ...
    bus_append = np.full(
        (ppci["bus"].shape[0], bus_cols_se), np.nan, dtype=ppci["bus"].dtype
    )
    ...
    new_in_line_buses = np.setdiff1d(
        np.arange(ppci["bus"].shape[0]), map_bus[map_bus >= 0]
    )
    bus_append[new_in_line_buses, 2] = 0.0
    bus_append[new_in_line_buses, 3] = 1e-6
    bus_append[new_in_line_buses, 4] = 0.0
    bus_append[new_in_line_buses, 5] = 1e-6

    # Create empty append array for branch measurements
    branch_append = np.full(
        (ppci["branch"].shape[0], branch_cols_se), np.nan, dtype=ppci["branch"].dtype
    )
...
def _add_zero_injection(net, ppci, bus_append, zero_injection):
    ...
    if zero_injection is not None:
        ...
        if net._pd2ppc_lookups["aux"]:
            aux_bus_lookup = np.concatenate(
                [v for k, v in net._pd2ppc_lookups["aux"].items() if k != "xward"]
            )
            ...
            if zero_injection in ["zero_pwr_bus", "no_inj_bus"]:
                ...
                if zero_injection == "no_inj_bus":
                    ...
                    for element in pq_elements:
                        ...
                        in_service = (
                            (tab["in_service"])
                            & (net.bus["in_service"][tab["bus"].values])
                        )
                ...
                raise UserWarning(
                    "zero injection parameter is not correctly initialized"
                )
...
def _build_measurement_vectors(ppci, update_meas_only=False):
    ...
    z = np.concatenate(
        (
            ppci["bus"][p_bus_not_nan, bus_cols + P],
    ...
    imag_meas = np.concatenate(
        (
            np.zeros(sum(p_bus_not_nan)),
    ...
    if ppci.algorithm == "af-wls":
        balance_eq_meas = np.zeros(ppci["rated_power_clusters"].shape[0]).astype(
            np.float64
        )
        ...
        z = np.concatenate(
            (
                z,
                balance_eq_meas[ppci.non_slack_bus_mask],
                balance_eq_meas[ppci.non_slack_bus_mask],
                af_vmeas,
            )
        )
    ...
    if not update_meas_only:
        ...
        pp_meas_indices = np.concatenate(
            (
                ppci["bus"][p_bus_not_nan, bus_cols + P_IDX],
        ...
        r_cov = np.concatenate(
            (
                ppci["bus"][p_bus_not_nan, bus_cols + P_STD],
        ...
        if ppci.algorithm == "af-wls":
            ...
            P_balance_dev_std = np.sqrt(
                np.sum(
                    np.square(
                        ppci["rated_power_clusters"][
                            :, 2 * num_clusters : 3 * num_clusters
                        ]
                    ),
                    axis=1,
                )
            ...
            r_cov = np.concatenate(
                (
                    r_cov,
                    P_balance_dev_std[ppci.non_slack_bus_mask],
                    Q_balance_dev_std[ppci.non_slack_bus_mask],
                    af_vmeas_dev_std,
                )
            )
...
def pp2eppci(
    ...
    if isinstance(eppci, ExtendedPPCI):
        ...
        eppci.data = _add_measurements_to_ppci(
            net, eppci.data, zero_injection, algorithm
        )
        eppci.update_meas()
...
class ExtendedPPCI(UserDict):
    def __init__(self, ppci, algorithm):
        ...
        # check slack bus
        self.non_slack_buses = np.argwhere(
            ppci["bus"][:, pypower_BUS_TYPE] != 3
        ).ravel()
        self.non_slack_bus_mask = (ppci["bus"][:, pypower_BUS_TYPE] != 3).ravel()
        self.num_non_slack_bus = np.sum(self.non_slack_bus_mask)
        self.delta_v_bus_mask = np.r_[
            self.non_slack_bus_mask,
            np.ones(self.non_slack_bus_mask.shape[0], dtype=bool),
        ].ravel()
    ...
    def get_Y(self):
        ...
        if "Ybus" in self["internal"] and self["internal"]["Ybus"].size:
            ...
            self["internal"]["Ybus"], self["internal"]["Yf"], self["internal"]["Yt"] = (
                Ybus,
                Yf,
                Yt,
            )
