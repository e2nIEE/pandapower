...
class Group(JSONSerializableClass):
    ...
    def __init__(self, net, elements_dict, element_column=None, name="", index=None,
        ...
        INPUT:
            ...
            **elements_dict** (dict: key as string, values as pd.Index (even if element_column is not None)) -
            dict of indices belonging to this group. Within initialization or by update_elements_dict()
            ...
            you may get problems after using drop_elements_and_group()
            or other functions. There are different pandapower toolbox functions
            which may help you to define 'elements_dict', such as get_connecting_branches(),
            get_inner_branches(), get_connecting_elements_dict().

        OPTIONAL:
            **element_column** (str, None) - If given, the elements_dict should not refer to DataFrames index
            but to another column. It is highly relevant that the element_column exists in all
        ...
        self.define_element_type(net)
        self.update_elements_dict(net, verbose=kwargs.get("verbose", True))
    ...
    def define_element_type(self, net):
        """
        Defines self.element_type with respect to the net[elm].dtypes
        """
        if self.element_column is not None:
            for i, (elm, _) in enumerate(self.elements_dict.items()):
                if self.element_column not in net[elm].columns:
                    raise ValueError(f"element_column '{self.element_column}' doesn't exist in net[{elm}].")
                if i == 0:
                    if pd.api.types.is_integer_dtype(net[elm][self.element_column]):
                        self.element_type = "int"
                    elif pd.api.types.is_numeric_dtype(net[elm][self.element_column]):
                        self.element_type = "float"
                    else:  # pd.api.types.is_object_dtype(net[elm][self.element_column])
                        self.element_type = "object"
                elif self.element_type == "int" and not pd.api.types.is_integer_dtype(
                        net[elm][self.element_column]) or self.element_type == "float" and not \
                        pd.api.types.is_numeric_dtype(net[elm][self.element_column]) or \
                        self.element_type == "object" and not pd.api.types.is_object_dtype(
                            net[elm][self.element_column]):
                    raise ValueError(
                        f"The dtypes of net[*].{self.element_column} does not fit toghether (* in [" +
                        ", ".join(self.elements_dict.keys()) + "]).")
        else:
            self.element_type = "int"
    ...
    def get_element_type(self):
        if self.element_type == "int":
            ...
            raise NotImplementedError(f"self.element_type {self.element_type} is unknown.")

    def _update_elements_dict(self, net, elements_dict, element_column, element_type, verbose=True):
        """
        Internal function of update_elements_dict().
        """
        elms = list(elements_dict.keys())
        for elm in elms:
            if elm not in net.keys() or not isinstance(net[elm], pd.DataFrame):
                if verbose:
                    logger.info(f"{elm} is in elements_dict but is no dataframe in net")
                del elements_dict[elm]
            elif not net[elm].shape[0]:
                if verbose:
                    logger.info(f"net.{elm} has no entries, so elements_dict[{elm}] is removed.")
                del elements_dict[elm]
            else:
                elements_dict[elm] = pd.Index(ensure_iterability(elements_dict[elm]),
                                          dtype=element_type).dropna()
                if element_column is None:
                    new_idx = elements_dict[elm].intersection(net[elm].index)
                else:
                    new_idx = elements_dict[elm].intersection(pd.Index(set(net[elm][element_column].astype(
                        element_type))))
                if len(new_idx) != len(elements_dict[elm]) and verbose:
                    logger.info("In %s, the number of indices has been corrected from %i to %i." % (
                        elm, len(elements_dict[elm]), len(new_idx)))
                if len(new_idx):
                    elements_dict[elm] = new_idx
                else:
                    del elements_dict[elm]

    def update_elements_dict(self, net, verbose=True):
        """
        * Set correct types of elements_dict values (pandas.Index)
        * Remove element keys and elements indices if these are not available in net.
        """
        self._update_elements_dict(net, self.elements_dict, self.element_column, self.get_element_type(),
                               verbose=verbose)

    def get_index(self, net, element):
        """
        Returns indices of the requested element dataframe, considering self.element_column.
        """
        if element in self.elements_dict.keys():
            return self.elements_dict[element] if self.element_column is None else net[element].index[net[element][
                self.element_column].isin(self.elements_dict[element])]
        else:
            return pd.Index([], dtype=int)

    def set_element_column(self, net, element_column):
        """
        Sets new self.element_column and updates self.elements_dict. If self.elements_dict is not up-to-date,
        update_elements_dict() is needed before using this function.
        """
        dupl_elements = list()
        elements_dict = dict()
        for elm in self.elements_dict.keys():

            # fill nan values in net[elm][element_column] with unique names
            if element_column not in net[elm].columns:
                net[elm][element_column] = pd.Series([None]*net[elm].shape[0], dtype=object)
            if pd.api.types.is_object_dtype(net[elm][element_column]):
                idxs = net[elm].index[net[elm][element_column].isnull()]
                net[elm][element_column].loc[idxs] = ["%s_%i_%s" % (elm, idx, str(uuid.uuid4())) for idx in
                                               idxs]

            # determine duplicated values which would corrupt Groups functionality
            if (net[elm][element_column].duplicated() | net[elm][element_column].isnull()).any():
                dupl_elements.append(elm)

            # update elements_dict
            if not len(dupl_elements):
                elements_dict[elm] = pd.Index(net[elm].loc[self.get_index(net, elm), element_column])

        if len(dupl_elements):
            raise ValueError(f"In net[*].{element_column} are duplicated or nan values - * in {dupl_elements}")
    ...
    def set_value(self, net, value, column, replace=True, append_column=True):
        ...
        """
        for elm, idx in self.elements_dict.items():
            if append_column or column in net[elm].columns:
                if self.element_column is None:
                    ...
                    if replace or column not in net[elm].columns:
                        ix = net[elm].index[net[elm][self.element_column].isin(idx)]
                    else:
                        ix = net[elm].index[net[elm][self.element_column].isin(idx) &
    ...
    def drop_elements_and_group(self, net):
        """
        ...
        if keep_everything_else:
            ...
            for elm in pp_elements():
                if group_net[elm].shape[0]:
                    keep = self.get_index(net, elm) if elm in self.elements_dict.keys() else pd.Index(
                        [], dtype=self.get_element_type())
    ...
    def append_to_group(self, elements_dict_to_append, net=None):
        """
        ...
        """
        if net is not None:
            self._update_elements_dict(net, elements_dict_to_append, self.element_column, self.get_element_type())
        for elm, idx in elements_dict_to_append.items():
            if elm in self.elements_dict.keys():
                self.elements_dict[elm] = pd.Index(ensure_iterability(
                    elements_dict_to_append[elm]), dtype=self.get_element_type()).dropna().union(
                        self.elements_dict[elm])
            else:
                self.elements_dict[elm] = pd.Index(ensure_iterability(
                    elements_dict_to_append[elm]), dtype=self.get_element_type()).dropna()

    def drop_from_group(self, elements_dict_to_drop):
        """
        Drops elements that are given in elements_dict_to_drop from this group. This is the reverse
        function of append_to_group().
        """
        for elm, idx2drop in elements_dict_to_drop.items():
            if elm in self.elements_dict.keys():
                self.elements_dict[elm] = self.elements_dict[elm].drop(self.elements_dict[elm].intersection(
                    pd.Index(ensure_iterability(idx2drop), dtype=self.get_element_type()).dropna()))
                if not len(self.elements_dict[elm]):
                    del self.elements_dict[elm]
    ...
    def compare_group(self, other_group_object, raise_false_str=None):
        ...
        for key, val in vars(self).items():
            ...
            if key == "elements_dict":
                if not self.compare_elements_dict(vars(ogo)[key], verbose=False):
                    return_raise_false(raise_false_str)
            else:
                try:
    ...
    def compare_elements_dict(self, elements_dict_to_compare_with, verbose=True):
        """
        Compares the elements of this group with the given ones in elements_dict_to_compare_with.
        ...
        OUTPUT:
            bool: whether self.elements_dict and elements_dict_to_compare_with include the same elements
        """
        ed1 = self.elements_dict
        ed2 = dict()
        for elm, idx in elements_dict_to_compare_with.items():
            pd_idx = pd.Index(ensure_iterability(idx), dtype=self.get_element_type()).dropna()
            if len(pd_idx):
                ed2[elm] = pd_idx
        ...
        if len(key_diff1) and verbose:
            logger.info("Missing element keys in elements_dict_to_compare_with: " + str(key_diff1))
        if len(key_diff2) and verbose:
            logger.info("Missing element keys in self.elements_dict: " + str(key_diff2))
