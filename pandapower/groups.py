...
def drop_group(net: pandapowerNet, index: int) -> None:
    ...
    Parameters:
        net: pandapower net
        index: index of the group which should be dropped
    """
...
def drop_group_and_elements(net: pandapowerNet, index: int) -> None:
    ...
    Parameters:
        ...
        index: index of the group which should be dropped
    """
    # functions like drop_trafos, drop_lines, drop_buses are not considered since all elements should be included in
    # elements_dict
...
def attach_to_groups(
    ...
    Parameters:
        net: pandapower net
        index: index of the considered group
        element_types: defines, together with 'elements', which net elements belong to the group
        element_indices: together with 'element_types' defines which net elements belong to the group
        reference_columns: If given, the elements_dict should not refer to DataFrames index but to another column.
            It is highly relevant that the reference_column exists in all DataFrames of the grouped
            elements and have the same dtype

    See Also:
        :func:`attach_to_group`
    """
    for idx in index:
...
def attach_to_group(
    ...
    Parameters:
        ...
        index: index of the considered group
        element_types: defines, together with 'elements', which net elements belong to the group
        element_indices: defines, together with 'element_types', which net elements belong to the group
        reference_columns: If given, the elements_dict should not refer to DataFrames index but to another column.
            It is highly relevant that the reference_column exists in all DataFrames of the grouped
            elements and have the same dtype
        take_existing_reference_columns: Determines the behavior if the given reference_columns does not match the
            reference_column existing in net.group. If True, the existing reference_column is taken for both. If False,
            an error is raised.
    """
    ...
    complete_new: dict[str, list] = {"name": [], "element_type": [], "element_index": [], "reference_column": []}
    ...
    for et, elm, rc in zip(element_types, element_indices, reference_columns):
        ...
        no_row: int = np.sum(group_et)

        # prepare adding new rows to net.group because element type is not present
        if no_row == 0:
        ...
        # element entry of existing is appended
            ...
            if existing_rc != rc:
                if take_existing_reference_columns:
                    ...
                    raise UserWarning(
                        f"The reference column of existing group {index} for element type '{et}' and of the elements to"
                        f" append differ. Use set_group_reference_column() to change the reference column of net.group "
                        "before, or pass appropriate data to attach_to_group()."
                    )

            # append
            prev_elm = net.group.element_index.loc[group_et].at[index]
            prev_elm = np.atleast_1d(prev_elm).tolist()
            net.group.iat[
                np.arange(len(group_et), dtype=int)[group_et][0],  # type: ignore[index]
...
@deprecated("Use detach_from_groups instead")
...
def detach_from_groups(
        net: pandapowerNet, element_type: str, element_index: int | list[int], index: int | None = None
) -> None:
    """
    Detaches elements from the group with the given group index 'index'. No errors are raised if elements are passed to
    be dropped from groups which already don't have these elements as members.
    A reverse function is available -> :func:`group.attach_to_group`

    Parameters:
        net: pandapower net
        index: Index of the group from which the element should be dropped
        element_type: The element type of which elements should be dropped from the group(s), e.g. "bus"
        element_index: indices of the elements which should be dropped from the group
    """
    if index is None:
        index = net.group.index.tolist()  # type: ignore[assignment]
...
def _get_lists_from_df(df: pd.DataFrame, cols: list[str]) -> list[list]:
...
def group_index(net: pandapowerNet, name: str) -> int:
    ...
    Parameters:
        net: pandapower net
        name: the group to get the index from
...
def group_element_index(net: pandapowerNet, index: int, element_type: str) -> pd.Index:
    ...
    Parameters:
        net: pandapower net
        index: Index of the group
        element_type: name of the element table to which the returned indices of the elements of the group belong to
...
def group_row(net: pandapowerNet, index: int, element_type: str) -> pd.Series:
    """
    ...
    Parameters:
        net: pandapower net
        index: index of the group
        element_type: element type (defines which row of the data of the group should be returned)
...
def isin_group(
    ...
    Parameters:
        net: pandapower net
        element_type: element type of the elements to be found in the groups, e.g. "gen" or "load"
        element_index: indices of the element table which should be found in the groups
        index: Can narrow the number of groups in which the elements are searched
        drop_empty_lines: This parameter decides whether empty entries should be removed (the complete row in
            net.group)
    ...
    single_element = np.isscalar(element_index)
    element_index = np.atleast_1d(element_index).tolist()
    if index is None:
        index_: Collection[int] = list(set(net.group.index))
    else:
    ...
    return isin[0] if single_element else isin
...
def element_associated_groups(
    ...
    Parameters:
        ...
        element_type: element type of the elements to be found in the groups, e.g. "gen" or "load"
        element_index: indices of the element table which should be found in the groups
        return_empties: whether entries with an empty list of associated groups should be returned
    ...
    associated_: dict = associated.to_dict()
    return associated_[index[0]] if single_element else associated_



def count_group_elements(net: pandapowerNet, index: int) -> pd.Series:
    """
...
def groups_equal(
    ...
    Parameters:
        net: pandapower net
        index1: index of the first group to compare
        index2: index of the second group to compare

    Keyword Arguments:
        These are passed to :func:`pandas.testing.assert_frame_equal`
    """
...
def compare_group_elements(net: pandapowerNet, index1: int, index2: int) -> bool:
    ...
    et1: list[str] = net.group.loc[[index1], "element_type"].tolist()
    et2: list[str] = net.group.loc[[index2], "element_type"].tolist()
...
def check_unique_group_rows(
        net: pandapowerNet,
        raise_error: bool = True,
        log_level: Literal["error", "warning", "info", "debug", "UserWarning"] = "warning"):
    """
    Checks whether all groups have unique names. raise_error decides whether duplicated names lead to error or log
    message.

    Parameters:
        net: pandapower net
        raise_error: decides whether duplicated names lead to error or log message.
        log_level: the level for logs, relevant if raise_error is False
    ...
    if any(len(names) != 1 for names in net.group.reset_index().groupby("index")["name"].agg(set)):
...
def group_entries_exist_in_element_table(
    ...
    """
    Returns an array of booleans whether the entries in net.group.element exist in net[element_type], also considering
    reference_column

    Parameters:
        net: pandapower net
        index: Index of the group
        element_type: element type which entries should be checked, e.g. "bus"
...
def set_group_in_service(net: pandapowerNet, index: int):
    """
...
def set_group_out_of_service(net: pandapowerNet, index: int):
    """
...
def set_value_to_group(
    ...
    Parameters:
        net: pandapower net
        index: index of the considered group
        value: value to be written to all group members into the element tables
        column: column to be manipulated
        replace: If False, value is only written to places where no value exist before (column doesn't exist
            before or value is nan)
        append_column: Decides whether the column should be added to element tables where this doesn't exist
            before
    """
...
def group_res_power_per_bus(net: pandapowerNet, index: int) -> pd.DataFrame:
    ...
    for et in net.group.loc[[index], "element_type"].tolist():
        ...
        sides = [0] if et not in bra_ets else [0, 1]
        sides = sides if et != "trafo3w" else [0, 1, 2]
        for side in sides:
...
def group_res_p_mw(net: pandapowerNet, index: int):
    """
...
def group_res_q_mvar(net: pandapowerNet, index: int):
    """
...
def set_group_reference_column(
    ...
    if element_type is None:
        element_type_: Collection[str] = net.group.loc[[index], "element_type"].tolist()
    else:
        element_type_ = ensure_iterability(element_type)
    ...
    for et in element_type:
        ...
        if not len(dupl_elements):
            ...
            pos: int = np.arange(len(pos_bool), dtype=np.int64)[pos_bool][0]

            if reference_column is None:
                element_index = group_element_index(net, index, et).tolist()
            else:
                element_index = net[et].loc[group_element_index(net, index, et), reference_column].tolist()
...
def return_group_as_net(
    ...
    Parameters:
        net: pandapower net
        index: index of the considered group
        keep_everything_else: Decides whether other data than element tables are kept
        verbose: Decides whether logging messages are triggered
...
def elements_connected_to_group(
    ...
    Parameters:
        net: the net of the group
        index: index of the group
        element_types: element types of which connected elements are searched
        find_buses_only_from_buses: if True, connected buses are not searched considering bus elements and branch
            elements of the group but only considering the buses of the group as done by
            pandapower.toolbox.get_connected_buses(). In that case it is ignored whether branch elements between the
            buses of the groups and the connected buses are in the group or not
        respect_switches: True -> open switches will be respected.
            False -> open switches will be ignored.
        respect_in_service: True -> in_service status of connected lines will be respected.
    ...
    connected: dict[str, Collection[int]] = get_connected_elements_dict(
    ...
    if not find_buses_only_from_buses:
        ...
        conn_buses: set[int] = set()
        bed.update({tpl[0]: [tpl[1]] for tpl in element_bus_tuples(branch_elements=False)})

        for row in net.group.loc[index].itertuples():
            et = row.element_type  # type: ignore[assignment]
            e_id: list = list(ensure_iterability(row.element_index))
            if et == "bus":
                continue
            for bus_col in bed[et]:
                ...
                if respect_in_service and "in_service" in net[et].columns:
                    bed_buses = bed_buses.loc[net[et].in_service.loc[e_id]]
                if respect_switches:
                    if et == "switch":
                        ...
                        switches: pd.DataFrame = net.switch[["bus", "closed"]].loc[  # type: ignore[assignment]
                            (net.switch.et == element_type_for_switch_et(et)) &
                            net.switch.element.isin(e_id)]
                        if switches.shape[0]:
        ...
        if respect_in_service:
            conn_index = pd.Index(conn_buses)
