...
class CharacteristicControl(Controller):
    ...
    INPUT:
        ...
        **output_element_type** (str) - name of the element table in pandapower net where the values are adjusted by the controller
        ...
        **input_element_type** (str) - name of the element table or the element result table in pandapower net that provides input values for the controller
    ...
    """
    def __init__(self, net, output_element_type, output_variable, output_element_index, input_element_type,
        ...
        if matching_params is None:
            matching_params = {"element_type": output_element_type, "input_variable": input_variable,
        ...
        self.output_element_type = output_element_type
        ...
        self.write_flag, self.output_variable = _detect_read_write_flag(
            net, output_element_type, output_element_index, output_variable)
        self.read_flag, self.input_variable = _detect_read_write_flag(
            net, input_element_type, input_element_index, input_variable)
    ...
    def is_converged(self, net):
        ...
        """
        # read input values
        input_values = read_from_net(net, self.input_element_type, self.input_element_index, self.input_variable,
                                     self.read_flag)
        ...
        output_values = read_from_net(net, self.output_element_type, self.output_element_index, self.output_variable,
                                      self.write_flag)
        # compare old and new set values
        diff = self.values - output_values
        # write new set values
        write_to_net(net, self.output_element_type, self.output_element_index, self.output_variable, self.values,
                     self.write_flag)
    ...
    def __str__(self):
        return super().__str__() + " [%s.%s.%s.%s]" % (
            self.input_element_type, self.input_variable, self.output_element_type,
