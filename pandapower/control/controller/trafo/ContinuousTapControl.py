...
class ContinuousTapControl(TrafoController):
    ...
    def _set_t_nom(self, net):
        vn_hv_kv = read_from_net(
            net, self.element, self.element_index, "vn_hv_kv", self._read_write_flag
        )
        hv_bus = read_from_net(
            net, self.element, self.element_index, "hv_bus", self._read_write_flag
        )
        vn_hv_bus_kv = read_from_net(net, "bus", hv_bus, "vn_kv", self._read_write_flag)

        if self.element == "trafo3w" and self.side == "mv":
            vn_mv_kv = read_from_net(
                net, self.element, self.element_index, "vn_mv_kv", self._read_write_flag
            )
            mv_bus = read_from_net(
                net, self.element, self.element_index, "mv_bus", self._read_write_flag
            )
            vn_mv_bus_kv = read_from_net(
                net, "bus", mv_bus, "vn_kv", self._read_write_flag
            )
            self.t_nom = vn_mv_kv / vn_hv_kv * vn_hv_bus_kv / vn_mv_bus_kv
        else:
            vn_lv_kv = read_from_net(
                net, self.element, self.element_index, "vn_lv_kv", self._read_write_flag
            )
            lv_bus = read_from_net(
                net, self.element, self.element_index, "lv_bus", self._read_write_flag
            )
            vn_lv_bus_kv = read_from_net(
                net, "bus", lv_bus, "vn_kv", self._read_write_flag
            )
    ...
    def initialize_control(self, net):
        ...
        if not self.nothing_to_do(net):
            self._set_t_nom(
                net
            )  # in case some of the trafo elements change their in_service in between runs
    ...
    def control_step(self, net):
        ...
        delta_vm_pu = (
            read_from_net(net, "res_bus", self.trafobus, "vm_pu", self._read_write_flag)
            - self.vm_set_pu
        )
    ...
    def is_converged(self, net):
        ...
        vm_pu = read_from_net(
            net, "res_bus", self.trafobus, "vm_pu", self._read_write_flag
        )
