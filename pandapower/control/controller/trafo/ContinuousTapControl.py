...
class ContinuousTapControl(TrafoController):
    ...
    def __init__(self, net, element_index, vm_set_pu, tol=1e-3, side="lv", element_type="trafo", in_service=True,
        ...
        super().__init__(net, element_index=element_index, side=side, tol=tol, in_service=in_service,
                         element_type=element_type, level=level, order=order,
    ...
    def _set_t_nom(self, net):
        vn_hv_kv = read_from_net(net, self.element_type, self.controlled_element_index, 'vn_hv_kv', self._read_write_flag)
        hv_bus = read_from_net(net, self.element_type, self.controlled_element_index, 'hv_bus', self._read_write_flag)
        vn_hv_bus_kv = read_from_net(net, "bus", hv_bus, 'vn_kv', self._read_write_flag)

        if self.element_type == "trafo3w" and self.side == "mv":
            vn_mv_kv = read_from_net(net, self.element_type, self.controlled_element_index, 'vn_mv_kv', self._read_write_flag)
            mv_bus = read_from_net(net, self.element_type, self.controlled_element_index, 'mv_bus', self._read_write_flag)
            ...
            vn_lv_kv = read_from_net(net, self.element_type, self.controlled_element_index, 'vn_lv_kv', self._read_write_flag)
            lv_bus = read_from_net(net, self.element_type, self.controlled_element_index, 'lv_bus', self._read_write_flag)
    ...
    def control_step(self, net):
        ...
        if net[self.element_type].tap_pos.dtype != "float":
            net[self.element_type].tap_pos = net[self.element_type].tap_pos.astype(float)
