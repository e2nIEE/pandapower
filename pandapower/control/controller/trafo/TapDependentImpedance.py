...
class TapDependentImpedance(CharacteristicControl):
        ...
        **kwargs,
        ...
        if matching_params is None:
            ...
            super().__init__(
                ...
                output_variable=output_variable,
                output_element_index=element_index,
                input_element=element,
                input_variable="tap_pos",
                input_element_index=element_index,
    ...
    def initialize_control(self, net):
        if self.restore:
            self.initial_values = (
                net[self.output_element]
                .loc[self.output_element_index, self.output_variable]
                .copy()
            )

    def finalize_control(self, net):
        if self.restore:
            net[self.output_element].loc[
                self.output_element_index, self.output_variable
            ] = self.initial_values
