...
class TapDependentImpedance(CharacteristicControl):
    ...
    def __init__(self, net, element_index, characteristic_index, element_type="trafo",
        ...
        if matching_params is None:
            ...
            super().__init__(
                net, output_element_type=element_type, output_variable=output_variable,
                output_element_index=element_index, input_element_type=element_type,
        ...
        self.initial_values = net[element_type].loc[element_index, output_variable].copy()

    def initialize_control(self, net):
        if self.restore:
            self.initial_values = net[self.output_element_type].loc[
                self.output_element_index, self.output_variable].copy()

    def finalize_control(self, net):
        if self.restore:
            net[self.output_element_type].loc[self.output_element_index, self.output_variable] = \
                self.initial_values
