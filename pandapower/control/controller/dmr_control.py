...
class DmrControl(Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if matching_params is None:
            matching_params = {
                "dmr_line": dmr_line,
                "dc_plus_line": dc_plus_line,
                "dc_minus_line": dc_minus_line,
            }
        ...
        if not np.all(net.line_dc.index.isin([dmr_line, dc_plus_line, dc_minus_line])):
            raise ValueError(
                "Wrong dc line index given. Please check if all lines are in line_dc!"
            )
    ...
    def finalize_control(self, net):
        ...
        net.res_line_dc.loc[self.dmr_line, "loading_percent"] = (
            np.abs(dcp - dcm) / (max_i_ka * parallel) * 100.0
        )
