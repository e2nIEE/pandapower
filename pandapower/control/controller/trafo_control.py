...
class TrafoController(Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            net, in_service=in_service, level=level, order=order, recycle=recycle
        )
    ...
    def _set_read_write_flag(self, net):
        ...
        self._read_write_flag, _ = _detect_read_write_flag(
            net, self.element, self.element_index, "tap_pos"
        )
    ...
    def nothing_to_do(self, net):
        element_in_service = read_from_net(
            net, self.element, self.element_index, "in_service", self._read_write_flag
        )
        ext_grid_bus = np.isin(
            self.trafobus, net.ext_grid.loc[net.ext_grid.in_service, "bus"].values
        )
        element_index_in_net = np.isin(
            self.element_index, net[self.element].index.values
        )
        self.controlled = np.logical_and(
            np.logical_and(element_in_service, element_index_in_net),
            np.logical_not(ext_grid_bus),
        )
        if isinstance(self.element_index, np.int64) or isinstance(
            self.element_index, int
        ):
            # if the controller shouldn't do anything, return True
            if (
                not element_in_service
                or ext_grid_bus
                or not element_index_in_net
                or (
                    self._read_write_flag != "single_index"
                    and len(self.element_index) == 0
                )
            ):
                return True
            return False
            ...
            # if the controller shouldn't do anything, return True
            if (
                np.all(~element_in_service[self.controlled])
                or np.all(ext_grid_bus[self.controlled])
                or np.all(~element_index_in_net[self.controlled])
                or (
                    self._read_write_flag != "single_index"
                    and len(self.element_index) == 0
                )
    ...
    def _set_tap_side_coeff(self, net):
        tap_side = read_from_net(
            net, self.element, self.element_index, "tap_side", self._read_write_flag
        )
        if (
            len(np.setdiff1d(tap_side, ["hv", "lv"])) > 0 and self.element == "trafo"
        ) or (
            len(np.setdiff1d(tap_side, ["hv", "lv", "mv"])) > 0
            and self.element == "trafo3w"
        ):
            raise ValueError(
                "Trafo tap side (in net.%s) has to be either hv or lv, "
                "but received: %s for trafo %s"
                % (self.element, tap_side, self.element_index)
            )
    ...
    def _set_side(self, side):
        if self.element == "trafo":
            if side not in ["hv", "lv"]:
                raise UserWarning(
                    "side has to be 'hv' or 'lv' for high/low voltage, "
                    "received %s" % side
                )
        elif self.element == "trafo3w":
            if side not in ["hv", "mv", "lv"]:
                raise UserWarning(
                    "side has to be 'hv', 'mv' or 'lv' for high/middle/low voltage, "
                    "received %s" % side
                )
        else:
    ...
    def _set_tap_parameters(self, net):
        self.tap_min = read_from_net(
            net, self.element, self.element_index, "tap_min", self._read_write_flag
        )
        self.tap_max = read_from_net(
            net, self.element, self.element_index, "tap_max", self._read_write_flag
        )
        self.tap_neutral = read_from_net(
            net, self.element, self.element_index, "tap_neutral", self._read_write_flag
        )
        self.tap_step_percent = read_from_net(
            net,
            self.element,
            self.element_index,
            "tap_step_percent",
        ...
        self.tap_step_degree = read_from_net(
            net,
            self.element,
            self.element_index,
            "tap_step_degree",
        ...
        if self._read_write_flag == "single_index":
            self.tap_sign = (
                1
                if np.isnan(self.tap_step_degree)
                else np.sign(np.cos(np.deg2rad(self.tap_step_degree)))
            )
            ...
            self.tap_sign = np.where(
                np.isnan(self.tap_step_degree),
                1,
                np.sign(np.cos(np.deg2rad(self.tap_step_degree))),
            )
            self.tap_sign = np.where(
                (self.tap_sign == 0) | (np.isnan(self.tap_sign)), 1, self.tap_sign
            )
            self.tap_pos = np.where(
                np.isnan(self.tap_pos), self.tap_neutral, self.tap_pos
            )

        if (
            np.any(np.isnan(self.tap_min))
            or np.any(np.isnan(self.tap_max))
            or np.any(np.isnan(self.tap_step_percent))
        ):
            logger.error(
    ...
    def set_recycle(self, net):
        ...
        if (
            net.controller.at[self.index, "recycle"] is False
            or self.element not in allowed_elements
        ):
