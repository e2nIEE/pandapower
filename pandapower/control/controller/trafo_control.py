...
class TrafoController(Controller):
    ...
    INPUT:
       ...
       **element_type** (string) - Type of the controlled trafo ("trafo" or "trafo3w")

    OPTIONAL:
        **recycle** (bool, True) - Re-use of internal-data in a time series loop.
    """

    def __init__(self, net, element_index, side, tol, in_service, element_type, level=0, order=0,
                 recycle=True, **kwargs):
    ...
    def _set_read_write_flag(self, net):
        ...
        self._read_write_flag, _ = _detect_read_write_flag(net, self.element_type, self.element_index, "tap_pos")
    ...
    def _set_tap_side_coeff(self, net):
        tap_side = read_from_net(net, self.element_type, self.controlled_element_index, 'tap_side', self._read_write_flag)
        if (len(np.setdiff1d(tap_side, ['hv', 'lv'])) > 0 and self.element_type == "trafo") or \
            (len(np.setdiff1d(tap_side, ['hv', 'lv', 'mv'])) > 0 and self.element_type == "trafo3w"):
            raise ValueError("Trafo tap side (in net.%s) has to be either hv or lv, "
                             "but received: %s for trafo %s" % (self.element_type, tap_side, self.controlled_element_index))
    ...
    def _set_side(self, side):
        if self.element_type == "trafo":
            if side not in ["hv", "lv"]:
                raise UserWarning("side has to be 'hv' or 'lv' for high/low voltage, "
                                  "received %s" % side)
        elif self.element_type == "trafo3w":
            if side not in ["hv", "mv", "lv"]:
                raise UserWarning("side has to be 'hv', 'mv' or 'lv' for high/middle/low voltage, "
                                  "received %s" % side)
        else:
            raise UserWarning("unknown trafo type, received %s" % self.element_type)

        self.side = side

    def _set_valid_controlled_index_and_bus(self, net):
        self.trafobus = read_from_net(net, self.element_type, self.element_index, self.side + '_bus', self._read_write_flag)
        element_in_service = read_from_net(net, self.element_type, self.element_index, 'in_service', self._read_write_flag)
        ext_grid_bus = np.isin(self.trafobus, net.ext_grid.loc[net.ext_grid.in_service, 'bus'].values)
        element_index_in_net = np.isin(self.element_index, net[self.element_type].index.values)
    ...
    def _set_tap_parameters(self, net):
        self.tap_min = read_from_net(net, self.element_type, self.controlled_element_index, "tap_min", self._read_write_flag)
        self.tap_max = read_from_net(net, self.element_type, self.controlled_element_index, "tap_max", self._read_write_flag)
        self.tap_neutral = read_from_net(net, self.element_type, self.controlled_element_index, "tap_neutral", self._read_write_flag)
        self.tap_step_percent = read_from_net(net, self.element_type, self.controlled_element_index, "tap_step_percent", self._read_write_flag)
        self.tap_step_degree = read_from_net(net, self.element_type, self.controlled_element_index, "tap_step_degree", self._read_write_flag)
    ...
    #     self.tap_pos = net[self.element_type].at[self.controlled_element_index, "tap_pos"]

    def __repr__(self):
        s = '%s of %s %s' % (self.__class__.__name__, self.element_type, self.element_index)
        return s
