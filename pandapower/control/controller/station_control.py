...
class BinarySearchControl(Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if input_inverted is None or (
            isinstance(input_inverted, Sequence) and len(input_inverted) == 0
        ):
        ...
        self.output_values_distribution = np.array(
            output_values_distribution, dtype=np.float64
        ) / np.sum(output_values_distribution)
        n = len(self.output_element_index)
        if gen_Q_response is None or (
            isinstance(gen_Q_response, Sequence) and len(gen_Q_response) == 0
        ):
            # empty, then set all entries to 1
            ...
            if len(gen_Q_response) < n:
                gen_Q_response += [1] * (
                    n - len(gen_Q_response)
                )  # missing entries with +1
        ...
        self.write_flag, self.output_variable = _detect_read_write_flag(
            net, output_element, output_element_index, output_variable
        )
        ...
        for input_index in self.input_element_index:
            ...
            if isinstance(input_variable, list):
                read_flag_temp, input_variable_temp = _detect_read_write_flag(
                    net, self.input_element, input_index, input_variable[counter]
                )
            else:
                read_flag_temp, input_variable_temp = _detect_read_write_flag(
                    net, self.input_element, input_index, input_variable
                )
    ...
    def initialize_control(self, net):
        self.output_values = read_from_net(
            net,
            self.output_element,
    ...
    def is_converged(self, net):
        ...
        for input_index in self.input_element_index:
            if self.input_element == "res_line":
                ...
                self.input_element_in_service.append(
                    net.trafo3w.in_service[input_index]
                )
            elif self.input_element == "res_switch":
        ...
        for output_index in self.output_element_index:
            if self.output_element == "gen":
                ...
                self.output_element_in_service.append(
                    net.shunt.in_service[output_index]
                )
        # check if at least one input and one output element is in_service
        if not (
            any(self.input_element_in_service) and any(self.output_element_in_service)
        ):
            logger.warning(
                "Input and/or output elements for controller %i out of service, putting controller "
        ...
        for input_index in self.input_element_index:
            input_values.append(
                read_from_net(
                    ...
                    self.input_element,
                    input_index,
                    self.input_variable[counter],
                    self.read_flag[counter],
                )
            )
            counter += 1
    ...
    def _binarysearchcontrol_step(self, net):
        ...
        if self.output_values_old is None:
            self.output_values_old, self.output_values = (
                self.output_values,
                self.output_values + 1e-3,
            )
        else:
            step_diff = self.diff - self.diff_old
            x = self.output_values - self.diff * (
                self.output_values - self.output_values_old
            ) / np.where(step_diff == 0, 1e-6, step_diff)

            rel_cap = 2
            cap = (
                rel_cap * (np.abs(self.output_values) + 1e-6) + 50
            )  # add epsilon to avoid zero; absolute cap +50 MVAr
            ...
            x = (
                ...
                else sum(x) * self.output_values_distribution
            )
...
class DroopControl(Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if voltage_ctrl and value is None:
            raise ValueError(
                "vm_set_pu_bsc missing in input variables of droop controller!"
            )
        else:
        ...
        self.tol = tol
        self.applied = False
        self.read_flag, self.input_variable = _detect_read_write_flag(
            net, "res_bus", bus_idx, "vm_pu"
        )
    ...
    def is_converged(self, net):
        if (
            not net.controller.at[self.controller_idx, "object"].in_service
            or net.controller.at[self.controller_idx, "object"].converged
        ):
            self.converged = True
            return self.converged
        if self.voltage_ctrl:
            self.diff = net.controller.at[
                self.controller_idx, "object"
            ].set_point - read_from_net(
                net, "res_bus", self.bus_idx, "vm_pu", self.read_flag
            )
        else:
            counter = 0
            input_values = []
            for input_index in net.controller.at[
                self.controller_idx, "object"
            ].input_element_index:
                input_values.append(
                    read_from_net(
                        net,
                        net.controller.at[self.controller_idx, "object"].input_element,
                        input_index,
                        net.controller.at[self.controller_idx, "object"].input_variable[
                            counter
                        ],
                        net.controller.at[self.controller_idx, "object"].read_flag[
                            counter
                        ],
                    )
                )
                counter += 1
            input_sign = np.asarray(
                net.controller.at[self.controller_idx, "object"].input_sign
            )
            input_values = (input_sign * np.asarray(input_values)).tolist()
            self.diff = net.controller.at[
                self.controller_idx, "object"
            ].set_point - sum(input_values)
    ...
    def _droopcontrol_step(self, net):
        self.vm_pu_old = self.vm_pu
        self.vm_pu = read_from_net(
            net, "res_bus", self.bus_idx, "vm_pu", self.read_flag
        )
        if not self.voltage_ctrl:
            if self.q_set_mvar_bsc is None:
                self.q_set_mvar_bsc = net.controller.at[
                    self.controller_idx, "object"
                ].set_point
            ...
            if self.q_set_mvar is not None:
                net.controller.at[
                    self.controller_idx, "object"
                ].set_point = self.q_set_mvar

        else:
            input_element = net.controller.at[
                self.controller_idx, "object"
            ].input_element
            input_element_index = net.controller.at[
                self.controller_idx, "object"
            ].input_element_index
            input_variable = net.controller.at[
                self.controller_idx, "object"
            ].input_variable
            ...
            self.vm_set_pu_new = (
                self.vm_set_pu_bsc + sum(input_values) / self.q_droop_mvar
            )  # net.controller.at[self.controller_idx, "object"].gen_Q_response[0] *
            net.controller.at[
                self.controller_idx, "object"
            ].set_point = self.vm_set_pu_new
...
class VDroopControl_local(Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tol = tol
        self.applied = False
        gen_idx = net.controller.at[self.controller_idx, "object"].input_element_index[
            0
        ]
        self.read_flag, self.input_variable = _detect_read_write_flag(
            net, "res_bus", bus_idx, "vm_pu"
    ...
    def _Vdroopcontrol_step(self, net):
        ...
        self.vm_set_pu_new = (
            self.vm_set_pu_bsc
            - (sum(input_values) - self.q_set_mvar) / self.q_droop_mvar
        )
