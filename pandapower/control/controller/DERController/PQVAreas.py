...
class BasePQVArea(BaseArea):
    ...
    def in_area(self, p_pu, q_pu, vm_pu):
        return self.pq_area.in_area(p_pu, q_pu, vm_pu) & self.qv_area.in_area(
            p_pu, q_pu, vm_pu
        )
    ...
    def q_flexibility(self, p_pu, vm_pu):
        ...
        if n_no_flex := sum(no_flexibility):
            raise ValueError(
                f"For {n_no_flex} elements, the q flexibility of the PQ area provides "
                "max_q < min_q."
            )
        ...
        if n_no_flex := sum(no_flexibility):
            raise ValueError(
                f"For {n_no_flex} elements, the q flexibility of the QV area provides "
                "max_q < min_q."
            )
        ...
        if n_no_flex := sum(no_flexibility):
            error_msg = (
                f"For {n_no_flex} elements, max_q > min_q result from considering both "
...
class PQAreaPOLYGON(BaseArea):
    ...
    def __init__(self, p_points_pu, q_points_pu):
        ...
        self.polygon = Polygon(
            [(p_pu, q_pu) for p_pu, q_pu in zip(p_points_pu, q_points_pu)]
        )

    def in_area(self, p_pu, q_pu, vm_pu=None):
        return np.array(
            [self.polygon.contains(Point(pi, qi)) for pi, qi in zip(p_pu, q_pu)]
        )
    ...
    def q_flexibility(self, p_pu, vm_pu=None):
        def _q_flex(p_pu):
            ...
            if line.intersects(self.polygon):
                points = [
                    point[1]
                    for point in LineString([(p_pu, -1), (p_pu, 1)])
                    .intersection(self.polygon)
                    .coords
                ]
                if len(points) == 1:
                    return [points[0]] * 2
...
class QVAreaPOLYGON(BaseArea):
    ...
    def in_area(self, p_pu, q_pu, vm_pu):
        return np.array(
            [self.polygon.contains(Point(vmi, qi)) for vmi, qi in zip(vm_pu, q_pu)]
        )
    ...
    def q_flexibility(self, p_pu, vm_pu):
        ...
        def _q_flex(vm_pu):
            ...
            if line.intersects(self.polygon):
                return [
                    point[1]
                    for point in LineString([(vm_pu, -1), (vm_pu, 1)])
                    .intersection(self.polygon)
                    .coords
                ]
            else:
...
class PQArea4120(BaseArea):
    ...
    def __init__(self, min_q_pu, max_q_pu, version=2018, **kwargs):
        ...
        self.min_q_pu, self.max_q_pu = min_q_pu, max_q_pu
        self.q_max_under_p_point = kwargs.get("q_max_under_p_point", 0.0)
    ...
    def in_area(self, p_pu, q_pu, vm_pu=None):
        ...
        is_in_area[
            (p_pu < self.p_points_pu[0])
            & ((q_pu < -0.05) | (q_pu > self.q_max_under_p_point))
        ] = False
        if all(~is_in_area):
            return is_in_area
        is_in_area[
            (p_pu > self.p_points_pu[1])
            & ((q_pu < self.min_q_pu) | (q_pu > self.max_q_pu))
        ] = False
        if all(~is_in_area):
            return is_in_area
        is_in_area[
            (q_pu < -0.1 - (p_pu - self.p_points_pu[0]) * self.linear_factor_ind)
            | (q_pu > 0.1 + (p_pu - self.p_points_pu[0]) * self.linear_factor_cap)
        ] = False
        return is_in_area
    ...
    def q_flexibility(self, p_pu, vm_pu=None):
        ...
        q_flex[part] = np.c_[
            -0.1 + (p_pu[part] - self.p_points_pu[0]) * self.linear_factor_ind,
            0.1 + (p_pu[part] - self.p_points_pu[0]) * self.linear_factor_cap,
        ]

        part = p_pu < self.p_points_pu[0]
        q_flex[part] = np.c_[
            [-0.05] * sum(part), [self.q_max_under_p_point] * sum(part)
        ]
...
class QVArea4120(BaseArea):
    ...
    def q_flexibility(self, p_pu, vm_pu):
        ...
        min_max_q[part] = np.c_[
            self.max_q_pu - self.linear_factor * (vm_pu[part] - self.min_vm_pu),
            [self.max_q_pu] * sum(part),
        ]

        part = (self.min_vm_pu + self.delta_vm_pu < vm_pu) & (
            vm_pu <= self.max_vm_pu - self.delta_vm_pu
        )
        min_max_q[part] = np.repeat(
            np.array([[self.min_q_pu, self.max_q_pu]]), sum(part), axis=0
        )

        part = (self.max_vm_pu - self.delta_vm_pu < vm_pu) & (vm_pu <= self.max_vm_pu)
        min_max_q[part] = np.c_[
            [self.min_q_pu] * sum(part),
...
class QVArea4130(QVArea4120):
    ...
    def __init__(self, min_q_pu, max_q_pu, vn_kv, variant):
        self.min_q_pu, self.max_q_pu, self.vn_kv, self.variant = (
            min_q_pu,
            max_q_pu,
            vn_kv,
            variant,
        ...
        if not np.any(np.isclose(np.array([380, 220]), vn_kv)):
            raise ValueError(
                f"QVArea4130 is defined for 380kV and 220kV only, not for {vn_kv} kV."
            )
        if variant == 1:
            if vn_kv == 380:
                ...
                self.min_vm_points_pu = (
                    np.array([193 - epsilon, 193, 220, 233.5]) / vn_kv
                )
                self.max_vm_points_pu = np.array([245, 253]) / vn_kv
            self.min_q_points_pu = np.array(
                [
                    self.max_q_pu,
                    self.max_q_pu * np.sin(np.arccos(0.95)) / np.sin(np.arccos(0.9)),
                    0,
                    self.min_q_pu,
                ]
            )
            ...
            self.min_q_points_pu = np.array(
                [
                    self.max_q,
                    self.max_q * np.sin(np.arccos(0.95)) / np.sin(np.arccos(0.925)),
                    0,
                    self.min_q,
                ]
            )
    ...
    def q_flexibility(self, p_pu, vm_pu):
        return np.c_[
            np.interp(vm_pu, self.min_vm_points_pu, self.min_q_points_pu),
...
class PQVArea4130Base(BasePQVArea):
    ...
    def __init__(
        self, min_q_pu, max_q_pu, variant, vn_kv=380, raise_merge_overlap=True
    ):
        super().__init__(raise_merge_overlap=raise_merge_overlap)
...
class PQVArea4130V1(PQVArea4130Base):
    ...
    def __init__(self, vn_kv=380, raise_merge_overlap=True):
        super().__init__(
            -0.227902, 0.484322, 1, vn_kv=vn_kv, raise_merge_overlap=raise_merge_overlap
        )
...
class PQVArea4130V2(PQVArea4130Base):
    ...
    def __init__(self, vn_kv=380, raise_merge_overlap=True):
        super().__init__(
            -0.328684, 0.410775, 2, vn_kv=vn_kv, raise_merge_overlap=raise_merge_overlap
        )
...
class PQVArea4130V3(PQVArea4130Base):
    ...
    def __init__(self, vn_kv=380, raise_merge_overlap=True):
        super().__init__(
            -0.410775, 0.328684, 3, vn_kv=vn_kv, raise_merge_overlap=raise_merge_overlap
        )
...
class PQArea4110(PQAreaPOLYGON):
    def __init__(self):
        p_points_pu = (-1e-7, 0.05, 0.05, 1.0, 1.0, 0.05, 0.05, 1e-7, -1e-7)
        q_points_pu = (
            -1e-7,
            -1e-7,
            -0.01961505,
...
class QVArea4110(QVAreaPOLYGON):
    ...
    def __init__(self):
        q_points_pu = (0.0, -0.484322, -0.484322, 0.0, 0.484322, 0.484322, 0.0)
...
class PQArea4105(PQAreaPOLYGON):
    ...
    def __init__(self, variant):
        if variant == 1:
            ...
            p_points_pu = (0.0, 1, 1, 0.0)
            q_points_pu = (0.0, -0.328684, 0.328684, 0.0)
        elif variant == 2:
            # commented due to the note of the docstring
            # p_points_pu = (0., 0.9, 0.9, 0.)
            # q_points_pu = (0., -0.36, 0.36, 0.)
            p_points_pu = (0.0, 1, 1, 0.0)
            q_points_pu = (0.0, -0.484322, 0.484322, 0.0)
        else:
...
class QVArea4105(QVAreaPOLYGON):
    ...
    def __init__(self, variant):
        if variant == 1:
            q_points_pu = (0.0, -0.328684, -0.328684, 0.0, 0.328684, 0.328684, 0.0)
            vm_points_pu = (0.9, 0.95, 1.1, 1.1, 1.05, 0.9, 0.9)
        elif variant == 2:
            q_points_pu = (0.0, -0.484322, -0.484322, 0.0, 0.484322, 0.484322, 0.0)
            vm_points_pu = (0.9, 0.95, 1.1, 1.1, 1.05, 0.9, 0.9)
