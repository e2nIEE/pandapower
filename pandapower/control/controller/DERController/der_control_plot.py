...
def _plot_pq_area(
    ...
    if tex:
        texts = (
            "$Q_{min}$",
            "$Q_{max}$",
            r"underexcited $\leftarrow------ Q/S_N ------\rightarrow$ overexcited",
            "$P/S_N$",
            "$S_{max}$",
        )
    else:
    ...
    if not np.isnan(saturate_sn_pu):
        ...
        y, x = generate_circle_segment(
            0, 0, saturate_sn_pu, -circle_segment, circle_segment, 1
        )
        ...
        ax.set_xlim(
            min(min_max_q[:, 0]) - 0.05 * x_diff, max(min_max_q[:, 1]) + 0.05 * x_diff
        )
    ax.fill_betweenx(
        p_samples,
        np.max(np.c_[min_max_q[:, 0], -y_circ], axis=1),
        np.min(np.c_[min_max_q[:, 1], y_circ], axis=1),
        color="green",
        alpha=0.2,
    )
    plt.tight_layout()
...
def plot_qv_area(
    qv_area, title=None, ax=None, prune_to_flexibility=False, vm_samples=None, tex=False
):
    ...
    min_max_q = qv_area.q_flexibility(
        p_pu=0.5, vm_pu=vm_samples
    )  # p should be irrelevant since
    ...
    if prune_to_flexibility:
        start = (
            min(
                np.arange(len_, dtype=int)[
                    ~np.isclose(min_max_q[:, 0], min_max_q[0, 0])
                ]
            )
            - 1
        )
        stop = (
            max(
                np.arange(len_, dtype=int)[
                    ~np.isclose(min_max_q[:, 1], min_max_q[-1, 1])
                ]
            )
            + 2
        )
    ...
    ax.grid(alpha=0.8)
    ax.fill_between(
        vm_samples, min_max_q[:, 0], min_max_q[:, 1], color="green", alpha=0.2
    )
