...
schema = pa.DataFrameSchema(  # in methodcall but not parameter docu: generator_type, max_i_ka, kappa, lrc_pu
    {
        ...
        "min_p_mw": pa.Column(float, required=False, description="maximum active power [MW]"),
        "max_p_mw": pa.Column(float, required=False, description="minimum active power [MW]"),
        "min_q_mvar": pa.Column(float, required=False, description="maximum reactive power [MVAr]"),
        "max_q_mvar": pa.Column(float, required=False, description="minimum reactive power [MVAr]"),
        ...
        "k": pa.Column(
            float, pa.Check.ge(0), required=False, description="ratio of short circuit current to nominal current"
        ),
        "rx": pa.Column(
            float,
            pa.Check.ge(0),
