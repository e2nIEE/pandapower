...
def create_lower_than_column_check(first_element: str, second_element: str) -> pa.Check:
    """
    Create a Pandera check that validates one column is less than another.

    This check verifies that values in the first column are less than
    corresponding values in the second column. The check passes (returns True) if:
    ...
    ----------
    first_element : str
        Name of the first column (should contain lower values).
    second_element : str
        Name of the second column (should contain higher values).
    ...
    Examples
    --------
    >>> check = create_lower_than_column_check("min_value", "max_value")
    >>> schema = pa.DataFrameSchema({
    ...     "min_value": pa.Column(float, checks=[check]),
    ...     "max_value": pa.Column(float)
    ... })
