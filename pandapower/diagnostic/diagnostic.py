...
class Diagnostic:
    ...
    def register_function(self, diagnostic_function: DiagnosticFunction, argument_names: list[str] | None, name: str | None):
        """
        register a diagnostic function to run when running network diagnostics and the associated report function.

        :param diagnostic_function: instance of class implementing the DiagnosticFunction Base Class
        :param argument_names: the kwargs that should be passed to the diagnostic function. If None is provided all
            kwargs will be passed.
        :param name: name to use for results dict and reports, if None will use name of the class
        """
        if name is None:
    ...
    def diagnose_network(
        ...
        **kwargs,
        ...
        >>> d = Diagnostic()
        >>> d.diagnose_network(net, report_style='compact', warnings_only=True)
        >>> d.compact_report()
        >>> d.detailed_report()
    ...
    def compact_report(self, warnings_only: bool = False):
        ...
        :raises RuntimeError: When called and no diagnostic results are available.
        """
    ...
    def report(self, compact_report: bool = True, warnings_only: bool = False) -> None:
        ...
        :param bool compact_report: diagnostic report should be compact or detailed
        :param bool warnings_only: diagnostic report should be only warnings or all info
        """
        if self.net is None:
            raise RuntimeError(
