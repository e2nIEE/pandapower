...
def _add_sgen_sc_z(net, ppc):
    ...
    sgen_wd = net.sgen.loc[
        net.sgen.in_service & (net.sgen.generator_type == "async_doubly_fed")
    ]
    if len(sgen_wd) > 0:
        ...
        base_z_ohm = (
            vn_net**2
        )  # / net.sn_mva  # by logic it must by divided by sn_mva, but why is it wrong?

        z_wd_ohm = (
            np.sqrt(2)
            * sgen_wd.kappa
            * net.bus.loc[sgen_buses, "vn_kv"].values
            / (np.sqrt(3) * sgen_wd.max_ik_ka)
        )
        ...
        bus_idx, sgen_gs, sgen_bs = _sum_by_group(
            sgen_buses_ppc, y_wd_pu.real, y_wd_pu.imag
        )
    ...
    if len(sgen_g) > 0:
        ...
        z_g_complex_ohm = (
            (sgen_g.rx + 1j) * z_g_ohm / (np.sqrt(1 + np.square(sgen_g.rx)))
        )
        z_g_complex_pu = z_g_complex_ohm / base_z_ohm
        y_g_pu = 1 / z_g_complex_pu.values

        bus_idx, sgen_gs, sgen_bs = _sum_by_group(
            sgen_buses_ppc, y_g_pu.real, y_g_pu.imag
        )
        ppc["bus"][bus_idx, GS] = sgen_gs
        ppc["bus"][bus_idx, BS] = sgen_bs
...
def _add_gen_sc_z_kg_ks(net, ppc):
    ...
    lv_gens, hv_gens = (vn_gen <= 1.0), (vn_gen > 1.0)
    ...
    _, gen_gs_p, gen_bs_p = _sum_by_group(
        gen_buses_ppc, y_gen_p_pu.real, y_gen_p_pu.imag
    )
    ...
    if np.any(~gen.power_station_trafo.isnull().values):
        ...
        ps_trafo_ix = gen.loc[ps_gen_mask, "power_station_trafo"].values.astype(
            np.int64
        )
        ps_trafo = net.trafo.loc[ps_trafo_ix, :]
        _ps_trafo_real_ix = (
            pd.Series(index=net.trafo.index.values, data=np.arange(net.trafo.shape[0]))
            .loc[ps_trafo_ix]
            .values
        )
        ...
        v_trafo_hv, v_trafo_lv = (
            ps_trafo["vn_hv_kv"].values,
            ps_trafo["vn_lv_kv"].values,
        )
        ...
        R_t = (
            ps_trafo["vkr_percent"].values / 100 * np.square(v_trafo_hv) / sn_trafo_mva
        )
        ...
        if np.any(np.isnan(p_t)):
            ...
            p_t[np.isnan(p_t)] = (
                -(
                    ...
                    * (ps_trafo["tap_max"].values - ps_trafo["tap_neutral"].values)
                )[np.isnan(p_t)]
                / 100
            )
        ...
        if np.any(ps_trafo_oltc_mask):
            ...
            ks = (
                (v_q**2 / v_g**2)
                * (v_trafo_lv**2 / v_trafo_hv**2)
                * ps_cmax
                / (1 + np.abs(x_g - x_t) * sin_phi_gen[ps_gen_mask])
            )
            ppc["bus"][ps_gen_buses_ppc[ps_trafo_oltc_mask], K_SG] = ks[
                ps_trafo_oltc_mask
            ]
            ppc["branch"][ps_trafo_ppc_ix[ps_trafo_oltc_mask], K_ST] = ks[
                ps_trafo_oltc_mask
            ]

            # kg for sc calculation inside power station units
            kg = ps_cmax / (1 + x_g * sin_phi_gen[ps_gen_mask])
            ppc["bus"][ps_gen_buses_ppc[ps_trafo_oltc_mask], K_G] = kg[
                ps_trafo_oltc_mask
            ]
        ...
        if np.any(~ps_trafo_oltc_mask):
            ...
            kso = (
                (v_q / (v_g * (1 + p_g)))
                * (v_trafo_lv / v_trafo_hv)
                * (1 - p_t)
                * ps_cmax
                / (1 + x_g * sin_phi_gen[ps_gen_mask])
            )

            ppc["bus"][ps_gen_buses_ppc[~ps_trafo_oltc_mask], K_SG] = kso[
                ~ps_trafo_oltc_mask
            ]
            ppc["branch"][ps_trafo_ppc_ix[~ps_trafo_oltc_mask], K_ST] = kso[
                ~ps_trafo_oltc_mask
            ]
...
def _create_k_updated_ppci(net, ppci_orig, ppci_bus, zero_sequence=False):
    ...
    if np.any(ps_trafo_mask):
        ...
        ps_trafo_ppci_lv_bus = ppci["branch"][ps_trafo_mask, T_BUS].real.astype(
            np.int64
        )
        ps_trafo_ppci_hv_bus = ppci["branch"][ps_trafo_mask, F_BUS].real.astype(
            np.int64
        )
    ...
    if np.any(ps_gen_bus_mask):
        ppci["bus"][np.ix_(ps_gen_bus_mask, [GS_P, BS_P])] /= ppci["bus"][
            np.ix_(ps_gen_bus_mask, [K_SG])
        ]
        ppci["bus"][np.ix_(ps_gen_bus_mask, [GS, BS])] += (
            1 / ppci["bus"][np.ix_(ps_gen_bus_mask, [K_SG])] - 1
        ) * ppci["bus"][np.ix_(ps_gen_bus_mask, [GS_GEN, BS_GEN])]
        # Then, the R and X are multiplied by K_S (named K_ST here)
        ppci["branch"][np.ix_(ps_trafo_mask, [BR_X, BR_R])] *= ppci["branch"][
            np.ix_(ps_trafo_mask, [K_ST])
        ]

    gen_bus_mask = np.isnan(ppci["bus"][:, K_SG]) & (~np.isnan(ppci["bus"][:, K_G]))
    if np.any(gen_bus_mask):
        ppci["bus"][np.ix_(gen_bus_mask, [GS_P, BS_P])] /= ppci["bus"][
            np.ix_(gen_bus_mask, [K_G])
        ]
        ppci["bus"][np.ix_(gen_bus_mask, [GS, BS])] += (
            1 / ppci["bus"][np.ix_(gen_bus_mask, [K_G])] - 1
    ...
    if ps_gen_bus.size > 0:
        for bus in ps_gen_bus:
            ...
            # Correct ps gen bus
            ppci_gen["bus"][bus, [GS_P, BS_P]] /= (
                ppci_gen["bus"][bus, K_G] / ppci_gen["bus"][bus, K_SG]
            )
            ppci_gen["bus"][bus, [GS, BS]] += (
                1 / (ppci_gen["bus"][bus, K_G]) - 1 / (ppci_gen["bus"][bus, K_SG])
            ...
            ppci_gen["branch"][trafo_ix, [BR_X, BR_R]] /= ppci_gen["branch"][
                trafo_ix, K_ST
            ]
