...
def _kappa_method_c(net, ppc):
    ...
    ppc_c["bus"][np.ix_(~np.isnan(ppc_c["bus"][:, GS_P]), [BS, GS])] = ppc_c["bus"][
        np.ix_(~np.isnan(ppc_c["bus"][:, GS_P]), [BS_P, GS_P])
    ]
...
def _kappa_method_b(net, ppc):
    ...
    if topology == "meshed":
        ...
        kappa_korr = np.full(ppc["bus"].shape[0], 1.0)
    if topology == "auto":
        kappa_korr = np.full(ppc["bus"].shape[0], 1.0)
...
def nxgraph_from_ppc(net, ppc):
    ...
    mg.add_edges_from(
        (int(branch[T_BUS]), int(branch[F_BUS]), {"r": branch[BR_R], "x": branch[BR_X]})
        for branch in ppc["branch"].real
    )
    mg.add_node("earth")
    vs_buses_pp = list(
