...
from pandapower.shortcircuit.currents import (
    _calc_ikss,
    _calc_ikss_1ph,
    _calc_ip,
    _calc_ith,
    _calc_branch_currents,
...
def calc_sc(
    ...
    if branch_results:
        logger.warning(
            "Branch results are in beta mode and might not always be reliable, "
...
def _calc_current(net, ppci_orig, bus):
    ...
    # update ppci
    non_ps_gen_ppci_bus, non_ps_gen_ppci, ps_gen_bus_ppci_dict = _create_k_updated_ppci(
        net, ppci_orig, ppci_bus=ppci_bus
    )
    ...
    for calc_bus in ps_gen_ppci_bus + [non_ps_gen_ppci_bus]:
        if isinstance(calc_bus, np.ndarray):
            ...
            this_ppci, this_ppci_bus = (
                ps_gen_bus_ppci_dict[calc_bus],
                np.array([calc_bus]),
            )
        ...
        _copy_result_to_ppci_orig(
            ppci_orig, this_ppci, this_ppci_bus, calc_options=net._options
        )
