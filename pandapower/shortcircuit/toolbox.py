...
def detect_power_station_unit(
    net, mode="auto", max_gen_voltage_kv=80, max_distance_km=0.01
):
    """
    ...
    """
    logger.info(
        "This function will overwrites the value 'power_station_trafo' in gen table"
    )
    net.gen["power_station_trafo"] = np.nan

    required_gen = net.gen.loc[
        net.bus.loc[net.gen.bus.values, "vn_kv"].values < max_gen_voltage_kv, :
    ]
    required_gen_bus = required_gen.loc[:, "bus"].values

    if mode.lower() == "auto":
        required_trafo = net.trafo.loc[
            net.bus.loc[net.trafo.lv_bus.values, "vn_kv"].values < max_gen_voltage_kv, :
        ]
        ...
        if "power_station_unit" in net.trafo.columns:
            ...
            logger.warning(
                "Using mode 'trafo' requires 'power_station_unit' defined for trafo! Using 'auto' mode instead!"
            )
            required_trafo = net.trafo.loc[
                net.bus.loc[net.trafo.lv_bus.values, "vn_kv"].values
                < max_gen_voltage_kv,
                :,
            ]
    ...
    g = create_nxgraph(
        net, respect_switches=True, nogobuses=None, notravbuses=trafo_hv_bus
    )

    for t_ix in required_trafo.index:
        t_lv_bus = required_trafo.at[t_ix, "lv_bus"]
        bus_dist = pd.Series(
            nx.single_source_dijkstra_path_length(g, t_lv_bus, weight="weight")
        )
        ...
        if len(gen_bus_at_lv_side) == 1:
            ...
            if not len(np.intersect1d(connected_bus_at_lv_side, trafo_lv_bus)) == 1:
                raise UserWarning(
                    "Failure in power station units detection! Parallel trafos on generator detected!"
                )
            if np.isin(required_gen_bus, gen_bus_at_lv_side).sum() > 1:
                logger.info(
                    "More than 1 gen detected at the lv side of a power station trafo! Will not be considered as power station unit"
                )
                continue
            net.gen.loc[
                np.isin(net.gen.bus.values, gen_bus_at_lv_side), "power_station_trafo"
            ] = t_ix
...
def _create_element_from_exisiting(net, ele_type, ele_ix):
    ...
    ps = pd.Series(
        net[ele_type].loc[ele_ix, :].to_dict(),
...
def _create_aux_net(net, line_ix, distance_to_bus0):
    if distance_to_bus0 < 0 or distance_to_bus0 > 1:
        raise UserWarning(
            "Calculating SC current on line failed! distance_to_bus0 must be between 0-1!"
        )
    ...
    aux_bus = create_bus(
        aux_net,
        vn_kv=aux_net.bus.at[aux_net.line.at[line_ix, "from_bus"], "vn_kv"],
    ...
    aux_net.line.at[aux_line0, "length_km"] = (
        distance_to_bus0 * aux_net.line.at[line_ix, "length_km"]
    )
    aux_net.line.at[aux_line0, "to_bus"] = aux_bus
    aux_net.line.at[aux_line0, "name"] += "_aux_line0"

    aux_net.line.at[aux_line1, "length_km"] = (1 - distance_to_bus0) * aux_net.line.at[
        line_ix, "length_km"
    ]
    ...
    for switch_ix in aux_net.switch.query(f" et == 'l' and element == {line_ix}").index:
        ...
        if (
            aux_net.switch.at[aux_switch_ix, "bus"]
            == aux_net.line.at[line_ix, "from_bus"]
        ):
