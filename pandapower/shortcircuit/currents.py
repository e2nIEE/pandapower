...
def _calc_ikss(net, ppc):
    ...
    if fault == "3ph":
        ppc["bus_sc"][:, IKSS1] = c / z_equiv  / ppc["bus"][:, BASE_KV] / np.sqrt(3) * ppc["baseMVA"]
    elif fault == "2ph":
        ppc["bus_sc"][:, IKSS1] = c / z_equiv / ppc["bus"][:, BASE_KV] / 2 * ppc["baseMVA"]
...
def _current_source_current(net, ppc):
    ...
    ppc["bus_sc"][buses, IKCV] = ikcv_pu
    ppc["bus_sc"][:, IKSS2] = abs(1 / np.diag(Zbus) * np.dot(Zbus, ppc["bus_sc"][:, IKCV] *-1j) / baseI)
    ppc["bus_sc"][buses, IKCV] /= baseI[buses]

def _calc_ip(net, ppc):
    ip = np.sqrt(2) * (ppc["bus_sc"][:, KAPPA] * ppc["bus_sc"][:, IKSS1] + ppc["bus_sc"][:, IKSS2])
    ppc["bus_sc"][:, IP] = ip
...
def _calc_ith(net, ppc):
    ...
    ppc["bus_sc"][:, M] = m
    ith = (ppc["bus_sc"][:, IKSS1] + ppc["bus_sc"][:, IKSS2])  * np.sqrt(m + n)
    ppc["bus_sc"][:, ITH] = ith
...
def _calc_branch_currents(net, ppc):
    ...
    V_ikss = (ppc["bus_sc"][:, IKSS1] * baseI) * Zbus
    ...
    current_sources = any(ppc["bus_sc"][:, IKCV]) > 0
    if current_sources:
        current = np.tile(-ppc["bus_sc"][:, IKCV], (n,1))
        np.fill_diagonal(current, current.diagonal() + ppc["bus_sc"][:, IKSS2])
    ...
    ppc["branch_sc"][:, IKSS_F] = minmax(ikss_all_f, axis=1) / baseI[fb]
    ppc["branch_sc"][:, IKSS_T] = minmax(ikss_all_t, axis=1) / baseI[tb]
    ...
    if net._options["ip"]:
        ...
        ppc["branch_sc"][:, IP_F] = minmax(abs(ip_all_f), axis=1) / baseI[fb]
        ppc["branch_sc"][:, IP_T] = minmax(abs(ip_all_t), axis=1) / baseI[tb]
    ...
    if net._options["ith"]:
        ...
        ppc["branch_sc"][:, ITH_F] = minmax(ith_all_f, axis=1) / baseI[fb]
        ppc["branch_sc"][:, ITH_T] = minmax(ith_all_t, axis=1) / baseI[fb]
