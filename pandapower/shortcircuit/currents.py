...
def _calc_ikss_1ph(net, ppci_0, ppci_1, ppci_2, bus_idx):
    ...
    # ToDo: 2Ph-G, hier überhaupt ein Unterschied in den Gleichungen?
    elif fault == "2ph-g":
    ...
    if net["_options"]["inverse_y"]:
        ...
        # ToDo: 2Ph-G, fehlende Gleichungen
        elif fault == "2ph-g":
            V_ikss_0 = 0 - ikss1 * Zbus_0[:, bus_idx] / 3  # initial value for zero-sequence voltage is 0
            V_ikss_1 = V0 - ikss1 * Zbus_1[:, bus_idx] / 3 if valid_V else 0 - ikss1 * Zbus_1[:, bus_idx] / 3
            V_ikss_2 = 0 - ikss1 * Zbus_2[:, bus_idx] / 3  # initial value for negative-sequence voltage is 0
        ...
        for ix, b in enumerate(bus_idx):
            ...
            # ToDo: 2Ph-G, siehe oben fehlende Gleichungen
            elif fault == "2ph-g":
                V_ikss_0[:, [ix]] = 0 - ybus_fact_0(ikss) / 3
                V_ikss_1[:, [ix]] = V0[:, [ix]] - ybus_fact_1(ikss) / 3 if valid_V else 0 - ybus_fact_1(ikss) / 3
                V_ikss_2[:, [ix]] = 0 - ybus_fact_2(ikss) / 3
    ...
    # ToDo: 2Ph-G, hier überhaupt ein Unterschied in den Gleichungen und ppci_1 und 2 notwendig?
...
def _current_source_current(net, ppci, bus_idx, sequence=1):
    ...
    if sgen_angle is not None and (fault == "3ph" or fault == "1ph" and type_c): #Todo auch 2Ph-G?
    ...
    if net["_options"]["inverse_y"]:
        ...
        if sgen_angle is None and (fault == "3ph" or fault == "1ph" and type_c): #Todo auch 2Ph-G?
    ...
    ppci["bus"][:, PHI_IKSS2_DEGREE] = np.angle(i_kss_2, deg=True) if (fault == "3ph" or fault == "1ph" and type_c) else 0 #Todo auch 2Ph-G?
