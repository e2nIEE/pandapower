...
def _determine_costs_dict(net, opf_task_overview):
    ...
    for flex_element in flex_elements:
        ...
        for cost_df in cost_dfs:
            idx_with_cost = set(
                net[cost_df].element[net[cost_df].et == flex_element].astype(np.int64)
            )
...
def res_power_columns(element_type, side=0):
    ...
    if element_type in pp_elements(branch_elements=False, other_elements=False):
        return ["p_mw", "q_mvar"]
    elif element_type in pp_elements(
        bus=False, bus_elements=False, other_elements=False
    ):
        ...
        if side != "all":
            ...
            cols = res_power_columns(element_type, side=0) + res_power_columns(
                element_type, side=1
            )
        ...
        raise ValueError(
            f"{element_type=} cannot be considered by res_power_columns()."
        )
