...
def get_connected_elements(
    net, element_type, buses, respect_switches=True, respect_in_service=False
):
    """
    ...
    if respect_switches and element_type in ["l", "t", "t3w"]:
        open_switches = get_connected_switches(
            net, buses, consider=element_type, status="open"
        )
        if open_switches:
...
def get_connected_buses(
    ...
    if "t" in consider or "trafo" in consider:
        ...
        cb |= set(
            net.trafo.loc[net.trafo.index.isin(connected_lvb_trafos)].hv_bus.values
        )
        cb |= set(
            net.trafo.loc[net.trafo.index.isin(connected_hvb_trafos)].lv_bus.values
        )
...
def get_connected_buses_at_element(
    net, element_index, element_type, respect_in_service=False
):
    """
...
def get_connected_buses_at_switches(net, switches):
    ...
    for element_type, et in zip(
        ["bus", "line", "trafo", "trafo3w"], ["b", "l", "t", "t3"]
    ):
        ...
        for bus_col in bebd[element_type]:
            buses |= set(
                net[element_type].loc[net.switch.element.loc[switches[is_et]], bus_col]
            )
    return buses
    ...
    for et in consider:
        if et == "b":
            ...
            if include_element_connections:
                ...
                element_buses = net[element_type].loc[
                    list(net.switch.element.loc[sw_idx]), bebd[element_type]
                ]
                isin_df = pd.concat(
                    [element_buses[col].isin(buses) for col in element_buses], axis=1
                )
    ...
    for et, bus_types in branch_dict.items():
        for bus1 in bus_types:
            for bus2 in bus_types:
                if bus2 != bus1:
                    idx = net[et].index[
                        net[et][bus1].isin(buses1) & net[et][bus2].isin(buses2)
                    ]
                    if et == "switch":
...
def false_elm_links(net, element_type, col, target_element_type):
    ...
    if isinstance(target_element_type, str):
        return net[element_type][col].index[
            ~net[element_type][col].isin(net[target_element_type].index)
        ]
    else:  # target_element_type is an iterable, e.g. a Series such as net["poly_cost"]["et"]
        df = pd.DataFrame(
            {
                "element": net[element_type][col].values,
                "et": target_element_type,
...
def false_elm_links_loop(net, element_types=None):
    ...
    element_types = (
        ...
        else pp_elements(bus=False, cost_tables=True)
    )
    ...
    for element_type in element_types:
        if net[element_type].shape[0]:
            ...
            if element_type in bebd.keys():
                ...
                fl = fl.union(
                    false_elm_links(
                        net, element_type, "element", net[element_type]["et"]
                    )
                )
            elif element_type == "measurement":
                fl = fl.union(
                    false_elm_links(
                        net, element_type, "element", net[element_type]["element_type"]
                    )
                )
            else:
...
def branch_element_bus_dict(include_switch=False, sort=None):
    ...
    """
    msg = (
        "The parameter 'sort' is deprecated to function branch_element_bus_dict() with "
...
def element_bus_tuples(bus_elements=True, branch_elements=True, res_elements=False):
    ...
    if Version(__version__) < Version("2.13"):
        logger.debug(
            "element_bus_tuples() returns a list of tuples instead of a set of tuples "
...
def count_elements(net, return_empties=False, **kwargs):
    ...
    return pd.Series(
        {
            et: net[et].shape[0]
            for et in pp_elements(**kwargs)
            if return_empties or bool(net[et].shape[0])
        },
        dtype=np.int64,
    )
