...
def select_subnet(
    ...
    ):  # Info: keep_everything_else might help to keep controllers but
    ...
    if "trafo_characteristic_table" in net:
        p2["trafo_characteristic_table"] = net["trafo_characteristic_table"][
            net["trafo_characteristic_table"].id_characteristic.isin(
                p2["trafo"].id_characteristic_table.values
            )
            | net["trafo_characteristic_table"].id_characteristic.isin(
                p2["trafo3w"].id_characteristic_table.values
            )
        ]
    for table in ["shunt_characteristic_table", "gen_characteristic_table"]:
        if table in net:
            p2[table] = net[table][
                net[table].id_characteristic.isin(
                    p2[table[:-21]].id_characteristic_table.values
                )
            ]

    if "trafo_characteristic_spline" in net:
        p2["trafo_characteristic_spline"] = net["trafo_characteristic_spline"][
            net["trafo_characteristic_spline"].id_characteristic.isin(
                p2["trafo"].id_characteristic_spline.values
            )
            | net["trafo_characteristic_spline"].id_characteristic.isin(
                p2["trafo3w"].id_characteristic_spline.values
            )
        ]
    for table in ["shunt_characteristic_spline", "gen_characteristic_spline"]:
        if table in net:
            p2[table] = net[table][
                net[table].id_characteristic.isin(
                    p2[table[:-22]].id_characteristic_spline.values
                )
            ]
        ...
        for table in net.keys():
            if (
                ...
                p2[table] = net[table].loc[
                    pd.Index(buses).intersection(net[table].index)
                ]
            else:
                p2[table] = net[table].loc[
...
def merge_nets(net1, net2, validate=True, merge_results=True, tol=1e-9, **kwargs):
    ...
    new_params = {
        "std_prio_on_net1",
        "return_net2_reindex_lookup",
        "net2_reindex_log_level",
    }
    msg1 = (
        "Since pandapower version 2.11.0, merge_nets() keeps element indices "
        + "and prioritize net1 standard types by default."
    )
    msg2 = f"Parameters {old_params} are deprecated."
    msg3 = (
        "To silence this warning, explicitely pass at least one of the new parameters "
        + f"{new_params}."
    )
    ...
    return _merge_nets(
        net1, net2, validate=validate, merge_results=merge_results, tol=tol, **kwargs
    )
...
def _merge_nets(
    ...
    **runpp_kwargs,
    ...
    elm_types = [
        elm_type
        for elm_type, df in net2.items()
        if (
            not elm_type.startswith("_")
            and isinstance(df, pd.DataFrame)
            and df.shape[0]
            and (not elm_type.startswith("res_") or (merge_results and not validate))
        )
    ]
    ...
    for elm_type in elm_types:
        ...
        if any(is_dupl):
            start = (
                max(net1[elm_type].index.max(), net2[elm_type].index[~is_dupl].max())
                + 1
            )
            old_indices = net2[elm_type].index[is_dupl]
            if elm_type == "group":
                old_indices = (
                    pd.Series(old_indices)
                    .loc[~pd.Series(old_indices).duplicated()]
                    .tolist()
                )
                ...
                if elm_type == "trafo_characteristic_table":
                    ...
                    reindex_lookup[elm_type] = {
                        ...
                        for old_id, new_id in zip(
                            sorted(combined_ids), range(id_start, id_max + 1)
                        )
                    }
            reindex_elements(net2, elm_type, lookup=reindex_lookup[elm_type])
    if len(reindex_lookup.keys()):
        log_to_level(
            "net2 elements of these types has been reindexed by merge_nets() because "
            + f"these exist already in net1: {list(reindex_lookup.keys())}",
            logger,
            net2_reindex_log_level,
        )
    ...
    if validate:
        ...
        dev1 = max(
            abs(
                net.res_bus.loc[net1.bus.index].vm_pu.values - net1_res_bus.vm_pu.values
            )
        )
        dev2 = max(
            abs(
...
def set_isolated_areas_out_of_service(net, respect_switches=True):
    ...
    if len(unsupplied):
        ...
        logger.info(
            f"Set {already_oos} of {len(unsupplied)} unsupplied buses out of service"
        )
    ...
    if len(net.trafo3w):
        ...
        status = trafo3ws[["hv_bus", "mv_bus", "lv_bus"]].apply(
            lambda col: bus_in_service.loc[col].values
        )
        ...
        mask_all3_open = (
            trafo3ws.index.to_series().map(open_counts).fillna(0).eq(3).values
        )
    ...
    if len(open_l) and len(net.line):
        ...
        other_bus = np.where(
            j["bus"].values == j["from_bus"].values,
            j["to_bus"].values,
            j["from_bus"].values,
        )
    ...
    if closed_switches:
        logger.info(
            f"closed {len(closed_switches)} switches: {sorted(closed_switches)}"
        )
...
def merge_same_bus_generation_plants(
    ...
    for bus in gen_df["bus"].loc[gen_df["bus"].duplicated()].unique():
        ...
        if "profiles" in net and col == "p_mw":
            ...
            elm_p = "%s.p_mw" % elm
            net.profiles[elm_p].loc[:, uniq_idx] = (
                net.profiles[elm_p].loc[:, gen_df["index"].loc[idxs]].sum(axis=1)
            )
            net.profiles[elm_p] = net.profiles[elm_p].drop(
                columns=gen_df["index"].loc[idxs[1:]]
            )
            if elm == "gen":
...
def fuse_buses(net, b1, b2, drop=True, fuse_bus_measurements=True):
    ...
    net["switch"].loc[
        (net["switch"]["et"] == "b") & (net["switch"]["element"].isin(b2)), "element"
    ] = b1
...
def drop_elements_simple(net, element_type, element_index):
    ...
    if element_type in ["bus", "line", "trafo", "trafo3w"]:
        logger.warning(
            f"drop_elements_simple() is not appropriate to drop {element_type}s. "
    ...
    if res_element_type in net.keys() and isinstance(
        net[res_element_type], pd.DataFrame
    ):
...
def drop_trafos(net, trafos, table="trafo"):
    ...
    logger.debug(
        f"Dropped {len(trafos)} {table}{plural_s(len(trafos))} with {num_switches} switches"
    )
...
def drop_lines(net, lines):
    ...
    if "line_geodata" in net:
        net["line_geodata"].drop(
            set(lines) & set(net["line_geodata"].index), inplace=True
        )
    res_lines = net.res_line.index.intersection(lines)
    net["res_line"].drop(res_lines, inplace=True)
    logger.debug(
        f"Dropped {len(lines)} line{plural_s(len(lines))} with {len(i)} line switches"
    )
...
def drop_elements_at_buses(
    ...
    """
    for element_type, column in element_bus_tuples(
        ...
        if element_type == "switch":
            ...
            if element_type == "line":
                ...
                for cost_elm in ["poly_cost", "pwl_cost"]:
                    net[cost_elm] = net[cost_elm].drop(
                        net[cost_elm].index[
...
def drop_measurements_at_elements(net, element_type, idx=None, side=None):
    ...
    bool3 = (
        ...
        if side is not None
        else np.full(net.measurement.shape[0], 1, dtype=bool)
    )
...
def drop_out_of_service_elements(net):
    ...
    if ebt:
        bus_series_list = [
            net[elm][bus_col] for elm, bus_col in ebt if bus_col in net[elm].columns
        ]
        if bus_series_list:
            do_not_delete = pd.Index(
                pd.concat(bus_series_list, ignore_index=True).unique()
            )
    ...
    for elm in elements:
        ...
        if elm == "impedance" and zero_impedance:
            ...
            branch_zero.update(
                net[elm]
                .loc[
                    (np.abs(net[elm].rft_pu) <= min_rft_pu)
                    & (np.abs(net[elm].xft_pu) <= min_xft_pu)
                    & (np.abs(net[elm].rtf_pu) <= min_rtf_pu)
                    & (np.abs(net[elm].xtf_pu) <= min_xtf_pu)
                ]
                .index.tolist()
            )
...
def replace_impedance_by_line(
    ...
    """
    index = (
        list(ensure_iterability(index))
        if index is not None
        else list(net.impedance.index)
    )
    ...
    for (idx, imp), max_i in zip(net.impedance.loc[index].iterrows(), max_i_ka):
        ...
        new_index.append(
            create_line_from_parameters(
                ...
                c_nf_per_km=0,
                max_i_ka=max_i,
                r0_ohm_per_km=imp.rft0_pu * Zni
                if "rft0_pu" in net.impedance.columns
                else np.nan,
                x0_ohm_per_km=imp.xft0_pu * Zni
                if "xft0_pu" in net.impedance.columns
                else np.nan,
                c0_nf_per_km=0,
                parallel=1,
                name=imp.name,
    ...
    _replace_group_member_element_type(
        net, index, "impedance", new_index, "line", detach_from_gr=False
    )
    drop_elements_simple(net, "impedance", index)

    # --- result data
    _adapt_result_tables_in_replace_functions(
        net, "impedance", index, "line", new_index
    )
...
def replace_line_by_impedance(net, index=None, sn_mva=None, only_valid_replace=True):
    ...
    """
    index = (
        list(ensure_iterability(index)) if index is not None else list(net.line.index)
    )
    ...
    for idx, line_ in net.line.loc[index].iterrows():
        if line_.c_nf_per_km or line_.g_us_per_km:
            if only_valid_replace:
                ...
                continue
            logger.error(
                f"Capacitance and dielectric conductance of line {idx} cannot be "
        ...
        new_index.append(
            create_impedance(
                ...
                sn_mva=sn_mva[i],
                rft0_pu=line_.r0_ohm_per_km * l / p / Zni
                if "r0_ohm_per_km" in cols
                else None,
                xft0_pu=line_.x0_ohm_per_km * l / p / Zni
                if "x0_ohm_per_km" in cols
                else None,
                gf0_pu=line_.g0_us_per_km * 1e-6 * Zni * l * p
                if "g0_us_per_km" in cols
                else None,
        ...
        i += 1
    _replace_group_member_element_type(
        net, index, "line", new_index, "impedance", detach_from_gr=False
    )
    drop_lines(net, index)

    # --- result data
    _adapt_result_tables_in_replace_functions(
        net, "line", index, "impedance", new_index
    )
...
def replace_ext_grid_by_gen(
    ...
    # --- result data
    _adapt_result_tables_in_replace_functions(
        net, "ext_grid", ext_grids, "gen", new_idx
    )
...
def replace_pq_elmtype(
    ...
    if old_element_type == new_element_type:
        logger.warning(
            f"'old_element_type' and 'new_element_type' are both '{old_element_type}'. "
    ...
    existing_cols_to_keep = (
        net[old_element_type]
        .loc[old_indices]
        .dropna(axis=1)
        .columns.intersection(cols_to_keep)
    )
    # add missing columns to net[new_element_type] which should be kept
    missing_cols_to_keep = existing_cols_to_keep.difference(
        net[new_element_type].columns
    )
    for col in missing_cols_to_keep:
    ...
    new_idx = []
    for oelm, index in zip(
        net[old_element_type].loc[old_indices].itertuples(), new_indices
    ):
        controllable = (
            False
            if "controllable" not in net[old_element_type].columns
            else oelm.controllable
        )
        ...
        if new_element_type == "load":
            ...
            args = {
                "max_e_mwh": 1
                if "max_e_mwh" not in net[old_element_type].columns
                else net[old_element_type].max_e_kwh.loc[old_indices]
            }
        idx = fct(
            net,
            oelm.bus,
    ...
    if sign == -1:
        for col1, col2 in zip(
            ...
            if col1 in existing_cols_to_keep:
                net[new_element_type].loc[new_idx, col2] = (
                    sign * net[old_element_type].loc[old_indices, col1].values
                )
                already_considered_cols |= {col1}
    net[new_element_type].loc[
...
def replace_ward_by_internal_elements(net, wards=None, log_level="warning"):
    ...
    for ward in net.ward.loc[wards].itertuples():
        ...
        attach_to_groups(
            net, ass[ward.Index], ["load", "shunt"], [[load_idx], [shunt_idx]]
        )
...
def replace_xward_by_internal_elements(net, xwards=None, set_xward_bus_limits=False):
    ...
    for xward in net.xward.loc[xwards].itertuples():
        ...
        vm_lims = (
            net.bus.loc[xward.bus, ["min_vm_pu", "max_vm_pu"]].tolist()
            if set_xward_bus_limits
            else default_vm_lims
        )
        new_bus = create_bus(
            net,
        ...
        new_gen = create_gen(
            net, new_bus, 0, xward.vm_pu, in_service=xward.in_service, name=xward.name
        )
        new_imp = create_impedance(
            net,
            xward.bus,
            new_bus,
            xward.r_ohm / (vn**2),
    ...
    if net.res_xward.shape[0]:
        log_to_level(
            "Implementations to move xward results to new internal elements are missing.",
            logger,
            "info",
        )
...
def replace_xward_by_ward(net, index=None, drop=True):
    ...
    for xi in index:
        wi = create_ward(
            net,
            net.xward.at[xi, "bus"],
            net.xward.at[xi, "ps_mw"],
            net.xward.at[xi, "qs_mvar"],
            net.xward.at[xi, "pz_mw"],
            net.xward.at[xi, "qz_mvar"],
            f"REPLACEMENT_xward_{xi}",
            net.xward.at[xi, "in_service"],
        )
        new_index.append(wi)

    _replace_group_member_element_type(
        net, index, "xward", new_index, "ward", detach_from_gr=False
    )
    if drop:
    ...
    for gr_index in gr_et.index:
        ...
        if any(isin):
            attach_to_group(
                ...
                gr_index,
                new_element_type,
                [new_elements.loc[isin].tolist()],
                reference_columns=gr_et.at[gr_index, "reference_column"],
            )
    if detach_from_gr:
        detach_from_groups(
            net, old_element_type, old_elements
        )  # sometimes done afterwarts when
...
def _adapt_profiles_in_replace_functions(
    ...
    keys_old = [
        ...
        if (key.startswith(f"{et_old}.") or key.startswith(f"res_{et_old}."))
    ]
    for key_old in keys_old:
        ...
        if key_new in net.profiles.keys():
            net.profiles[key_new] = pd.concat(
                [net.profiles[key_new], to_add], sort=True
            )
        else:
