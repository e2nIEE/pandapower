...
from pandapower.pypower.idx_brch_dc import DC_F_BUS, DC_T_BUS, DC_BR_STATUS, DC_BR_R
from pandapower.pypower.idx_bus import BUS_I
from pandapower.pypower.idx_vsc import VSC_BUS, VSC_BUS_DC, VSC_STATUS, VSC_X
...
def makeBdc(bus, branch, bus_dc, branch_dc, vsc, return_csr=True):
    """Builds the B matrices and phase shift injections for DC power flow.
    ...
    nvsc = vsc.shape[0]        ## number of vsc
    ...
    Pfinj, Pbusinj = phase_shift_injection(np.concatenate([b, b_vsc, b_dc]), np.concatenate([branch[:, SHIFT], np.zeros(nvsc + nl_dc)]), Cft)
...
def calc_b_from_vsc_dc(vsc):
    ...
    b = zeros_like(stat, dtype=float64)
    divide(stat, br_x, out=b, dtype=float64)  # series susceptance
    # if setpoint p available, dann das b=0 todo
    return b
