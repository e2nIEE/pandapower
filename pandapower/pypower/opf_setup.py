...
def opf_setup(ppc, ppopt):
        ...
        if nusr or nw:  # pragma: no cover
            ...
            if nw and (ppc["N"].shape[1] >= 2 * nb + 2 * ng):
                ...
                if ppc["N"][:, acc].nnz > 0:
                    ...
                    if any(ppc["Cw"][ii]) | (
                        ("H" in ppc) & (len(ppc["H"]) > 0) & any(any(ppc["H"][:, ii]))
                    ):
                        stderr.write(
    ...
    pwl1 = find(
        (ppc["gencost"][:, MODEL] == PW_LINEAR) & (ppc["gencost"][:, NCOST] == 2)
    )
    # p1 = array([])
    if len(pwl1) > 0:
        ...
        ppc["gencost"][pwl1, COST : COST + 2] = r_[
            "1", m.reshape(len(m), 1), b.reshape(len(b), 1)
    ...
    if (
        alg == 545 or alg == 550
    ):  ## SC-PDIPM or TRALM, no CCV cost vars # pragma: no cover
        ny = 0
        Ay = None
        by = array([])
    else:
