...
def printpf(baseMVA, bus=None, gen=None, branch=None, f=None, success=None,
    ...
    ## parameters
    ties = find(bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] !=
                   bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA])
                            ## area inter-ties
    ...
    if isDC:
        ...
        loss = baseMVA * abs(V[e2i[ branch[:, F_BUS].real.astype(np.int64) ]] / tap -
                             V[e2i[ branch[:, T_BUS].real.astype(np.int64) ]])**2 / \
                    (branch[:, BR_R] - 1j * branch[:, BR_X])

    fchg = abs(V[e2i[ branch[:, F_BUS].real.astype(np.int64) ]] / tap)**2 * branch[:, BR_B].real * baseMVA / 2
    tchg = abs(V[e2i[ branch[:, T_BUS].real.astype(np.int64) ]]      )**2 * branch[:, BR_B].real * baseMVA / 2
    loss[out] = zeros(nout)
    fchg[out] = zeros(nout)
    tchg[out] = zeros(nout)
    ...
    if OUT_AREA_SUM:
        ...
        for i in range(len(s_areas)):
            ...
            ig = find((bus[e2i[gen[:, GEN_BUS].astype(np.int64)], BUS_AREA] == a) & ~isload(gen))
            igon = find((bus[e2i[gen[:, GEN_BUS].astype(np.int64)], BUS_AREA] == a) & (gen[:, GEN_STATUS] > 0) & ~isload(gen))
            ildon = find((bus[e2i[gen[:, GEN_BUS].astype(np.int64)], BUS_AREA] == a) & (gen[:, GEN_STATUS] > 0) & isload(gen))
            inzld = find((bus[:, BUS_AREA] == a) & logical_or(bus[:, PD], bus[:, QD]))
            inzsh = find((bus[:, BUS_AREA] == a) & logical_or(bus[:, GS], bus[:, BS]))
            ibrch = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] == a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] == a))
            in_tie = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] == a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] != a))
            out_tie = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] != a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] == a))
            if not any(xfmr + 1):
                nxfmr = 0
            else:
                nxfmr = len(find((bus[e2i[branch[xfmr, F_BUS].real.astype(np.int64)], BUS_AREA] == a) & (bus[e2i[branch[xfmr, T_BUS].real.astype(np.int64)], BUS_AREA] == a)))
...
#                sum(gen[ong, PG]), sum(gen[ong, QG]) ))
        ...
        for i in range(len(s_areas)):
            a = s_areas[i]
            inzsh   = find((bus[:, BUS_AREA] == a) & logical_or(bus[:, GS], bus[:, BS]))
            ibrch   = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] == a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] == a) & branch[:, BR_STATUS].astype(bool))
            in_tie  = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] != a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] == a) & branch[:, BR_STATUS].astype(bool))
            out_tie = find((bus[e2i[branch[:, F_BUS].real.astype(np.int64)], BUS_AREA] == a) & (bus[e2i[branch[:, T_BUS].real.astype(np.int64)], BUS_AREA] != a) & branch[:, BR_STATUS].astype(bool))
    ...
    if OUT_GEN:
        if isOPF:
            genlamP = bus[e2i[gen[:, GEN_BUS].astype(np.int64)], LAM_P]
            genlamQ = bus[e2i[gen[:, GEN_BUS].astype(np.int64)], LAM_Q]
    ...
    if isOPF:
        ...
        if (ppopt['OPF_FLOW_LIM'] == 1) | isDC:  ## P limit
            ...
            Ff = abs( (branch[:, PF] + 1j * branch[:, QF]) / V[e2i[branch[:, F_BUS].real.astype(np.int64)]] )
            Ft = abs( (branch[:, PT] + 1j * branch[:, QT]) / V[e2i[branch[:, T_BUS].real.astype(np.int64)]] )
