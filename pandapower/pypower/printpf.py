...
def printpf(
    ...
    OUT_ANY = OUT_ANY | (
        (OUT_ALL == -1)
        and (OUT_SYS_SUM or OUT_AREA_SUM or OUT_BUS or OUT_BRANCH or OUT_GEN)
    )
    ...
    OUT_ANY = OUT_ANY or (
        (OUT_ALL_LIM == -1) and (OUT_V_LIM or OUT_LINE_LIM or OUT_PG_LIM or OUT_QG_LIM)
    )
    ...
    ## parameters
    ties = find(
        bus[e2i[branch[:, F_BUS].real.astype(int64)], BUS_AREA]
    ...
    if isDC:
        ...
        loss = (
            baseMVA
            * abs(
                V[e2i[branch[:, F_BUS].real.astype(int64)]] / tap
                - V[e2i[branch[:, T_BUS].real.astype(int64)]]
            )
            ** 2
    ...
    fchg = (
        abs(V[e2i[branch[:, F_BUS].real.astype(int64)]] / tap) ** 2
        * branch[:, BR_G]
        * baseMVA
        / 2
    )
    tchg = (
        abs(V[e2i[branch[:, T_BUS].real.astype(int64)]]) ** 2
        * branch[:, BR_G]
        * baseMVA
        / 2
    )
    ...
    if OUT_SYS_SUM:
        ...
        fd.write(
            ...
            % (nb, sum(gen[allg, PMAX]), sum(gen[allg, QMIN]), sum(gen[allg, QMAX]))
        )
        fd.write(
            "\nGenerators     %5d     On-line Capacity     %7.1f       %7.1f to %.1f"
            % (len(allg), sum(gen[ong, PMAX]), sum(gen[ong, QMIN]), sum(gen[ong, QMAX]))
        )
        fd.write(
        ...
        fd.write(
            ...
            % (
                len(ties),
                sum(abs(branch[ties, PF] - branch[ties, PT])) / 2,
    ...
    if OUT_AREA_SUM:
        ...
        for i in range(len(s_areas)):
            ...
            ig = find(
                (bus[e2i[gen[:, GEN_BUS].astype(int64)], BUS_AREA] == a) & ~isload(gen)
            )
            igon = find(
                (bus[e2i[gen[:, GEN_BUS].astype(int64)], BUS_AREA] == a)
                & (gen[:, GEN_STATUS] > 0)
                & ~isload(gen)
            )
            ildon = find(
            ...
            ibrch = find(
                (bus[e2i[branch[:, F_BUS].real.astype(int64)], BUS_AREA] == a)
                & (bus[e2i[branch[:, T_BUS].real.astype(int64)], BUS_AREA] == a)
            )
            in_tie = find(
                (bus[e2i[branch[:, F_BUS].real.astype(int64)], BUS_AREA] == a)
                & (bus[e2i[branch[:, T_BUS].real.astype(int64)], BUS_AREA] != a)
            ...
            if not any(xfmr + 1):
                ...
                nxfmr = len(
                    find(
                        (
                            bus[e2i[branch[xfmr, F_BUS].real.astype(int64)], BUS_AREA]
                            == a
                        )
                        & (
                            bus[e2i[branch[xfmr, T_BUS].real.astype(int64)], BUS_AREA]
                            == a
                        )
                    )
                )
        ...
        for i in range(len(s_areas)):
            ...
            fd.write(
                ...
                % (
                    a,
                    -sum(bus[inzsh, VM] ** 2 * bus[inzsh, GS]),
                    ...
                    sum(branch[in_tie, PT])
                    + sum(branch[out_tie, PF])
                    - sum(real(loss[r_[in_tie, out_tie]])) / 2,
                    sum(branch[in_tie, QT])
                    + sum(branch[out_tie, QF])
    ...
    if OUT_GEN:
        ...
        for k in range(len(ong)):
            ...
            if (gen[i, GEN_STATUS] > 0) & logical_or(gen[i, PG], gen[i, QG]):
                ...
                fd.write("       -         -  ")
            if isOPF:
                fd.write("%10.2f%10.2f" % (genlamP[i], genlamQ[i]))
        ...
        for i in range(nb):
            ...
            if logical_or(bus[i, PD], bus[i, QD]) | any(ld + 1):
                if any(ld + 1):
                    fd.write(
                        "%10.2f*%9.2f*"
                        % (bus[i, PD] - sum(gen[ld, PG]), bus[i, QD] - sum(gen[ld, QG]))
                    )
                else:
    ...
    if isOPF:
        ...
        if (ppopt["OPF_FLOW_LIM"] == 1) | isDC:  ## P limit
            ...
            Ff = abs(
                (branch[:, PF] + 1j * branch[:, QF])
                / V[e2i[branch[:, F_BUS].real.astype(int64)]]
            )
            Ft = abs(
                (branch[:, PT] + 1j * branch[:, QT])
                / V[e2i[branch[:, T_BUS].real.astype(int64)]]
            )
            strg = (
        ...
        if (OUT_LINE_LIM == 2) | (
            ...
            for i in range(nl):
                if (OUT_LINE_LIM == 2) | (
                    ...
                    fd.write(
                        "%9.2f%10.2f%10.2f" % (Ff[i], branch[i, RATE_A].real, Ft[i])
                    )
