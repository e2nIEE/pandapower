...
def makeAy(baseMVA, ng, gencost, pgbas, qgbas, ybas):
    ...
    m = sum( gencost[iycost, NCOST].astype(np.int64) )  ## total number of cost points
    ...
    k = 0
    for i in iycost:
        ns = gencost[i, NCOST].astype(np.int64)         ## # of cost points segments = ns-1
...
#        Ay[k:k + ns - 1, sidx] = m
    ...
    j = 0
    for i in iycost:
        ns = gencost[i, NCOST].astype(np.int64)
        ## FIXME: Bug in SciPy 0.7.2 prevents setting with a sequence
