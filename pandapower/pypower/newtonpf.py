...
from pandapower.pf.makeYbus_facts import (
    makeYbus_svc,
    makeYft_tcsc,
    calc_y_svc_pu,
    makeYbus_ssc_vsc,
...
from pandapower.pypower.idx_svc import (
    SVC_BUS,
    SVC_STATUS,
    SVC_CONTROLLABLE,
    SVC_X_L,
    ...
    SVC_X_PU,
    SVC_SET_VM_PU,
    SVC_THYRISTOR_FIRING_ANGLE,
    SVC_MAX_FIRING_ANGLE,
    SVC_MIN_FIRING_ANGLE,
    SVC_Q,
)
from pandapower.pypower.idx_ssc import (
    SSC_BUS,
    SSC_R,
    SSC_X,
    SSC_SET_VM_PU,
...
from pandapower.pf.create_jacobian_facts import (
    create_J_modification_svc,
    create_J_modification_tcsc,
    create_J_modification_ssc_vsc,
    create_J_modification_hvdc,
)
...
def newtonpf(Ybus, Sbus, V0, ref, pv, pq, ppci, options, makeYbus=None):
    ...
    ssc = ppci["ssc"]
    vsc = ppci["vsc"]
    source_dc = ppci["source_dc"]
    slack_weights = bus[:, SL_FAC].astype(
        float64
    ...
    x_control_ssc = ssc[ssc_branches, SSC_X_CONTROL_VM].real * np.exp(
        1j * ssc[ssc_branches, SSC_X_CONTROL_VA].real
    )
    ...
    vsc_gl_pu = vsc[
        vsc_branches, VSC_PL_DC
    ]  # / np.square(bus_dc[vsc[vsc_branches, VSC_BUS_DC].astype(np.int64), DC_BASE_KV])
    any_vsc = num_vsc > 0
    ...
    if any_vsc:
        bus_dc[vsc[:, VSC_BUS_DC].astype(int), DC_BUS_TYPE] = (
            DC_P  # todo implement vsc / sources / dc slacks
        )
    # bus_dc[4, 1] = 2.0
    ...
    num_branch_dc = (
        len(hvdc_branches) + num_vsc
    )  # internal vsc resistance is also a dc branch
    with np.errstate(all="raise"):
    ...
    # todo vsc
    vsc_group_buses_p, P_dc_sum, vsc_group_buses_p_number = _sum_by_group(
        vsc_dc_tb[p_set_point_index],
        -vsc_value_dc[p_set_point_index],
        np.ones(sum(p_set_point_index)),
    )
    # P_dc[vsc_group_buses_p] = P_dc_sum
    vsc_slack_p_dc_bus, _, _ = _sum_by_group(
        vsc_dc_tb[vsc_mode_ac == VSC_MODE_AC_SL],
        vsc_dc_tb[vsc_mode_ac == VSC_MODE_AC_SL],
        vsc_dc_tb[vsc_mode_ac == VSC_MODE_AC_SL],
    )
    ...
    num_facts_controllable = (
        num_svc_controllable + num_tcsc_controllable
    )  # + 2 * num_ssc_controllable
    ...
    if len(vsc_value_dc[v_set_point_index]) > 0:
        ...
        V_dc[vsc[v_set_point_index, VSC_BUS_DC].astype(np.int64)] = vsc_value_dc[
            v_set_point_index
        ]
    ...
    # SSC
    Ybus_ssc_not_controllable, Ybus_ssc_controllable, Ybus_ssc = makeYbus_ssc_vsc(
        Ybus, ssc_y_pu, ssc_fb, ssc_tb, ssc_controllable
    )
    # VSC
    Ybus_vsc_not_controllable, Ybus_vsc_controllable, Ybus_vsc = makeYbus_ssc_vsc(
        Ybus, vsc_y_pu, vsc_fb, vsc_tb, vsc_controllable
    )
    # HVDC
    Ybus_vsc_dc = make_Ybus_facts(
        vsc_dc_fb, vsc_dc_tb, vsc_g_pu, num_bus_dc, ysf_pu=vsc_gl_pu, dtype=np.float64
    )
    Ybus_hvdc = (
        make_Ybus_facts(hvdc_fb, hvdc_tb, hvdc_y_pu, num_bus_dc, dtype=np.float64)
        + Ybus_vsc_dc
    )
    Yf_vsc_dc, Yt_vsc_dc = make_Yft_facts(
        vsc_dc_fb, vsc_dc_tb, vsc_g_pu, num_bus_dc, ysf_pu=vsc_gl_pu
    )
    ...
    pvpq_lookup = zeros(
        max((Ybus + Ybus_svc + Ybus_tcsc + Ybus_ssc + Ybus_vsc).indices) + 1,
        dtype=np.int64,
    )
    if dist_slack:
    ...
    pq_lookup = np.array([-1] * (max(refpvpq) + 1), dtype=np.int64)
    ...
    j7 = j6c
    j6d = j6c + len(dc_ref)  # number of dc slacks
    ...
    F = _evaluate_Fx(
        Ybus + Ybus_svc + Ybus_tcsc + Ybus_ssc + Ybus_vsc,
        V,
        Sbus,
        ref,
        pv,
    ...
    if any_facts_controllable or any_ssc_controllable or any_vsc_controllable:
        mis_facts = _evaluate_Fx_facts(
            V,
            pq,
            svc_buses[svc_controllable],
            svc_set_vm_pu[svc_controllable],
            tcsc_controllable,
            tcsc_set_p_pu,
            tcsc_tb,
            Ybus_tcsc,
            ssc_fb[ssc_controllable],
            ssc_tb[ssc_controllable],
            Ybus_ssc,
            ssc_controllable,
            ssc_set_vm_pu,
            F,
    ...
    if tdpf:
        ...
        r_ref_pu = (
            branch[tdpf_lines, BR_R_REF_OHM_PER_KM].real
            * branch[tdpf_lines, BR_LENGTH_KM].real
            / z_base_ohm
        )
        ...
        a0, a1, a2, tau = calc_a0_a1_a2_tau(
            ...
            alpha_pu=alpha_pu,
            solar_absorptivity=branch[tdpf_lines, GAMMA].real,
            emissivity=branch[tdpf_lines, EPSILON].real,
            T_base=T_base,
            i_base_a=i_base_a,
        )
    ...
    while not converged and i < max_it:
        ...
        if tdpf:
            ...
            branch[tdpf_lines, BR_R] = r = r_ref_pu * (
                1 + alpha_pu * np.clip(np.nan_to_num(T - t_ref_pu), -50, 250 / T_base)
            )
        ...
        if tdpf:
            ...
            J = create_J_tdpf(
                ...
                alpha_pu,
                r_ref_pu,
                refpvpq if dist_slack else pvpq,
                pq,
                pvpq_lookup,
                pq_lookup,
                tau,
        ...
        if any_facts_controllable:
            K_J = vstack(
                [
                    eye(J.shape[0], format="csr"),
                    csr_matrix((num_facts_controllable, J.shape[0])),
                ],
                format="csr",
            )
            J = K_J * J * K_J.T  # this extends the J matrix with 0-rows and 0-columns
        if any_svc:
            J_m_svc = create_J_modification_svc(
                J,
                svc_buses,
                refpvpq if dist_slack else pvpq,
                pq,
                ...
                svc_x_l_pu,
                svc_x_cvar_pu,
                num_svc_controllable,
                svc_controllable,
            )
            J = J + J_m_svc
        if any_tcsc:
            J_m_tcsc = create_J_modification_tcsc(
                J,
                V,
                y_tcsc_pu,
                x_control_tcsc,
        ...
        if any_ssc_controllable:
            J_m_ssc = create_J_modification_ssc_vsc(
                J,
                V,
                Vm,
                ssc_y_pu[ssc_controllable],
                ssc_fb[ssc_controllable],
                ssc_tb[ssc_controllable],
                refpvpq if dist_slack else pvpq,
                pq,
        ...
        if any_vsc_controllable:
            J_m_vsc = create_J_modification_ssc_vsc(
                J=J,
                V=V,
                Vm=Vm,
                y_pu=vsc_y_pu[vsc_controllable],
                f=vsc_fb[vsc_controllable],
                t=vsc_tb[vsc_controllable],
                pvpq=refpvpq if dist_slack else pvpq,
                pq=pq,
                pvpq_lookup=pvpq_lookup,
                pq_lookup=pq_lookup,
                control_v=(vsc_mode_ac == VSC_MODE_AC_V)
                | (vsc_mode_ac == VSC_MODE_AC_SL),
                control_delta=vsc_mode_ac == VSC_MODE_AC_SL,
            )
            J = J + J_m_vsc
        if any_branch_dc:
        ...
        if any_svc_controllable:
            Ybus_svc = makeYbus_svc(
                Ybus, x_control_svc, svc_x_l_pu, svc_x_cvar_pu, svc_buses
            )
        ...
        if any_facts_controllable or any_ssc_controllable or any_vsc_controllable:
            mis_facts = _evaluate_Fx_facts(
                ...
                pq_lookup,
                vsc_controllable,
                vsc_fb[vsc_controllable],
                vsc_tb[vsc_controllable],
                Ybus_vsc,
                Ybus_vsc_dc,
                Yf_vsc_dc,
                Yt_vsc_dc,
        ...
        if tdpf:
            i_square_pu, p_loss_pu = calc_i_square_p_loss(
                branch, tdpf_lines, g, b, Vm, Va
            )
            if tdpf_update_r_theta:
                r_theta_pu = calc_r_theta(t_air_pu, a0, a1, a2, i_square_pu, p_loss_pu)
            T_calc = calc_T_frank(
                p_loss_pu, t_air_pu, r_theta_pu, tdpf_delay_s, T0, tau
            )
    ...
    if any_tcsc:
        ...
        tcsc[tcsc_branches, TCSC_X_PU] = 1 / calc_y_svc_pu(
            x_control_tcsc, tcsc_x_l_pu, tcsc_x_cvar_pu
        )
    ...
    # todo move this to pfsoln
    # calculate the power and currents running through the dc buses and lines
    if any_branch_dc:
        ...
        # calculate the dc sources and fill them with the correct values
        if source_dc.shape[0] > 0:
            source_bus = flatnonzero(bus_dc[relevant_bus_dc, DC_BUS_TYPE] == DC_REF)
            # source_dc[source_bus, SOURCE_DC_PG] = bus_dc[source_bus, DC_PD]
            source_dc[source_dc[:, SOURCE_DC_BUS] == source_bus, SOURCE_DC_PG] = bus_dc[
                source_bus, DC_PD
            ]
...
def _evaluate_Fx(
    Ybus, V, Sbus, ref, pv, pq, slack_weights=None, dist_slack=False, slack=None
):
    # evalute F(x)
    if dist_slack:
...
def _evaluate_Fx_facts(
    V,
    pq,
    svc_buses=None,
    svc_set_vm_pu=None,
    tcsc_controllable=None,
    tcsc_set_p_pu=None,
    tcsc_tb=None,
    Ybus_tcsc=None,
    ssc_fb=None,
    ssc_tb=None,
    Ybus_ssc=None,
    ssc_controllable=None,
    ssc_set_vm_pu=None,
    old_F=None,
    pq_lookup=None,
    vsc_controllable=None,
    vsc_fb=None,
    vsc_tb=None,
    Ybus_vsc=None,
    Ybus_vsc_dc=None,
    ...
    if tcsc_tb is not None and len(tcsc_tb) > 0 and len(tcsc_controllable) > 0:
    ...
    if num_branch_dc > 0:  # todo if hvdc lines
        ...
        if len(vsc_dc_mode_p) > 0:
            mis_hvdc[vsc_dc_tb[vsc_dc_mode_p]] = (
                p_vsc_dc_f[vsc_dc_mode_p] - vsc_value_dc[vsc_dc_mode_p]
            )

        if len(vsc_dc_mode_v) > 0:
            mis_hvdc[vsc_dc_tb[vsc_dc_mode_v]] = (
                V_dc[vsc_dc_fb[vsc_dc_mode_v]] - vsc_value_dc[vsc_dc_mode_v]
            )
    ...
    if len(vsc_controllable) > 0:
        ...
        if np.any(ac_mode_q):
            ...
            mis_vsc_q = (
                sf_vsc.imag[ac_mode_q] - vsc_value_ac[ac_mode_q]
            )  # todo test for when they share same bus
        ...
        if np.any(ac_mode_sl):
            mis_vsc_delta = (
                np.angle(V[vsc_fb[ac_mode_sl]]) - 0
            )  # <- here we set delta set point to zero, but can be a parameter in the future
            ...
            vsc_slack_p_dc_bus, vsc_slack_p_dc, _ = _sum_by_group(
                vsc_dc_tb[ac_mode_sl], vsc_slack_p, vsc_slack_p
            )
        ...
        # find the P DC buses
        vsc_set_p_pu = np.zeros(
            len(vsc_fb[vsc_controllable]), dtype=np.float64
        )  # power sign convention issue
        ...
        if np.any(vsc_dc_ref):
            ...
            vsc_set_p_pu[vsc_dc_ref] = -Pbus_hvdc[
                vsc_dc_tb[vsc_dc_ref]
            ]  # - Pbus_hvdc[vsc_dc_fb[vsc_dc_ref]]
        ...
        mis_vsc_p = (
            Sbus_vsc[vsc_tb[vsc_controllable]].real - vsc_set_p_pu
        )  # this is coupling the AC and the DC sides # todo old
