...
from pandapower.pypower.pipsopf_solver import (
    pipsopf_solver,
)  # temporary changed import to match bugfix path
    ...
    if success:
        if not dc:
            ...
            if (ll["N"]["PQh"] > 0) | (ll["N"]["PQl"] > 0):  # pragma: no cover
                ...
                results["gen"] = update_mupq(
                    results["baseMVA"], results["gen"], mu_PQh, mu_PQl, Apqdata
                )
            ...
            if ppopt["RETURN_RAW_DER"]:  # pragma: no cover
                ...
                if "dg" not in raw:
                    ...
                    g, geq, dg, dgeq = opf_consfcn(
                        results["x"], om, Ybus, Yf, Yt, ppopt
                    )
                    raw["g"] = r_[geq, g]
        ...
        if ll["N"]["ang"] > 0:
            ...
            results["branch"][iang, MU_ANGMIN] = (
                results["mu"]["lin"]["l"][ll["i1"]["ang"] : ll["iN"]["ang"]] * pi / 180
            )
            results["branch"][iang, MU_ANGMAX] = (
                results["mu"]["lin"]["u"][ll["i1"]["ang"] : ll["iN"]["ang"]] * pi / 180
            )
    else:
