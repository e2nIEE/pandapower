...
def pipsopf_solver(om, ppopt, out_opt=None):
    ...
    hess_fcn = lambda x, lmbda, cost_mult: opf_hessfcn(
        x, lmbda, om, Ybus, Yf[il, :], Yt[il, :], ppopt, il, cost_mult
    )

    solution = pips(f_fcn, x0, A, l, u, xmin, xmax, gh_fcn, hess_fcn, opt)
    x, f, info, lmbda, output = (
        solution["x"],
        solution["f"],
        solution["eflag"],
        solution["lmbda"],
    ...
    Sf = V[branch[:, F_BUS].astype(int64)] * conj(
        Yf * V
    )  ## cplx pwr at "from" bus, p["u"].
    St = V[branch[:, T_BUS].astype(int64)] * conj(
        Yt * V
    ...
    muSt = zeros(nl)
    if len(il) > 0:
        muSf[il] = 2 * lmbda["ineqnonlin"][:nl2] * branch[il, RATE_A] / baseMVA
        muSt[il] = (
    ...
    bus[:, LAM_P] = lmbda["eqnonlin"][nn["i1"]["Pmis"] : nn["iN"]["Pmis"]] / baseMVA
    bus[:, LAM_Q] = lmbda["eqnonlin"][nn["i1"]["Qmis"] : nn["iN"]["Qmis"]] / baseMVA
