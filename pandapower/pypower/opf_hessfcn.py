...
def opf_hessfcn(x, lmbda, om, Ybus, Yf, Yt, ppopt, il=None, cost_mult=1.0):
    ...
    i = r_[
        arange(vv["i1"]["Pg"], vv["iN"]["Pg"]), arange(vv["i1"]["Qg"], vv["iN"]["Qg"])
    ]
    ...
    if issparse(N) and N.nnz > 0:  # pragma: no cover
        ...
        d2f = (
            d2f
            + AA * H * AA.T
            + 2 * N.T * M * QQ * sparse((HwC, (arange(nw), arange(nw))), (nw, nw)) * N
        )
    d2f = d2f * cost_mult
    ...
    d2G = vstack(
        [
            hstack(
                [
                    vstack([hstack([Gpaa, Gpav]), hstack([Gpva, Gpvv])]).real
                    + vstack([hstack([Gqaa, Gqav]), hstack([Gqva, Gqvv])]).imag,
                    sparse((2 * nb, nxtra)),
                ]
            ),
            hstack([sparse((nxtra, 2 * nb)), sparse((nxtra, nxtra))]),
        ],
        "csr",
    )
    ...
    if ppopt["OPF_FLOW_LIM"] == 2:  ## current
        ...
        if ppopt["OPF_FLOW_LIM"] == 1:  ## real power
            Hfaa, Hfav, Hfva, Hfvv = d2ASbr_dV2(
                dSf_dVa.real, dSf_dVm.real, Sf.real, Cf, Yf, V, muF
            )
            Htaa, Htav, Htva, Htvv = d2ASbr_dV2(
                dSt_dVa.real, dSt_dVm.real, St.real, Ct, Yt, V, muT
            )
        else:  ## apparent power
    ...
    d2H = vstack(
        [
            hstack(
                [
                    vstack([hstack([Hfaa, Hfav]), hstack([Hfva, Hfvv])])
                    + vstack([hstack([Htaa, Htav]), hstack([Htva, Htvv])]),
                    sparse((2 * nb, nxtra)),
                ]
            ),
            hstack([sparse((nxtra, 2 * nb)), sparse((nxtra, nxtra))]),
