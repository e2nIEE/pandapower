...
def _make_bibc_bcbv(bus, branch, graph):
    ...
    branches_lst = list(
        zip(
            branch[:, F_BUS].real.astype(np.int64),
            branch[:, T_BUS].real.astype(np.int64),
        )
    )
    ...
    branches_ind_dict.update(
        dict(zip(zip(branches_arr[:, 1], branches_arr[:, 0]), range(0, nobranch)))
    )
    ...
    for ref in refs:
        ...
        if norefs > 1:
            branches_sub_mask = np.isin(
                branches_arr[:, 0], buses_ordered_bfs
        ...
        if G_tree.nnz < branches.shape[0]:
            ...
            branches_tree = np.sort(
                np.array([G_tree_nnzs[0], G_tree_nnzs[1]]).T, axis=1
            )
            branches_loops = set(zip(branches[:, 0], branches[:, 1])) - set(
                zip(branches_tree[:, 0], branches_tree[:, 1])
            )
        ...
        for loop_i, brch_loop in enumerate(branches_loops):
            path_lens, path_preds = csgraph.shortest_path(
                G_tree, directed=False, indices=brch_loop, return_predecessors=True
            )
            init, end = brch_loop
            loop = [end]
            while init != end:
    ...
    BIBC = csr_matrix(
        (data_BIBC, (rowi_BIBC, np.array(coli_BIBC) - norefs)),
        shape=(nobranch, nobranch),
    )
    BCBV = csr_matrix(
        (data_BCBV, (rowi_BIBC, np.array(coli_BIBC) - norefs)),
        shape=(nobranch, nobranch),
    ).transpose()
...
def _get_bibc_bcbv(ppci, options, bus, branch, graph):
    ...
    if (
        isinstance(recycle, dict)
        and not recycle["trafo"]
        and ppci["internal"]["DLF"].size
    ):
        DLF, buses_ordered_bfs_nets = (
            ppci["internal"]["DLF"],
        ...
        if isinstance(recycle, dict) and not recycle["trafo"]:
            ppci["internal"]["DLF"], ppci["internal"]["buses_ord_bfs_nets"] = (
                DLF,
                buses_ordered_bfs_nets,
            )
...
def _makeYsh_bfsw(bus, branch, baseMVA):
    ...
    Gch = np.bincount(
        branch[:, F_BUS].real.astype(np.int64), weights=ysh_f.real, minlength=nobus
    ) + np.bincount(
        branch[:, T_BUS].real.astype(np.int64), weights=ysh_t.real, minlength=nobus
    )
    Bch = np.bincount(
        branch[:, F_BUS].real.astype(np.int64), weights=ysh_f.imag, minlength=nobus
    ) + np.bincount(
        branch[:, T_BUS].real.astype(np.int64), weights=ysh_t.imag, minlength=nobus
    )
    ...
    **kwargs,
    ...
    while not converged and n_iter < max_it:
        ...
        while not inner_loop_converged and len(pv) > 0:
            pvi = (
                pv - norefs
            )  # internal PV buses indices, assuming reference node is always 0
            ...
            if enforce_q_lims:  # check Q violation limits
                ...
                if qg_min_lim.any():
                    ...
                    bus[gen[qg_min_lim, GEN_BUS].astype(np.int64), BUS_TYPE] = (
                        1  # convert to PQ bus
                    )

                if qg_max_lim.any():
                    gen[qg_max_lim, QG] = gen[qg_max_lim, QMAX]
                    bus[gen[qg_max_lim, GEN_BUS].astype(np.int64), BUS_TYPE] = (
                        1  # convert to PQ bus
                    )
            ...
            Sbus = (
                makeSbus(baseMVA, bus, gen, vm=abs(V))
                if voltage_depend_loads
                else makeSbus(baseMVA, bus, gen)
            )
...
def _run_bfswpf(ppci, options, **kwargs):
    ...
    enforce_q_lims, tolerance_mva, max_iteration, calculate_voltage_angles, numba = (
    ...
    G = csr_matrix((np.ones(nobranch), (bus_from, bus_to)), shape=(nobus, nobus))
    ...
    # #-----  run the power flow  -----
    V_final, success, iterations = _bfswpf(
        DLF,
        bus,
        gen,
        branch,
    ...
    if calculate_voltage_angles and any_trafo_shift:
        ...
        trafos_shift = dict(
            list(
                zip(
                    list(
                        zip(
                            branch[brch_shift_mask, F_BUS].real.astype(np.int64),
        ...
        for trafo_ind, shift_degree in trafos_shift.items():
            ...
            if np.argwhere(buses_ordered_bfs == trafo_ind[0]) < np.argwhere(
                buses_ordered_bfs == trafo_ind[1]
            ):
                lv_bus = trafo_ind[1]
                shift_degree *= -1
            else:
    ...
    ppci = _store_results_from_pf_in_ppci(
        ppci, bus, gen, branch, success, iterations, et
    )
