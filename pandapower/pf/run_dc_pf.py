...
from pandapower.pypower.makeBdc import (
    makeBdc,
...
def _run_dc_pf(ppci, recycle=False):
    ...
    if (
        ...
        and "Bbus" in ppci["internal"]
        and np.array_equal(ppci["internal"]["branch"][:, TAP], ppci["branch"][:, TAP])
    ):
    ...
    gen[ref_gens, PG] = real(
        gen[ref_gens, PG]
        + (B[refgenbus, :] * Va - Pbus[refgenbus]) * baseMVA / ext_grids_bus[refgenbus]
    )
