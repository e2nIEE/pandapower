...
def _get_elements(params, net, element, phase, typ):
    ...
    if len(elm):
        if element == "load" or element == "sgen":
            ...
            vl = elm[active, scaling].ravel()
            p = {
                "a": net[element].columns.get_loc("p_a_mw"),
                "b": net[element].columns.get_loc("p_b_mw"),
                "c": net[element].columns.get_loc("p_c_mw"),
            }
            q = {
                "a": net[element].columns.get_loc("q_a_mvar"),
...
def runpp_3ph(
    ...
    **kwargs,
    ...
    # calculate_voltage_angles = True        # Already default to True in arguments
    if init == "results" and net.res_bus_3ph.empty:
        init = "auto"
    if init == "auto":
        init = "dc" if calculate_voltage_angles else "flat"
    default_max_iteration = {"nr": 30, "bfsw": 10, "gs": 10000, "fdxb": 30, "fdbx": 30}
    ...
    (
        ...
        y_1_t,
        y_2_t,
    ) = _get_y_bus(ppci0, ppci1, ppci2, recycle)
    # =========================================================================
    ...
    while (s_mismatch > outer_tolerance_mva).any() and count < max_iteration:
        ...
        i_abc_it_delta = np.matmul(
            i_del_xfmn,
            (np.divide(s_abc_delta_pu, np.matmul(v_del_xfmn, v_abc_it))).conjugate(),
        )
