...
# #    J_C_C_d[:, tcsc_j] = A * np.sin(np.angle(V[tcsc_i]) - phi_tcsc_ij - np.angle(V[tcsc_j]))
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pvpq, nf_pvpq), (tcsc_in_pvpq_t, mt_pvpq, nt_pvpq)):
        i = pvpq_lookup[m]
        j = pvpq_lookup[n]
        J_C_C_d[x_control_lookup==1, i] = np.abs(V[m]) * np.abs(Ybus_tcsc[m, n]) * np.abs(V[n]) * np.sin(np.angle(V[m]) - np.angle(V[n]) + np.angle(Ybus_tcsc[m,n]))  #### A_ij = A_ji    phi_tcsc_ij = phi_tcsc_ji
