...
def create_J_modification_tcsc(J, branch, pvpq_lookup, pq_lookup, Ybus_tcsc, V, tcsc_i, tcsc_j, tcsc_fb, tcsc_tb, pvpq, pq, tcsc_branches,
    ...
    p_tcsc_ij = np.zeros(shape=(Ybus_tcsc.shape[0], 1), dtype=np.float64)
    q_tcsc_ii = np.zeros(shape=(Ybus_tcsc.shape[0], 1), dtype=np.float64)
    q_tcsc_ij = np.zeros(shape=(Ybus_tcsc.shape[0], 1), dtype=np.float64)
    ...
    for in_pq, m, n in zip([tcsc_in_pq_t], [mt_pq], [nt_pq]):
        i = pq_lookup[m]
        j = pq_lookup[n]
        J_C_Q_u[i, i] = 2*q_tcsc_ii[i] + q_tcsc_ij[m]
        J_C_Q_u[i, j] = q_tcsc_ij[m]
...
# #    J_C_C_d[:, tcsc_j] = A * np.sin(np.angle(V[tcsc_i]) - phi_tcsc_ij - np.angle(V[tcsc_j]))
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pvpq, nf_pvpq), (tcsc_in_pvpq_t, mt_pvpq, nt_pvpq)):
        i = pvpq_lookup[m]
        j = pvpq_lookup[n]
        J_C_C_d[x_control_lookup==1, i] = A_ij * np.sin(np.angle(V[n]) + phi_tcsc_ij[m] - np.angle(V[m]))  #### A_ij = A_ji    phi_tcsc_ij = phi_tcsc_ji
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pq, nf_pq), (tcsc_in_pvpq_t, mt_pq, nt_pq)):
        ...
        j = pq_lookup[n]
        if len(m) == 0: continue
        J_C_C_u[x_control_lookup==1, i] = A_ij / abs(V[m]) * np.cos(np.angle(V[n]) - np.angle(V[m]) + phi_tcsc_ij[m])
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pvpq, nf_pvpq), (tcsc_in_pvpq_t, mt_pvpq, nt_pvpq)):
        ...
        B = abs(V[m]) * abs(V[n]) * np.cos(np.angle(V[n]) - np.angle(V[m]) + phi_tcsc_ij[m])
...
def calc_tcsc_p_pu(Ybus, V, tcsc_fb, tcsc_tb):
...
def calc_tcsc_q_pu(Ybus, V, tcsc_fb, tcsc_tb):
    Vm_f = np.abs(V[tcsc_fb])
    Va_f = np.angle(V[tcsc_fb])
    Vm_t = np.abs(V[tcsc_tb])
    Va_t = np.angle(V[tcsc_tb])
    ...
    y_ii_pu = Ybus[tcsc_fb, tcsc_fb]
    y_jj_pu = Ybus[tcsc_tb, tcsc_tb]
    y_ij_pu = Ybus[tcsc_fb, tcsc_tb]
    y_ji_pu = Ybus[tcsc_tb, tcsc_fb]
