...
def create_J_modification_tcsc(J, branch, pvpq_lookup, pq_lookup, Ybus_tcsc, V, tcsc_i, tcsc_j, tcsc_fb, tcsc_tb, pvpq, pq, tcsc_branches,
    ...
    p_tcsc_ij = (V * np.conj(Ybus_tcsc * V)).real
    ...
    q_tcsc_ij = (V * np.conj(Ybus_tcsc * V)).imag - q_tcsc_ii
...
# #    J_C_C_d[:, tcsc_j] = A * np.sin(np.angle(V[tcsc_i]) - phi_tcsc_ij - np.angle(V[tcsc_j]))
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pvpq, nf_pvpq), (tcsc_in_pvpq_t, mt_pvpq, nt_pvpq)):
        i = pvpq_lookup[m]
        j = pvpq_lookup[n]
        J_C_C_d[x_control_lookup==1, i] = np.abs(V[m]) * np.abs(Ybus_tcsc[m, n]) * np.abs(V[n]) * np.sin(np.angle(V[m]) - np.angle(V[n]) + 0*np.angle(Ybus_tcsc[m,n]))  #### A_ij = A_ji    phi_tcsc_ij = phi_tcsc_ji
    ...
    for in_pvpq, m, n in ((tcsc_in_pvpq_f, mf_pq, nf_pq), (tcsc_in_pvpq_t, mt_pq, nt_pq)):
        ...
        j = pq_lookup[n]
        if len(m) == 0: continue
        J_C_C_u[x_control_lookup==1, i] = np.abs(V[m]) * np.abs(Ybus_tcsc[m, n]) * np.abs(V[n]) / abs(V[m]) * np.cos(np.angle(V[m]) - np.angle(V[n]) + np.angle(Ybus_tcsc[m,n]))
