...
def create_J_modification_svc(
    ...
    if np.any(svc_controllable):
        jcqc_values = (
            2
            * np.square(np.abs(Vm[svc_buses]))
            * (np.cos(2 * x_control) - 1)
            / (np.pi * svc_x_l_pu)
        )
...
def create_J_modification_tcsc(
    ...
    S_Fi_dx = (
        -1j * dY_TCSC_dx / y_tcsc_pu * (S_Fii + S_Fik)
    )  # ybus_tcsc_pu already has -1j but we want it without -1j
    ...
    rows = np.r_[
        rows,
        pvpq_lookup[f[f_in_pvpq]],
        pvpq_lookup[f[f_in_pvpq & t_in_pvpq]],
        pvpq_lookup[t[t_in_pvpq & f_in_pvpq]],
        pvpq_lookup[t[t_in_pvpq]],
    ]
    cols = np.r_[
        cols,
        pvpq_lookup[f[f_in_pvpq]],
        pvpq_lookup[t[f_in_pvpq & t_in_pvpq]],
        pvpq_lookup[f[t_in_pvpq & f_in_pvpq]],
        pvpq_lookup[t[t_in_pvpq]],
    ]
    data = np.r_[
        data,
        -Q_Fik[f_in_pvpq],
        Q_Fik[f_in_pvpq & t_in_pvpq],
        Q_Fki[t_in_pvpq & f_in_pvpq],
        -Q_Fki[t_in_pvpq],
    ]
    ...
    rows = np.r_[
        rows,
        pvpq_lookup[f[f_in_pvpq & f_in_pq]],
        pvpq_lookup[f[f_in_pvpq & t_in_pq]],
        pvpq_lookup[t[t_in_pvpq & f_in_pq]],
        pvpq_lookup[t[t_in_pvpq & t_in_pq]],
    ]
    cols = np.r_[
        cols,
        lpvpq + pq_lookup[f[f_in_pvpq & f_in_pq]],
        lpvpq + pq_lookup[t[f_in_pvpq & t_in_pq]],
        lpvpq + pq_lookup[f[t_in_pvpq & f_in_pq]],
        lpvpq + pq_lookup[t[t_in_pvpq & t_in_pq]],
    ]
    data = np.r_[
        data,
        (P_Fik / Vmf)[f_in_pvpq & f_in_pq],
        (P_Fik / Vmt)[f_in_pvpq & t_in_pq],
        (P_Fki / Vmf)[t_in_pvpq & f_in_pq],
        (P_Fki / Vmt)[t_in_pvpq & t_in_pq],
    ]
    ...
    rows = np.r_[
        rows,
        lpvpq + pq_lookup[f[f_in_pq & f_in_pvpq]],
        lpvpq + pq_lookup[f[f_in_pq & t_in_pvpq]],
        lpvpq + pq_lookup[t[t_in_pq & f_in_pvpq]],
        lpvpq + pq_lookup[t[t_in_pq & t_in_pvpq]],
    ]
    cols = np.r_[
        cols,
        pvpq_lookup[f[f_in_pq & f_in_pvpq]],
        pvpq_lookup[t[f_in_pq & t_in_pvpq]],
        pvpq_lookup[f[t_in_pq & f_in_pvpq]],
        pvpq_lookup[t[t_in_pq & t_in_pvpq]],
    ]
    data = np.r_[
        data,
        P_Fik[f_in_pq & f_in_pvpq],
        -P_Fik[f_in_pq & t_in_pvpq],
        -P_Fki[t_in_pq & f_in_pvpq],
        P_Fki[t_in_pq & t_in_pvpq],
    ]
    ...
    rows = np.r_[
        rows,
        lpvpq + pq_lookup[f[f_in_pq]],
        lpvpq + pq_lookup[f[f_in_pq & t_in_pq]],
        lpvpq + pq_lookup[t[t_in_pq & f_in_pq]],
        lpvpq + pq_lookup[t[t_in_pq]],
    ]
    cols = np.r_[
        cols,
        lpvpq + pq_lookup[f[f_in_pq]],
        lpvpq + pq_lookup[t[f_in_pq & t_in_pq]],
        lpvpq + pq_lookup[f[t_in_pq & f_in_pq]],
        lpvpq + pq_lookup[t[t_in_pq]],
    ]
    data = np.r_[
        data,
        ((2 * Q_Fii + Q_Fik) / Vmf)[f_in_pq],
        (Q_Fik / Vmt)[f_in_pq & t_in_pq],
        (Q_Fki / Vmf)[t_in_pq & f_in_pq],
        ((2 * Q_Fkk + Q_Fki) / Vmt)[t_in_pq],
    ]
    ...
    rows = np.r_[
        rows,
        pvpq_lookup[f[f_in_pvpq & tcsc_controllable]],
        pvpq_lookup[t[t_in_pvpq & tcsc_controllable]],
    ]
    cols = np.r_[
        cols,
        lpvpq + lpq + nsvc : lpvpq
        + lpq
        + nsvc
        + np.count_nonzero(f_in_pvpq & tcsc_controllable),
        lpvpq + lpq + nsvc : lpvpq
        + lpq
        + nsvc
        + np.count_nonzero(t_in_pvpq & tcsc_controllable),
    ]
    data = np.r_[
        data,
        P_Fi_dx[f_in_pvpq & tcsc_controllable],
        P_Fk_dx[t_in_pvpq & tcsc_controllable],
    ]
    ...
    rows = np.r_[
        rows,
        lpvpq + pq_lookup[f[f_in_pq & tcsc_controllable]],
        lpvpq + pq_lookup[t[t_in_pq & tcsc_controllable]],
    ]
    cols = np.r_[
        cols,
        lpvpq + lpq + nsvc : lpvpq
        + lpq
        + nsvc
        + np.count_nonzero(f_in_pq & tcsc_controllable),
        lpvpq + lpq + nsvc : lpvpq
        + lpq
        + nsvc
        + np.count_nonzero(t_in_pq & tcsc_controllable),
    ]
    data = np.r_[
    ...
    data = np.r_[
        data,
        Q_Fik[f_in_pvpq & tcsc_controllable],
        -Q_Fik[t_in_pvpq & tcsc_controllable],
    ]
    ...
    data = np.r_[
        data,
        (P_Fik / Vmf)[f_in_pq & tcsc_controllable],
        (P_Fik / Vmt)[t_in_pq & tcsc_controllable],
    ]
    ...
    rows = np.r_[
        rows,
        lpvpq + lpq + nsvc : lpvpq + lpq + nsvc + np.count_nonzero(tcsc_controllable),
    ]
    cols = np.r_[
        cols,
        lpvpq + lpq + nsvc : lpvpq + lpq + nsvc + np.count_nonzero(tcsc_controllable),
...
def create_J_modification_ssc_vsc(
    ...
    data = np.r_[
        ...
        Q_Fik[f_in_pvpq & t_in_pvpq],
        np.where(
            control_delta[f_in_pvpq & t_in_pvpq],
            one[f_in_pvpq & t_in_pvpq],
            Q_Fki[f_in_pvpq & t_in_pvpq],
        ),
    ...
    rows = np.r_[
        ...
        pvpq_lookup[f[f_in_pvpq]],
        pvpq_lookup[f[f_in_pvpq & t_in_pq]],
        pvpq_lookup[t[f_in_pvpq & t_in_pq]],
        pvpq_lookup[t[t_in_pvpq & t_in_pq]],
    ]

    cols = np.r_[
        cols,
        len(pvpq) + pq_lookup[f[f_in_pq]],
        len(pvpq) + pq_lookup[t[f_in_pvpq & t_in_pq]],
        len(pvpq) + pq_lookup[f[f_in_pvpq & t_in_pq]],
        len(pvpq) + pq_lookup[t[t_in_pvpq & t_in_pq]],
    ]
    ...
    data = np.r_[
        data,
        ((2 * P_Fii + P_Fik) / Vmf)[f_in_pvpq],
        (P_Fik / Vmt)[f_in_pvpq & t_in_pq],
        np.where(
            control_delta[f_in_pvpq & t_in_pq],
            zero[f_in_pvpq & t_in_pq],
            (P_Fki / Vmf)[f_in_pvpq & t_in_pq],
        ),
        np.where(
            control_delta[t_in_pvpq & t_in_pq],
            zero[t_in_pvpq & t_in_pq],
            ((2 * P_Fkk + P_Fki) / Vmt)[t_in_pvpq & t_in_pq],
        ),
    ]
    ...
    rows = np.r_[
        rows,
        len(pvpq) + pq_lookup[f[f_in_pq]],
        len(pvpq) + pq_lookup[f[f_in_pq & t_in_pvpq]],
        len(pvpq) + pq_lookup[t[f_in_pvpq & t_in_pq]],
        len(pvpq) + pq_lookup[t[t_in_pq]],
    ]
    ...
    data = np.r_[
        ...
        -P_Fik[f_in_pq & t_in_pvpq],
        np.where(
            control_v[f_in_pvpq & t_in_pq],
            zero[f_in_pvpq & t_in_pq],
            P_Fik[f_in_pvpq & t_in_pq],
        ),
    ...
    rows = np.r_[
        rows,
        len(pvpq) + pq_lookup[f[f_in_pq]],
        len(pvpq) + pq_lookup[f[f_in_pq & t_in_pq]],
        len(pvpq) + pq_lookup[t[f_in_pq & t_in_pq]],
        len(pvpq) + pq_lookup[t[t_in_pq]],
    ]
    ...
    data = np.r_[
        ...
        (Q_Fik / Vmt)[f_in_pq & t_in_pq],
        np.where(
            control_v[f_in_pq & t_in_pq],
            one[f_in_pq & t_in_pq],
            ((2 * Q_Fii + Q_Fik) / Vmf)[f_in_pq & t_in_pq],
        ),
...
def create_J_modification_hvdc(
    ...
    rows = np.r_[
        dc_p_lookup[vsc_dc_fb],
        dc_p_lookup[vsc_dc_fb],
        dc_p_lookup[vsc_dc_tb],
        dc_p_lookup[vsc_dc_tb],
    ]
    # rows = np.r_[vsc_dc_fb, vsc_dc_fb, vsc_dc_tb, vsc_dc_tb]
    cols = np.r_[
        dc_p_lookup[vsc_dc_fb],
        dc_p_lookup[vsc_dc_tb],
        dc_p_lookup[vsc_dc_fb],
    ...
    # cols = np.r_[vsc_dc_fb, vsc_dc_tb, vsc_dc_fb, vsc_dc_tb]
    ...
    data = np.r_[
        ...
        np.where(
            vsc_dc_slack,
            one,
            np.where(vsc_dc_mode_p, (2 * P_Fii + P_Fik) / Vmf, P_Fki / Vmf),
        ),
        np.where(
            vsc_dc_slack,
            zero,
            np.where(vsc_dc_mode_p, P_Fik / Vmt, (2 * P_Fkk + P_Fki) / Vmt),
        ),
    ]
    ...
    # add dc_slack, a 1 has to be added at the required
    for el in dc_ref:
        ele = dc_ref_lookup[el]
        J_combined[ele, :] = 0.0
        J_combined[:, ele] = 0.0
        J_combined[ele, ele] = 1.0
    # J_combined[:, 4] = 0.
    # J_combined[4, :] = 0.
    # J_combined[4, 4] = 1.
    ...
    J_m[offset : offset + num_p, offset : offset + num_p] = (
        J_combined  # [dc_p, :][:, dc_p]
    )
