...
def _update_p(baseMVA, bus, gen, ref, gbus, on, Sbus):
    ## update Pg for slack bus(es)
    ## inj P + local Pd
    for gen_bus in ref:
        refgen = find(gbus == gen_bus)  ## which is(are) the reference gen(s)?
        p_bus = Sbus[refgen[0]].real * baseMVA + bus[gen_bus, PD]
        if len(refgen) > 1:  ## more than one generator at this ref bus
            # subtract off what is generated by other gens at this bus
            ext_grids = find((gen[refgen, QMAX] == 0.) & (gen[refgen, QMIN] == 0.))
            gens = setdiff1d(refgen, ext_grids)
            p_ext_grids = p_bus - sum(gen[on[refgen[gens]],PG])
            gen[ext_grids, PG] = p_ext_grids / len(ext_grids)
        else:
            gen[on[refgen[0]], PG] = p_bus
