...
def diagnostic_report(
    net, diag_results, diag_errors, diag_params, compact_report, warnings_only
):
    diag_report = DiagnosticReports(
        net, diag_results, diag_errors, diag_params, compact_report
    )
    if warnings_only:
...
class DiagnosticReports:
    ...
    def report_disconnected_elements(self):
        if "disconnected_elements" in self.diag_results:
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s disconnected element(s) found." % (element_counter)
                )
        elif "disconnected_elements" in self.diag_errors:
            logger.warning(
                "Check for disconnected elements failed due to the following error:"
            )
        ...
        if "different_voltage_levels_connected" in self.diag_results:
            ...
            for key in diag_result:
                ...
                for element in diag_result[key]:
                    buses = list(
                        get_connected_buses_at_element(self.net, element, key[0])
                    )
                    if self.compact_report:
                        logger.warning(
                            "%s %s: buses %s" % (element_type, element, buses)
                        )
                    else:
                        logger.warning(
    ...
    def report_impedance_values_close_to_zero(self):
        if "impedance_values_close_to_zero" in self.diag_results:
                ...
                for element in diag_result[key]:
                    ...
                    logger.warning(
                        ...
                        % (
                            ...
                            element,
                            min_r_type,
                            self.diag_params["min_" + min_r_type],
                            min_x_type,
                            self.diag_params["min_" + min_x_type],
                        )
                    )

            if len(self.diag_results["impedance_values_close_to_zero"]) > 1:
                switch_replacement = self.diag_results[
                ...
                if switch_replacement["loadflow_converges_with_switch_replacement"]:
                    ...
                    logger.warning(
                        "Power flow still does not converge after replacing implausible "
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s element(s) with impedance values close to zero found."
                    % (element_counter)
                )
        elif "impedance_values_close_to_zero" in self.diag_errors:
            logger.warning(
                "Check for elements with impedance values close to zero failed due "
                "to the following error:"
            )
            logger.warning(self.diag_errors["impedance_values_close_to_zero"])
        else:
            logger.info(
                "PASSED: No elements with impedance values close to zero found..."
            )
    ...
    def report_nominal_voltages_dont_match(self):
        if "nominal_voltages_dont_match" in self.diag_results:
                    ...
                    if element == "trafo":
                        if self.compact_report:
                            ...
                            if key == "hv_lv_swapped":
                                ...
                                for trafo in diag_result[element][key]:
                                    logger.warning(
                                        "Trafo %s: Nominal voltage on lv_side "
                                        "(%s kV) and voltage_level of lv_bus "
                                        "(bus %s with voltage_level %s kV) "
                                        "deviate more than +/- %s percent."
                                        % (
                                            trafo,
                                            self.net.trafo.vn_lv_kv.at[trafo],
                    ...
                    if element == "trafo3w":
                        if self.compact_report:
                            ...
                            if key == "connectors_swapped_3w":
                                logger.warning(
                                    "Trafo3w %s: connectors seem to "
                                    "be swapped" % (diag_result[element][key])
                                )
                            elif key == "hv_bus":
                                for trafo3w in diag_result[element][key]:
                                    logger.warning(
                                        "Trafo3w %s: Nominal voltage on hv_side "
                                        "(%s kV) and voltage_level of hv_bus "
                                        "(bus %s with voltage_level %s kV) "
                                        "deviate more than +/- %s percent."
                                        % (
                                            trafo3w,
                                            self.net.trafo3w.vn_hv_kv.at[trafo3w],
                                ...
                                for trafo3w in diag_result[element][key]:
                                    logger.warning(
                                        "Trafo3w %s: Nominal voltage on mv_side "
                                        "(%s kV) and voltage_level of mv_bus "
                                        "(bus %s with voltage_level %s kV) "
                                        "deviate more than +/- %s percent."
                                        % (
                                            trafo3w,
                                            self.net.trafo3w.vn_mv_kv.at[trafo3w],
                                ...
                                for trafo3w in diag_result[element][key]:
                                    logger.warning(
                                        "Trafo3w %s: Nominal voltage on lv_side "
                                        "(%s kV) and voltage_level of lv_bus "
                                        "(bus %s with voltage_level %s kV) "
                                        "deviate more than +/- %s percent."
                                        % (
                                            trafo3w,
                                            self.net.trafo3w.vn_lv_kv.at[trafo3w],
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s component(s) with deviating nominal voltages found"
                    % (element_counter)
                )
        elif "nominal_voltages_dont_match" in self.diag_errors:
            logger.warning(
                "Check for components with deviating nominal voltages failed due "
                "to the following error:"
            )
    ...
    def report_invalid_values(self):
        ...
        if "invalid_values" in self.diag_results:
            ...
            for element_type in diag_result:
                ...
                for inv_value in diag_result[element_type]:
                    if self.compact_report:
                        ...
                        logger.warning(
                            "Invalid value found: '%s %s' with attribute '%s' = %s "
            ...
            logger.warning(
                "Check for invalid values failed due to the following error:"
            )
    ...
    def report_overload(self):
        if "overload" in self.diag_results:
            ...
            if not diag_result["load"] and not diag_result["generation"]:
                logger.warning(
                    "Overload check failed: Power flow still does not "
                ...
                logger.warning(
                    "overload found: Power flow converges "
    ...
    def report_wrong_switch_configuration(self):
        if "wrong_switch_configuration" in self.diag_results:
            ...
            if diag_result:
                logger.warning(
                    "Possibly wrong switch configuration found: power flow "
        ...
        # message summary
            ...
            logger.warning(
                "Check for wrong switch configuration failed due to the following error:"
            )
    ...
    def report_no_ext_grid(self):
        if "no_ext_grid" in self.diag_results:
            ...
            if diag_result is True:
                logger.warning(
                    "No ext_grid found. There has to be at least one ext_grid!"
                )
    ...
    def report_multiple_voltage_controlling_elements_per_bus(self):
        if "multiple_voltage_controlling_elements_per_bus" in self.diag_results:
            ...
            if self.compact_report:
                ...
                logger.warning(
                    "Checking for multiple gens and/or external grids per bus..."
                )
            logger.warning("")
                ...
                if self.compact_report:
                    ...
                    for bus in diag_result[feeder_type]:
                        if feeder_type == "buses_with_mult_ext_grids":
                            logger.warning(
                                "External grids %s are connected to bus %s. Only one "
                                "external grid per bus is allowed."
                                % (
                                    list(
                                        self.net.ext_grid[
                                            self.net.ext_grid.bus == bus
                                        ].index
                                    ),
                                    bus,
                                )
                            ...
                            logger.warning(
                                "Generator(s) %s and external grid(s) %s are connected "
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s bus(ses) with multiple gens and/or ext_grids "
                    "found." % (element_counter)
                )
        elif "multiple_voltage_controlling_elements_per_bus" in self.diag_errors:
            logger.warning(
                "Check for multiple voltage controlling elements per bus failed due "
                "to the following error:"
            )
            logger.warning(
                self.diag_errors["multiple_voltage_controlling_elements_per_bus"]
            )
        else:
        ...
        if "wrong_reference_system" in self.diag_results:
            ...
            for element_type in diag_result:
                if element_type == "loads":
                    if self.compact_report:
                        logger.warning(
                            "loads %s: wrong reference system."
                            % (diag_result[element_type])
                        )
                    else:
                        for load in diag_result[element_type]:
                            logger.warning(
                                "Found load %s: '%s' with p_mw = %s. In load reference "
                                "system p_mw should be positive."
                                % (
                                    load,
                                    self.net.load.name.at[load],
                    ...
                    if self.compact_report:
                        logger.warning(
                            "gens %s: wrong reference system."
                            % (diag_result[element_type])
                        )
                    else:
                        for gen in diag_result[element_type]:
                            logger.warning(
                                "Found gen %s: '%s' with p_mw = %s. In load reference "
                                "system p_mw should be negative."
                                % (
                                    gen,
                                    self.net.gen.name.at[gen],
                                    self.net.gen.p_mw.at[gen],
                                )
                            )
                    ...
                    if self.compact_report:
                        logger.warning(
                            "sgens %s: wrong reference system."
                            % (diag_result[element_type])
                        )
                    else:
                        for sgen in diag_result[element_type]:
                            logger.warning(
                                "Found sgen %s: '%s' with p_mw = %s. In load reference "
                                "system p_mw should be negative."
                                % (
                                    sgen,
                                    self.net.sgen.name.at[sgen],
                                    self.net.sgen.p_mw.at[sgen],
                                )
                            )
            ...
            if not self.compact_report:
                ...
                if "loads" in diag_result:
                    logger.warning(
                        "SUMMARY: Found %s load(s) with negative p_mw. In load "
                ...
                if "gens" in diag_result:
                    logger.warning(
                        "SUMMARY: Found %s gen(s) with positive p_mw. In load "
                ...
                if "sgens" in diag_result:
                    logger.warning(
                        "SUMMARY: Found %s sgen(s) with positive p_mw. In load "
                        "reference system, p_mw should be negative. If the intention "
                        "was to model a load, please use a load instead."
            ...
            logger.warning(
                "Check for wrong reference system failed due to the following error:"
            )
    ...
    def report_deviation_from_std_type(self):
        if "deviation_from_std_type" in self.diag_results:
            ...
            for et in diag_result:
                for eid in diag_result[et]:
                    ...
                    if values["std_type_in_lib"]:
                        ...
                        logger.warning(
                            "%s %s: No valid std_type or std_type not in net.std_types"
                            % (et, eid)
                        )
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s elements with deviations from std_type found."
                    % (element_counter)
                )

        elif "deviation_from_std_type" in self.diag_errors:
            logger.warning(
                "Check for deviation from std_type failed due to the following error:"
            )
    ...
    def report_numba_comparison(self):
        if "numba_comparison" in self.diag_results:
            ...
            if self.compact_report:
                ...
                logger.warning(
                    "Checking for deviations between numba = True vs. False..."
                )
            logger.warning("")
            ...
            for element_type in diag_result:
                for res_type in diag_result[element_type]:
                    if self.compact_report:
                        ...
                        for idx in diag_result[element_type][res_type].index:
                            ...
                            logger.warning(
                                "%s.%s at index %s: absolute deviation = %s"
                ...
                if not self.compact_report:
                    logger.warning("")
                    logger.warning(
                        "SUMMARY: %s results with deviations between numba = True vs. \
                                    False found."
                        % (element_counter)
                    )
            ...
            logger.info(
                "PASSED: No results with deviations between numba = True vs. False found."
            )
    ...
    def report_parallel_switches(self):
        if "parallel_switches" in self.diag_results:
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s occurences of parallel switches found."
                    % len(diag_result)
                )
        elif "parallel_switches" in self.diag_errors:
            logger.warning(
                "Check for parallel_switches failed due to the following error:"
            )
        ...
        if "missing_bus_indices" in self.diag_results:
            ...
            for element_type in diag_result:
                for element in diag_result[element_type]:
                    element_counter += 1
                    logger.warning(
                        "%s %s: %s (%s) not in net.bus.index"
                        % (element_type, element[0], element[1], element[2])
                    )
            ...
            if not self.compact_report:
                logger.warning("")
                logger.warning(
                    "SUMMARY: %s missing bus indices found." % element_counter
                )

        elif "missing_bus_indices" in self.diag_errors:
            logger.warning(
                "Check for missing bus indices failed due to the following error:"
            )
