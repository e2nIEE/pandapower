        ...
        if "wrong_reference_system" in self.diag_results:
            ...
            for element_type in diag_result:
                if element_type == "loads":
                    logger.compact(f"loads {diag_result[element_type]}: wrong reference system.")
                    for load in diag_result[element_type]:
                        logger.detailed(
                            f"Found load {load}: '{self.net.load.name.at[load]}' with p_mw = "
                            f"{self.net.load.p_mw.at[load]}. In load reference system p_mw should be positive."
                        )

                elif element_type == "gens":
                    logger.compact(f"gens {diag_result[element_type]}: wrong reference system.")
                    for gen in diag_result[element_type]:
                        logger.detailed(
                            f"Found gen {gen}: '{self.net.gen.name.at[gen]}' with p_mw = "
                            f"{self.net.gen.p_mw.at[gen]}. In load reference system p_mw should be negative."
                        )

                elif element_type == "sgens":
                    logger.compact(f"sgens {diag_result[element_type]}: wrong reference system.")
                    for sgen in diag_result[element_type]:
                        logger.detailed(
                            f"Found sgen {sgen}: '{self.net.sgen.name.at[sgen]}' with p_mw = "
                            f"{self.net.sgen.p_mw.at[sgen]}. In load reference system p_mw should be negative."
                        )

            # message summary
            if 'loads' in diag_result:
                logger.detailed(
                    f"\nSUMMARY: Found {len(diag_result['loads'])} load(s) with negative p_mw. In load reference "
                    f"system, p_mw should be positive. If the intention was to model a constant generation, "
                    f"please use an sgen instead."
                )
            if 'gens' in diag_result:
                logger.detailed(
                    f"\nSUMMARY: Found {len(diag_result['gens'])} gen(s) with positive p_mw. "
                    "In load reference system, p_mw should be negative. If the intention was to model a load, "
                    "please use a load instead."
                )
            ...
            for element_type in diag_result:
                for res_type in diag_result[element_type]:
                    ...
                    for idx in diag_result[element_type][res_type].index:
                        ...
                        logger.detailed(
                            f"{element_type}.{res_type} at index {idx}: absolute deviation = {dev}"
                        )

                # message summary
                logger.detailed("")
                logger.detailed(
                    f"SUMMARY: {element_counter} results with deviations between numba = True vs. False found."
                )
        ...
        if "missing_bus_indices" in self.diag_results:
            ...
            for element_type in diag_result:
                for element in diag_result[element_type]:
                    element_counter += 1
                    logger.warning(
                        f"{element_type} {element[0]}: {element[1]} ({element[2]}) not in net.bus.index"
                    )
