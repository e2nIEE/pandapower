...
error : element table queried before assingment
reproduce : pp.get_connected_elements_dict(network , bus,
respect_switches=False,respect_in_service=True)
solution : query if there is the df before removing the in_service false
elements
"""
import copy
import gc
...
from pandapower.auxiliary import _preserve_dtypes, get_indices, pandapowerNet
    ...
    if res_elements:
        ...
        ebts.update(
            [
                ("res_" + ebt[0], ebt[1])
                for ebt in ebts
                if ebt[0] not in elements_without_res
            ]
        )
    return ebts
...
def signing_system_value(elm):
    ...
    if elm in generator_viewpoint_elms:
        ...
        raise ValueError(
            f"This function is defined for bus elements, not for '%s'." % str(elm)
        )
...
def _pq_from_cosphi(s, cosphi, qmode, pmode):
    # pylint : disable = consider-using-in,consider-using-f-string
    if qmode in ("ind", "cap"):
        logger.warning(
    ...
    if qmode in ("ind", "underexcited"):
        ...
        raise ValueError(
            f'Unknown mode %s - specify "underexcited" (Q absorption, decreases voltage'
            f') or "overexcited" (Q injection, increases voltage)' % qmode
        )
    ...
    if pmode == "load":
        ...
        raise ValueError(f'Unknown mode %s - specify "load" or "gen"' % pmode)
...
def _cosphi_from_pq_bulk(p, q, len_=None):
    ...
    qmode = np.array(["underexcited", "underexcited", "overexcited"])[
        np.sign(q).astype(int)
    ]
    return cosphi, s, qmode, pmode
...
def dataframes_equal(df1, df2, ignore_index_order=True, **kwargs):
    ...
    if "tol" in kwargs:
        if "atol" in kwargs:
            raise ValueError(
                "'atol' and 'tol' are given to dataframes_equal(). Don't use 'tol' "
                "anymore."
            )
        logger.warning(
            "in dataframes_equal() parameter 'tol' is deprecated. Use 'atol' instead."
        )
...
def compare_arrays(x, y):
    ...
    """
    # pylint : disable = comparison-with-itself,consider-using-f-string
    if x.shape == y.shape:
...
def ensure_iterability(var, len_=None):
    ...
    if hasattr(var, "__iter__") and not isinstance(var, str):
        if isinstance(len_, int) and len(var) != len_:
            raise ValueError(f"Length of variable differs from %i." % len_)
    else:
        len_ = len_ or 1
        var = [var] * len_
    return var
...
def lf_info(net, numv=1, numi=2):  # pragma: no cover
    ...
    for _, r in (
        net.res_bus.sort_values("vm_pu", ascending=False).iloc[:numv].iterrows()
    ):
    ...
    if net.res_trafo is not None:
        for _, r in (
            net.res_trafo.sort_values("loading_percent", ascending=False)
            .iloc[:numi]
            .iterrows()
        ):
            logger.info(
                "  %s loading at trafo %s (%s)",
                r.loading_percent,
                r.name,
    ...
    for _, r in (
        net.res_line.sort_values("loading_percent", ascending=False)
        .iloc[:numi]
        .iterrows()
    ):
        logger.info(
            "  %s loading at line %s (%s)",
            r.loading_percent,
            r.name,
...
def _determine_flexibilities_dict(net, data, delta_pq, **kwargs):
    ...
    for elm, controllable_default in flex_tuple:
        for power_type in ["P", "Q"]:
            ...
            if (
                len(constraints) >= 2
                and pd.Series(constraints[:2]).isin(net[elm].columns).all()
            ):
            ...
            if (
                elm == "dcline"
                and power_type == "Q"
                and len(controllables)
                and pd.Series(constraints[2:4]).isin(net[elm].columns).all()
            ):
                controllables = _find_idx_without_numerical_difference(
                    net[elm],
                    constraints[2],
            ...
            data[key] = _cluster_same_floats(
                net[elm].loc[controllables], constraints, **kwargs
            )
            shorted = [
                col[:3] if col[:3] in ["min", "max"] else col
...
def _find_idx_without_numerical_difference(
    df, column1, column2, delta, idx=None, equal_nan=False
):
    """
    ...
    idx_no_delta = idx_without_null[
        (df.loc[idx_without_null, column1] - df.loc[idx_without_null, column2])
        .abs()
        .values
        <= delta
    ]
...
def _determine_costs_dict(net, opf_task_overview):
    ...
    for flex_element in flex_elements:
        ...
        keys = [
            power_type + flex_element
            for power_type in ["P", "Q"]
            if (power_type + flex_element) in opf_task_overview["flexibilities"].keys()
        ]
        ...
        for key in keys:
            idx_without_cost |= set(
                chain(*opf_task_overview["flexibilities"][key]["index"])
            )
        ...
        for cost_df in cost_dfs:
            idx_with_cost = set(
                net[cost_df].element[net[cost_df].et == flex_element].astype(int)
            )
            if len(idx_with_cost - idx_without_cost):
                logger.warning(
                    "These "
                    + flex_element
                    + "s have cost data but aren't flexible or"
        ...
        if len(idx_without_cost):
            opf_task_overview["flexibilities_without_costs"][flex_element] = list(
                idx_without_cost
            )
...
def _cluster_same_floats(df, subset=None, **kwargs):
    ...
    if df.index.duplicated().any():
        logger.error(
            "There are duplicated indices in df. Clusters will be determined but remain "
            + "ambiguous."
        )
    subset = (
        subset
    ...
    cluster_df = pd.DataFrame(
        np.empty((sum(uniq), len(subset) + 1)), columns=["index"] + subset
    )
...
def _check_overlapping_constraints(opf_task_overview):
    ...
    """
    # pylint : disable = too-many-branches , too-complex ,too-many-nested-blocks
    overlap = []
    for dict_key in ["flexibilities", "network_constraints"]:
        for key, df in opf_task_overview[dict_key].items():
            ...
            for i_col in range(n_col):
                assert min_col[i_col].replace("min", "") == max_col[i_col].replace(
                    "max", ""
                )
    ...
    if len(overlap):
        logger.error(
            "At these variables, there is a minimum constraint exceeding the maximum "
            + "constraint value: "
            + str(overlap)
        )
...
def _log_opf_task_overview(opf_task_overview):
    ...
    """
    # pylint : disable =  too-complex ,too-many-nested-blocks,no-else-continue,consider-using-f-string
    s = ""
        ...
        for key, elm in zip(keys, elms):
            ...
            if dict_key in ["flexibilities", "network_constraints"]:
                ...
                if df.shape[0] > 1:
                    ...
                    if not len(
                        set(df.columns).symmetric_difference({"index", "min", "max"})
                    ):
                        s += "\n    %g <= %ix %s (all) <= %g" % (
                            df.loc[0, "min"],
                            len_idx,
                        ...
                        s += "\n    %ix %s (all) with these constraints:" % (
                            len_idx,
                            key,
                        )
...
def switch_info(net, sidx):  # pragma: no cover
    ...
    if switch_type == "b":
        ...
        logger.info(
            "Switch %u connects bus %u (%s) with bus %u (%s)"
            % (sidx, bidx, bus_name, eidx, bus2_name)
        )
    elif switch_type == "l":
        line_name = net.line.at[eidx, "name"]
        logger.info(
            "Switch %u connects bus %u (%s) with line %u (%s)"
            % (sidx, bidx, bus_name, eidx, line_name)
        )
    elif switch_type == "t":
        trafo_name = net.trafo.at[eidx, "name"]
        logger.info(
            "Switch %u connects bus %u (%s) with trafo %u (%s)"
            % (sidx, bidx, bus_name, eidx, trafo_name)
        )
...
def overloaded_lines(net, max_load=100):
    ...
    if net.converged:
        ...
        raise UserWarning(
            "The last loadflow terminated erratically, results are invalid!"
        )
...
def violated_buses(net, min_vm_pu, max_vm_pu):
    ...
    """
    if net.converged:
        return net["bus"].index[
            (net["res_bus"]["vm_pu"] < min_vm_pu)
...
def nets_equal(
    ...
    **kwargs,
    ...
    if len(not_checked_keys) > 0:
        logger.warning(
            "These keys were ignored by the comparison of the networks: %s"
            % (", ".join(not_checked_keys))
        )

    if len(not_equal) > 0:
        logger.warning(
            "Networks do not match in DataFrame(s): %s" % (", ".join(not_equal))
        )
        return False
    else:
        return True
...
def _nets_equal_keys(
    ...
    **kwargs,
    ...
    if check_without_results and check_only_results:
        raise UserWarning(
            "Please provide only one of the options to check without results or to "
    ...
    if name_selection is not None:
        ...
        net1_keys = [
            ...
            if key.startswith("res_") and key not in exclude_elms
        ]
        net2_keys = [
            key
        ...
        net1_keys = [
            ...
            if not (
                key.startswith("_")
                or key in exclude_elms
                or key == "et"
                or key.startswith("res_")
                and check_without_results
            )
        ]
        net2_keys = [
            key
    ...
    for key in list(keys_to_check):
        ...
        if isinstance(net1[key], pd.DataFrame):
            if not isinstance(net2[key], pd.DataFrame):
                ...
                if (
                    "object" in net1[key].columns
                    and "object" in net2[key].columns
                    and isinstance(net1[key].object.dtype, object)
                    and isinstance(net1[key].object.dtype, object)
                ):
                    logger.warning("net[%s]['object'] cannot be compared." % key)
                    if not dataframes_equal(
                        net1[key][net1[key].columns.difference(["object"])],
            ...
            isinstance(net1[key], int)
            or isinstance(net1[key], float)
            or isinstance(net1[key], complex)
        ):
            ...
            if GRAPHS_EQUAL_POSSIBLE:
                ...
                if (
                    net1[key].nodes != net2[key].nodes
                    or net1[key].edges != net2[key].edges
                ):
                    not_equal.append(key)
...
def clear_result_tables(net):
    ...
    for key in net.keys():
        if (
            isinstance(net[key], pd.DataFrame)
            and key[:3] == "res"
            and net[key].shape[0]
        ):
...
def reindex_elements(net, element, new_indices, old_indices=None):
    ...
    if element in ["line", "trafo", "trafo3w"]:
        affected = net.measurement[
            (net.measurement.element_type == element)
    ...
    if element in ["line", "trafo"]:
        affected = net.switch[
            (net.switch.et == element[0]) & (net.switch.element.isin(old_indices))
        ]
        if len(affected):
    ...
    for cost_df in ["pwl_cost", "poly_cost"]:
        ...
        if sum(element_in_cost_df):
            net[cost_df].element.loc[element_in_cost_df] = get_indices(
    ...
    for elm in list(elements):
        ...
        if elm in net and isinstance(net[elm], pd.DataFrame):
            if elm in ["bus_geodata", "line_geodata"]:
                logger.info(
                    elm
                    + " don't need to bo included to 'add_df_to_reindex'. It is "
                    + "already included by elm=='"
                    + elm.split("_")[0]
                    + "'."
                )
            else:
...
def set_scaling_by_type(net, scalings, scale_load=True, scale_sgen=True):
    ...
    if not isinstance(scalings, dict):
        raise UserWarning(
            "The parameter scaling has to be a dictionary, " "see docstring"
        )
                ...
                if key in new_net and col in new_net[key].columns:
                    ...
                    if set(item.columns) == set(new_net[key]):
                        if version.parse(pd.__version__) < version.parse("0.21"):
                            net[key] = net[key].reindex_axis(
                                new_net[key].columns, axis=1
                            )
                        else:
                    ...
                    if version.parse(pd.__version__) < version.parse("0.20.0"):
                        net[key][col] = net[key][col].astype(
                            new_net[key][col].dtype, raise_on_error=False
                        )
                    else:
                        net[key][col] = net[key][col].astype(
...
def close_switch_at_line_with_two_open_switches(net):
    ...
    if len(closed_switches) > 0:
        logger.info(
            "closed %d switches at line with 2 open switches (switches: %s)"
...
def fuse_buses(net, b1, b2, drop=True, fuse_bus_measurements=True):
    ...
    net["switch"]["element"].loc[
        (net["switch"]["et"] == "b") & (net["switch"]["element"].isin(b2))
    ] = b1
...
def drop_elements_at_buses(
    net, buses, bus_elements=True, branch_elements=True, drop_measurements=True
):
    """
    drop elements connected to given buses
    """
    for element, column in element_bus_tuples(
        bus_elements, branch_elements, res_elements=False
    ):
        if element == "switch":
            ...
            if element == "line":
                ...
                if res_element in net.keys() and isinstance(
                ...
                if net[element].shape[0] < n_el:
                    logger.info(
                        "dropped %d %s elements"
                        % (n_el - net[element].shape[0], element)
                    )
...
def drop_trafos(net, trafos, table="trafo"):
    ...
    i = net["switch"].index[
        (net["switch"]["element"].isin(trafos)) & (net["switch"]["et"] == et)
    ]
    net["switch"].drop(i, inplace=True)
    num_switches = len(i)
    ...
    logger.info(
        "dropped %d %s elements with %d switches" % (len(trafos), table, num_switches)
    )
...
def drop_measurements_at_elements(net, element_type, idx=None, side=None):
    ...
    bool3 = (
        ...
        if side is not None
        else [True] * net.measurement.shape[0]
    )
...
def drop_duplicated_measurements(net, buses=None, keep="first"):
    ...
    # drop duplicates
    if not analyzed_meas.duplicated(
        subset=["measurement_type", "element_type", "side", "element"], keep=keep
    ).empty:
        idx_to_drop = analyzed_meas.index[
            analyzed_meas.duplicated(
                subset=["measurement_type", "element_type", "side", "element"],
                keep=keep,
            )
        ]
    ...
    for elm, bus_types in branch_dict.items():
        for bus1 in bus_types:
            for bus2 in bus_types:
                if bus2 != bus1:
                    idx = net[elm].index[
                        net[elm][bus1].isin(buses1) & net[elm][bus2].isin(buses2)
                    ]
                    if elm == "switch":
...
def set_element_status(net, buses, in_service):
    ...
    for element in net.keys():
        if (
            ...
            try:
                idx = list(get_connected_elements(net, element, buses))
...
def set_isolated_areas_out_of_service(net, respect_switches=True):
    ...
    closed_switches = set()
    unsupplied = list(unsupplied_buses(net, respect_switches=respect_switches))
    logger.info(
        "set %d of %d unsupplied buses out of service"
        % (len(net.bus.loc[unsupplied].query("~in_service")), len(unsupplied))
    )
    set_element_status(net, unsupplied, False)

    # remove this loop after unsupplied_buses are fixed
    for tr3w in net.trafo3w.index.values:
        ...
        open_tr3w_switches = net.switch.loc[
            (net.switch.et == "t3") & ~net.switch.closed & (net.switch.element == tr3w)
        ]
    ...
    for element, et in zip(["line", "trafo"], ["l", "t"]):
        ...
        oos_switches = net.switch[
            (net.switch.et == et) & net.switch.element.isin(oos_elements)
        ].index

        closed_switches.update(
            [i for i in oos_switches.values if not net.switch.at[i, "closed"]]
        )
...
def drop_out_of_service_elements(net):
    ...
    other_branch_elms = pp_elements(
        bus=False, bus_elements=False, branch_elements=True, other_elements=False
    ...
    for elm in pp_elements(
        ...
        if "in_service" not in net[elm].columns:
            if elm not in ["measurement", "switch"]:
                logger.info(
                    "Out-of-service elements cannot be dropped since 'in_service' is "
...
def select_subnet(
    ...
    p2.dcline = net.dcline[
        (net.dcline.from_bus.isin(buses)) & (net.dcline.to_bus.isin(buses))
    ]
    p2.trafo = net.trafo[
        (net.trafo.hv_bus.isin(buses)) & (net.trafo.lv_bus.isin(buses))
    ]
    p2.trafo3w = net.trafo3w[
        (net.trafo3w.hv_bus.isin(buses))
        & (net.trafo3w.mv_bus.isin(buses))
        & (net.trafo3w.lv_bus.isin(buses))
    ]
    p2.impedance = net.impedance[
        (net.impedance.from_bus.isin(buses)) & (net.impedance.to_bus.isin(buses))
    ]
    p2.measurement = net.measurement[
        (
    ...
    if include_results:
        for table in net.keys():
            if (
                net[table] is None
                or not isinstance(net[table], pd.DataFrame)
                or not net[table].shape[0]
                or not table.startswith("res_")
                or table[4:] not in net.keys()
                or not isinstance(net[table[4:]], pd.DataFrame)
                or not net[table[4:]].shape[0]
            ):
                continue
            elif table == "res_bus":
                p2[table] = net[table].loc[buses.intersection(net[table].index)]
            else:
                p2[table] = net[table].loc[
                    p2[table[4:]].index.intersection(net[table].index)
                ]
    if "bus_geodata" in net:
        p2["bus_geodata"] = net.bus_geodata.loc[
            p2.bus.index.intersection(net.bus_geodata.index)
        ]
    if "line_geodata" in net:
        p2["line_geodata"] = net.line_geodata.loc[
            p2.line.index.intersection(net.line_geodata.index)
        ]

    # switches
    p2["switch"] = net.switch.loc[
        net.switch.bus.isin(p2.bus.index)
        & pd.concat(
            [
...
def merge_nets(
    ...
    **kwargs,
    ...
    def adapt_element_idx_references(net, element, element_type, offset=0):
        ...
        et_col = (
            "et" if element in ["switch", "poly_cost", "pwl_cost"] else "element_type"
        )
        elements = net[element][net[element][et_col] == et]
        new_index = [
            net[element_type].index.get_loc(ix) + offset
            for ix in elements.element.values
        ]
        if len(new_index):
    ...
    for element, table in net.items():
        if (
            ...
            or element == "dtypes"
            or (element.startswith("res") and (validate or not merge_results))
        ):
            continue
        if isinstance(table, pd.DataFrame) and (
            len(table) > 0 or len(net2[element]) > 0
        ):
            if element in ["switch", "measurement"]:
                adapt_element_idx_references(
                    net2, element, "line", offset=len(net1.line)
                )
                adapt_element_idx_references(net, element, "line")
                adapt_element_idx_references(
                    net2, element, "trafo", offset=len(net1.trafo)
                )
                adapt_element_idx_references(net, element, "trafo")
            if element in ["poly_cost", "pwl_cost"]:
                net[element]["element"] = [
                    np.nan
                    if row.element not in net1[row.et].index.values
                    else net1[row.et].index.get_loc(row.element)
                    for row in net1[element].itertuples()
                ]
                if net[element]["element"].isnull().any():
                    # this case could also be checked using get_false_links()
                    logger.warning(
                        f"Some net1[{element}] does not link to an existing element."
                        "These are dropped."
                    )
                    net[element].drop(
                        net[element].index[net[element].element.isnull()], inplace=True
                    )
                    net[element]["element"] = net[element]["element"].astype(int)
                for et in ["gen", "sgen", "ext_grid", "load", "dcline", "storage"]:
                    adapt_element_idx_references(
                        net2, element, et, offset=len(net1[et])
                    )
            if element == "line_geodata":
                ...
                ni = [
                    net2.line.index.get_loc(ix) + len(net1.line)
            ...
            try:
                net[element] = pd.concat(
                    [net[element], net2[element]], ignore_index=ignore_index, sort=False
                )
            except:
                # pandas legacy < 0.21
                net[element] = pd.concat(
                    [net[element], net2[element]], ignore_index=ignore_index
                )
            _preserve_dtypes(net[element], dtypes)
    # update standard types of net by data of net2
    for type_ in net.std_types.keys():
        net.std_types[type_].update(
            net2.std_types[type_]
        )  # net2.std_types have priority
    if validate:
        runpp(net, **kwargs)
        dev1 = max(
            abs(
                net.res_bus.loc[net1.bus.index].vm_pu.values - net1.res_bus.vm_pu.values
            )
        )
        dev2 = max(
            abs(
                net.res_bus.iloc[len(net1.bus.index) :].vm_pu.values
                - net2.res_bus.vm_pu.values
            )
        )
        if dev1 > tol or dev2 > tol:
            raise UserWarning(
                "Deviation in bus voltages after merging: %.10f" % max(dev1, dev2)
            )
    return net
    ...
    **kwargs - additional line parameters you want to set for the new line
    ...
    for bus in net.line.at[idx, "to_bus"], net.line.at[idx, "from_bus"]:
        if (
            ...
            create_switch(
                net, bus=bus, element=new_idx, closed=False, et="l", type="LBS"
            )
...
def merge_same_bus_generation_plants(
    net, add_info=True, error=True, gen_elms=["ext_grid", "gen", "sgen"]
):
    """
    ...
    if add_info:
        for elm in gen_elms:
            ...
            if (
                "includes_other_plants" not in net[elm].columns
                or net[elm]["includes_other_plants"].dtype != bool
            ):
    ...
    for bus in gen_df["bus"].loc[gen_df["bus"].duplicated()].unique():
        ...
        if (
            "vm_pu" in gen_df.columns
            and len(gen_df.vm_pu.loc[idxs].dropna().unique()) > 1
        ):
            message = (
                "Generation plants connected to bus %i have different vm_pu." % bus
            )
            if error:
        ...
        if "profiles" in net and col == "p_mw":
            elm = (
                "gen" if "gen" in gen_df["elm_type"].loc[idxs[1:]].unique() else "sgen"
            )
            net.profiles["%s.p_mw" % elm].loc[:, uniq_idx] = (
                net.profiles["%s.p_mw" % elm]
                .loc[:, gen_df["index"].loc[idxs]]
                .sum(axis=1)
            )
            net.profiles["%s.p_mw" % elm].drop(
                columns=gen_df["index"].loc[idxs[1:]], inplace=True
            )
            if elm == "gen":
                net.profiles["%s.vm_pu" % elm].drop(
                    columns=gen_df["index"].loc[idxs[1:]], inplace=True
                )
        ...
        for elm in gen_df["elm_type"].loc[idxs[1:]].unique():
            dupl_idx_elm = gen_df.loc[
                gen_df.index.isin(idxs[1:]) & (gen_df.elm_type == elm), "index"
            ].values
...
def create_replacement_switch_for_branch(net, element, idx):
    ...
    if element in ["line", "trafo"]:
        is_closed = all(
            net.switch.loc[
                (net.switch.element == idx) & (net.switch.et == element[0]), "closed"
            ]
        )
    ...
    logger.debug(
        "created switch %s (%d) as replacement for %s %s"
...
def replace_zero_branches_with_switches(
    ...
    for elm in elements:
        ...
        if elm == "line" and zero_length:
            branch_zero.update(
                net[elm].loc[net[elm].length_km <= min_length_km].index.tolist()
            )
        ...
        if elm == "line" and zero_impedance:
            branch_zero.update(
                net[elm]
                .loc[
                    (net[elm].r_ohm_per_km <= min_r_ohm_per_km)
        ...
        if elm == "impedance" and zero_impedance:
            branch_zero.update(
                net[elm]
                .loc[
                    (net[elm].rft_pu <= min_rft_pu)
                    & (net[elm].xft_pu <= min_xft_pu)
                    & (net[elm].rtf_pu <= min_rtf_pu)
                    & (net[elm].xtf_pu <= min_xtf_pu)
                ]
                .index.tolist()
            )
...
def replace_impedance_by_line(
    net, index=None, only_valid_replace=True, max_i_ka=np.nan
):
    """
    ...
    for (_, imp), max_i in zip(net.impedance.loc[index].iterrows(), max_i_ka):
        if not np.isclose(imp.rft_pu, imp.rtf_pu) or not np.isclose(
            imp.xft_pu, imp.xtf_pu
        ):
            if only_valid_replace:
                continue
            logger.error(
                "impedance differs in from or to bus direction. lines always "
    ...
    for idx, line_ in net.line.loc[index].iterrows():
        if line_.c_nf_per_km or line_.g_us_per_km:
            if only_valid_replace:
                continue
            logger.error(
                "Capacitance and dielectric conductance of line %i cannot be " % idx
        ...
        new_index.append(
            create_impedance(
                ...
                line_.x_ohm_per_km * line_.length_km / Zni,
                sn_mva[i],
                name=line_.name,
...
def replace_ext_grid_by_gen(
    ...
    if gen_indices is None:
        ...
        raise ValueError(
            "The length of 'gen_indices' must be the same as 'ext_grids' but is "
    ...
    cols_to_keep = (
        ...
        else ["max_p_mw", "min_p_mw", "max_q_mvar", "min_q_mvar"]
    )
    if isinstance(add_cols_to_keep, list) and len(add_cols_to_keep):
        ...
        raise ValueError(
            "'add_cols_to_keep' must be a list or None but is a %s"
            % str(type(add_cols_to_keep))
        )
    cols_to_keep = list(
        set(cols_to_keep)
        - {"bus", "vm_pu", "p_mw", "name", "in_service", "controllable"}
    ...
    existing_cols_to_keep = (
        net.ext_grid.loc[ext_grids].dropna(axis=1).columns.intersection(cols_to_keep)
    )
    ...
    for ext_grid, index in zip(net.ext_grid.loc[ext_grids].itertuples(), gen_indices):
        p_mw = (
            0
            if ext_grid.Index not in net.res_ext_grid.index
            else net.res_ext_grid.at[ext_grid.Index, "p_mw"]
        )
        idx = create_gen(
            net,
            ext_grid.bus,
    ...
    net.gen.loc[new_idx, existing_cols_to_keep] = net.ext_grid.loc[
        ext_grids, existing_cols_to_keep
    ].values
    ...
    for table in ["pwl_cost", "poly_cost"]:
        if net[table].shape[0]:
            to_change = net[table].index[
                (net[table].et == "ext_grid") & (net[table].element.isin(ext_grids))
            ]
            if len(to_change):
...
def replace_gen_by_ext_grid(
    net, gens=None, ext_grid_indices=None, cols_to_keep=None, add_cols_to_keep=None
):
    """
    ...
    if ext_grid_indices is None:
        ...
        raise ValueError(
            "The length of 'ext_grid_indices' must be the same as 'gens' but is "
    ...
    cols_to_keep = list(
        set(cols_to_keep) - {"bus", "vm_pu", "va_degree", "name", "in_service"}
    )

    existing_cols_to_keep = (
        net.gen.loc[gens].dropna(axis=1).columns.intersection(cols_to_keep)
    )
    ...
    for gen, index in zip(net.gen.loc[gens].itertuples(), ext_grid_indices):
        va_degree = (
            0.0
            if gen.bus not in net.res_bus.index
            else net.res_bus.va_degree.at[gen.bus]
        )
        idx = create_ext_grid(
            net,
            gen.bus,
    ...
    for table in ["pwl_cost", "poly_cost"]:
        if net[table].shape[0]:
            to_change = net[table].index[
                (net[table].et == "gen") & (net[table].element.isin(gens))
            ]
            if len(to_change):
...
def replace_gen_by_sgen(
    net, gens=None, sgen_indices=None, cols_to_keep=None, add_cols_to_keep=None
):
    """
    ...
    if sgen_indices is None:
        ...
        raise ValueError(
            "The length of 'sgen_indices' must be the same as 'gens' but is "
    ...
    for gen, index in zip(net.gen.loc[gens].itertuples(), sgen_indices):
        q_mvar = (
            0.0
            if gen.Index not in net.res_gen.index
            else net.res_gen.at[gen.Index, "q_mvar"]
        )
        controllable = (
            True if "controllable" not in net.gen.columns else gen.controllable
        )
        idx = create_sgen(
            net,
            gen.bus,
...
def replace_sgen_by_gen(
    net, sgens=None, gen_indices=None, cols_to_keep=None, add_cols_to_keep=None
):
    """
    ...
    if gen_indices is None:
        ...
        raise ValueError(
            "The length of 'gen_indices' must be the same as 'sgens' but is "
    ...
    existing_cols_to_keep = (
        net.sgen.loc[sgens].dropna(axis=1).columns.intersection(cols_to_keep)
    )
    # add missing columns to net.gen which should be kept
    missing_cols_to_keep = existing_cols_to_keep.difference(net.gen.columns)
    for col in missing_cols_to_keep:
    ...
    for sgen, index in zip(net.sgen.loc[sgens].itertuples(), gen_indices):
        if sgen.bus in net.res_bus.index:
            ...
            if sgen.bus in net.gen.bus.values:
                ...
                log_warning = True
        controllable = (
            False if "controllable" not in net.sgen.columns else sgen.controllable
        )
        idx = create_gen(
            net,
            sgen.bus,
    ...
    net.gen.loc[new_idx, existing_cols_to_keep] = net.sgen.loc[
        sgens, existing_cols_to_keep
    ].values

    if log_warning:
        logger.warning(
            "In replace_sgen_by_gen(), for some generator 'vm_pu' is assumed as 1.0 "
    ...
    for table in ["pwl_cost", "poly_cost"]:
        if net[table].shape[0]:
            to_change = net[table].index[
                (net[table].et == "sgen") & (net[table].element.isin(sgens))
            ]
            if len(to_change):
                net[table].et.loc[to_change] = "gen"
                net[table].element.loc[to_change] = new_idx
...
def replace_pq_elmtype(
    ...
    if old_elm == new_elm:
        logger.warning(
            "'old_elm' and 'new_elm' are both '%s'. No replacement is done." % old_elm
        )
        return old_indices
    ...
    if new_indices is None:
        ...
        raise ValueError(
            "The length of 'new_indices' must be the same as of 'old_indices' but "
    ...
    existing_cols_to_keep = (
        net[old_elm].loc[old_indices].dropna(axis=1).columns.intersection(cols_to_keep)
    )
    ...
    for oelm, index in zip(net[old_elm].loc[old_indices].itertuples(), new_indices):
        controllable = (
            False if "controllable" not in net[old_elm].columns else oelm.controllable
        )
        ...
        if new_elm == "load":
            ...
            args = {
                "max_e_mwh": 1
                if "max_e_mwh" not in net[old_elm].columns
                else net[old_elm].max_e_kwh.loc[old_indices]
            }
        idx = fct(
            net,
            oelm.bus,
    ...
    if sign == -1:
        for col1, col2 in zip(
            ...
            if col1 in existing_cols_to_keep:
                net[new_elm].loc[new_idx, col2] = (
                    sign * net[old_elm].loc[old_indices, col1].values
                )
                already_considered_cols |= {col1}
    net[new_elm].loc[
        new_idx, existing_cols_to_keep.difference(already_considered_cols)
    ] = (
        net[old_elm]
        .loc[old_indices, existing_cols_to_keep.difference(already_considered_cols)]
        .values
    )
    ...
    for table in ["pwl_cost", "poly_cost"]:
        if net[table].shape[0]:
            to_change = net[table].index[
                (net[table].et == old_elm) & (net[table].element.isin(old_indices))
            ]
            if len(to_change):
    ...
    if net["res_" + old_elm].shape[0]:
        ...
        if version.parse(pd.__version__) < version.parse("0.23"):
            ...
            net["res_" + new_elm] = pd.concat(
                [net["res_" + new_elm], to_add], sort=True
            )
...
def replace_ward_by_internal_elements(net, wards=None):
    ...
    for ward in net.ward.loc[wards].itertuples():
        load_idx = create_load(
            net,
            ward.bus,
            ward.ps_mw,
            ward.qs_mvar,
            in_service=ward.in_service,
            name=ward.name,
        )
    ...
    if net.res_ward.shape[0]:
        ...
        to_add_load.p_mw = (
            net.ward.ps_mw.loc[wards].values * sign_in_service * sign_not_isolated
        )
        to_add_load.q_mvar = (
            net.ward.qs_mvar.loc[wards].values * sign_in_service * sign_not_isolated
        )
        ...
        to_add_shunt.p_mw = (
            net.res_ward.vm_pu.loc[wards].values ** 2
            * net.ward.pz_mw.loc[wards].values
            * sign_in_service
            * sign_not_isolated
        )
        to_add_shunt.q_mvar = (
            net.res_ward.vm_pu.loc[wards].values ** 2
            * net.ward.qz_mvar.loc[wards].values
            * sign_in_service
            * sign_not_isolated
        )
...
def replace_xward_by_internal_elements(net, xwards=None):
    ...
    for xward in net.xward.loc[xwards].itertuples():
        ...
        bus_idx = create_bus(
            net, net.bus.vn_kv[xward.bus], in_service=xward.in_service, name=xward.name
        )
        create_load(
            net,
            xward.bus,
        ...
        create_shunt(
            net,
            xward.bus,
            q_mvar=xward.qz_mvar,
            p_mw=xward.pz_mw,
            in_service=xward.in_service,
            name=xward.name,
        )
        create_gen(
            net, bus_idx, 0, xward.vm_pu, in_service=xward.in_service, name=xward.name
        )
        create_impedance(
            net,
            xward.bus,
    ...
    if net.res_xward.shape[0]:
        logger.debug(
            "The implementation to move xward results to new internal elements is "
            + "missing."
        )
...
def get_connected_elements(
    net, element, buses, respect_switches=True, respect_in_service=False
):
    """
    ...
    if element in ["line", "l"]:
        ...
        connected_elements = set(
            net.line.index[net.line.from_bus.isin(buses) | net.line.to_bus.isin(buses)]
        )
        ...
        connected_elements = set(
            net["impedance"].index[
                (net.impedance.from_bus.isin(buses))
    ...
    if respect_switches and element in ["l", "t", "t3w"]:
        open_switches = get_connected_switches(
            net, buses, consider=element, status="open"
        )
        if open_switches:
            open_and_connected = net.switch.loc[
                net.switch.index.isin(open_switches)
                & net.switch.element.isin(connected_elements)
            ].index
            connected_elements -= set(net.switch.element[open_and_connected])

    if respect_in_service and not element_table.empty:
...
def get_connected_buses(
    ...
    if "l" in consider or "line" in consider:
        ...
        opened_lines = (
            set(
                net.switch.loc[
                    (~net.switch.closed) & (net.switch.et == "l")
                ].element.unique()
            )
            if respect_switches
            else set()
        )
        connected_fb_lines = set(
            net.line.index[
                (net.line.from_bus.isin(buses))
                & ~net.line.index.isin(opened_lines)
                & in_service_constr
            ]
        )
        connected_tb_lines = set(
            net.line.index[
    ...
    if "s" in consider or "switch" in consider:
        cs = get_connected_switches(
            net, buses, consider="b", status="closed" if respect_switches else "all"
        )
    ...
    if "t" in consider or "trafo" in consider:
        ...
        opened_trafos = (
            set(
                net.switch.loc[
                    (~net.switch.closed) & (net.switch.et == "t")
                ].element.unique()
            )
            if respect_switches
            else set()
        )
        connected_hvb_trafos = list(
            set(
                net.trafo.index[
                    (net.trafo.hv_bus.isin(buses))
                    & ~net.trafo.index.isin(opened_trafos)
                    & in_service_constr
                ]
            )
        )
        connected_lvb_trafos = list(
            set(
                net.trafo.index[
    ...
    if "t3" in consider or "trafo3w" in consider:
        ...
        if respect_switches:
            opened_buses_hv = set(
                net.switch.loc[
                    ~net.switch.closed
                    & (net.switch.et == "t3")
                    & net.switch.bus.isin(net.trafo3w.hv_bus)
                ].bus.unique()
            )
            opened_buses_mv = set(
                net.switch.loc[
                    ~net.switch.closed
                    & (net.switch.et == "t3")
                    & net.switch.bus.isin(net.trafo3w.mv_bus)
                ].bus.unique()
            )
            opened_buses_lv = set(
                net.switch.loc[
                    ~net.switch.closed
                    & (net.switch.et == "t3")
                    & net.switch.bus.isin(net.trafo3w.lv_bus)
                ].bus.unique()
            )
        else:
        ...
        hvb_trafos3w = list(
            set(
                net.trafo3w.index[
                    net.trafo3w.hv_bus.isin(buses)
                    & ~net.trafo3w.hv_bus.isin(opened_buses_hv)
                    & in_service_constr3w
                ]
            )
        )
        mvb_trafos3w = list(
            set(
                net.trafo3w.index[
                    net.trafo3w.mv_bus.isin(buses)
                    & ~net.trafo3w.mv_bus.isin(opened_buses_mv)
                    & in_service_constr3w
                ]
            )
        )
        lvb_trafos3w = list(
            set(
                net.trafo3w.index[
                    net.trafo3w.lv_bus.isin(buses)
                    & ~net.trafo3w.lv_bus.isin(opened_buses_lv)
                    & in_service_constr3w
                ]
            )
        )
        ...
        cb |= (
            set(net.trafo3w.loc[hvb_trafos3w].mv_bus)
            | set(net.trafo3w.loc[hvb_trafos3w].lv_bus)
            - opened_buses_mv
            - opened_buses_lv
        )
        cb |= (
            set(net.trafo3w.loc[mvb_trafos3w].hv_bus)
            | set(net.trafo3w.loc[mvb_trafos3w].lv_bus)
            - opened_buses_hv
            - opened_buses_lv
        )
        cb |= (
            set(net.trafo3w.loc[lvb_trafos3w].hv_bus)
            | set(net.trafo3w.loc[lvb_trafos3w].mv_bus)
            - opened_buses_hv
            - opened_buses_mv
        )
    ...
    if "i" in consider or "impedance" in consider:
        ...
        connected_fb_impedances = set(
            net.impedance.index[
                (net.impedance.from_bus.isin(buses)) & in_service_constr
            ]
        )
        connected_tb_impedances = set(
            net.impedance.index[(net.impedance.to_bus.isin(buses)) & in_service_constr]
        )
        cb |= set(
            net.impedance[net.impedance.index.isin(connected_tb_impedances)].from_bus
        )
        cb |= set(
            net.impedance[net.impedance.index.isin(connected_fb_impedances)].to_bus
        )
...
def get_connected_switches(net, buses, consider=("b", "l", "t", "t3"), status="all"):
    ...
    if status == "closed":
        ...
        if status != "all":
            logger.warning(
                'Unknown switch status "%s" selected! '
...
def get_connected_elements_dict(
    ...
    connected_buses=True,
    connected_bus_elements=True,
    connected_branch_elements=True,
    connected_other_elements=True,
):
    """Returns a dict of lists of connected elements."""
    pp_elms = pp_elements(
        bus=connected_buses,
        bus_elements=connected_bus_elements,
        branch_elements=connected_branch_elements,
        other_elements=connected_other_elements,
        res_elements=False,
    )
    connected = {}
    for elm in pp_elms:
        if elm == "bus":
            conn = get_connected_buses(
                net,
                buses,
...
def false_elm_links(net, elm, col, target_elm):
    ...
    if isinstance(target_elm, str):
        ...
        for et in df.index:
            false_links = false_links.union(
                pd.Index(
                    df.loc[et].indices.loc[~df.loc[et].element.isin(net[et].index)]
                )
            )
        return false_links
    ...
    for elm in elms:
        if net[elm].shape[0]:
            ...
            if elm in bebd.keys():
                ...
                fl = fl.union(
                    false_elm_links(net, elm, "element", net[elm]["element_type"])
                )
            else:
