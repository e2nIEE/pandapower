...
-------------------------------
- [FIXED] short-circuit: the impedance of an ``ext_grid`` is now derived with the voltage factor ``c`` of the nominal voltage at the short-circuit location, as required by IEC 60909-0, 6.2, equation (6), instead of the one at the ext_grid bus. **This changes results.** Affected are short circuits in a voltage level whose ``c`` differs from that of the feeding ext_grid: the ``max`` case with ``lv_tol_percent=6`` and an ext_grid above 1 kV, and the ``min`` case with any ``lv_tol_percent``, because ``c_min`` is 1.0 above 1 kV but 0.9 or 0.95 in low voltage. Previously ``Ikss`` came out up to ~4.8 % too low for a max-case LV fault fed from a strong MV feeder. The IEC 60909-4 reference examples are unaffected.
