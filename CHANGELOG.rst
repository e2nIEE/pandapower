...
- [FIXED] JAO converter: the ``Susceptance_B(μS)`` column of the lines and tielines sheets was written straight into ``c_nf_per_km``, so a susceptance in µS was stored as a capacitance in nF. Dividing by ``2*pi*f`` (and scaling µ to n) makes the imported shunt capacitance a factor ``1e3/(2*pi*50) = 3.18`` larger; the 380 kV lines of the test file now come out at 6.9 ... 16.4 nF/km instead of 2.2 ... 5.1 nF/km, in line with pandapower's own 380 kV overhead line standard types (11 ... 14.6 nF/km) and with the equivalent conversion already used by the UCTE converter.
