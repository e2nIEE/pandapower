...
from test.consistency_checks import runpp_with_consistency_checks, rundcpp_with_consistency_checks
...
def test_recycle_pq(recycle_net):
    ...
    runpp_with_consistency_checks(net, recycle={"trafo": False, "gen": False, "bus_pq": True})
    ...
    net["load"].at[0, "q_mvar"] = ql
    runpp_with_consistency_checks(net, recycle={"bus_pq": True, "trafo": False, "gen": False})
...
def test_recycle_gen(recycle_net):
    ...
    runpp_with_consistency_checks(net, recycle={"bus_pq": False, "trafo": False, "gen": True})
...
def test_recycle_trafo(recycle_net):
    ...
    runpp_with_consistency_checks(net, recycle={"bus_pq": False, "trafo": True, "gen": False})
...
def test_recycle_trafo_bus_gen(recycle_net):
    ...
    net["gen"].at[0, "vm_pu"] = u_set
    net["trafo"].at[0, "tap_pos"] = 0
    runpp_with_consistency_checks(net, recycle={"bus_pq": True, "trafo": True, "gen": True})
    assert np.allclose(net.res_gen.at[0, "p_mw"], ps)
    assert np.allclose(net.res_gen.at[0, "vm_pu"], u_set)
    assert np.allclose(net.res_load.at[0, "p_mw"], pl)
    assert np.allclose(net.res_load.at[0, "q_mvar"], ql)
...
def test_result_index_unsorted():
    ...
    runpp(net_recycle)
    runpp(net_recycle, recycle={"bus_pq": True, "trafo": True, "gen": True})
    runpp(net)
...
def test_recycle_dc(recycle_net):
    ...
    ql = 0.0
    net["load"].at[0, "q_mvar"] = ql
    rundcpp_with_consistency_checks(net, recycle={"bus_pq": True, "trafo": False, "gen": False})
    assert np.allclose(net.res_load.at[0, "p_mw"], pl)
    pl = 0.8
    ql = 0.55
    net.load.at[0, "p_mw"] = pl
...
def test_recycle_dc_trafo_shift(recycle_net):
    ...
    ql = 0.0
    net["load"].at[0, "q_mvar"] = ql
    rundcpp_with_consistency_checks(net, recycle={"bus_pq": True, "trafo": True, "gen": False})
    assert np.allclose(net.res_load.at[0, "p_mw"], pl)
    pl = 0.8
    ql = 0.55
    net.load.at[0, "p_mw"] = pl
