...
from test.consistency_checks import (
    runpp_3ph_with_consistency_checks,
    runpp_with_consistency_checks,
    trafo_currents_consistent_3ph,
)
from test.loadflow.PF_Results import get_PF_Results
from test.conftest import result_test_network
from test.helper_functions import add_grid_connection
from test.loadflow.test_runpp import get_isolated
...
def check_it(net):
    bus_pp = np.abs(net.res_bus_3ph[["vm_a_pu", "vm_b_pu", "vm_c_pu"]][~np.isnan(net.res_bus_3ph.vm_a_pu)].values)
    bus_pf = np.abs(np.array([[0.96742893, 1.01302766, 1.019784], [0.74957533, 1.09137945, 1.05124282]]))
    assert np.max(np.abs(bus_pp - bus_pf)) < 1.1e-6
    line_pp = np.abs(
        net.res_line_3ph[~np.isnan(net.res_line_3ph.i_a_from_ka)][
            [
                "i_a_from_ka",
                "i_a_to_ka",
                "i_b_from_ka",
...
def test_2bus_network_single_isolated_busses(test_net):
    # -o---o o x
    create_bus(test_net, vn_kv=110)
    create_bus(test_net, vn_kv=110, in_service=False)
    add_zero_impedance_parameters(test_net)
    runpp_3ph_with_consistency_checks(test_net)
    assert test_net["converged"]
    check_it(test_net)
...
def test_2bus_network_isolated_net_part(test_net):
    # -o---o o---o
    b1 = create_bus(test_net, vn_kv=110)
    b2 = create_bus(test_net, vn_kv=110)
    create_line(test_net, from_bus=b1, to_bus=b2, length_km=50.0, std_type="example_type")
    create_asymmetric_load(test_net, b2, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5)
    add_zero_impedance_parameters(test_net)
    runpp_3ph_with_consistency_checks(test_net)
    assert test_net["converged"]
    check_it(test_net)
...
def test_2bus_network_singel_oos_bus(test_net):
    # -o---x---o
    b1 = create_bus(test_net, vn_kv=110)
    test_net.bus.loc[5, "in_service"] = False
    create_line(test_net, from_bus=5, to_bus=b1, length_km=10.0, std_type="example_type")
    create_asymmetric_load(test_net, b1, p_a_mw=-5, q_a_mvar=5, p_b_mw=-1, q_b_mvar=1.5, p_c_mw=-1, q_c_mvar=0.5)
    add_zero_impedance_parameters(test_net)
    runpp_3ph_with_consistency_checks(test_net)
    assert test_net["converged"]
...
def test_out_serv_load(test_net):
    ...
    check_it(test_net)
    create_asymmetric_load(
        test_net, 5, p_a_mw=50, q_a_mvar=100, p_b_mw=29, q_b_mvar=38, p_c_mw=10, q_c_mvar=5, in_service=False
    )
    runpp_3ph_with_consistency_checks(test_net)
    assert test_net["converged"]
    check_it(test_net)
...
def test_2bus_network_one_of_two_ext_grids_oos(test_net):
...
def test_4bus_network(init, recycle):
    ...
    create_ext_grid(
        net, bus=busn, vm_pu=1.0, name="Grid Connection", s_sc_max_mva=5000, rx_max=0.1, r0x0_max=0.1, x0x_max=1.0
    )
    create_std_type(
        net,
        {
    ...
    create_asymmetric_load(net, busk, p_a_mw=50, q_a_mvar=20, p_b_mw=80, q_b_mvar=60, p_c_mw=20, q_c_mvar=5)
    create_asymmetric_load(net, busm, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5)
    create_asymmetric_load(net, busp, p_a_mw=50, q_a_mvar=20, p_b_mw=60, q_b_mvar=20, p_c_mw=10, q_c_mvar=5)
    ...
    bus_pf = np.abs(
        np.array(
            [
                [0.98085729, 0.97711997, 1.04353786],
...
def test_3ph_bus_mapping_order():
    ...
    assert np.allclose(
        ...
        net.res_line_3ph.p_a_from_mw + net.res_line_3ph.p_b_from_mw + net.res_line_3ph.p_c_from_mw,
    )
...
def test_3ph_two_bus_line_powerfactory():
    ...
    create_asymmetric_load(
        net, b2, p_a_mw=0.020, q_a_mvar=0.010, p_b_mw=0.015, q_b_mvar=0.005, p_c_mw=0.025, q_c_mvar=0.010
    )
    runpp_3ph_with_consistency_checks(net)
    assert net["converged"]
    ...
    bus_pf = np.abs(
        np.array([[0.99939853552, 1.0013885141, 0.99921580141], [0.97401782343, 0.98945593737, 0.96329605983]])
    )
    ...
    line_load_pp = np.abs(
        net.res_line_3ph[["loading_a_percent", "loading_b_percent", "loading_c_percent", "loading_percent"]].values
    )
...
def check_bus_voltages(net, result, trafo_vector_group):
    ...
    tolerances = {"YNyn": 1e-05, "Dyn": 1e-05, "Yzn": 1e-03}

    assert np.allclose(result, res_vm_pu, atol=tolerances[trafo_vector_group], rtol=0), (
        f"Incorrect results for {trafo_vector_group}"
    )
...
def check_line_currents(net, result, trafo_vector_group):
    ...
    tolerances = {"YNyn": 1e-07, "Dyn": 1e-07, "Yzn": 1e-04}
...
def check_trafo_currents(net, result, trafo_vector_group):
    ...
    tolerances = {"YNyn": 1e-03, "Dyn": 1e-03, "Yzn": 1e-03}
...
def make_nw(net, bushv, tap_ps, case, vector_group):
    b1 = create_bus(net, bushv, zone=vector_group, index=get_free_id(net.bus))
    b2 = create_bus(net, 0.4, zone=vector_group)
    b3 = create_bus(net, 0.4, zone=vector_group)
    create_ext_grid(net, b1, s_sc_max_mva=10000, rx_max=0.1, r0x0_max=0.1, x0x_max=1.0)
    create_transformer_from_parameters(
        net,
        hv_bus=b1,
        lv_bus=b2,
    ...
    create_line_from_parameters(
        ...
        length_km=0.5,
        r_ohm_per_km=0.1941,
        x_ohm_per_km=0.07476991,
        c_nf_per_km=1160.0,
        max_i_ka=0.421,
        endtemp_degree=70.0,
        r0_ohm_per_km=0.7766,
    ...
    if case == "bal_wye":
        # Symmetric Load
        create_load(net, b3, 0.08, 0.012, type="wye")
    elif case == "delta_wye":
...
def test_trafo_asym():
    nw_dir = os.path.abspath(os.path.join(test_path, "loadflow"))
...
def test_3ph_isolated_nodes():
    ...
    create_bus(net, vn_kv=20.0, in_service=False, index=3)
    busy = create_bus(net, vn_kv=20.0, in_service=True, index=0, name="busy")

    create_ext_grid(net, bus=busn, vm_pu=1.0, name="Grid Connection", s_sc_max_mva=5000, rx_max=0.1)
    net.ext_grid["r0x0_max"] = 0.1
    net.ext_grid["x0x_max"] = 1.0
    create_std_type(
        net,
    ...
    create_asymmetric_load(net, busk, p_a_mw=50, q_a_mvar=50, p_b_mw=10, q_b_mvar=15, p_c_mw=10, q_c_mvar=5)
    ...
    assert net["converged"]
    assert np.allclose(
        net.res_bus_3ph.T[[0, 2, 3]].T[["vm_a_pu", "va_a_degree", "vm_b_pu", "va_b_degree", "vm_c_pu", "va_c_degree"]],
        np.nan,
        equal_nan=True,
    )
    assert np.allclose(
        net.res_bus_3ph.T[[0, 2, 3]].T[["p_a_mw", "q_a_mvar", "p_b_mw", "q_b_mvar", "p_c_mw", "q_c_mvar"]], 0.0
    )
...
def test_shunt_3ph():
    ...
    bus_expected = np.abs(
        np.array(
            [
                [1.0, 0.0, 1.0, -120.0, 1.0, 120.0],
                [
                    0.9950250311521571,
                    -8.955222949441059e-06,
                    0.995025031152157,
                    -120.00000895522294,
                    0.995025031152157,
                    119.99999104477705,
...
def test_gen_3ph_loadflow():
    net = pandapowerNet(name="test_gen_3ph_loadflow", f_hz=50, add_stdtypes=False)
    add_zero_impedance_parameters(net)

    create_bus(net, vn_kv=110.0, index=0)
    create_bus(net, vn_kv=110.0, index=1)
    create_bus(net, vn_kv=110.0, index=2)
