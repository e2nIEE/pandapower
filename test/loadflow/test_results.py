...
from test.loadflow.result_test_network_generator import (
    add_test_enforce_qlims,
    add_test_gen,
    add_test_enforce_qlims_sgen,
    add_test_enforce_plims,
    add_test_enforce_plims_sgen
)
...
def test_trafo(result_test_network, v_tol=1e-6, i_tol=1e-6, s_tol=1e-2, l_tol=1e-3, va_tol=1e-2):
    net = result_test_network
    buses = net.bus[net.bus.zone == "test_trafo"].index
    trafos = net.trafo[net.trafo.hv_bus.isin(buses)].index
    t1 = trafos[0]
    t2 = trafos[1]
    t3 = trafos[2]
    b2 = buses[1]
...
def test_trafo3w_tap(tap_pos, tap_side, tap_step_degree):
    results = pd.read_csv(os.path.join(test_path, "test_files", "test_results_files", "trafo_3w_tap_results.csv"),
                          sep=";", decimal=",")
    ...
    net = from_json(os.path.join(test_path, "test_files", "test_trafo3w_tap.json"))  #
...
def test_trafo3w_tap_neutral_not_zero(tap_pos, tap_side, tap_step_degree):
    results = pd.read_csv(
        os.path.join(test_path, "test_files", "test_results_files", "trafo_3w_tap_results_neutral_not_zero.csv"),
        sep=";", decimal=",")
