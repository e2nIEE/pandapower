...
from test.helper_functions import assert_net_equal, create_test_network, create_test_network2
...
def net_charactistics():
    return from_json(os.path.join(test_path, "test_files", "from_excel_characteristics.json"))
...
def test_json_controller_none():
    ...
    """
    from_json(os.path.join(test_path, 'test_files', 'controller_containing_NoneNan.json'), convert=False)
...
def test_convert_format():  # TODO what is this thing testing ?
    net = from_pickle(os.path.join(test_path, "api", "old_net.p"))
    runpp(net)
...
def test_replace_elements_json_string(net_in):
    ...
    net_load = from_json_string(json_string, replace_elements={
        r'pandapower.control.controller.const_control': r'test.api.input_files.test_control',
